...
public class HorizontalListViewAdapter extends BaseAdapter{
	...
	private static PKGIcongetter iconGetter;
	public HorizontalListViewAdapter(Context context, ArrayList<AppEntry> entries, int itemId, PackageManager pm){
		...
		iconGetter = PKGIcongetter.getInstance(mContext);
	...
	public static Drawable getApplicationIcon(ApplicationInfo info, PackageManager pm){
		...
		if(iconGetter == null){
			iconGetter = PKGIcongetter.getInstance(activity);
		}
		Drawable drawable = allDrawables.get(info.packageName);
		if(drawable == null){
			Drawable iconTmp = iconGetter.getIconDrawable(info.packageName);
			if(iconTmp == null){
