...
//        mPermissionApps.refresh(true);
    ...
    private void bindUi(Fragment fragment, PermissionApps permissionApps) {
//        final Drawable icon = permissionApps.getIcon();
//        final CharSequence label = permissionApps.getLabel();
        final Toolbar ab = ((MstActivity)fragment.getActivity()).getToolbar();
        if (ab != null) {
            ab.setTitle(mMstPermission.mstGetPkgPermLabel(groupName));
        }        
    }
...
//        List<MstAppGroup> appGroupList =  permGroup.getAppGroupList();
        Map<String, MstAppGroup> appGroupMap = permGroup.getAppGroupMap();
        if(appGroupMap.size() > 0) {
        	...
        	MstAppGroup app = null;
        	for (Map.Entry<String, MstAppGroup> mapEntry: appGroupMap.entrySet()) {
				app = mapEntry.getValue();
    ...
    private void addPreferenceSelect(PreferenceScreen screen, List<MstAppGroup> appGroupList) {
    	...
    	for (MstAppGroup app : appGroupList) {
    		...
    		pref.setSelected(permEntry, mMstPermission);
    		screen.addPreference(pref);
    	}
    }
