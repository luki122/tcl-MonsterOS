...
public  class ApplicationIconLoader {
    ...
    private static final String CACHE_NAME = "pkg_icon";
    ...
    private PKGIcongetter mIconGetter;
    
    
    public ApplicationIconLoader(Context context) {
        mResources = context.getResources();
        mIconGetter = PKGIcongetter.getInstance(context);
        if(context instanceof Activity){
        	addImageCache((Activity)context, CACHE_NAME);
        }
    }

    

    public void loadImage(String packageName, ImageView imageView, int position) {
        ...
        if (mImageCache != null) {
            value = mImageCache.getBitmapFromMemCache(getCacheName(packageName));
        }
        ...
        if (value != null) {
            ...
            Log.d("icon", "from cache");
        } else if (cancelPotentialWork(packageName, imageView)) {
            ...
            task.executeOnExecutor(AsyncTask.DUAL_THREAD_EXECUTOR, packageName);
        }
    }
    ...
    public void addImageCache(Activity activity, String diskCacheDirectoryName) {
     ...
     void addImageCache(Activity activity, String diskCacheDirectoryName, String updateName) {
    ...
    protected  Bitmap processBitmap(String packageName){
    	Drawable dw = mIconGetter.getIconDrawable(packageName);
    	if(dw != null){
    		Bitmap bitmap = PhotoUtils.drawable2bitmap(dw);
    		return bitmap;
    	}
    	return null;
    }
    ...
    public static boolean cancelPotentialWork(Object data, ImageView imageView) {
        ...
        if (bitmapWorkerTask != null) {
            final Object bitmapData = bitmapWorkerTask.packageName;
    ...
    private class BitmapWorkerTask extends AsyncTask<String, Void, BitmapDrawable> {
        ...
        protected BitmapDrawable doInBackground(String... params) {
            ...
            final String dataString = packageName;
            Bitmap bitmap = null;
...
//                bitmap = mIconGetter.standardThirdPardIcon(drawable);
//            }
            if (bitmap == null && !isCancelled() && getAttachedImageView() != null
                    && !mExitTasksEarly) {
            	try{
                bitmap = processBitmap(packageName);
            	}catch(Exception e){
