...
import cn.tcl.note.util.DensityUtil;
...
public class MatrixImageView extends ImageView {
    ...
    private float DEFAULT_PADDING_TOP = 0;//dp
    private float DEFAULT_PADDING_BOTTOM = 0;
    ...
    public MatrixImageView(Context context, AttributeSet attrs) {
        ...
        mPaddingBottom = a.getDimension(R.styleable.MatrixImageView_matrix_paddingBottom, DEFAULT_PADDING_BOTTOM);
        a.recycle();
    ...
    public void setImagePaddingTopByDp(float imagePaddingTopByDp){
        mPaddingTop =  DensityUtil.dip2px(this.getContext(),imagePaddingTopByDp);
    }

    public void setImagePaddingBottomByDp(float imagePaddingBottomByDp){
        mPaddingBottom =  DensityUtil.dip2px(this.getContext(),imagePaddingBottomByDp);
    }
    ...
    public void setImageBitmap(Bitmap bm) {
        ...
        NoteLog.d(TAG, "bitmap width=" + mMatrix);
    }
    ...
    private Matrix getInitMatrix(){
        ...
        if((mViewHeight - mPaddingTop - mPaddingBottom)/mViewWidth < mImageHeight/mImageWidth){
            mRealHeight = mViewHeight - mPaddingTop - mPaddingBottom;
        ...
        NoteLog.i(TAG,"getInitMatrix matrix = " + matrix);
        return matrix;
    }
    ...
    public class MatrixTouchListener implements OnTouchListener, ScaleGestureDetector.OnScaleGestureListener {
        ...
        public void setDragMatrix(MotionEvent event) {
            if (isZoomChanged()) {
                ...
                NoteLog.d(TAG, "drag x=" + dx + "  dy=" + dy);
                if (Math.sqrt(dx * dx + dy * dy) > 10f) {
                    NoteLog.d(TAG, "drag img");
                    startPoint.set(event.getX(), event.getY());
        ...
        private void scaleImgWithAnima(final float startScale, final float endScale) {
            NoteLog.d(TAG, "startScale=" + startScale + "  endScale=" + endScale + " current Matri=" + mCurrentMatrix);
            ValueAnimator valueAnimator = ValueAnimator.ofFloat(startScale, endScale);
            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    float scale = (float) valueAnimator.getAnimatedValue();
                    NoteLog.d(TAG, "update scale=" + scale);
                    float[] oldValue = new float[9];
                    mCurrentMatrix.getValues(oldValue);
                    float scaleFactor = scale / oldValue[Matrix.MSCALE_X];
                    NoteLog.d(TAG, "scaleFactor=" + scaleFactor);
                    mCurrentMatrix.postScale(scaleFactor, scaleFactor, mScaleGestureDetector.getFocusX(), mScaleGestureDetector.getFocusY());
                    NoteLog.i(TAG,"onAnimationUpdate scaled mCurrentMatrix = " + mCurrentMatrix);
                    float[] currentValue = new float[9];
//                    if(currentValue[Matrix.MTRANS_Y] < mTransY){
//                        MeetingLog.i(TAG,"onAnimationUpdate currentValue[Matrix.MTRANS_Y] = " + currentValue[Matrix.MTRANS_Y]);
//                        mCurrentMatrix.postTranslate(currentValue[Matrix.MTRANS_X],mTransY);
//                        MeetingLog.i(TAG,"onAnimationUpdate translate mCurrentMatrix = " + mCurrentMatrix);
//                    }
//                    mCurrentMatrix.setScale(scale,scale);
    ...
    private void requestParentIntercept(boolean result) {
        NoteLog.d(TAG, "requestParentIntercept result=" + result);
