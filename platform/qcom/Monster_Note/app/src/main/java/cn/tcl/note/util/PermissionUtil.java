...
public class PermissionUtil {
    ...
    public static boolean requestWritePermission(Activity activity) {
        return requestWritePermission(activity, REQUEST_CODE_WRITE);
    }

    public static boolean requestWritePermission(Activity activity, int requestCode) {
        if (ContextCompat.checkSelfPermission(activity, WRITE_PERMISSION) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(activity, new String[]{WRITE_PERMISSION}, requestCode);
            NoteLog.d(TAG, "Activity don't have write sd permission,so request");
            return false;
        } else {
            NoteLog.d(TAG, "Activity have had write sd permission");
            return true;
        }
    }
    ...
    public static boolean requestRecordPermission(Activity activity) {
        ...
        if (ContextCompat.checkSelfPermission(activity, RECORD_PERMISSION) != PackageManager.PERMISSION_GRANTED) {
            NoteLog.d(TAG, "Activity don't have record permission,so request");
        ...
        if (size > 0) {
            ...
            NoteLog.d(TAG, "Activity have had write sd and record permission");
            return true;
        }
    }
