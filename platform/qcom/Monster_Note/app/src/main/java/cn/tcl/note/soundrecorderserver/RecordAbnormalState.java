...
public class RecordAbnormalState {
    ...
    private NoteEditorActivity mContext;
    ...
    public RecordAbnormalState(NoteEditorActivity context) {
        mContext = context;
    }
    ...
    private void setState(double value, int minValue, int maxValue) {
        NoteLog.d(TAG, "set state value = " + value);
        if (value >= maxValue) {
    ...
    public void manager() {
        if (NoteLog.DEBUG) {
            for (int i : mStateList) {
                NoteLog.d(TAG, "state list= " + i);
            }
        }
        NoteLog.d(TAG, "mode=" + mMode);
        int state = -1;
    ...
    private int getLatestStateItem() {
        ...
        NoteLog.d(TAG, "latest state =" + state);
        return state;
    }
    ...
    private void showStartDialog(int state) {
        switch (state) {
            ...
            case STATE_LOW_BATTERY_10:
                ...
                }, R.string.dialog_back_title, R.string.dialog_start_battery_10, R.string.dialog_start_record, R.string.dialog_cancel);
                break;
            ...
            case STATE_LOW_SIZE_50MB:
                ...
                }, R.string.dialog_back_title, R.string.dialog_space_50m_start, R.string.dialog_space_clear, R.string.dialog_cancel);
                break;
        }
    }
        ...
        for (ActivityManager.RunningAppProcessInfo appProcessInfo : appProcessInfos) {
            if (appProcessInfo.processName.equals(mContext.getPackageName())) {
                if (appProcessInfo.importance == ActivityManager.RunningAppProcessInfo
                    ...
                    NoteLog.d(TAG, "this is run in background: " + appProcessInfo.processName);
                    return true;
                } else {
                    NoteLog.d(TAG, "this is run in foreground: " + appProcessInfo.processName);
                    return false;
                }
            }
        }
