        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case EVENT_STORAGE_FULL:
                    showToast(R.string.confirm_device_info_full);
                    break;
                case EVENT_SAVE_SECCESS:
                    String path = (String) msg.obj;
                    showToast(path);
                    showRecordSuccessNotification(path);
                    break;
                case EVENT_STORAGE_UNMOUNTED:
                    showToast(R.string.ext_media_badremoval_notification_title);
                    break;
...
//            }
    ...
    private void showToast(int resId) {
        showToast(InCallApp.getInstance().getApplicationContext().getResources()
                .getText(resId).toString());
    }
    
    private void showToast(String res) {
		Toast toast = Toast.makeText(InCallApp.getInstance(), res,
				Toast.LENGTH_SHORT);
		toast.setGravity(Gravity.TOP, 0, 0);
		toast.show();
    }
    
    private void showRecordSuccessNotification(String path) {
    	  NotificationManager mNotificationManager =
                (NotificationManager) InCallApp.getInstance().getSystemService(Context.NOTIFICATION_SERVICE);
    	  final Notification.Builder builder = new Notification.Builder(InCallApp.getInstance());

          // Set up the main intent to send the user to the in-call screen
          final Intent intent = new Intent("com.mst.recordhistory");
          intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
          final PendingIntent inCallPendingIntent = PendingIntent.getActivity(InCallApp.getInstance(), 0, intent, 0);
          builder.setContentIntent(inCallPendingIntent);
          // Set the content
          builder.setSmallIcon(R.drawable.ic_call_white_24dp);
          builder.setContentText(path);
          builder.setContentTitle(InCallApp.getInstance().getResources().getString(R.string.call_record));
