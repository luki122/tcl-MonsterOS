...
public class Recorder implements OnErrorListener {
    ...
    public static final int SECCESS = 4;
...
//        String prefix = dateFormat.format(new Date());
        String prefix = RecorderUtils.getRecordFileName();
        File sampleDir = new File(RecorderUtils.getExternalStorageDefaultPath());
...
//            mSampleFile = File.createTempFile(prefix, extension, sampleDir);
        	mSampleFile = new File(sampleDir.getAbsolutePath() + "/" +  prefix + extension);
        	mSampleFile.createNewFile();
            log("mSampleFile name = " + mSampleFile.getName());
        } catch (IOException e) {
        ...
        mPrivateId = RecorderUtils.getPrivateId();
...
//        mRecorder.setAudioSource(MediaRecorder.AudioSource.MIC);
    ...
    public void stopRecording() {
        ...
        //add by lgy
      	if(mSampleFile != null) {
      	  String path = mPrivateId > 0 ? RecorderUtils.getPrivateRecordPath(mPrivateId)  : RecorderUtils.getRecordPath();
          File newFilePath = new File(path, RecorderUtils.changeRecordFileName(mSampleFile.getName(), mSampleStart + "_" + mSampleLength));
          mSampleFile.renameTo(newFilePath);
    	}        
        mRecorder.stop();
        mRecorder.release();
        mRecorder = null;
