 ...
 *
 */
public class AnswerPresenterMst extends Presenter<AnswerPresenterMst.AnswerUi>
    ...
    private static final String TAG = AnswerPresenterMst.class.getSimpleName();

    //modify by lgy for 2846370
    private static int mPhoneCount = TelephonyManager.from(InCallApp.getInstance()).getPhoneCount();
//    private String mCallId[] = new String[InCallServiceImpl.sPhoneCount];
//    private Call mCall[] = new Call[InCallServiceImpl.sPhoneCount];
	private String mCallId[] = new String[mPhoneCount];
	private Call mCall[] = new Call[mPhoneCount];
    ...
    public void onCallChanged(Call call) {
        ...
        if (call.getState() != Call.State.INCOMING && call.getState() != Call.State.CALL_WAITING ) {
            ...
            //add by lgy
            boolean isDisconnectPending = false;
            if(call.getState() == Call.State.DISCONNECTED || call.getState() == Call.State.DISCONNECTING) {
            	if(!mCalls.hasLiveCall()) {
            		isDisconnectPending = true;
            	}
            }
            boolean isActivityStart = InCallPresenter.getInstance().getInCallActivity() != null;     
            if(InCallApp.getInCallActivity() != null) {
                InCallApp.getInCallActivity().setMainAlpha(isActivityStart && isDisconnectPending ? 0 : 1);   
            }
    ...
    public void onDecline(Context context) {
        ...
        if (mCall[phoneId].getSessionModificationState()
            ...
            mLastRejectNumber =  mCall[phoneId].getNumber();
            mLastRejectSubId = mCall[phoneId].getSubId();
    ...
    private int mLastRejectSubId;
    private String mLastRejectNumber;
    ...
    public void onDeclineForeground(Context context) {
        int phoneId = getActivePhoneId();
        Call activeCall = mCalls.getActiveOrBackgroundCall();       
        Log.d(this, "onDeclineForeground callId:" + activeCall.getId() + "phoneId:" + phoneId);
        if (activeCall.getSessionModificationState()
                == Call.SessionModificationState.RECEIVED_UPGRADE_TO_VIDEO_REQUEST) {
            InCallPresenter.getInstance().declineUpgradeRequest(context);
        } else {
            TelecomAdapter.getInstance().disconnectCall(activeCall.getId());
        }
        onAnswer(VideoProfile.STATE_AUDIO_ONLY, context);
    }
    
    private void startSmsAnim() {
    	InCallApp.getInCallActivity().hideDown(null);
    }
    
    public boolean isShowRejctAndAnswer() {
        TelephonyManager tm = TelephonyManager.from(InCallApp.getInstance());
        if(mCalls.getIncomingOrActive() != null) {
            int type =  tm.getCurrentPhoneType(mCalls.getIncomingOrActive().getSubId());
            Log.d(this, "isShowRejctAndAnswer mCalls.getActiveSubId(): " + mCalls.getIncomingOrActive().getSubId() + " phone type =" +type);
            return type !=  PhoneConstants.PHONE_TYPE_CDMA;
        } else {
            return true;
        }
    }
    ...
    public void sendMessage(String message) {
        sendMessage(getUi().getContext(), mLastRejectNumber,
                    message, mLastRejectSubId); 
    }
    
    /**
     * Reject the call with the specified message. If message is null this call is ignored.
     */
    private void sendMessage(Context context, String phoneNumber, String textMessage,
            int subId) {
    ...
    private void showMessageSentToast(final String phoneNumber, final Context context) {
        ...
        Toast t = Toast.makeText(context, styledConfirmationMsg,
                Toast.LENGTH_LONG);
        t.getWindowParams().type = WindowManager.LayoutParams.TYPE_STATUS_BAR_PANEL;  
        t.getWindowParams().privateFlags |= WindowManager.LayoutParams.PRIVATE_FLAG_SHOW_FOR_ALL_USERS;  
        t.getWindowParams().flags |= WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED;  
        t.show();
