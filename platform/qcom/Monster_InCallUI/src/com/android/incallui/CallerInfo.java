...
import com.mediatek.telecom.recording.RecordUtils;
import com.mst.privacy.PrivacyDataUtils;
import com.mst.privacy.PrivacyUtils;
import com.mst.tms.AreaManager;
import com.mst.tms.MarkManager;
import com.mst.tms.MarkResult;
import com.mst.utils.DatabaseUtil;
import com.mst.utils.NoteUtils;
...
//                        context, info.lookupKeyOrNull, info.userType);
                ...
                //add by lgy                
                int privateData[] = PrivacyDataUtils.getPrivateData(contactRef);
                long currentPrivateId = PrivacyUtils.getCurrentAccountId();
...
//                        info.name = InCallApp.getString(R.string.private_unknown_contact);
                    } 
                } else  {
                    info.mRawContactId = DatabaseUtil.getRawContactId(contactRef);
                }
                                               
                RecordUtils.handleAutoRecord(context, info.phoneNumber,  info.mRawContactId);
    ...
    public String mArea;
    public String mNote;
    public MarkResult mMark;
    public long mPrivateId = 0;
    public int mPrivateNotiType = 0; 
    
    
    public void updataMstInfo(String number){
        mArea = AreaManager.getArea(number);
        //18682313086 诈骗电话
//        mMark = MarkManager.getMark(16, "18682313086");
        String userMark =MarkManager.getUserMark(InCallApp.getInstance(), number);
        if(!TextUtils.isEmpty(userMark)) {
            mMark = new MarkResult(0, number, userMark, 0, -1, null, 0, null, null);
        } else {
            mMark = MarkManager.getMark(16, number);
        }

        mNote = NoteUtils.getNote(InCallApp.getInstance(), mRawContactId);
    }
}
