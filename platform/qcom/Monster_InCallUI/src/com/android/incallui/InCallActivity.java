...
import com.mst.floatview.FloatWindowManager;
/**
...
public class InCallActivity extends TransactionSafeActivity implements FragmentDisplayManager {
    ...
    private CallButtonFragmentMst mCallButtonFragment;
    private CallCardFragmentMst mCallCardFragment;
    private AnswerFragmentMst mAnswerFragment;
    private DialpadFragmentMst mDialpadFragment;
...
//        if(speed) {
//            Log.i(this, "onCreate speed =  " + speed);
//            setMainAlpha(0);
//        }
        ...
        Fragment existingFragment =
                getFragmentManager().findFragmentByTag(DialpadFragmentMst.class.getName());
        if (existingFragment != null) {
            mDialpadFragment = (DialpadFragmentMst) existingFragment;
        }
        ...
        if (isDsdaEnabled ) {
		 ...
		 boolean hasReadWindowPermission = PermissionsUtil.hasPermission(this,WINDOW_PERMISSION);
         if(!hasReadContactPermission || !hasReadWindowPermission){
       	   requestPermissions(new String[] {CONTACT_PERMISSION, WINDOW_PERMISSION}, CONTACT_PERMISSION_REQUEST_CODE);
             return;
         }
    ...
    protected void onStart() {
        ...
        setFragementListeners(mCallCardFragment);
        setFragementListeners(mAnswerFragment);
        setFragementListeners(mDialpadFragment);                    
        setFragementListeners(mConferenceManagerFragment);
        setFragementListeners(mCallButtonFragment);
    }
...
//        InCallPresenter.getInstance().setThemeColors();
    ...
    protected void onStop() {
        ...
        if(CallList.getInstance().hasLiveCall()) {
   		    ...
   		    if(hasWindowPermission) {
   		    	if(!mIsNotShowStatusbarNextTime) {
   		    		FloatWindowManager.createStatusBarWindow(InCallApp.getInstance());
   		    	} else {
   		    		mIsNotShowStatusbarNextTime = false;
   		    	}
   		    }
        }
        super.onStop();
    ...
    public void onFragmentAttached(Fragment fragment) {
        if (fragment instanceof DialpadFragmentMst) {
            mDialpadFragment = (DialpadFragmentMst) fragment;
        } else if (fragment instanceof AnswerFragmentMst) {
            mAnswerFragment = (AnswerFragmentMst) fragment;
        } else if (fragment instanceof CallCardFragmentMst) {
            mCallCardFragment = (CallCardFragmentMst) fragment;
        ...
        } else if (fragment instanceof CallButtonFragmentMst) {
            mCallButtonFragment = (CallButtonFragmentMst) fragment;
        }
    }
    ...
    public CallButtonFragmentMst getCallButtonFragment() {
        return mCallButtonFragment;
    }

    public CallCardFragmentMst getCallCardFragment() {
        return mCallCardFragment;
    }

    public AnswerFragmentMst getAnswerFragment() {
    ...
    private void internalResolveIntent(Intent intent) {
        ...
        if (action.equals(Intent.ACTION_MAIN)) {
            ...
            //add by lgy
            Log.d(this, "- internalResolveIntent: answer: " + intent.getBooleanExtra("answer", false));
            if(intent.getBooleanExtra("answer", false)) {
                InCallPresenter.getInstance().answerIncomingCall(
                        this, VideoProfile.STATE_AUDIO_ONLY);
            }
...
//                        InCallPresenter.getInstance());
            ...
            if (pendingAccountSelectionCall != null) {
                ...
                DialogFragment dialogFragment = SelectPhoneAccountDialogFragment.newInstance(
                        R.string.select_phone_account_for_calls, false, phoneAccountHandles,
                        mSelectAcctListener);
                dialogFragment.show(getFragmentManager(), TAG_SELECT_ACCT_FRAGMENT);
                getWindow().setNavigationBarColor(0x01f5f5f5);
    ...
    private Fragment createNewFragmentForTag(String tag) {
        if (TAG_DIALPAD_FRAGMENT.equals(tag)) {
            mDialpadFragment = new DialpadFragmentMst();
...
//            }
            ...
            mCallCardFragment = new CallCardFragmentMst();
            return mCallCardFragment;
        }
    ...
    public void showCallCardFragment(boolean show) {
        Log.d(this, "showCallCardFragment show = " + show);
    ...
    public void showAnswerFragment(boolean show) {
        Log.i(this, "showAnswerFragment show =  " + show);
        showFragment(TAG_ANSWER_FRAGMENT, show, true);
        configCallCardFragment(show);
    }
	...
	private static final String WINDOW_PERMISSION = "android.permission.SYSTEM_ALERT_WINDOW";
...
//            	finish();
    ...
    private void configCallCardFragment(boolean incoming) {
        if(mCallCardFragment != null) {
            mCallCardFragment.configCallCardFragment(incoming);
        }
    ...
    public void hideDown(final Runnable r) {
     	...
     	PropertyValuesHolder pvha = PropertyValuesHolder.ofFloat("alpha",
 				1,  0);
 		ObjectAnimator mainAnimator = ObjectAnimator.ofPropertyValuesHolder(
 				main, pvhy, pvha);
    ...
    private boolean mIsNotShowStatusbarNextTime = false;
    public void hideDownNoStatusbar(final Runnable r) {
    	mIsNotShowStatusbarNextTime = true;
    	hideDown(r);
    }
    ...
    public void updateTransparentBackground(boolean transparent) {
        if (mTransparentBackground == transparent) {
            return;
        }
        mTransparentBackground = transparent;
        if (transparent) {
    ...
    private void restoreState() {
        Log.i(this, "restoreState InCallPresenter.getInstance().getInCallState() = " + InCallPresenter.getInstance().getInCallState());
//        mHandler.postDelayed(new Runnable() {
//            public void run(){
...
//        if(InCallPresenter.getInstance().getInCallState() == InCallState.NO_CALLS) {
            Log.i(this, "restoreStateInternal");
            showAnswerFragment(false);   
//            configCallCardFragment(false);            
            unsetFragementListeners(mCallCardFragment);
            unsetFragementListeners(mAnswerFragment);
            unsetFragementListeners(mDialpadFragment);                    
            unsetFragementListeners(mConferenceManagerFragment);
            unsetFragementListeners(mCallButtonFragment);
            InCallPresenter.getInstance().unsetActivity(InCallActivity.this);
            initUiState();
//        }
    
    }
    
    private void restoreAfter() {
        mHandler.postDelayed(new Runnable() {
            public void run() {
                View main = findViewById(R.id.main);
    ...
    private void initUiState() {
        if(mCallCardFragment != null) {
            mCallCardFragment.restoreState();
        }
        if(mAnswerFragment != null) {
            mAnswerFragment.restoreState();
        }
        if(mDialpadFragment != null) {
        ...
        if(mCallButtonFragment != null) {
            mCallButtonFragment.restoreState();
        }        
    }
    
    private void unsetFragementListeners (BaseFragment f) {
        Log.i(this, "unsetFragementListeners f = " + f);
        if(f != null) {
            if(f.getPresenter().getUi() != null) {
                f.mstExit();
            }
        }
    }
    
    private void setFragementListeners(BaseFragment f) {
        Log.i(this, "setFragementListeners f = " + f);
        if (f != null) {
            if(f.getPresenter().getUi() == null) {
                f.mstInit();
            }
    ...
    void setMainAlpha(int alpha) {
        Log.i(this, "setMainAlpha alpha = " + alpha);
        View main = findViewById(R.id.main);
        main.setAlpha(alpha);
    }
}
