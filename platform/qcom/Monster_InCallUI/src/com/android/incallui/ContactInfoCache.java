...
public class ContactInfoCache implements ContactsAsyncHelper.OnImageLoadCompleteListener {
    ...
    class PhoneNumberServiceListener implements PhoneNumberService.NumberLookupListener,
        ...
        public void onPhoneNumberInfoComplete(
            ...
            entry.area= AreaManager.getArea(entry.number);
            entry.mark = MarkManager.getMark(16, entry.number);
            entry.note = NoteUtils.getNote(entry.number);
...
//                && (ContactsUtils.FLAG_N_FEATURE || info.contactIdOrZero != 0)) {
    ...
    public static void populateCacheEntry(Context context, CallerInfo info, ContactCacheEntry cce,
        ...
        Log.d(TAG, "  ==> CallerInfo: area '" + info.mArea);
        cce.area= info.mArea;
        cce.mark = info.mMark;
        cce.note = info.mNote;
        cce.mPrivateId = info.mPrivateId;
        cce.mPrivateNotiType = info.mPrivateNotiType;
    }
...
//                    mContext.getResources().getDrawable(R.drawable.img_no_image_automirrored);
          		   mContext.getResources().getDrawable(R.drawable.img_no_image);        		  
        }
    ...
    public static class ContactCacheEntry {
        ...
        public String toString() {
            return MoreObjects.toStringHelper(this)
                    ...
                    .add("area", area)
                    .add("mark", mark != null ? mark.getName() : "null")
                    .add("note", note)
                    .toString();
        }
    }
    ...
    //add by lgy
    public ContactCacheEntry findInfoInCache(final Call call) {
//        Preconditions.checkState(Looper.getMainLooper().getThread() == Thread.currentThread());
