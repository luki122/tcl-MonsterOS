 ...
 */
public class CallButtonPresenterMst extends Presenter<CallButtonPresenterMst.CallButtonUi>
    ...
    public void holdClicked(boolean checked) {
        ...
        //add by lgy for cdma hold
        TelephonyManager tm = TelephonyManager.from(InCallApp.getInstance());
        int type =  tm.getCurrentPhoneType(mCall.getSubId());
        if(type ==  PhoneConstants.PHONE_TYPE_CDMA) {
    ...
    public void swapClicked() {
        ...
        if(getCurrentPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            Log.i(this, "Swapping the call cdma" );
            TelecomAdapter.getInstance().swap(mCall.getId());
        } else {
            Log.i(this, "Swapping the call: gsm" );
            Call call = CallList.getInstance().getBackgroundCall();
            if(call != null) {
...
//        muteClicked(true);
    ...
    private void updateButtonsState(Call call) {
        ...
        ui.showButton(BUTTON_HOLD, !showSwap);
        ui.setHold(isCallOnHold);
//        ui.showButton(BUTTON_MUTE, showMute);
        ui.showButton(BUTTON_MUTE, true);
        ui.showButton(BUTTON_ADD_CALL, !showMerge);
...
//        }
    ...
    private void updateButtonsEnableState(Call call) {
        Log.v(this, "updateButtonsEnableState");
        final CallButtonUi ui = getUi();

        final boolean showSwap = call.can(
                android.telecom.Call.Details.CAPABILITY_SWAP_CONFERENCE);
        //modify by lgy for 3408758
//        final boolean showHold = !showSwap
//                && call.can(android.telecom.Call.Details.CAPABILITY_SUPPORT_HOLD)
//                && call.can(android.telecom.Call.Details.CAPABILITY_HOLD);
      final boolean showHold = !showSwap && (call.getState() == Call.State.ONHOLD || call.getState() == Call.State.ACTIVE);
        
        final boolean showMerge = call.can(
                android.telecom.Call.Details.CAPABILITY_MERGE_CONFERENCE);
        boolean enable = call.getState() == Call.State.ACTIVE||call.getState() == Call.State.ONHOLD;
//        boolean hold  = call.getState() == Call.State.ONHOLD || call.getState() == Call.State.ACTIVE;
        boolean add = call.getState() == Call.State.IDLE || call.getState() == Call.State.ACTIVE;         

        ui.enableButton(BUTTON_AUDIO, true);
        ui.enableButton(BUTTON_SWAP, true);
        ui.enableButton(BUTTON_HOLD, showHold && !showMerge);
        ui.enableButton(BUTTON_MUTE, enable);
        ui.enableButton(BUTTON_ADD_CALL, add && !showSwap);
        ui.enableButton(BUTTON_DIALPAD, true);
        ui.enableButton(BUTTON_MERGE, true);
        ui.enableButton(BUTTON_RECORD, enable);

        ui.updateButtonStates();
    }
    
    private int getCurrentPhoneType() {
        TelephonyManager tm = TelephonyManager.from(InCallApp.getInstance());
        return tm.getCurrentPhoneType(mCall.getSubId());
    }
}
