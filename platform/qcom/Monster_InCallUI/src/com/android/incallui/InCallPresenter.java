...
import com.mst.manager.OverTurnManager;
import com.mst.manager.VibratorManager;
...
public class InCallPresenter implements CallList.Listener,
    ...
    private AnswerPresenterMst mAnswerPresenter = new AnswerPresenterMst();
...
//        InCallZoomController.getInstance().setUp(mContext);
        ...
        //add by lgy
        RecordManager.getInstance().setUp(mContext);
        VibratorManager.getInstance().setUp(mContext);
        AntiTouchManager.getInstance().setUp(mContext);
        OverTurnManager.getInstance().setUp(mContext);
...
//        removeDetailsListener(SessionModificationCauseNotifier.getInstance());
        ...
        //add by lgy
        RecordManager.getInstance().tearDown();
        VibratorManager.getInstance().tearDown();
        AntiTouchManager.getInstance().tearDown();
        OverTurnManager.getInstance().tearDown();
    }
...
//        }
        ...
        //add by lgy
    	if(mInCallState != InCallState.INCOMING && FloatWindowManager.isWindowShowing()) {
...
//        	if((mInCallState == InCallState.INCOMING || mInCallActivity.hasPendingDialogs()) 
//            		&& keyguardManager.isKeyguardLocked()) {
          if(keyguardManager.isKeyguardLocked()) {
        		mInCallActivity.updateTransparentBackground(true);
	        } else {
	            mInCallActivity.updateTransparentBackground(false);
	        }
        }
    }
    ...
    public void onIncomingCall(Call call) {
        ...
        Log.i(this, "onIncomingCall Phone switching state: " + oldState + " -> " + newState);
        mInCallState = newState;
...
//            mInCallActivity.getCallCardFragment().animateForNewOutgoingCall();
    ...
    public void addIncomingCallListener(IncomingCallListener listener) {
        ...
        //modify by lgy for speed
        if(!mIncomingCallListeners.contains(listener)) {
...
//        mInCallActivity.getWindow().setStatusBarColor(color);
    ...
    AnswerPresenterMst getAnswerPresenter() {
