...
import com.mst.ui.MstAnswerArrow;
...
public class GlowPadView extends View {
    ...
    public GlowPadView(Context context, AttributeSet attrs) {
        ...
        mMstAnswerArrow = new MstAnswerArrow(context); 
    }
...
//                }
    ...
    private void dispatchTriggerEvent(int whichTarget) {
        ...
        if (mOnTriggerListener != null) {
        	mHasTrigger = true;
            Log.i("lgy", "dispatchTriggerEvent ");
    ...
    private void hideTargets(boolean animate, boolean expanded) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            target.setState(TargetDrawable.STATE_INACTIVE);
            if(i == 0) {
                mTargetAnimations.add(Tweener.to(target, 0,
                        "ease", interpolator,
                        "alpha", 0.0f,
                        "scaleX", targetScale,
                        "scaleY", targetScale,
        ...
        mMstAnswerArrow.waveManager.setAlpha(0f);
        mTargetAnimations.add(Tweener.to(mMstAnswerArrow.waveManager, duration,
                "ease", interpolator,
                "alpha", new float[]{0f, 1f},
                "delay", delay,
                "onUpdate", mUpdateListener));
    ...
    private void showTargets(boolean animate) {
        ...
        mMstAnswerArrow.waveManager.setAlpha(1.0f);
        mTargetAnimations.add(Tweener.to(mMstAnswerArrow.waveManager, duration,
                "ease", Ease.Cubic.easeOut,
                "alpha", new float[]{1.0f, 0f},
                "delay", delay,
                "onUpdate", mUpdateListener));	
    ...
    private void internalSetTargetResources(int resourceId) {
        ...
        if (mMaxTargetWidth != maxWidth || mMaxTargetHeight != maxHeight) {
            ...
            mMstAnswerArrow.setCenter(mWaveCenterX, mWaveCenterY);
        }
    }
    /**
...
//            }
          ...
          final AnimationBundleV2 waveAnimations = mWaveAnimationsV2;

          // Don't do a wave if there's already one in progress
          if (waveAnimations.size() > 0 && waveAnimations.get(0).isRunning()) {
              long t = waveAnimations.get(0).getCurrentPlayTime();
              if (t < WAVE_ANIMATION_DURATION/2) {
                  doWaveAnimation = false;
              }
          }
...
//                    }
                if(ntargets == 2) {
                    for (int i = 0; i < ntargets; i++) {
                        ...
                        double targetMaxRad;
                        if(i == 0) {
                            targetMinRad = - Math.PI ; 
                            targetMaxRad = 0;
                        } else {
                            targetMinRad =  0 ; 
                            targetMaxRad =  Math.PI;
                        }
                    ...
                    //向下是正的，方向顺时针，从最左的－PI 转一圈到＋PI 
                     for (int i = 0; i < ntargets; i++) {
                          ...
                          double targetMaxRad;
                          if(i == 0) {
                              targetMinRad = - Math.PI + Math.PI /6; 
                              targetMaxRad = 0;
                          } else if(i ==1) {
                              targetMinRad =  Math.PI - Math.PI / 6; 
                              targetMaxRad = Math.PI + Math.PI / 6;
                          } else {
                              targetMinRad = 0; 
                              targetMaxRad = Math.PI - Math.PI / 6 ;
                          }
        ...
        if (activeTarget != -1) {
            ...
            updateGlowPositionWhenMove(x, y);
        } else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mMstAnswerArrow.setScale(mRingScaleFactor);
        mMstAnswerArrow.setCenter(newWaveCenterX, newWaveCenterY);
    ...
    private void updateTargetPositions(float centerX, float centerY) {
        ...
        float alpha;
        if(size == 3) {
            alpha = (float) (-2.0f * Math.PI / 4);
        } else {
            alpha = (float) (-2.0f * Math.PI / size);
        }
        //add by lgy
        final float base = (float) (- Math.PI * 0.5f );
...
//            targetIcon.setY(getRingHeight() / 2 * (float) Math.sin(angle));
            if(size ==3 && i ==1) {
                int smsOffset = getContext().getResources().getDimensionPixelOffset(
                        R.dimen.sms_reject_offset);
                targetIcon.setX(smsOffset * (float) Math.cos( base +angle));
            } else {
                targetIcon.setX(getRingWidth() / 2 * (float) Math.cos( base +angle));
            }
          targetIcon.setY(getRingHeight() / 2 * (float) Math.sin(base + angle));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        if(!mIsTargetShow || mAnimatingTargets) {
            mMstAnswerArrow.draw(canvas);
        }
//        mOuterRing.draw(canvas);
    ...
    private MstAnswerArrow mMstAnswerArrow;
    private static int mState = STATE_IDLE;
    private boolean isTouchState() {
    	return mState != STATE_FINISH && mState != STATE_IDLE;
    }
    private boolean mIsTargetShow = false;
    
    private void updateGlowPositionWhenMove(float x, float y) {
        float dx = x - mOuterRing.getX();
        float dy = y - mOuterRing.getY();
        dx *= 1f / mRingScaleFactor;
        dy *= 1f / mRingScaleFactor; 
        if( y < 0) {
        	 mPointCloud.glowManager.setX(mOuterRing.getX());
        } else {
        	 mPointCloud.glowManager.setX(mOuterRing.getX() + dx);
        }
...
//        if( y > mOuterRadius / 4 ) {
//        	if(!mAnimatingTargets && !mIsTargetShow) {
//        		showTargets(true);        		
//        	}
//        } else {
//        	if(mIsTargetShow && !mAnimatingTargets) {
...
//        }
        ...
        if(y < 0) {        	
	        float rate =1 + y/mOuterRadius;
	        Log.i("lgy", "rate = " + rate);
	        if(rate < 0) rate = 0;
	        if(rate > 1) rate = 1;
	        this.setAlpha(rate);
	        mMstAnswerArrow.waveManager.setDeltaY2((1-rate) * 100);
	        InCallApp.getInCallActivity().getCallCardFragment().configCallCardFragment(rate);
        } else {
            float rate = y/mOuterRadius;
	        if(rate < 0) rate = 0;
	        if(rate > 1) rate = 1;
	        mMstAnswerArrow.waveManager.setDeltaY2(- rate * 100);
        }     
    }
    
    private AnimationBundleV2 mWaveAnimationsV2 = new AnimationBundleV2();
    private class AnimationBundleV2 extends ArrayList<ObjectAnimator> {
        private static final long serialVersionUID = 0xA84D78726F127468L;
        private boolean mSuspended;
    ...
    private void startWaveAnimationV2() {
        Log.i("lgy", "startWaveAnimationV2 ");
        mWaveAnimationsV2.cancel();        
        mWaveAnimationsV2.add(getArrowWiggleAnimator());        
        mWaveAnimationsV2.add(getCenterHandleWiggleAnimator());        
        mWaveAnimationsV2.start();
    }
    
    private ObjectAnimator getCenterHandleWiggleAnimator() {        	    	
        //0.08s~0.28s~0.28s~0.08s   0~30%~100~30%~0
    	mHandleDrawable.setY(-10);
    	PropertyValuesHolder pvh = PropertyValuesHolder.ofFloat("y",
				-10f, 10f);
    ...
    private ObjectAnimator getArrowWiggleAnimator() {        	    	
        //0.08s~0.28s~0.28s~0.08s   0~30%~100~30%~0
    	mMstAnswerArrow.waveManager.setDeltaY(-10);
    	PropertyValuesHolder pvh = PropertyValuesHolder.ofFloat("DeltaY",
				-10f, 10f);
		ObjectAnimator animator = ObjectAnimator.ofPropertyValuesHolder(
				mMstAnswerArrow.waveManager, pvh);
		animator.setInterpolator(new AccelerateDecelerateInterpolator());
		animator.addUpdateListener(mUpdateListener);
		animator.setDuration(250);
		animator.setRepeatMode(Animation.REVERSE);
    ...
    private void restoreDimens() {
        Log.i("lgy", "restoreDimens ");
    	if(!mHasTrigger) {
	        this.setAlpha(1);
	        if(InCallApp.getInCallActivity() != null) {
		        InCallApp.getInCallActivity().getCallCardFragment().configCallCardFragment(2);
	        }
    	} else {
    		mHasTrigger = false;
    	}
        mMstAnswerArrow.waveManager.setDeltaY2(0);
    }    
