 ...
 */
public class PinnedHeaderListView extends AutoScrollListView implements
	...
	public interface PinnedHeaderAdapter {
		 ...
		 */
		View getPinnedHeaderView(int viewIndex, View convertView,
				ViewGroup parent);
		...
		/**
		 * Returns the list position to scroll to if the pinned header is
		 * touched. Return -1 if the list does not need to be scrolled.
		 */
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (mAdapter != null) {
			...
			for (int i = 0; i < mSize; i++) {
				...
				mHeaders[i].view = mAdapter.getPinnedHeaderView(i,
						mHeaders[i].view, this);
			}

			mAnimationTargetTime = System.currentTimeMillis()
					+ mAnimationDuration;
		...
		if (selectedView != null) {
			if (selectedView.getTop() < windowTop) {
				...
				setSelectionFromTop(position,
						windowBottom - selectedView.getHeight());
			}
		}
	 ...
	 * @param animate
	 *            true if the transition to the new coordinate should be
	 *            animated
	 */
	...
	public void setFadingHeader(int viewIndex, int position, boolean fade) {
		...
		if (fade) {
			...
			if (bottom < headerHeight) {
				...
				header.alpha = MAX_ALPHA * (headerHeight + portion)
						/ headerHeight;
	...
	public void setHeaderInvisible(int viewIndex, boolean animate) {
		PinnedHeader header = mHeaders[viewIndex];
		if (header.visible && (animate || header.animating)
				&& header.state == BOTTOM) {
			header.sourceY = header.y;
			if (!header.animating) {
				header.visible = true;
	...
	private void ensurePinnedHeaderLayout(int viewIndex) {
		...
		if (view.isLayoutRequested()) {
			...
			if (layoutParams != null && layoutParams.width > 0) {
				...
				widthSpec = View.MeasureSpec.makeMeasureSpec(mHeaderWidth,
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mScrollState == SCROLL_STATE_IDLE) {
			...
			for (int i = mSize; --i >= 0;) {
				PinnedHeader header = mHeaders[i];
				// For RTL layouts, this also takes into account that the
				// scrollbar is on the left
				// side.
				final int padding = getPaddingLeft();
				if (header.visible && header.y <= y
						&& header.y + header.height > y && x >= padding
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (hasVisibleHeaders) {
			...
			// If the first item is visible and if it has a positive top that is
			// greater than the
			// first header's assigned y-value, use that for the first header's
			// y value. This way,
			...
			if (mSize > 0 && getFirstVisiblePosition() == 0) {
				...
				if (firstHeader != null) {
					int firstHeaderTop = firstChild != null ? firstChild
							.getTop() : 0;
					firstHeader.y = Math.max(firstHeader.y, firstHeaderTop);
				}
			}

			// First draw top headers, then the bottom ones to handle the Z axis
			// correctly
			for (int i = mSize; --i >= 0;) {
				PinnedHeader header = mHeaders[i];
				if (header.visible
						&& (header.state == TOP || header.state == FADING)) {
	...
	private void drawHeader(Canvas canvas, PinnedHeader header, long currentTime) {
		if (header.animating) {
			...
			if (timeLeft <= 0) {
				...
				header.y = header.targetY + (header.sourceY - header.targetY)
						* timeLeft / mAnimationDuration;
			}
		}
		if (header.visible) {
			View view = header.view;
			int saveCount = canvas.save();
			int translateX = ViewUtil.isViewLayoutRtl(this) ? getWidth()
					- mHeaderPaddingStart - view.getWidth()
					: mHeaderPaddingStart;
