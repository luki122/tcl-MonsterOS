...
/**
 * Arranges contacts favorites according to provided {@link DisplayType}. Also
 * allows for a configurable number of columns and {@link DisplayType}
 */
public class ContactTileAdapter extends BaseAdapter {
	...
	/**
	 * Index of the first NON starred contact in the {@link Cursor} Only valid
	 * when {@link DisplayType#STREQUENT} is true
	 */
	...
	/**
	 * Configures the adapter to filter and display contacts using different
	 * view types. TODO: Create Uris to support getting Starred_only and
	 * Frequent_only cursors.
	 */
	public enum DisplayType {
		...
		/**
		 * Display all contacts from a group in the cursor Use
		 * {@link com.android.contacts.GroupMemberLoader} when passing
		 * {@link Cursor} into loadFromCusor method.
		 * 
		 * Group member logic has been moved into GroupMemberTileAdapter. This
		 * constant is still needed by calling classes.
		 */
		GROUP_MEMBERS
	}

	public ContactTileAdapter(Context context,
			ContactTileView.Listener listener, int numCols,
		...
		mPaddingInPixels = mContext.getResources().getDimensionPixelSize(
				R.dimen.contact_tile_divider_padding);
		mWhitespaceStartEnd = mContext.getResources().getDimensionPixelSize(
				R.dimen.contact_tile_start_end_whitespace);
	...
	/**
	 * Sets the column indices for expected {@link Cursor} based on
	 * {@link DisplayType}.
	 */
	 ...
	 * 
	 * @param cursor
	 *            The cursor to get number of frequents from.
	 */
	protected void saveNumFrequentsFromCursor(Cursor cursor) {
		...
		case STARRED_ONLY:
			...
			break;
		case STREQUENT:
			mNumFrequents = cursorIsValid(cursor) ? cursor.getCount()
					- mDividerPosition : 0;
			break;
		case FREQUENT_ONLY:
	...
	/**
	 * Iterates over the {@link Cursor} Returns position of the first NON
	 * Starred Contact Returns -1 if {@link DisplayType#STARRED_ONLY} Returns 0
	 * if {@link DisplayType#FREQUENT_ONLY}
	 */
	protected int getDividerPosition(Cursor cursor) {
		...
		case FREQUENT_ONLY:
			...
			return 0;
		default:
			throw new IllegalStateException("Unrecognized DisplayType "
					+ mDisplayType);
		}
	}

	protected ContactEntry createContactEntryFromCursor(Cursor cursor,
			int position) {
		...
		contact.namePrimary = (name != null) ? name : mResources
		...
		if (statusMessage == null && presence != 0) {
			statusMessage = ContactStatusUtil.getStatusString(mContext,
					presence);
		}
		contact.status = statusMessage;

		if (!cursor.isNull(mAccountTypeIndex)
	...
	public int getCount() {
		...
		case STREQUENT:
			...
			// Compute the frequent row count which is 1 plus the number of
			// frequents
	...
	/**
	 * Returns the number of rows required to show the provided number of
	 * entries with the current number of columns.
	 */
	...
	/**
	 * Returns an ArrayList of the {@link ContactEntry}s that are to appear on
	...
	public ArrayList<ContactEntry> getItem(int position) {
		...
		switch (mDisplayType) {
		case FREQUENT_ONLY:
			resultList.add(createContactEntryFromCursor(mContactCursor,
					position));
			break;
		case STARRED_ONLY:
		...
		case STREQUENT:
			if (position < getRowCount(mDividerPosition)) {
				...
				/*
				 * Current position minus how many rows are before the divider
				 * and Minus 1 for the divider itself provides the relative
				 * index of the frequent contact being displayed. Then add the
				 * dividerPostion to give the offset into the contacts cursor to
				 * get the absoulte index.
				 */
				contactIndex = position - getRowCount(mDividerPosition) - 1
						+ mDividerPosition;
				resultList.add(createContactEntryFromCursor(mContactCursor,
						contactIndex));
			}
	...
	public long getItemId(int position) {
		...
		// we can not determine a stable id. But as we don't rely on ListView's
		// selection,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (itemViewType == ViewTypes.DIVIDER) {
			...
			final TextView textView = (TextView) (convertView == null ? getDivider()
					: convertView);
		...
		contactTileRowView
				.configureRow(contactList, position == getCount() - 1);
		return contactTileRowView;
	}

	/**
	 * Divider uses a list_seperator.xml along with text to denote the most
	...
	private void setDividerPadding(TextView headerTextView, boolean isFirstRow) {
		MoreContactUtils.setHeaderViewBottomPadding(mContext, headerTextView,
				isFirstRow);
	}
	...
	public int getItemViewType(int position) {
		/*
		 * Returns view type based on {@link DisplayType}. {@link
		 * DisplayType#STARRED_ONLY} and {@link DisplayType#GROUP_MEMBERS} are
		 * {@link ViewTypes#STARRED}. {@link DisplayType#FREQUENT_ONLY} is
		 * {@link ViewTypes#FREQUENT}. {@link DisplayType#STREQUENT} mixes both
		 * {@link ViewTypes} and also adds in {@link ViewTypes#DIVIDER}.
		 */
		switch (mDisplayType) {
		case STREQUENT:
			if (position < getRowCount(mDividerPosition)) {
	...
	private class ContactTileRow extends FrameLayout {
		...
		/**
		 * Configures the row to add {@link ContactEntry}s information to the
		 * views
		 */
		public void configureRow(ArrayList<ContactEntry> list, boolean isLastRow) {
			int columnCount = mItemViewType == ViewTypes.FREQUENT ? 1
					: mColumnCount;

			// Adding tiles to row and filling in contact information
			for (int columnCounter = 0; columnCounter < columnCount; columnCounter++) {
				ContactEntry entry = columnCounter < list.size() ? list
						.get(columnCounter) : null;
				addTileFromEntry(entry, columnCounter, isLastRow);
			}
		}

		private void addTileFromEntry(ContactEntry entry, int childIndex,
				boolean isLastRow) {
			final ContactTileView contactTile;

			if (getChildCount() <= childIndex) {
				contactTile = (ContactTileView) inflate(mContext, mLayoutResId,
						null);
				// Note: the layoutparam set here is only actually used for
				// FREQUENT.
				// We override onMeasure() for STARRED and we don't care the
				// layout param there.
				Resources resources = mContext.getResources();
				FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(
						ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				params.setMargins(mWhitespaceStartEnd, 0, mWhitespaceStartEnd,
						0);
			...
			case ViewTypes.STARRED:
				// Set padding between tiles. Divide mPaddingInPixels between
				// left and right
				// tiles as evenly as possible.
				contactTile.setPaddingRelative((mPaddingInPixels + 1) / 2, 0,
						mPaddingInPixels / 2, 0);
				break;
			case ViewTypes.FREQUENT:
				contactTile
						.setHorizontalDividerVisibility(isLastRow ? View.GONE
								: View.VISIBLE);
				break;
			default:
				break;
		...
		private void onLayoutForTiles() {
			...
			// Amount of margin needed on the left is based on difference
			// between offset and padding
			int childLeft = mWhitespaceStartEnd - (mPaddingInPixels + 1) / 2;

			// Just line up children horizontally.
			for (int i = 0; i < count; i++) {
				final int rtlAdjustedIndex = ViewUtil.isViewLayoutRtl(this) ? count
						- i - 1
						: i;
		...
		private void onMeasureForTiles(int widthMeasureSpec) {
			...
			// If we have a remainder, some tiles will have 1 pixel larger width
			// than its height.
			//
			...
			final int totalWhitespaceInPixels = (mColumnCount - 1)
					* mPaddingInPixels + mWhitespaceStartEnd * 2;

			// Preferred width / height for images (excluding the padding).
			// The actual width may be 1 pixel larger than this if we have a
			// remainder.
			final int imageSize = (width - totalWhitespaceInPixels)
					/ mColumnCount;
			final int remainder = width - (imageSize * mColumnCount)
					- totalWhitespaceInPixels;
