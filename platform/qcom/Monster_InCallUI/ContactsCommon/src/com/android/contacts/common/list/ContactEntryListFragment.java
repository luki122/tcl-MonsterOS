...
public abstract class ContactEntryListFragment<T extends ContactEntryListAdapter>
		extends Fragment implements OnItemClickListener, OnScrollListener,
		OnFocusChangeListener, OnTouchListener, OnItemLongClickListener,
	...
	/**
	 * Used to save the scrolling state of the list when the fragment is not
	 * recreated.
	 */
	...
	/**
	 * @param position
	 *            Please note that the position is already adjusted for header
	 *            views, so "0" means the first list item below header views.
	 */
	...
	public void onSaveInstanceState(Bundle outState) {
		...
		outState.putBoolean(KEY_VISIBLE_SCROLLBAR_ENABLED,
		...
		if (mListView != null) {
			outState.putParcelable(KEY_LIST_STATE,
	...
	public void restoreSavedState(Bundle savedState) {
		...
		mSectionHeaderDisplayEnabled = savedState
				.getBoolean(KEY_SECTION_HEADER_DISPLAY_ENABLED);
		...
		mAdjustSelectionBoundsEnabled = savedState
				.getBoolean(KEY_ADJUST_SELECTION_BOUNDS_ENABLED);
		mIncludeProfile = savedState.getBoolean(KEY_INCLUDE_PROFILE);
		mSearchMode = savedState.getBoolean(KEY_SEARCH_MODE);
		mVisibleScrollbarEnabled = savedState
				.getBoolean(KEY_VISIBLE_SCROLLBAR_ENABLED);
		...
		for (int i = 0; i < partitionCount; i++) {
			...
			if (partition instanceof DirectoryPartition) {
				...
				if (directoryPartition.getStatus() == DirectoryPartition.STATUS_NOT_LOADED) {
					if (directoryPartition.isPriorityDirectory()
							|| !mLoadPriorityDirectoriesOnly) {
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		if (id == DIRECTORY_LOADER_ID) {
			...
			loader.setLocalInvisibleDirectoryEnabled(ContactEntryListAdapter.LOCAL_INVISIBLE_DIRECTORY_ENABLED);
			return loader;
		} else {
			CursorLoader loader = createCursorLoader(mContext);
			long directoryId = args != null
					&& args.containsKey(DIRECTORY_ID_ARG_KEY) ? args
		...
		return new CursorLoader(context, null, null, null, null, null) {
			...
			protected Cursor onLoadInBackground() {
				try {
					...
					// We don't even know what the projection should be, so no
					// point trying to
					// return an empty MatrixCursor with the correct projection
					// here.
					Log.w(TAG,
	...
	private void startLoadingDirectoryPartition(int partitionIndex) {
		DirectoryPartition partition = (DirectoryPartition) mAdapter
				.getPartition(partitionIndex);
	 ...
	 * request.
	 */
	private void loadDirectoryPartitionDelayed(int partitionIndex,
		...
		mDelayedDirectorySearchHandler.sendMessageDelayed(msg,
				DIRECTORY_SEARCH_DELAY_MILLIS);
	}

	/**
	 * Loads the directory partition.
	 */
	protected void loadDirectoryPartition(int partitionIndex,
			DirectoryPartition partition) {
		Bundle args = new Bundle();
			...
			if (isSearchMode()) {
				...
				if (directorySearchMode != DirectoryListLoader.SEARCH_MODE_NONE) {
					if (mDirectoryListStatus == STATUS_NOT_LOADED) {
						...
						getLoaderManager().initLoader(DIRECTORY_LOADER_ID,
								null, this);
					} else {
						startLoading();
					}
	...
	/**
	 * Shows a view at the top of the list with a pseudo local profile prompting
	 * the user to add a local profile. Default implementation does nothing.
	 */
	...
	/**
	 * Enter/exit search mode. This is method is tightly related to the current
	 * query, and should only be called by {@link #setQueryString}.
	 * 
	 * Also note this method doesn't call {@link #reloadData()};
	 * {@link #setQueryString} does it.
	 */
	...
	public void setQueryString(String queryString, boolean delaySelection) {
		if (!TextUtils.equals(mQueryString, queryString)) {
			if (mShowEmptyListForEmptyQuery && mAdapter != null
					&& mListView != null) {
	...
	protected void onCreateView(LayoutInflater inflater, ViewGroup container) {
		...
		ContactListViewUtils.applyCardPaddingToView(getResources(), mListView,
				mView);
	}
	...
	public void onHiddenChanged(boolean hidden) {
		...
		if (getActivity() != null && getView() != null && !hidden) {
			// If the padding was last applied when in a hidden state, it may
			// have been applied
			// incorrectly. Therefore we need to reapply it.
			ContactListViewUtils.applyCardPaddingToView(getResources(),
					mListView, getView());
		}
	}
	...
	public void onPause() {
		...
		mListViewTopOffset = (v == null) ? 0 : (v.getTop() - mListView
				.getPaddingTop());
	...
	public void onResume() {
		...
		// This has to be done manually because if the list view has its
		// emptyView set,
		// the scrolling state will be reset when clearPartitions() is called on
		// the adapter.
	...
	public void onPickerResult(Intent data) {
		throw new UnsupportedOperationException(
				"Picker result handler is not implemented.");
	}
