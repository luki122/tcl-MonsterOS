...
/**
 * A cursor adapter for the {@link ContactsContract.Contacts#CONTENT_TYPE}
 * content type. Also includes support for including the
 * {@link ContactsContract.Profile} record in the list.
 */
public abstract class ContactListAdapter extends ContactEntryListAdapter {
	...
	public boolean isSelectedContact(int partitionIndex, Cursor cursor) {
		...
		return directoryId != Directory.DEFAULT
				...
				&& getSelectedContactId() == cursor
						.getLong(ContactQuery.CONTACT_ID);
	}

	@Override
	protected ContactListItemView newView(Context context, int partition,
			Cursor cursor, int position, ViewGroup parent) {
		ContactListItemView view = super.newView(context, partition, cursor,
				position, parent);
	...
	protected void bindSectionHeaderAndDivider(ContactListItemView view,
	...
	protected void bindPhoto(final ContactListItemView view,
			int partitionIndex, Cursor cursor) {
		...
		if (!cursor.isNull(ContactQuery.CONTACT_ACCOUNT_TYPE)
			...
			final String accountType = cursor
					.getString(ContactQuery.CONTACT_ACCOUNT_TYPE);
			final String accountName = cursor
					.getString(ContactQuery.CONTACT_ACCOUNT_NAME);
			account = new Account(accountName, accountType);
		}
		if (photoId != 0) {
			getPhotoLoader().loadThumbnail(view.getPhotoView(), photoId,
					account, false, getCircularPhotos(), null);
		} else {
			final String photoUriString = cursor
					.getString(ContactQuery.CONTACT_PHOTO_URI);
			...
			getPhotoLoader().loadDirectoryPhoto(view.getPhotoView(), photoUri,
					account, false, getCircularPhotos(), request);
		}
	}

	protected void bindNameAndViewId(final ContactListItemView view,
			Cursor cursor) {
	...
	protected void bindPresenceAndStatusMessage(final ContactListItemView view,
			Cursor cursor) {
		view.showPresenceAndStatusMessage(cursor,
				ContactQuery.CONTACT_PRESENCE_STATUS,
				ContactQuery.CONTACT_CONTACT_STATUS);
	}

	protected void bindSearchSnippet(final ContactListItemView view,
			Cursor cursor) {
		...
		while (cursor.moveToNext()) {
			if (mSelectedContactLookupKey != null) {
				String lookupKey = cursor
						.getString(ContactQuery.CONTACT_LOOKUP_KEY);
			...
			if (mSelectedContactId != 0
					&& (mSelectedContactDirectoryId == Directory.DEFAULT || mSelectedContactDirectoryId == Directory.LOCAL_INVISIBLE)) {
				long contactId = cursor.getLong(ContactQuery.CONTACT_ID);
