...
public class DefaultContactListAdapter extends ContactListAdapter {
	...
	private static final String TIME_SINCE_LAST_USED_SEC = "(strftime('%s', 'now') - "
			+ Contacts.LAST_TIME_CONTACTED + "/1000)";

	private static final String STREQUENT_SORT = "(CASE WHEN "
			+ TIME_SINCE_LAST_USED_SEC + " < " + LAST_TIME_USED_3_DAYS_SEC
			+ " THEN 0 " + " WHEN " + TIME_SINCE_LAST_USED_SEC + " < "
			+ LAST_TIME_USED_7_DAYS_SEC + " THEN 1 " + " WHEN "
			+ TIME_SINCE_LAST_USED_SEC + " < " + LAST_TIME_USED_14_DAYS_SEC
			+ " THEN 2 " + " WHEN " + TIME_SINCE_LAST_USED_SEC + " < "
			+ LAST_TIME_USED_30_DAYS_SEC + " THEN 3 " + " ELSE 4 END), "
			+ Contacts.TIMES_CONTACTED + " DESC, " + Contacts.STARRED + " DESC";
	...
	public void configureLoader(CursorLoader loader, long directoryId) {
		if (loader instanceof ProfileAndContactsLoader) {
			((ProfileAndContactsLoader) loader)
					.setLoadProfile(shouldIncludeProfile());
		}
		...
		if (isSearchMode()) {
			...
			if (TextUtils.isEmpty(query)) {
				...
				final Builder builder = ContactsCompat.getContentUri()
						.buildUpon();
				appendSearchParameters(builder, query, directoryId);
				loader.setUri(builder.build());
				loader.setProjection(getProjection(true));
				if (flags.getBoolean(Experiments.FLAG_SEARCH_STREQUENTS_FIRST,
						false)) {
	...
	private void appendSearchParameters(Builder builder, String query,
			long directoryId) {
		...
		builder.appendQueryParameter(SearchSnippets.DEFERRED_SNIPPETING_KEY,
				"1");
	}
	...
	protected void configureUri(CursorLoader loader, long directoryId,
		...
		if (filter != null
			...
			if (lookupKey != null) {
				uri = Uri.withAppendedPath(Contacts.CONTENT_LOOKUP_URI,
						lookupKey);
			} else {
	...
	private void configureSelection(CursorLoader loader, long directoryId,
		...
		case ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS: {
			// We have already added directory=0 to the URI, which takes care of
			// this
			// filter
			break;
		}
		case ContactListFilter.FILTER_TYPE_SINGLE_CONTACT: {
			// We have already added the lookup key to the URI, which takes care
			// of this
			// filter
			break;
		}
		...
		case ContactListFilter.FILTER_TYPE_ACCOUNT: {
			// We use query parameters for account filter, so no selection to
			// add here.
			break;
		}
		...
		case ContactListFilter.FILTER_TYPE_CAN_SAVE_EMAIL: {
			...
			if (!TextUtils.isEmpty(emailFilter)) {
				appendUriQueryParameterWithoutSim(loader,
						RawContacts.ACCOUNT_NAME, emailFilter);
			}
			break;
		}
		}
	...
	protected void bindView(View itemView, int partition, Cursor cursor,
		...
		if (isQuickContactEnabled()) {
			bindQuickContact(view, partition, cursor,
					...
					ContactQuery.CONTACT_ACCOUNT_TYPE,
					ContactQuery.CONTACT_ACCOUNT_NAME);
		} else {
			if (getDisplayPhotos()) {
