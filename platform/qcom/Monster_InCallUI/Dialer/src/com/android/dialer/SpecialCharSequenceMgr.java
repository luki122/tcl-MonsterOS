...
/* ----------|----------------------|----------------------|----------------- */
/* 08/17/2016|      xian.jiang      |     Task-2670032     |[GMS][Hidenmenu]  */
/*           |                      |                      |input "###666#"   */
/*           |                      |                      |do disabling GMS  */
/* ----------|----------------------|----------------------|----------------- */
/* ========================================================================== */
...
//[FEATURE]-Add-BEGIN by TCTNB.xian.jiang,08/17/2016,Task-2670032
import android.content.pm.ApplicationInfo;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.os.SystemProperties;
//[FEATURE]-Add-END by TCTNB.xian.jiang
...
public class SpecialCharSequenceMgr {
    ...
    //[FEATURE]-Add-BEGIN by TCTNB.xian.jiang,08/17/2016,Task-2670032,
    //[GMS][Hidenmenu]input "###666#" do disabling GMS
    private static final String GOOGLE_APP_CONFIG_MODE = "###666#";
    ...
    public static boolean handleChars(Context context, String input, EditText textField) {
        ...
        if (handleDeviceIdDisplay(context, dialString)
                ...
                || handleGoogleApp(context, dialString)  //[FEATURE]-Add by TCTNB.xian.jiang,08/17/2016,Task-2670032,
    ...
    //[FEATURE]-Add-BEGIN by TCTNB.xian.jiang,08/17/2016,Task-2670032,
    //[GMS][Hidenmenu]input "###666#" do disabling GMS
    private static boolean handleGoogleApp(Context context, String dialString) {
        if (dialString.equals(GOOGLE_APP_CONFIG_MODE)) {//0=disable, 1=enable, 2=without google apk, 3=not initial
            Log.i(TAG, "handleGoogleAppConfig");
            Intent intent = new Intent(ACTION_GOOGLE_APP_CHANGED);
            String enabled = SystemProperties.get(PROPERTY_GOOGLEAPP_ENABLED, "3");
            String show = "";

            Log.i(TAG,"sys.boot_completed = " + SystemProperties.get("sys.boot_completed"));
            if ((enabled.equals("3")) && ("1".equals(SystemProperties.get("sys.boot_completed")))) {
                Log.i(TAG,"enabled.equals3 .....");
                ...
                for (int i = 0; i < packageInfos.size(); i++) {
                    ...
                    Log.i(TAG, "info="+info.packageName+" info.flags="+info.flags);
                    if (((info.packageName).startsWith("com.google")) /*&& (((info.flags & ApplicationInfo.FLAG_SYSTEM) != 0))*/) {
                        Log.i(TAG,"  set PROPERTY_GOOGLEAPP_ENABLED to 1");
            ...
            if (enabled.equals("0")) {
                show = context.getResources().getString(R.string.google_app_enable);
                intent.putExtra("googleConfig", "1");
                context.sendBroadcast(intent);
            } else if (enabled.equals("1")) {
                show = context.getResources().getString(R.string.google_app_disable);
                intent.putExtra("googleConfig", "0");
                context.sendBroadcast(intent);
            } else if (enabled.equals("2")) {
                show = context.getResources().getString(R.string.google_app_no);
            } else if (enabled.equals("3")) {
                show = context.getResources().getString(R.string.not_ready);
            }

            AlertDialog alert = new AlertDialog.Builder(context)
                .setTitle(R.string.google_config_title)
                .setMessage(show)
                .setPositiveButton(android.R.string.ok, null)
                .setCancelable(false)
                .show();
    ...
    static boolean handleCallTimeCountDisplay(Context context, String input) {
        if (input.equals(MMI_CALLDURATION_COUNT_DISPLAY)) {
    ...
    static void showCallTimeCountPanel(Context context) {
        ...
        data = tm.getCallDurationTime();

        //change  by guangchao.su for Defect 1050988,on 12/23/2015
        String durationInSec = "0";
        String threeMinitesDate = "";
        String[] time = null;//Add
        if (data.contains("||")) {
        ...
        String show;
        //[BUGFIX]-Mod by AMNJ yang-wang, 2016/03/21, PR-1845737
        if (!"".equals(threeMinitesDate)) {
            //show = parseDuration(durationInSec) + '\n' + '\n' + threeMinitesDate;
        ...
        //[BUGFIX]-Mod by AMNJ song.fei, 2015/12/09, PR-1059208
    }
