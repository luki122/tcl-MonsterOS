...
import com.android.phone.UssdAlertActivity;
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/12/2016, SOLUTION-2466214
//[HOMO][MSRU][IOT2][USSD] USSD menu which have internal menu not works
    public static UssdAlertActivity mUssdActivity = null;
...
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    ...
    private static Call.State previousCallState;
    private static PhoneConstants.State previousPhoneState;
    private static Vibrator vibrator;
    private static void reminder(AsyncResult mResult) {
        Phone fgPhone = (Phone)(mResult.result);
        CallManager mCM = CallManager.getInstance();
        PhoneConstants.State state = fgPhone.getState();
        Call.State fgCallState = fgPhone.getForegroundCall().getState();
        android.util.Log.e("TelephonyConnection","dingyicccc reminder--fgCallState :"+fgCallState+"  ,State:"+state);
        android.util.Log.e("TelephonyConnection","dingyicccc reminder--previousCallState :"+previousCallState+"  ,previousPhoneState:"+previousPhoneState);
        if ((fgCallState == Call.State.ACTIVE && (Call.State.DIALING == previousCallState || Call.State.ALERTING == previousCallState))
                || (state == PhoneConstants.State.OFFHOOK && PhoneConstants.State.RINGING == previousPhoneState)
                || (PhoneConstants.State.IDLE == state && PhoneConstants.State.IDLE != previousPhoneState)) {
            if (mCM.hasActiveBgCall()) {
                return;
            }
            SharedPreferences mSP = fgPhone.getContext()
                    .getSharedPreferences("com.android.phone_preferences",
                            Context.MODE_PRIVATE);
            boolean defPreferenceValue = fgPhone.getContext().getResources().getBoolean(R.bool.def_phone_vibrateReminder_on);
            boolean vibratorReminder = mSP.getBoolean("button_reminder_key" + fgPhone.getPhoneId(), defPreferenceValue);
            ...
            if (vibratorReminder) {
                vibrator = (Vibrator) fgPhone.getContext().getSystemService(Context.VIBRATOR_SERVICE);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case PHONE_STATE_CHANGED:
                        ...
                        previousCallState = fgPhone.getForegroundCall().getState();
                    break;
                //ADD-END by Dingyi  2016/08/12 SOLUTION 2473833
            }
        }
...
//[HOMO][MSRU][IOT2][USSD] USSD menu which have internal menu not works
        ...
        SharedPreferences settings = context.getSharedPreferences("com.android.phone_preferences", Context.MODE_WORLD_READABLE);
        String preState = settings.getString("phone_mmi_pre_state", "CANCELLED");
        if (DBG) log("displayMMICompleteExt preState: " + preState );
        settings.edit().putString("phone_mmi_pre_state", state.toString()).commit();
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
        ...
        switch (state) {
            case PENDING:
                ...
                //[SOLUTION]-Add-BEGIN by TCTNB.(Yubin.Ying), 08/16/2016, SOLUTION-2497601
                if (text != null) {
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/12/2016, SOLUTION-2466214
//[HOMO][MSRU][IOT2][USSD] USSD menu which have internal menu not works
                boolean isShowComplete = context.getResources().getBoolean(
                        R.bool.def_phone_showMmiCodeComplete);
                if (DBG) log("isShowComplete=" + isShowComplete);
                if (!isShowComplete && (text.toString().equals(context.getText(com.android.internal.R.string.mmiComplete)))) {
                    return;
                }
                boolean isShowError = context.getResources().getBoolean(R.bool.def_show_mmi_code_error);
                if (DBG) log("isShowError=" + isShowError);
                if (!isShowError && (preState == "PENDING") && text.toString().equals(context.getText(com.android.internal.R.string.mmiError))){
                    dismissUssdDialog();
                    return;
                }
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
...
*/
                boolean bCancelConfrimationOn = context.getResources().getBoolean(R.bool.feature_phone_ussdCancelConfirmationShow_on);
                Log.v("simply", "text=" + text);
                if (text == null || text.length() == 0) {
                    if (bCancelConfrimationOn && state == MmiCode.State.CANCELLED) {
                        //1070103 [USSD] when enter second USSD selection, pop up error message
                        displayMmiDialog(phone, context, context.getText(R.string.mmiCancelled), UssdAlertActivity.USSD_DIALOG_NOTIFICATION);
                    ...
                    String Mccmnc = TelephonyManager.getDefault().getSimOperator(phone.getSubId());
                    String volteState = Settings.System.getString(phone.getContext().getContentResolver(), "volteState" + (phone.getSubId()));
                    boolean isWillShowWarnOnLTEMode = "25099".equals(Mccmnc) &&//Mcc MNC: 25099 -> "Bee Line", "RU"
                            settingsNetworkMode == Phone.NT_MODE_LTE_ONLY &&
                            "true".equals(volteState) &&
                            context.getResources().getBoolean(R.bool.feature_phone_diaplayWarnMsgWhenSendUSSDOnLTE_on);
                    if (!(isWillShowWarnOnLTEMode)) {
                        displayMmiDialog(phone, context, text, UssdAlertActivity.USSD_DIALOG_NOTIFICATION);
                    }
                }
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
            } else {
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/12/2016, SOLUTION-2475544
//Main porting  BootCellBroadcastReceiver related solution
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/12/2016, SOLUTION-2466214
//[HOMO][MSRU][IOT2][USSD] USSD menu which have internal menu not works
    static void displayMmiDialog(Phone phone,Context context, CharSequence text, int type) {
        ...
        intent.putExtra(UssdAlertActivity.USSD_PHONE_ID,phone.getPhoneId());
        try{
            context.startActivity(intent);
        }catch(Exception e){
            Log.e(LOG_TAG, "failed to start UssdAlertActivity "+"reason: "+e);
        }
    }
