...
public class MobileNetworkSettings extends PreferenceActivity
    ...
    //private SwitchPreference mButton4glte;  //[BUGFIX]-Add-BEGIN by TCTNB.Dandan.Fang,08/15/2016,TASK2721235,
    ...
    protected void onCreate(Bundle icicle) {
      ...
      //[Telecom_Internal[GL/CN]] VoLTE contorl menu
        ...
        if (getResources().getBoolean(
                com.android.internal.R.bool.feature_tctfw_lteswitch_on)) {
    ...
    protected void onResume() {
      ...
      //[Telecom_Internal[GL/CN]] VoLTE contorl menu
        ...
        */
        ...
        //ly.The Notification should display when reboot or hotplug if Lte only is the
        //preferred networkmode.
        if (getResources().getBoolean( R.bool.feature_phone_lteOnlyDialogForBeeline_on) == true) {
            IntentFilter filter = new IntentFilter( "update.network.mode.as.auto");
            registerReceiver(modeAsAutoReceiver, filter);
        }
        //[FEATURE]-Add-END by TCTNB.Fuqiang.Song
    ...
    private void updateBody() {
        ...
        if (prefSet != null) {
            ...
            //prefSet.addPreference(mButton4glte);//[BUGFIX]-Add-BEGIN by TCTNB.Dandan.Fang,08/15/2016,TASK2721235,
        }
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/05/2016, SOLUTION-2474226
//[Settings]"Enhanced 4G LTE mode" item doesn\'t exist in search result
        if (!(ImsManager.isVolteEnabledByPlatform(this)
                && ImsManager.isVolteProvisionedOnDevice(this)) || (mPhone.getImsPhone() == null)
                || !getResources().getBoolean(com.android.internal.R.bool.def_telephony_enhanced_4g_lte_mode_on)) {
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    ...
    //[BUGFIX]-Add-END by TCTNB.Dandan.Fang
      ...
      //[BUGFIX]-Add-END by TCTNB.Dandan.Fang
        ...
        //mButton4glte.setEnabled(hasActiveSubscriptions && canChange4glte);  //[BUGFIX]-Add-BEGIN by TCTNB.Dandan.Fang,08/15/2016,TASK2721235,
    ...
    protected void onPause() {
        ...
        //preferred networkmode.
        if (getResources().getBoolean( R.bool.feature_phone_lteOnlyDialogForBeeline_on) == true) {
            unregisterReceiver(modeAsAutoReceiver);
        }
        //[FEATURE]-Add-END by TCTNB.Fuqiang.Song
        if (DBG) log("onPause:-");
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if (preference == mButtonPreferredNetworkMode) {
          ...
          if ( Integer.valueOf((String) objValue).intValue() != Phone.NT_MODE_LTE_ONLY ){
            ...
            if (buttonNetworkMode != settingsNetworkMode) {
               ...
               lteOnlyModeProcess(Phone.NT_MODE_LTE_ONLY);
           }
        } else if (preference == mButtonEnabledNetworks) {
    ...
    //For Russian Beeline,notify end user when set preferred network mode to Lte On-
    //ly.The Notification should display when reboot or hotplug if Lte only is the
    //preferred networkmode.Refer to 1989904,528518.
    private void lteOnlyModeProcess(int lteonly) {
          String mccmnc = TelephonyManager.getDefault().getSimOperator(mPhone.getSubId());

          //int settingsNetworkMode = getPreferredNetworkModeForSubId();
          int phoneSubId = mPhone.getSubId();
          int settingsNetworkMode = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
                                            Settings.Global.PREFERRED_NETWORK_MODE + phoneSubId,
                                            preferredNetworkMode);

          if (DBG) log(" settingsNetworkMode: " + settingsNetworkMode);

          if (settingsNetworkMode != lteonly) {
              boolean lteOnlyDialog = getResources().getBoolean(
                      R.bool.feature_phone_lteOnlyDialogForBeeline_on);
              int modemNetworkMode;
              if (lteOnlyDialog == true && !(mBeeline.equals(mccmnc))) {
                  AlertDialog dialog =new AlertDialog.Builder(this).setMessage(
                          getResources().getString(R.string.lteonly_dialog_warning))
                  .setTitle(R.string.lteonly_dialog_title)
                              ...
                              public void onClick(DialogInterface dialog, int which) {
                                  if (mButtonPreferredNetworkMode != null) {
                                      mButtonPreferredNetworkMode.setValue(Integer
                                              .toString(Phone.NT_MODE_LTE_ONLY));
                                      UpdatePreferredNetworkModeSummary(Phone.NT_MODE_LTE_ONLY);
                                  }
                                  Settings.Global.putInt(mPhone.getContext().getContentResolver(),
                                                   Settings.Global.PREFERRED_NETWORK_MODE+ phoneSubId,
                                                  Phone.NT_MODE_LTE_ONLY);
                                  // Set the modem network mode
                                  mPhone.setPreferredNetworkType(Phone.NT_MODE_LTE_ONLY,
      ...
      private BroadcastReceiver modeAsAutoReceiver = new BroadcastReceiver() {
          @Override
          public void onReceive(Context context, Intent intent) {
              if (intent.getAction().equals("update.network.mode.as.auto")) {
                  int settingsNetworkMode = getPreferredNetworkModeForPhoneId();
                  if (mButtonPreferredNetworkMode != null) {
                      mButtonPreferredNetworkMode.setValue(Integer.toString(settingsNetworkMode));
                      UpdatePreferredNetworkModeSummary(settingsNetworkMode);
                  }
