...
public class PhoneAccountSettingsFragment extends PreferenceFragment
    ...
    public void onResume() {
        ...
        if (shouldShowConnectionServiceList(allNonSimAccounts)) {
            ...
            if (!(getResources().getBoolean(R.bool.feature_phone_internetCall_on))
                    || (allNonSimAccounts.isEmpty() && allAccounts != null)) {
        ...
        if (ImsManager.isVtEnabledByPlatform(this.getActivity())) {
            boolean currentValue =
                    ImsManager.isEnhanced4gLteModeSettingEnabledByUser(this.getActivity())
                    ? PhoneGlobals.getInstance().phoneMgr.isVideoCallingEnabled(
                            this.getActivity().getOpPackageName()) : false;
            mEnableVideoCalling.setChecked(currentValue);
            mEnableVideoCalling.setOnPreferenceChangeListener(this);
        } else {
            getPreferenceScreen().removePreference(mEnableVideoCalling);
        }
        //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
        ...
        if (getPreferenceScreen() != null && mVibrateAfterConnected != null && mAccountList != null
            && !(getResources().getBoolean(R.bool.enableReminderFromQualcomm))) {
            mAccountList.removePreference(mVibrateAfterConnected);
        }
    ...
    public boolean onPreferenceChange(Preference pref, Object objValue) {
        //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/31/2016, SOLUTION- 2500194 And TASk-2781349
        //[Dialer][call][Dual]Set up SIM card 1 Turn on video calling options, SIM card 2 settings will change
        if (pref == mEnableVideoCalling) {
            if (ImsManager.isEnhanced4gLteModeSettingEnabledByUser(this.getActivity())) {
