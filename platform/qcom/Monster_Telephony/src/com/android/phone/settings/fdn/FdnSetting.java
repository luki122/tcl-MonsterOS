...
public class FdnSetting extends PreferenceActivity
    ...
    //[ForTest][FDN] Mobile data connection and USSD should be disabled when FDN is enabled
    private static boolean mLastDataEnabled = false;
    private ContentObserver mFDNObserver = new ContentObserver(new Handler()) {
        @Override
                        ...
                        if (ar.exception != null) {
                            if (ar.exception instanceof CommandException) {
                                ...
                                if (mPhone.getIccCard().getIccFdnEnabled()) {
                                    mLastDataEnabled = mPhone.getDataEnabled();
                                } else {
                                    if (mLastDataEnabled) {
                                        mPhone.setDataEnabled(true);
                                        mLastDataEnabled = false;
                                    }
                                }
                                //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
    ...
    private void updateEnableFDN() {
        if (mPhone.getIccCard().getIccFdnEnabled()) {
            ...
            //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 09/02/2016, SOLUTION- 2473826 And TASk-2781430
            //[ForTest][FDN] Mobile data connection and USSD should be disabled when FDN is enabled
            if(0 == Settings.System.getInt(getContentResolver(), "FDN_HAS_BEEN_ENABLED", 0)){
                Settings.System.putInt(getContentResolver(), "FDN_HAS_BEEN_ENABLED", 1);
                if(mPhone != null){
                    Settings.System.putInt(getContentResolver(), "DATA_STATUS_BEFORE_ENABLE_FDN", mPhone.getDataEnabled()?1:0);
                    Settings.System.putInt(getContentResolver(), "DATAROAMING_STATUS_BEFORE_ENABLE_FDN", mPhone.getDataRoamingEnabled()?1:0);
                }
            }
            queryFDNList();
            //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
            ...
            //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 09/02/2016, SOLUTION- 2473826 And TASk-2781430
            //[ForTest][FDN] Mobile data connection and USSD should be disabled when FDN is enabled
            if(1 == Settings.System.getInt(getContentResolver(), "FDN_HAS_BEEN_ENABLED", 0)){
                if(mPhone != null){
                    if(1 == Settings.System.getInt(getContentResolver(), "DATA_STATUS_BEFORE_ENABLE_FDN", 0)){
                        mPhone.setDataEnabled(true);
                    }else{
                        mPhone.setDataEnabled(false);
                    }

                    if(1 == Settings.System.getInt(getContentResolver(), "DATAROAMING_STATUS_BEFORE_ENABLE_FDN", 0)){
            ...
            Settings.System.putInt(getContentResolver(), "FDN_HAS_BEEN_ENABLED", 0);
            //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
        }
    }
    ...
    */
    ...
    protected void onCreate(Bundle icicle) {
        ...
        //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 09/07/2016, SOLUTION- 2473826 And TASk-2781430
        //[ForTest][FDN] Mobile data connection and USSD should be disabled when FDN is enabled
        Uri mFdnUri = FdnList.getContentUri(mSubscriptionInfoHelper);
        this.getContentResolver().registerContentObserver(mFdnUri, true, mFDNObserver);
        //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
    }
...
//            CallFeaturesSetting.goUpToTopLevelSetting(this, mSubscriptionInfoHelper);
    ...
    public void onDestroy() {
        this.getContentResolver().unregisterContentObserver(mFDNObserver);
        super.onDestroy();
    }

    private void queryFDNList() {
        Uri mFdnUri = null;
        mFdnUri = FdnList.getContentUri(mSubscriptionInfoHelper);

        if (mFdnUri != null) {
            new QueryHandler(mPhone.getContext()).startQuery(1, null, mFdnUri, null, null, null, null);
        }
    ...
    private class QueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            // While FDN is activated, the data connection should be possible
            // only when the SS code *99# is stored as a contact in the FDN
            // list.
            boolean flag = true;
            ...
            boolean enableDataFlag = getResources().getBoolean(R.bool.def_phone_enable_fdn_data_roaming);
            final int phoneSubId = mPhone.getSubId();
            TctLog.i(LOG_TAG, "onQueryComplete phoneSubId = " + phoneSubId);
            String FDNFlag = "FDN_DATA_CONNECTION_DISABLE_FLAG";
            if (phoneSubId != -1) {
                FDNFlag += phoneSubId;
            }
            if (flag && !enableDataFlag) {
                Settings.System.putInt(getContentResolver(), FDNFlag, 1);
                mPhone.setDataEnabled(false);
                try {
                    mPhone.setDataRoamingEnabled(false);
                } catch (Exception e) {
                    Log.e(LOG_TAG, "FdnSettings:" + e.toString());
                }
