...
//                                Intent intent = new Intent(CALL_WAITING_INTENT);
                                Intent intent = new Intent(CdmaCallOptions.this, CdmaCallWaitingOptions.class);
        ...
        Preference mCf =  prefSet.findPreference("button_cf_expand_key");
        if(mCf != null) { 
  			 prefSet.removePreference(mCf);
  	    }    
        //add by lgy
        CdmaVoicePrivacyCheckBoxPreference prefPrivacy = (CdmaVoicePrivacyCheckBoxPreference)
                prefScreen.findPreference("button_voice_privacy_key");
        if (prefPrivacy != null) {
   		        prefSet.removePreference(prefPrivacy);
        }
    }
