...
public class PhoneInterfaceManager extends ITelephony.Stub {
    ...
    private static final int CMD_SET_PREFERRED_NETWORK_MODE = 46;
    private static final int EVENT_SET_PREFERRED_NETWORK_MODE_DONE = 47;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_SIM_GET_ATR_DONE:
                    ...
                    break;
                /* ADD-BEGIN by Dingyi for dualsim 2016/08/03 FR 2655692*/
                case CMD_SET_PREFERRED_NETWORK_MODE:
                    request = (MainThreadRequest) msg.obj;
                    onCompleted = obtainMessage(EVENT_SET_PREFERRED_NETWORK_MODE_DONE, request);
                    int networkMode = (Integer) request.argument;
                    Log.e("LOG_TAG","CMD_SET_PREFERRED_NETWORK_MODE networkMode:"+networkMode+" ,subID:"+request.subId);
                    getPhoneFromRequest(request).setPreferredNetworkType(networkMode, onCompleted);
                    break;

                case EVENT_SET_PREFERRED_NETWORK_MODE_DONE:
                    ar = (AsyncResult)msg.obj;
                    request = (MainThreadRequest)ar.userObj;
                    int phoneSubId = request.subId;
                    int phoneId = SubscriptionManager.getPhoneId(phoneSubId);
                    int nwMode=(Integer) request.argument;
                    Log.e(LOG_TAG,"EVENT_SET_PREFERRED_NETWORK_MODE_DONE phoneSubId:"+phoneSubId+" ,phoneId:"+phoneId);
                    if(ar.exception == null){
                        Log.e(LOG_TAG,"EVENT_SET_PREFERRED_NETWORK_MODE_DONE ar.exception==null");
                        request.result = true;
                        ...
                        Log.e(LOG_TAG,"EVENT_SET_PREFERRED_NETWORK_MODE_DONE ar.exception=="+ar.exception);
                        request.result = false;
                        if (ar.exception instanceof CommandException) {
                            loge("setPreferredNetworkMode" + ": CommandException: " + ar.exception);
                        } else {
                            loge("setPreferredNetworkMode" + ": Unknown exception");
                        }
                    }
                    synchronized (request) {
                        request.notifyAll();
    ...
    public int getTetherApnRequired() {
        ...
        //[BUGFIX]-Mod-BEGIN by TCTNB.Fuqiang.Song,08/17/2016,2743064,
        //Tethering should be OK under single SIM and Dual SIM. Refer to 982370.
        int subId = mSubscriptionController.getDefaultDataSubId();
        ...
        boolean hasMatchedTetherApn = phone.hasMatchedTetherApnSetting();
        if (dunRequired == 2) {
            if (hasMatchedTetherApn) {
                dunRequired = 1;
            } else {
                dunRequired = 0;
            }
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466833
//Porting Framework Interface for jrdapp
    public boolean isDataPossibleForSubscription(int subId, String apnType) {
        return getPhone(subId).isDataConnectivityPossible(apnType);
    }

    public boolean getIccFdnEnabled(int phoneId) {
        final Phone mPhone = PhoneFactory.getPhone(phoneId);
        return mPhone.getIccCard().getIccFdnAvailable()
                && mPhone.getIccCard().getIccFdnEnabled();
    }
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    ...
    * @hide
    */
    public void setPreferredNetworkMode(int subId, int networkType) {
        enforceModifyPermissionOrCarrierPrivilege(subId);
        Boolean success = (Boolean) sendRequest(CMD_SET_PREFERRED_NETWORK_MODE, networkType, subId);
    }
    /* ADD-END by Dingyi for dualsim 2016/08/03 FR 2655692*/

//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/19/2016, SOLUTION-2743874
//Porting WifiCall IR94 Interface which Gapp needeed
    //add by yonganlin@tcl.com for Task:2419454 2016-6-29 begin
    public void setAcsNetworkConfig(int enableRcsMessaging){
        try {
            Phone phone = PhoneFactory.getDefaultPhone();
            if (phone != null) {
                phone.setAcsNetworkConfig(enableRcsMessaging);
            }
    ...
    public void getAcsNetworkConfig() {
        try {
            Phone phone = PhoneFactory.getDefaultPhone();
            if (phone != null) {
                phone.getAcsNetworkConfig();
            }
        } catch(IllegalStateException e) {
            e.printStackTrace();
        }
...
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    //ADD-BEGIN by Dingyi for london dual sim 2016/09/18 task 2827906
    public boolean getDataRoamingEnabled(int slotId){
        Phone phone = PhoneFactory.getPhone(slotId);
        if (phone != null) {
            boolean enable = phone.getDataRoamingEnabled();
            Log.d(LOG_TAG, "getDataRoamingEnabled enable:"+enable);
            return enable;
        }
        return false;
    }

    public boolean setDataRoamingEnabled(int slotId, boolean enable){
        Phone phone = PhoneFactory.getPhone(slotId);
        if (phone != null) {
            Log.d(LOG_TAG, "setDataRoamingEnabled enable:"+enable);
            phone.setDataRoamingEnabled(enable);
            return true;
        }
        return false;
    }
    //ADD-END by Dingyi for london dual sim 2016/09/18 task 2827906
}
