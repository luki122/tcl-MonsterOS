...
//preferred networkmode.Refer to 861817.
...
public class CallNotifier extends Handler {
    ...
    //ly.The Notification should display when reboot or hotplug if Lte only is the
    //preferred networkmode.Refer to 861817.
    public static final String mBeeline = "25099";
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/12/2016, SOLUTION-2476196
//The wakelock "PhoneApp" is not be released after answer a call
...
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    ...
    private void onSuppServiceFailed(AsyncResult r) {
        ...
        //[SOLUTION]-Add-BEGIN by TCTNB.(Caixia Chen), 08/09/2016, SOLUTION-2497208
        if (mCM.getActiveFgCall().getState() == Call.State.IDLE
                && mCM.getFirstActiveBgCall().getConnections().size() < 2
                && mCM.getFirstActiveBgCall().getState() == Call.State.HOLDING) {
            Log.d(LOG_TAG, " Only connection a hold call from SystemUI hangup the call no popup error message.");
            return;
        }
        ...
        for (int i = 0; i < subInfos.size(); i++) {
            ...
            if (!mPhoneStateListeners.containsKey(subId)) {
                ...
                mTelephonyManager.listen(listener,
                        ...
                        | PhoneStateListener.LISTEN_CALL_STATE  //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/16/2016, SOLUTION- 2724431 And TASk-2726394
                        //[FEATURE]-Add-END by TCTNB.Fuqiang.Song
                        );
    ...
    private class CallNotifierPhoneStateListener extends PhoneStateListener {
      ...
      //preferred networkmode.Refer to 861817
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            String mccmnc = TelephonyManager.getDefault().getSimOperator(mSubId);

            int phoneId = SubscriptionManager.getPhoneId(mSubId);
            Phone phone = PhoneFactory.getPhone(phoneId);

            lteOnlyDialog = mApplication.getResources().getBoolean(R.bool.feature_phone_lteOnlyDialogForBeeline_on);

            if(phone != null){
                int preNetWorkType = getPrefernetwork(phone);

                if(lteOnlyDialog && mBeeline.equals(mccmnc)) {
                    log("serviceState.getState(): " + serviceState.getState());
                    String rilTec = serviceState.rilRadioTechnologyToString(serviceState.getRilVoiceRadioTechnology());
                    if(serviceState.getState() == ServiceState.STATE_IN_SERVICE && "LTE".equals(rilTec) && preNetWorkType == Phone.NT_MODE_LTE_ONLY){
                        mApplication.notificationMgr.updateLteOnlyIcon(true);
                    }else{
                        mApplication.notificationMgr.updateLteOnlyIcon(false);
                    }
                    System.putString(mApplication.getContentResolver(), "volteState"+(mSubId),"false");
                }
            }
        }
        ...
        public void onVoLteServiceStateChanged(VoLteServiceState stateInfo) {
            ...
            log("onVoLteServiceStateChanged: " + stateInfo.toString());
            int netWorkType = TelephonyManager.getDefault().getNetworkType(mSubId);
            String mccmnc = TelephonyManager.getDefault().getSimOperator(mSubId);
            int preNetWorkType = 0;
            if(phone != null){
                preNetWorkType = getPrefernetwork(phone);
            }
            log("VoLteServiceState.getSrvccState(): " + stateInfo.getSrvccState());
            if (stateInfo.getSrvccState() == IMS_REGISTERED) {
                if(lteOnlyDialog && mBeeline.equals(mccmnc)) {
                    if(preNetWorkType == Phone.NT_MODE_LTE_ONLY && netWorkType == TelephonyManager.NETWORK_TYPE_LTE){
                        mApplication.notificationMgr.updateLteOnlyIcon(false);
                    }
                }
            }else if (stateInfo.getSrvccState() == IMS_UNREGISTERED){
                if(lteOnlyDialog && mBeeline.equals(mccmnc)
                        && netWorkType == TelephonyManager.NETWORK_TYPE_LTE && preNetWorkType == Phone.NT_MODE_LTE_ONLY){
                    mApplication.notificationMgr.updateLteOnlyIcon(true);
                }
            }
        }

        private int getPrefernetwork(Phone phone) {
            int preNetWorkType = 0;
            if(TelephonyManager.getDefault().isMultiSimEnabled()){
                try {
                    preNetWorkType = TelephonyManager.getIntAtIndex(
                            phone.getContext().getContentResolver(),
                            android.provider.Settings.Global.PREFERRED_NETWORK_MODE,
                            phone.getPhoneId());
                    android.util.Log.e(LOG_TAG, "-----preNetWorkType------" + preNetWorkType);
                } catch (SettingNotFoundException e) {
                    try {
                        preNetWorkType = android.provider.Settings.Global.getInt(
                            phone.getContext().getContentResolver(),
                            android.provider.Settings.Global.PREFERRED_NETWORK_MODE + phone.getSubId()
                            ,0);
                        if ( 0 == preNetWorkType ) {
                             e.printStackTrace();
                        }
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            ...
            switch (state) {
                case TelephonyManager.CALL_STATE_OFFHOOK:
                    if (!mPhoneFlag) {
                        mCurrentPhone = PhoneUtils.getPhoneFromSubId(this.mSubId);
