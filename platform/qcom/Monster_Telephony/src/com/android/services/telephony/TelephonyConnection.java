...
//Porting WifiCall IR94 Interface which Gapp needeed
                        ...
                        int SUPP_SERV_NOTIFICATION_TYPE_MT = 1;
                        if (mSsNotification.notificationType == SUPP_SERV_NOTIFICATION_TYPE_MT &&
                                (mSsNotification.code == SuppServiceNotification.MT_CODE_CALL_ON_HOLD ||
                                        mSsNotification.code == SuppServiceNotification.MT_CODE_CALL_RETRIEVED)) {
                            android.util.Log.d("ZZ", "TelephonyConnection getPhone() = " + getPhone() + "state = " + mSsNotification.code);
                            if (getPhone() != null) {
                                getPhone().notifyForVideoCallingStateToHold(mSsNotification.code);
                            }

                            /** fixme == temporary solution of 2127445 for GAPP == start */
                            try {
                                Context context = getPhone().getContext();
                                String action = null;
                                switch (mSsNotification.code) {
                                    case SuppServiceNotification.MT_CODE_CALL_ON_HOLD:
                                        action = "test.ims.vt.hold.ONHOLD";
                                        break;
                                    case SuppServiceNotification.MT_CODE_CALL_RETRIEVED:
                                        action = "test.ims.vt.hold.RECOVER";
                                        break;
                                }
                                if (action != null) {
...
//                                mDisplayName, Toast.LENGTH_LONG).show();
    ...
    void updateState() {
        ...
        if(mConnectionState == Call.State.DIALING 
        		&& newState == Call.State.ACTIVE 
        		&& this instanceof CdmaConnection) {
