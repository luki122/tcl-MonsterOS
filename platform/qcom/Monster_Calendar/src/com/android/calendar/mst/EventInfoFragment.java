...
public class EventInfoFragment extends DialogFragment implements OnCheckedChangeListener,
    ...
    private ImageView mEventReminder;
    ...
    public EventInfoFragment(Context context, Uri uri, long startMillis, long endMillis,
            ...
            ArrayList<ReminderEntry> reminders, Toolbar toolbar) {
    ...
    public EventInfoFragment(Context context, long eventId, long startMillis, long endMillis,
        ...
        this(context, ContentUris.withAppendedId(Events.CONTENT_URI, eventId), startMillis,
                endMillis, attendeeResponse, isDialog, windowStyle, reminders, toolbar);
        mEventId = eventId;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mWindowStyle == DIALOG_WINDOW_STYLE) {
            ...
            mView = inflater.inflate(R.layout.mst_event_info_fragment, container, false);
        }
        ...
        if (i >= 1) {
        	mView.findViewById(R.id.event_info_banner).setBackgroundResource(R.drawable.mst_event_banner_2);
        }

        mMonthDay = (TextView) mView.findViewById(R.id.monthday);
        mMonth = (TextView) mView.findViewById(R.id.month);
        mWeekDay = (TextView) mView.findViewById(R.id.weekday);
        mEventTitle = (TextView) mView.findViewById(R.id.event_title);
        mEventDiscription = (TextView) mView.findViewById(R.id.event_description);
        mBeginTime = (TextView) mView.findViewById(R.id.begin_time);
        mBeginDate = (TextView) mView.findViewById(R.id.begin_date);
        mEndTime = (TextView) mView.findViewById(R.id.end_time);
        mEndDate = (TextView) mView.findViewById(R.id.end_date);
        mEventReminder = (ImageView) mView.findViewById(R.id.event_reminder);
        mEventRepeat = (TextView) mView.findViewById(R.id.event_repeat);
        ...
        mEventReminder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mHasAlarm) {
                    String[] args = new String[] { Long.toString(mEventId) };
                    cr.delete(Reminders.CONTENT_URI, REMINDERS_WHERE, args);
                } else {
                ...
                mHasAlarm = !mHasAlarm;
                updateEventReminder(mHasAlarm);
                Toast.makeText(mContext, mHasAlarm ?
                        R.string.event_reminder_open : R.string.event_reminder_close, Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void updateEvent(View view) {
        ...
        if (eventName != null) {
            ...
            setTextCommon(view, R.id.event_title, eventName);
        }
        ...
        if (description != null && description.length() != 0) {
            ...
            mEventDiscription.setText(description);
        }
    ...
    private void updateBanner() {
        Resources res = getActivity().getResources();
        String[] monthArray = res.getStringArray(R.array.month_labels);
        String[] weekDayArray = res.getStringArray(R.array.weekday_labels);

        Time date = new Time();
        date.set(mStartMillis);

        int monthDay = date.monthDay;
        String monthDayStr = String.valueOf(monthDay);
        if (monthDay < 10) {
            monthDayStr= "0" + monthDayStr;
        }

        mMonthDay.setText(monthDayStr);
        mMonth.setText(monthArray[date.month]);
        mWeekDay.setText(weekDayArray[date.weekDay]);
    }

    private void updateTime() {
        String beginTime = DateUtils.formatDateTime(mContext, mStartMillis, DateUtils.FORMAT_SHOW_TIME);
        String beginDate = DateUtils.formatDateTime(mContext, mStartMillis, DateUtils.FORMAT_SHOW_DATE
                | DateUtils.FORMAT_SHOW_YEAR | DateUtils.FORMAT_ABBREV_MONTH);
        String endTime = DateUtils.formatDateTime(mContext, mEndMillis, DateUtils.FORMAT_SHOW_TIME);
        String endDate = DateUtils.formatDateTime(mContext, mEndMillis, DateUtils.FORMAT_SHOW_DATE
                | DateUtils.FORMAT_SHOW_YEAR | DateUtils.FORMAT_ABBREV_MONTH);
    ...
    private void updateEventReminder(boolean hasAlarm) {
        mEventReminder.setImageResource(hasAlarm ?
                R.drawable.mst_event_reminder_open : R.drawable.mst_event_reminder_close);
    }
