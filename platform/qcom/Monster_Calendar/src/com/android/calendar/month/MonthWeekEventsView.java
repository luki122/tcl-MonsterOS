...
import com.android.calendar.mst.CalendarFilterSettingsActivity;
import com.android.calendar.mst.legalholiday.LegalHoliday;
import com.android.calendar.mst.legalholiday.LegalHolidayUtils;
...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    public MonthWeekEventsView(Context context) {
        ...
        mDisplayFestivalDay = Utils.getSharedPreference(context, CalendarFilterSettingsActivity.KEY_FESTIVAL_HOLIDAY, true);
        mDisplayWorkAndShift = Utils.getSharedPreference(context, CalendarFilterSettingsActivity.KEY_HOLIDAY_WORKDAY, true);
        mDisplayLunarDate = Utils.getSharedPreference(context, CalendarFilterSettingsActivity.KEY_LUNAR_DATE, true);
        mDisplayEventTag = Utils.getSharedPreference(context, CalendarFilterSettingsActivity.KEY_EVENT_TAG, true);
    }
    ...
    @Override
    protected void onDraw(Canvas canvas) {
    	Log.d(TAG, "MonthWeekEventsView->onDraw()");
        initMstView();
    ...
    private int computeDayLeftPosition(int day) {
        ...
        if (mShowWeekNum) {
            ...
            xOffset = mMonthWeekSidePadding;
            effectiveWidth -= xOffset * 2;
        }
    ...
    private int mColorMonthDaySelected = 0;
    ...
    private int mColorToday = 0;
    private int mColorEventTag = 0;

    private int highlightTextColor = 0;

    private Paint mMonthDayPaint;
    private Paint mLunarMonthDayPaint;

    private int mMonthWeekSidePadding;

    private int mMonthDayTapRadius;
    // private int mMonthDayEventTagRadius;
    private float mMonthDayEventTagRadius = 1.5f;
    private int mMonthDayEventTagTopPadding;
    private int mMonthDayNumberTopPadding;
    private int mMonthDayTextTopPadding;

    private String mStrLegalHoliday;
    private String mStrLegalWorkShift;

    private LunarUtils mLunarUtils;
    private String termQingmingString;

    private boolean initMstView = false;

    private void initMstView() {
        if (initMstView) return;

        initMstView = true;

        Resources res = getContext().getResources();

        mMonthWeekSidePadding = res.getDimensionPixelOffset(R.dimen.month_week_side_padding);

        mMonthDayTapRadius = res.getInteger(R.integer.month_day_tap_radius);
        // mMonthDayEventTagRadius = res.getInteger(R.integer.month_day_event_tag_radius);
        mMonthDayEventTagTopPadding = res.getInteger(R.integer.month_day_event_tag_top_padding);
        ...
        mTextSizeSolarMonthDay = res.getInteger(R.integer.month_day_text_size);
        mTextSizeLunarMonthDay = res.getInteger(R.integer.month_day_other_text_size);

    	mBgColorCurrentMonthDay = res.getColor(R.color.month_day_bg_color);
        mBgColorSecondaryMonthDay = res.getColor(R.color.month_day_bg_color);

        mColorMonthDayDefault   = res.getColor(R.color.month_day_number_color);
        mColorMonthDaySelected = res.getColor(R.color.month_day_select_text_color);
        mColorMonthDaySecondary = res.getColor(R.color.month_day_other_number_color);
        mColorLunarMonthDayDefault   = res.getColor(R.color.month_day_extra_text_color);
        mColorLunarMonthDaySecondary = res.getColor(R.color.month_day_other_number_color);
        mColorTappedDay = res.getColor(R.color.month_day_tap_color);
        mColorToday = res.getColor(R.color.month_day_today_color);
        mColorEventTag = res.getColor(R.color.month_day_event_tag_color);
        highlightTextColor = res.getColor(R.color.month_day_highlight_text_color);

        mStrLegalHoliday = res.getString(R.string.legal_holiday);
        mStrLegalWorkShift = res.getString(R.string.legal_workday);

        if (mScale > 0 && mScale != 1.0F) {
            mTextSizeSolarMonthDay *= mScale;
            mTextSizeLunarMonthDay *= mScale;

            mMonthDayTapRadius *= mScale;
            mMonthDayEventTagRadius *= mScale;
            mMonthDayEventTagTopPadding *= mScale;
            mMonthDayNumberTopPadding *= mScale;
            mMonthDayTextTopPadding *= mScale;
        }

        Typeface typeface = Typeface.create("monster-normal", Typeface.NORMAL);
        if (typeface == null) {
        ...
        mMonthDayPaint.setTypeface(typeface);
        ...
        mLunarMonthDayPaint.setTypeface(typeface);

        mLunarUtils = LunarUtils.getInstance(getContext());
        termQingmingString = res.getString(R.string.term_qingming);
    }
    ...
    private void drawTappedDayBackground(Canvas canvas) {
        if (mClickedDayIndex != -1) {
            ...
            p.setAlpha(255);
            canvas.drawCircle((r.left + r.right) / 2, (r.top + r.bottom) / 2, mMonthDayTapRadius, p);
            p.setAlpha(alpha);
        }
    }

    private void drawWeekNumber(Canvas canvas) {
        int[] legalHolidayTags = new int[mNumDays];
        LegalHoliday legelHolidayUtils = LegalHolidayUtils.getInstance();

        for (int i = mFirstJulianDay; i < (mFirstJulianDay + mNumDays); ++i) {
            legalHolidayTags[i - mFirstJulianDay] = legelHolidayUtils.getDayType(i);
        }
    	...
    	for (int index = 0; index < mNumDays; ++index) {
            ...
            if (mClickedDayIndex == index) {
                ...
                mMonthDayPaint.setColor(mColorMonthDaySelected);
                mLunarMonthDayPaint.setColor(mColorMonthDaySelected);
            } else if (mHasToday && mTodayIndex == index) {
                drawSelectedDay = true;
                mMonthDayPaint.setColor(mColorToday);
                mLunarMonthDayPaint.setColor(mColorToday);
            } else if (mFocusDay[index] || isWeekMode) {
                mMonthDayPaint.setColor(mColorMonthDayDefault);
                // mLunarMonthDayPaint.setColor(mColorLunarMonthDayDefault);
                if (legalHolidayTags[index] != LegalHoliday.DAY_TYPE_NORMAL && mDisplayWorkAndShift) {
            ...
            int ySolarOffset = inHeight + mMonthDayNumberTopPadding;

			canvas.drawText(solarText, xSolarOffset, ySolarOffset, mMonthDayPaint);
			...
			if (isChineseEnvironment) {
	            ...
	            if (mDisplayFestivalDay && mDisplayWorkAndShift && mDisplayLunarDate) {
	                String allExtraString = mLunarUtils.getLunarFestivalChineseString(t.year, (t.month + 1), t.monthDay);
	                boolean isFestival = allExtraString.contains(LunarUtils.DELIM);
	                String[] temp = allExtraString.split(LunarUtils.DELIM);
	                String lunarText = temp[0];

	                if (lunarText.equals(termQingmingString)) isFestival = true;

	                if (!isFestival && legalHolidayTags[index] == LegalHoliday.DAY_TYPE_HOLIDAY) {
	                    lunarText = mStrLegalHoliday;
	                } else if (!isFestival && legalHolidayTags[index] == LegalHoliday.DAY_TYPE_WORKDAY) {
	                ...
	                if (legalHolidayTags[index] != LegalHoliday.DAY_TYPE_NORMAL) {
	                    isWorkShift = true;
	                }
	                ...
	                String lunarText = mLunarUtils.getLunarChineseString(t.year, (t.month + 1), t.monthDay);

	                if (legalHolidayTags[index] == LegalHoliday.DAY_TYPE_HOLIDAY) {
	                    lunarText = mStrLegalHoliday;
	                } else if (legalHolidayTags[index] == LegalHoliday.DAY_TYPE_WORKDAY) {
	                    lunarText = mStrLegalWorkShift;
	                }
				...
				int yLunarOffset = (int) (textBounds.height() + mMonthDayTextTopPadding);
    ...
    private void drawEventsTag(Canvas canvas) {
        ...
        for (int i = 0; i < hasEvents.length; ++i) {
            if (hasEvents[i]) {
                int left = computeDayLeftPosition(i);
                int right = computeDayLeftPosition(i + 1);

                int alpha = p.getAlpha();
                if (mClickedDayIndex == i /*|| (mHasToday && mTodayIndex == i)*/) {
                    p.setColor(mColorMonthDaySelected);
                } else if (mFocusDay[i] || isWeekMode) {
                    p.setColor(mColorEventTag);
                } else {
                    p.setColor(mColorMonthDaySecondary);
                }
                p.setAlpha(255);;
                canvas.drawCircle((left + right) / 2 , mMonthDayEventTagTopPadding + mMonthDayEventTagRadius,
                		mMonthDayEventTagRadius, p);
                p.setAlpha(alpha);
    		}
    	}
    }
    ...
    public int setAndReturnClickedDay(float xLocation) {
        ...
        return mFirstJulianDay + mClickedDayIndex;
    }
