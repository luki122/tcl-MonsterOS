...
import com.android.calendar.mst.BirthdayInfoActivity;
...
public class CalendarController {
    ...
    public interface EventType {
        ...
        final long VIEW_BIRTHDAY = 1L << 12;
    }
    ...
    public void sendEvent(Object sender, final EventInfo event) {
        ...
        if (!handled) {
            ...
            if (event.eventType == EventType.CREATE_EVENT) {
                ...
                return;
            } else if (event.eventType == EventType.VIEW_BIRTHDAY) {
                launchViewBirthday(event.startTime.toMillis(false));
            }
        }
    }
    ...
    private void launchViewBirthday(long millis) {
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setClass(mContext, BirthdayInfoActivity.class);
        intent.putExtra(BirthdayInfoActivity.BIRTHDAY_START_MILLIS, millis);
        mContext.startActivity(intent);
    }
...
//    }
    ...
    private String eventInfoToString(EventInfo eventInfo) {
        ...
        if ((eventInfo.eventType & EventType.GO_TO) != 0) {
            ...
            tmp = "Update title";
        } else if ((eventInfo.eventType & EventType.VIEW_BIRTHDAY) != 0) {
            tmp = "View brithday";
        }
        builder.append(tmp);
        builder.append(": id=");
