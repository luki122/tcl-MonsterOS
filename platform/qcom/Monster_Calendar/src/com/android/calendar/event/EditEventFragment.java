...
public class EditEventFragment extends Fragment implements EventHandler, OnColorSelectedListener {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (mUri == null && mView != null && mView.mTitleTextView != null) {
            mView.mTitleTextView.postDelayed(new Runnable() {
                @Override
                public void run() {
                    mView.mTitleTextView.requestFocus();
                    mInputMethodManager.showSoftInput(mView.mTitleTextView, 0);
                 }
             }, 200);
        }
    }
...
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
        ...
        if (mIsReadOnly) {
            ...
            view = inflater.inflate(R.layout.mst_edit_event/*edit_event*/, null);
        }
        ...
        View cancelActionView = view.findViewById(R.id.action_cancel);
        cancelActionView.setOnClickListener(mActionBarListener);
        View doneActionView = view.findViewById(R.id.action_done);
        doneActionView.setOnClickListener(mActionBarListener);
        TextView title = (TextView) view.findViewById(R.id.edit_event_title);
        title.setText(mUri == null ? R.string.create_event : R.string.edit_event);
    ...
    private boolean onActionBarItemSelected(int itemId) {
        if (itemId == R.id.action_done) {
            if (mView != null && mView.isTitleEmpty()) {
                Toast.makeText(mActivity, R.string.event_edit_save_no_title, Toast.LENGTH_SHORT).show();
                return true;
            }
            ...
            } else if (mView != null && mView.prepareForSave() && modelChanged()) {
                showCancelDialog();
            } else {
                cancelDone();
            }
    ...
    private void showCancelDialog() {
        AlertDialog dialog = new AlertDialog.Builder(mActivity)
                .setMessage(R.string.event_edit_cancel_info)
                .setNegativeButton(android.R.string.cancel, null)
                .setPositiveButton(R.string.event_edit_cancel, new OnClickListener() {
                    @Override
    ...
    private boolean isNewEventEmpty() {
        if (mModel.mOriginalStart != mModel.mStart || mModel.mOriginalEnd != mModel.mEnd) {
    		return false;
    	}

        return mView != null && mView.isContentEmpty();
    }

    private boolean modelChanged() {
        if (mOriginalModel.mTitle == null) {
            if (mModel.mTitle != null) {
                return true;
            }
        } else if (!mOriginalModel.mTitle.equals(mModel.mTitle)) {
            return true;
        }

        if (mModel.mOriginalStart != mModel.mStart || mModel.mOriginalEnd != mModel.mEnd ||
                mOriginalModel.mHasAlarm != mModel.mHasAlarm) {
            return true;
        }

        if (mOriginalModel.mRrule == null) {
            if (mModel.mRrule != null) {
                return true;
            }
        } else if (!mOriginalModel.mRrule.equals(mModel.mRrule)) {
            return true;
        }

        if (mOriginalModel.mDescription == null) {
            if (mModel.mDescription != null) {
                return true;
            }
        } else if (!mOriginalModel.mDescription.equals(mModel.mDescription)) {
            return true;
        }
    ...
    protected void displayEditWhichDialog() {
        if (mModification == Utils.MODIFY_UNINITIALIZED) {
            ...
            mModifyDialog = new AlertDialog.Builder(mActivity)
                    .setMessage(R.string.edit_repeat_event)
                    .setNeutralButton(R.string.edit_repeat_event_ok, null)
                    .show();
            mModifyDialog.setOnCancelListener(new OnCancelListener() {
                @Override
                public void onCancel(DialogInterface dialog) {
        ...
        public void run() {
            ...
            if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
                ...
                if (!mModel.mAttendeesList.isEmpty()) {
                    ...
                    if (mModel.mUri != null) {
                        stringResource = R.string.event_saved/*saving_event*/;
                    } else {
                        stringResource = R.string.event_created/*creating_event*/;
                    }
                }
    ...
    public void onBackPressed() {
        onActionBarItemSelected(R.id.action_cancel);
    }
