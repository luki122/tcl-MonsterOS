...
import mst.app.dialog.DateTimeDialog.OnDateTimeSetListener;
...
import com.android.calendar.mst.EventRepeatChooseActivity;
...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
    ...
    private class DateTimeListener implements OnDateTimeSetListener {
        ...
        @Override
        public void onDateTimeSet(DateTimePicker view, int year, int month, int monthDay,
                int hour, int minute) {
            Log.d(TAG, "onDateSet: " + year +  " " + month +  " " + monthDay + " " + hour + " " + minute);
            Time startTime = mStartTime;
            Time endTime = mEndTime;
            ...
            long endMillis;
            if (mView == mFromRow) {
                int orignalYear = startTime.year;
                int orignalMonth = startTime.month;
                int orignalMonthDay = startTime.monthDay;
                ...
                endTime.month = month + monthDuration;
                endTime.monthDay = monthDay + monthDayDuration;
                endTime.hour = hour + hourDuration;
                endTime.minute = minute + minuteDuration;
                endMillis = endTime.normalize(true);

                // If the start date has changed then update the repeats.
                if (orignalYear != year || orignalMonth != month || orignalMonthDay != monthDay) {
                    populateRepeats();
                }
    ...
    private class DateTimeClickListener implements View.OnClickListener {
        ...
        public DateTimeClickListener(Time time) {
            mTime = time;
        }
        ...
        public void onClick(View v) {
            ...
            final DateTimeListener listener = new DateTimeListener(v);
            if (mDateTimeDialog != null) {
                mDateTimeDialog.dismiss();
                mDateTimeDialog = null;
            }
            mDateTimeDialog = new DateTimeDialog(mActivity, listener,
                    mTime.year, mTime.month, mTime.monthDay, mTime.hour, mTime.minute);
    ...
    private void populateWhen() {
        ...
        mFromRow.setOnClickListener(new DateTimeClickListener(mStartTime));
        mToRow.setOnClickListener(new DateTimeClickListener(mEndTime));
    }
    ...
    private void populateRepeats() {
        ...
        if (!TextUtils.isEmpty(mRrule)) {
            ...
            repeatString = r.getString(R.string.event_no_repeat/*does_not_repeat*/);
            enabled = true;
        }

        // mRruleButton.setText(repeatString);
        mRruleView.setText(repeatString);
    ...
    public boolean isTitleEmpty() {
        return TextUtils.isEmpty(mTitleTextView.getText().toString().trim());
    }

    public boolean isContentEmpty() {
        return isTitleEmpty() && TextUtils.isEmpty(mDescriptionTextView.getText().toString().trim()) && TextUtils.isEmpty(mRrule);
    }
    ...
    public void onClick(View view) {
        if (view == mRruleButton) {
            ...
            intent.setClass(mActivity, EventRepeatChooseActivity.class);
            intent.putExtra(EventRepeatChooseActivity.EVENT_START_MILLIS, mStartTime.toMillis(false));
            intent.putExtra(EventRepeatChooseActivity.EVENT_TIME_ZONE, mStartTime.timezone);
            intent.putExtra(EventRepeatChooseActivity.EVENT_RRULE, mRrule);
            mActivity.startActivityForResult(intent, EventRepeatChooseActivity.EVENT_REPEAT_CHOOSE_CODE);
            return;
        }
    ...
    public EditEventView(Activity activity, View view, EditDoneRunnable done,
        ...
        mEventReminder = (ImageView) view.findViewById(R.id.event_reminder);
        ...
        mRruleView = (TextView) view.findViewById(R.id.event_repeat);
    ...
    public void setModel(CalendarEventModel model) {
        ...
        mEventReminder.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean hasAlarm = mReminderItems.size() > 0;
                if (hasAlarm) {
        ...
        if (mModel.mUri == null) {
            onItemSelected(mCalendarsSpinner, mCalendarsSpinner.getSelectedView(),
                    mCalendarsSpinner.getSelectedItemPosition(), mCalendarsSpinner.getSelectedItemId());
        }
    ...
    private void setViewStates(int mode) {
        ...
        if (mode == Utils.MODIFY_UNINITIALIZED || !EditEventHelper.canModifyEvent(mModel)) {
            ...
            for (View v : mEditOnlyList) {
                v.setVisibility(View.GONE/*VISIBLE*/);
            }
            for (View v : mEditViewList) {
                v.setEnabled(true);
                if (v.getTag() != null) {
            ...
            if (mModel.mUri == null) {
                mCalendarSelectorGroup.setVisibility(View.GONE/*VISIBLE*/);
                mCalendarStaticGroup.setVisibility(View.GONE);
            } else {
                mCalendarSelectorGroup.setVisibility(View.GONE);
                mCalendarStaticGroup.setVisibility(View.GONE/*VISIBLE*/);
            }
            if (mModel.mOriginalSyncId == null) {
            ...
            mRemindersGroup.setVisibility(View.GONE/*VISIBLE*/);

            mLocationGroup.setVisibility(View.GONE/*VISIBLE*/);
    ...
    private void updateRemindersVisibility(int numReminders) {
        ...
        mEventReminder.setImageResource(
        		mReminderItems.isEmpty() ? R.drawable.mst_event_reminder_close : R.drawable.mst_event_reminder_open);
    }
    ...
    protected void setAllDayViewsVisibility(boolean isChecked) {
        if (isChecked) {
            ...
            mTimezoneRow.setVisibility(View.GONE/*VISIBLE*/);
        }
