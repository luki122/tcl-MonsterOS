...
public class AlertReceiver extends BroadcastReceiver {
    ...
    public static NotificationWrapper makeBirthdayReminderNotification(Context context,
            ArrayList<AlertService.NotificationInfo> notificationInfos, String digestTitle, int notificationId,
            boolean expandable) {
        if (notificationInfos == null || notificationInfos.size() < 1) {
            return null;
        }
        ...
        clickIntent.setAction(DismissAlarmsService.SHOW_BIRTHDAY_ACTION);
        clickIntent.putExtra(AlertUtils.EVENT_IDS_KEY, eventIds);
        clickIntent.putExtra(AlertUtils.EVENT_STARTS_KEY, startMillis);
        clickIntent.putExtra(AlertUtils.EVENT_START_KEY, notificationInfos.get(0).startMillis);
        clickIntent.putExtra(AlertUtils.NOTIFICATION_ID_KEY, notificationId);
        PendingIntent pendingClickIntent = PendingIntent.getService(context, 0, clickIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        notificationBuilder.setContentText(res.getString(R.string.birthday_wish));
        notificationBuilder.setSmallIcon(R.drawable.mst_birthday_stat_notify);
        notificationBuilder.setContentIntent(pendingClickIntent);
        notificationBuilder.setDeleteIntent(pendingDeleteIntent);
        ...
        if (Utils.isJellybeanOrLater()) {
            ...
            RemoteViews contentView = new RemoteViews(context.getPackageName(),
                    R.layout.notification);
            contentView.setImageViewResource(R.id.image, R.drawable.mst_birthday_stat_notify);
            contentView.setTextViewText(R.id.title, digestTitle);
            contentView.setTextViewText(R.id.text, res.getString(R.string.birthday_wish));
            contentView.setViewVisibility(R.id.time, View.VISIBLE);
            contentView.setViewVisibility(R.id.map_button, View.GONE);
            contentView.setViewVisibility(R.id.call_button, View.GONE);
            contentView.setViewVisibility(R.id.email_button, View.GONE);
