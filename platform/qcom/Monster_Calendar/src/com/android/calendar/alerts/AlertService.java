...
public class AlertService extends Service {
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
        ...
        int currentNotificationId = /*1*/AlertUtils.BIRTHDAY_REMINDER_NOTIFICATION_ID + 1;
        ...
        if (numBirthdayReminder > 0) {
            ...
            if (numBirthdayReminder == 1) {
                titleText = context.getString(R.string.birthday_only_one, titleText);
            } else {
                titleText = context.getString(R.string.birthday_have_other, titleText, numBirthdayReminder);
            }
            NotificationWrapper notification = AlertReceiver.makeBirthdayReminderNotification(context,
                    birthdayReminderEvents, titleText, AlertUtils.BIRTHDAY_REMINDER_NOTIFICATION_ID, false);
            ...
            addNotificationOptions(notification, info.newAlert ? notificationPrefs.quietUpdate : true, titleText,
                    ...
                    true);
            nm.notify(AlertUtils.BIRTHDAY_REMINDER_NOTIFICATION_ID, notification);
        } else {
            nm.cancel(AlertUtils.BIRTHDAY_REMINDER_NOTIFICATION_ID);
        }
        ...
        if (currentNotificationId <= maxNotifications + 1) {
            nm.cancelAllBetween(currentNotificationId, maxNotifications + 1);
            if (DEBUG) {
    ...
    static int processQuery(final Cursor alertCursor, final Context context,
            ...
            ArrayList<NotificationInfo> birthdayReminderEvents) {
        ...
        try {
            while (alertCursor.moveToNext()) {
                ...
                boolean isBirthdayReminder = Utils.BIRTHDAY_REMINDER_ACCOUNT_NAME.equals(ownerAccount);
                boolean doReminderBirthday = Utils.getSharedPreference(context, CalendarSettingsActivity.KEY_BIRTHDAY_REMINDER, true);
                if (isBirthdayReminder && !(doReminderBirthday && endTime >= currentTime)) {
                    continue;
                }
