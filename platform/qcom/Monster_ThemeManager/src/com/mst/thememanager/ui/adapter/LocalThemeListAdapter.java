...
public class LocalThemeListAdapter extends AbsBaseAdapter<Theme> {
	...
	public LocalThemeListAdapter(Context context) {
		...
		final int height = DisplayUtils.getHeightPixels(context);
		final int width = DisplayUtils.getWidthPixels(context);
	        ImageResizer imageResizer = new ImageResizer(context, width/2, height/2);
	        imageResizer.addImageCache((Activity)context, "previews_cache");
	        imageResizer.setImageLoaderCallback(new ImageLoaderCallback() {
