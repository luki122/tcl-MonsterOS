	...
	public void run() {
		...
		if(systemIconBackupFile.exists() && systemIconBackupFile.isDirectory()){
			...
			if(childFiles != null && childFiles.length > 0){
				for(String desity:childFiles){
					if(desity.endsWith("xml")){
						continue;
					}
					ArrayList<String> iconList = new ArrayList<String>();
					File iconDir = new File(Config.THEME_APPLY_ICON_BACKUP_DIR+desity);
					...
					mSystemDefaultIconMap.put(desity, iconList);
				}
			}
		}
						...
						for (String tmpDensity : tmpDensities) {
							if (mSystemDefaultIconMap.containsKey(tmpDensity)) {
								...
								if (backupIcoms.length > tmpIcons.length) {
									List<String> detalIcons = CommonUtil.compareArray(tmpIcons, backupIcoms);
									for (String icon : detalIcons) {
