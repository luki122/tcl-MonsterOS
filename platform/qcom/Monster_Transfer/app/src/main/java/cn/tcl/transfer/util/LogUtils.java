...
public class LogUtils {
    ...
    /**	550
     * TCT: some feature log, tag always from TAG.
     * e.g: I/Email( 9178): [SearchMail]buildMessageListSelection,.
     */
    public static int logFeature(String featureName, String format,
                                 Object... args) {
        if (isLoggable(TAG, DEBUG)) {
            try {
                return LogUtils.d(TAG,
                        "[" + featureName + "] " + String.format(format, args));
                /**
                 * M: If the given log string contain some special string, such as "%s",
                 * JE will happened, it is not a best solution just catch it, but current code contains
                 * too many not format logs.
                 * Make sure email not crash, and throws a error log. @{
                 */
            } catch (Exception e) {
                return LogUtils.d(TAG, "[" + featureName + "] " + format + ": !!!! Log format exception:", e);
            }
            /** @} */
        }
        return 0;
