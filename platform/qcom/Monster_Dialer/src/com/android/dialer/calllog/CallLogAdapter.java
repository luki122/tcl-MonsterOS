...
VoicemailPlaybackPresenter.OnVoicemailDeletedListener {
	...
	public void updateActionMode(){
		if(isAllSelect()){
			actionMode.setPositiveText(mContext.getString(R.string.mst_actionmode_selectnone));
		}else{
			actionMode.setPositiveText(mContext.getString(R.string.mst_actionmode_selectall));
		}		

		actionMode.setTitle(String.format(
				mContext.getString(R.string.mst_menu_actionbar_selected_items),
				getCheckedCount()));	
	...
	private final View.OnClickListener mExpandCollapseListener = new View.OnClickListener() {
		...
		public void onClick(final View v) {
			if(v.getId()==R.id.slider_view1){

				new AlertDialog.Builder(mContext)
				.setTitle(null) 
				.setMessage(mContext.getString(R.string.mst_delete_call_log_message,1))
				.setPositiveButton(mContext.getString(R.string.mst_ok), new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						CallLogAsyncTaskUtil.deleteCalls(
								mContext, callIds.toString(), mCallLogAsyncTaskListener,mIsPrivate);
					}
				})
				.setNegativeButton(mContext.getString(R.string.mst_cancel), new DialogInterface.OnClickListener() {

					@Override
					public void onClick(DialogInterface dialog, int which) {
						int position=Integer.parseInt(v.getTag().toString());
						Log.d(TAG,"position:"+position);
						((SliderView)v).close(true);
					}
				})
				.show();
				return;
	...
	public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		...
		if(viewType==VIEW_TYPE_HEADER_ITEM){
			...
			linearParams.height = mContext.getResources().getDimensionPixelOffset(R.dimen.mst_calllog_listview_padding_top);			  
	...
	private ViewHolder createCallLogEntryViewHolder(ViewGroup parent) {
		...
		View view = inflater.inflate(R.layout.mst_call_log_list_item, parent, false);
		CallLogListItemViewHolder viewHolder = CallLogListItemViewHolder.create(
				view,
				mContext,
...
//		views.primaryActionView.setVisibility(View.VISIBLE);
		...
		if(is_editor_mode) {
			...
			if(views.slider!=null) views.slider.setLockDrag(true);
		}else {
			views.checkBox.setVisibility(View.GONE);
			views.primaryActionButtonViewParent.setVisibility(View.VISIBLE);
			if(views.slider!=null) views.slider.setLockDrag(false);
		}

		if(views.slider!=null&&views.slider.isOpened()){
			views.slider.close(false);
		}

		if(views.slider!=null) views.slider.setSwipeListener(new SwipeListener(){

			public void onClosed(SliderLayout view){
				currentSliderView=null;
			}
			public void onOpened(SliderLayout view){
				currentSliderView=views.slider;
			}
		...
		//		if(position==0) {
		//			Log.d(TAG,"bindcalllogview,position==0");
		//			
		//			if(itemHeight==0){
		//				itemHeight=views.slider.getMeasuredHeight();
		//			}
		//			
		//			int distance=mContext.getResources().getDimensionPixelOffset(R.dimen.mst_list_padding_top_or_bottom);
		//			ViewGroup.LayoutParams params = views.slider.getLayoutParams();
		//			params.height = itemHeight+distance;
		//			Log.d(TAG,"params.height:"+params.height);
		//			views.slider.setLayoutParams(params);
		//			views.slider.setPadding(0, distance, 0, 0);
		//		}
