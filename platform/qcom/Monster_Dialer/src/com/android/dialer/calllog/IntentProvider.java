...
public abstract class IntentProvider {
	...
	public static IntentProvider getReturnCallIntentProvider(final String number,
			final PhoneAccountHandle accountHandle,final int position,final CheckBox checkBox,final int slotId) {
		return new IntentProvider() {
			public int getPosition() {
				return position;
			}
			...
			public Intent getIntent(Context context) {
				...
				intent.putExtra("slot",slotId);//-1 不指定；0指定卡槽1拨号；1指定卡槽2拨号
				return intent;
			}
			@Override
			public CheckBox getCheckBox() {
	...
	public static IntentProvider getReturnCallIntentProvider(final String number,
			final int pos,final CheckBox checkBox,final int slotId) {
		return getReturnCallIntentProvider(number, null,pos,checkBox,slotId);
	}
