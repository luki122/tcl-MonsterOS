...
import mst.provider.ContactsContract.PhoneLookup;
...
public class ContactInfoHelper {
	...
	public ContactInfo lookupNumber(String number, String countryIso) {
		Log.d(TAG,"lookupNumber,number:"+number);
	...
	private ContactInfo lookupContactFromUri(Uri uri) {
		Log.d(TAG,"lookupContactFromUri,uri:"+uri);
		if (uri == null) {
			return null;
		}
		...
		final ContactInfo info;
		Cursor phonesCursor =
				mContext.getContentResolver().query(uri, PhoneQuery._PROJECTION, null, null, "_id DESC");
		...
		if (phonesCursor != null) {
			try {
				if (phonesCursor.moveToFirst()) {
					...
					Log.d(TAG,"info,label:"+info.label);
					...
					info.starred = phonesCursor.getInt(PhoneQuery.STARRED);
					info.rawContactId=phonesCursor.getInt(0);
				} else {
	...
	private ContactInfo queryContactInfoForPhoneNumber(String number, String countryIso) {
		Log.d(TAG,"queryContactInfoForPhoneNumber,number:"+number);
		if (TextUtils.isEmpty(number)) {
			return null;
		}
		...
		Uri uri = Uri.withAppendedPath(/*PhoneLookup.ENTERPRISE_CONTENT_FILTER_URI*/Uri.parse("content://com.android.contacts/mst_phone_lookup_enterprise"),//update by liyang
	...
	public void updateCallLogContactInfo(String number, String countryIso, ContactInfo updatedInfo,
		...
		try {
			if (countryIso == null) {
				...
				Log.d(TAG,"updateCallLogContactInfo4");
				mContext.getContentResolver().update(
						TelecomUtil.getCallLogUri(mContext),
						values,
						Calls.NUMBER + " = ? AND " + Calls.COUNTRY_ISO + " = ? AND privacy_id=?",
						new String[]{ number, countryIso,String.valueOf(PrivacyUtils.getCurrentAccountId())});
			}
