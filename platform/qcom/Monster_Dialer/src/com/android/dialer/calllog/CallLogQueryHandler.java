...
public class CallLogQueryHandler extends NoNullCursorAsyncQueryHandler {
    ...
    private static final int NUM_LOGS_TO_DISPLAY = 500;
     ...
     */
    public void fetchCalls(int callType, long newerThan,boolean isPrivate) {
        cancelFetch();
        if (PermissionsUtil.hasPhonePermissions(mContext)) {
            fetchCalls(QUERY_CALLLOG_TOKEN, callType, false /* newOnly */, newerThan,isPrivate);
        } else {
            updateAdapterData(null);
        }
    }

    public void fetchCalls(int callType,boolean isPrivate) {
        fetchCalls(callType, 0,isPrivate);
    }
    ...
    private void fetchCalls(int token, int callType, boolean newOnly, long newerThan,boolean isPrivate) {
        ...
        if(isPrivate) selection+=" AND privacy_id="+privacyId;
        else selection+=" AND privacy_id=0";//liyang add for privacy
        
        Uri uri = TelecomUtil.getCallLogUri(mContext).buildUpon()
                .appendQueryParameter(Calls.LIMIT_PARAM_KEY, Integer.toString(limit))
                .build();
        Log.d(TAG,"fetchCalls,uri:"+uri+" token:"+token+" selection:"+selection);
