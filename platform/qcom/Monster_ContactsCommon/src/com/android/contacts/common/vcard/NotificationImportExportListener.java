...
Handler.Callback {
	...
	private static final String TAG = "NotificationImportExportListener";
	...
	public void onImportProcessed(ImportRequest request, int jobId, int sequence) {
		...
		Log.d(DEFAULT_NOTIFICATION_TAG, "onImportProcessed is " + request.displayName
				+ ",jobId: " + jobId);
	...
	public void onImportFinished(ImportRequest request, int jobId, Uri createdUri) {
		final String description = mContext.getString(R.string.importing_vcard_finished_title,
				ImportVCardActivity.fileName!=null?ImportVCardActivity.fileName:request.displayName);
		final Intent intent;
		Log.d(TAG,"createdUri:"+createdUri);
		if (/*createdUri != null*/false) {
			...
			intent=getAppOpenIntentByPackageName(mContext, "com.android.contacts");
	...
	//		if (pkgContext != null && intent != null) { pkgContext.startActivity(intent); 
	//		return true;
	//		} 
	//		return false;
	//	}
	...
	public void onImportCanceled(ImportRequest request, int jobId) {
		...
		Log.d(DEFAULT_NOTIFICATION_TAG, "onImportCanceled is " + request.displayName
				+ ",jobId: " + jobId);
		mNotificationManager.notify(NotificationImportExportListener.DEFAULT_NOTIFICATION_TAG,
				jobId, notification);

		Intent intent=new Intent("CONTACTS_IMPORT_CANCEL_COMPLETE");
	...
	/* package */ static Notification constructProgressNotification(
		...
		Log.d(TAG,"constructProgressNotification,type:"+type+" description:"+description+" tickerText:"+tickerText+" jobId:"+jobId+" displayName:"+displayName
				+" totalCount:"+totalCount+" currentCount:"+currentCount);
		Intent notificationIntent = new Intent("android.intent.action.contacts.MstContactImportExportActivity");
		...
		.setSmallIcon(
				type == VCardService.TYPE_IMPORT ? android.R.drawable.stat_sys_download_done
						...
						/// @}
						.setContentIntent(PendingIntent.getActivity(context, 0, /*intent*/notificationIntent, 0));
		if (totalCount > 0) {
			String percentage =
	...
	public static Notification mstConstructProgressNotification(
			Context context, int type, String description, String tickerText,
			int jobId, String displayName, int totalCount, int currentCount,PendingIntent pendingIntent) {
		// Note: We cannot use extra values here (like setIntExtra()), as PendingIntent doesn't
		// preserve them across multiple Notifications. PendingIntent preserves the first extras
		// (when flag is not set), or update them when PendingIntent#getActivity() is called
		// (See PendingIntent#FLAG_UPDATE_CURRENT). In either case, we cannot preserve extras as we
	...
	/* package */ static Notification constructFinishNotification(int type,
		...
		Log.d(DEFAULT_NOTIFICATION_TAG, "[constructFinishNotification] intent = " +
				intent.toString());
		return new Notification.Builder(context)
		.setAutoCancel(true)
		.setColor(context.getResources().getColor(R.color.dialtacts_theme_color))
