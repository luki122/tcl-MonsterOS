...
implements OnScrollListener, OnItemSelectedListener {
	...
	/// M: whether to draw pinned header.
	private boolean mDrawPinnedHeader = true;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		/// M: Need get the mHeaderWidth before super.onLayout. @{
		mHeaderPaddingStart = getPaddingStart();
//		mHeaderPaddingStart=getResources().getDimensionPixelOffset(R.dimen.contact_listview_header_padding_left);
		mHeaderWidth = r - l - mHeaderPaddingStart - getPaddingEnd();
		/// @}
		Log.d(TAG,"onLayout,changed:"+changed+" mPinnedHeaderListAdapter.getCursor().isClosed():"
				+((mPinnedHeaderListAdapter.getCursor()==null)?"null":mPinnedHeaderListAdapter.getCursor().isClosed()));
		if(mPinnedHeaderListAdapter.getCursor()!=null && !mPinnedHeaderListAdapter.getCursor().isClosed())
			super.onLayout(changed, l, t, r, b);
	}

	@Override
	public void setAdapter(ListAdapter adapter) {
		mPinnedHeaderListAdapter=(PinnedHeaderListAdapter)adapter;
	...
	public void setFadingHeader(int viewIndex, int position, boolean fade) {
		/** M: wether to draw fading header @ { */
		if (!mDrawPinnedHeader) {
			return;
		}
		/** @} */
		ensurePinnedHeaderLayout(viewIndex);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		Log.d(TAG,"mHeaderTouched:"+mHeaderTouched);
		if (mHeaderTouched) {
	...
	private void drawHeader(Canvas canvas, PinnedHeader header, long currentTime) {
		/** M: wether to draw pinned header @ { */
		if (!mDrawPinnedHeader) {
			return;
		}
		/** @ } */
		...
		if (header.visible) {
			...
			int translateX = ViewUtil.isViewLayoutRtl(this) ?
					...
					view.setBackgroundColor(mContext.getResources().getColor(R.color.contact_main_background));
//					canvas.saveLayerAlpha(mBounds, 50, Canvas.ALL_SAVE_FLAG);
					view.draw(canvas);
	...
	/**
	 *  M: Set whether to draw pinned header.
	 */
	public void setDrawPinnedHeader(boolean needPinnedHeader) {
		mDrawPinnedHeader = needPinnedHeader;
	}
}
