...
public class ExportProcessor extends ProcessorBase {
    ...
    /** M: New Feature support Dialer use MTK ContactImportExport */
    public static final String DIALER_PACKAGENAME = "com.android.dialer";
    ...
    public ExportProcessor(VCardService service, ExportRequest exportRequest, int jobId,
        ...
        Log.i(LOG_TAG, "[ExportProcessor]new,jobId = " + jobId
    ...
    private void runInternal() {
        ...
        try {
            ...
            /// M: [COMMD_FOR_OP09] @{
            //removed,because Operator don't override this function
           // composer = ExtensionManager.getInstance().getImportExportExtension()
            ...
            if (mService.getQuerySelection() != null &&
                    mService.getQuerySelection().length() != 0) {
                querySelection = mService.getQuerySelection();
                Log.d(TAG,"querySelection:"+querySelection);
            }
            ...
            Intent intent=new Intent("EXPORT_TO_SD_CARD_DOING");
			intent.putExtra("index", 0);
			intent.putExtra("totalContacts", total);
			intent.putExtra("mJobId",mJobId);
			intent.putExtra("displayName",uri.getLastPathSegment());
            ...
            while (!composer.isAfterLast()) {
                ...
                if (/*current % 100 == 1*/true) {
                    doProgressNotification(uri, total, current);
                }
                
                Intent intent1=new Intent("EXPORT_TO_SD_CARD_DOING");
    			intent1.putExtra("index", current);
    			intent1.putExtra("totalContacts", total);
    			intent1.putExtra("mJobId",mJobId);
    			intent1.putExtra("displayName",uri.getLastPathSegment());
    ...
    private void doFinishNotification(final String title, final String description) {
        ...
        /**
         * M: support Dialer use mtk ContactImportExport and when export finished
         * it will jump to the start importExportActivity if click notification.cause by
         * mtk importExportActivity is Contacts process,so support Dialer need to change.@{
         */
        Log.d(LOG_TAG, "[doFinishNotification] mCallingActivity : " + mCallingActivity);
        if (mCallingActivity.indexOf(DIALER_PACKAGENAME) < 0) {// Contacts start
            intent.setClassName(mService, mCallingActivity);
        } else {// DialtactsActivity start
            intent.setComponent(new ComponentName(DIALER_PACKAGENAME, mCallingActivity));
        }
        Log.d(LOG_TAG, "[doFinishNotification] send callingActivity intent = " + intent.toString());
        // @}

        final Notification notification =
        // / M:
        NotificationImportExportListener.constructFinishNotification(VCardService.TYPE_EXPORT,
