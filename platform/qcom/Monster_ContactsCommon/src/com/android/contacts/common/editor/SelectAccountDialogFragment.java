...
public final class SelectAccountDialogFragment extends DialogFragment {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (mSubId > 0) {
                    ...
                    if (!SimCardUtils.isPhoneBookReady(mSubId)) {
                        ...
                        Log.w(TAG, "[onClick]PhoneBook is not ready for use");
                    } else {
                        onAccountSelected(mAccount);
                    }
                    /** @} */
                } else {
                    Log.w(TAG, "[onCreateDialog]subId is invalid: mSubId = " + mSubId);
                    onAccountSelected(mAccount);
                }
                /** M: @} */
            }
    ...
    public void onCreate(Bundle savedInstanceState) {
        if (mReceiver == null) {
            ...
            filter.addAction("android.intent.action.PHB_STATE_CHANGED");
            getActivity().registerReceiver(mReceiver, filter);
            Log.i(TAG, "[onCreate]registerReceiver mReceiver");
        }
    ...
    public void onDetach() {
        if (mReceiver != null) {
            ...
            mReceiver = null;
            Log.i(TAG, "[onDetach]unregisterReceiver mReceiver");
        }
        super.onDetach();
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.intent.action.PHB_STATE_CHANGED".equals(action)) {
                if (SubInfoUtils.getActivatedSubInfoCount() > 0) {
                    Log.i(TAG, "[onReceive] activity finsh,activity is: " + getActivity());
                    getActivity().finish();
                }
            }
        }
