...
//4.3 migration has deleted DataItem package
...
public class Contact {
    ...
    public RawContactDeltaList createRawContactDeltaList() {
        /** M: return null if the selected contact is deleted. @{ */
        ImmutableList<RawContact> rawContactslistList = getRawContacts();
        if (rawContactslistList != null) {
            return RawContactDeltaList.fromIterator(rawContactslistList.iterator());
        } else {
            return null;
        }
        /** @] */
    ...
    public Contact(Uri requestedUri, Uri uri, Uri lookupUri, long directoryId, String lookupKey,
            ...
            String photoUri, String displayName, String altDisplayName, String phoneticName,
            boolean starred, Integer presence, boolean sendToVoicemail, String customRingtone,
            boolean isUserProfile, int isSdnContact, boolean blockVideoCall, String jianPinYin) {
        this(requestedUri, uri, lookupUri, directoryId, lookupKey, id, nameRawContactId,
                displayNameSource, photoId, photoUri, displayName, altDisplayName, phoneticName,
                starred, presence, sendToVoicemail, customRingtone, isUserProfile, isSdnContact, blockVideoCall);
        mJianPinYin = jianPinYin;
    }
    ...
    private void initSIMMembers(Contact from) {
        Log.d(TAG, "initSIMMembers begin mIndicatePhoneOrSimContact , mSimIndex ,"
                + " mBlockVideoCall, mIsSdnContact : " + mIndicatePhoneOrSimContact + " , "
                + mSimIndex + " , " + mBlockVideoCall + " , " + mIsSdnContact);
        mIndicatePhoneOrSimContact = from.mIndicatePhoneOrSimContact;
        mSimIndex = from.mSimIndex;
        /** M: fix alps00943520 initiate Contacts's slot info. */
        mSlot = from.mSlot;
        mBlockVideoCall = from.mBlockVideoCall;
        mIsSdnContact = from.mIsSdnContact;
        Log.d(TAG, "initSIMMembers end mIndicatePhoneOrSimContact , mSimIndex ," +
                " mBlockVideoCall, mIsSdnContact : " + mIndicatePhoneOrSimContact + " , "
                + mSimIndex + " , " + mBlockVideoCall + " , " + mIsSdnContact);
    }
