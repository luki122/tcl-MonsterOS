...
public class ImportVCardActivity extends Activity {
	...
	private BroadcastReceiver mSubStateReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (("android.intent.action.PHB_STATE_CHANGED").equals(action)) {
				if (!isFinishing()) {
	...
	private void prepareProgressDialog(String title, String message,int max,final int jobId) {
		Log.d(LOG_TAG,"prepareProgressDialog,title:"+title+" message:"+message+" max:"+max);
		...
		mProgressDialog.setButton(getString(R.string.mst_cancel), new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				final CancelRequest request = new CancelRequest(jobId, null);
				mService.handleCancelRequest(request, null);
				finish();
			}
		});
	...
	// Runs on the UI thread.
	private class MstDialogDisplayer implements Runnable {
		private String title,message;
		private int type=0;
		public MstDialogDisplayer(String title,String message) {
			this.title=title;
			this.message=message;
		}

		public MstDialogDisplayer(int type) {
			this.type=type;
		}
	...
	//	private ContactsExportImportSDReceiver myReceiver;
	//	public class ContactsExportImportSDReceiver extends BroadcastReceiver{
	//
	//		@Override
	//		public void onReceive(Context context, Intent intent) {
	//			String action=intent.getAction();
	//			Bundle bundle=intent.getExtras();
	//			Log.d(LOG_TAG,"onReceive,action:"+action+" bundle:"+bundle);
	//			if(TextUtils.equals(action, "CONTACTS_IMPORT_FROM_SD_DOING")){
	//				if(bundle.getInt("index")==1){
	//					Log.d(LOG_TAG,"index 1");
	//					prepareProgressDialog(getString(R.string.mst_import_sim_contacts_doing_from_sd), getString(R.string.mst_import_sim_contacts_doing_from_sd), bundle.getInt("totalCount"),
	//							bundle.getInt("mJobId"));
	//				}else{
	//					mProgressDialog.incrementProgressBy(1);
	//					if(bundle.getInt("index")==bundle.getInt("totalCount")) {
	//						mProgressDialog.dismiss();
	//						mProgressDialog=null;
	//						Toast.makeText(ImportVCardActivity.this, getString(R.string.mst_import_sim_contacts_result,bundle.getInt("index")), Toast.LENGTH_LONG).show();
	//						ImportVCardActivity.this.finish();
	//					}
	//				}
	//			}
	...
	implements DialogInterface.OnClickListener, DialogInterface.OnCancelListener {
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			Toast.makeText(ImportVCardActivity.this, getString(R.string.mst_cancel_msg), Toast.LENGTH_LONG).show();
			setResult(Activity.RESULT_CANCELED);
			finish();
		}
		@Override
	...
	private void startVCardSelectAndImport() {
		...
		if (getResources().getBoolean(R.bool.config_import_all_vcard_from_sdcard_automatically) ||
			...
			if (size == 1) {
				runOnUiThread(new MstDialogDisplayer(getString(R.string.mst_menu_import_from_sd),getString(R.string.mst_import_from_sd_found_vcard_dialog_message,
						mAllVCardFileList.get(0).mName)));
				//                setResult(ImportExportUtils.RESULT_CODE);
			}
			/** @} */
			//            importVCardFromSDCard(mAllVCardFileList);
			...
			runOnUiThread(new MstDialogDisplayer(1));
	...
	private void importVCard(final String[] uriStrings) {
		Log.d(LOG_TAG,"importVCard,uriString:"+uriStrings);
	...
	protected void onCreate(Bundle bundle) {
		...
		//		myReceiver = new ContactsExportImportSDReceiver();
		//		IntentFilter intentFilter=new IntentFilter();
		//		intentFilter.addAction("CONTACTS_IMPORT_FROM_SD_DOING");
		//		//		intentFilter.addAction("CONTACTS_EXPORT_CONFICATION");
	...
	public String getRealPathFromUri(Context context, Uri contentUri) {
	    ...
	    try {
	        ...
	        int column_index = cursor.getColumnIndexOrThrow(MediaStore.Files.FileColumns.TITLE);
	        cursor.moveToFirst();
	        return cursor.getString(column_index)+".vcf";
	    } finally {
	        if (cursor != null) {
	            cursor.close();
	        }
	...
	protected Dialog onCreateDialog(int resId, Bundle bundle) {
		...
		case R.id.dialog_cache_vcard: {
			...
			if (mProgressDialogForCachingVCard == null) {
				...
				mProgressDialogForCachingVCard.setCanceledOnTouchOutside(false);
				startVCardService();
			}
	...
	private void doScanExternalStorageAndImportVCard() {
		...
		if (!file.exists() || !file.isDirectory() || !file.canRead()) {
			...
			//modify by lgy for 3445355
//			prepareProgressDialogSpinner(getString(R.string.mst_import_from_sd_searching_vcard),getString(R.string.mst_import_from_sd_searching_vcard));
			prepareProgressDialogSpinner(getString(R.string.mst_import_from_sd_searching_vcard_title), getString(R.string.mst_import_from_sd_searching_vcard));
			mVCardScanThread.start();
		}
	}
	...
	private void prepareProgressDialogSpinner(String title, String message) {
		if(mProgressDialogForScanVCard!=null) mProgressDialogForScanVCard.dismiss();

		mProgressDialogForScanVCard = new ProgressDialog(this);
		mProgressDialogForScanVCard.setTitle(title);
		mProgressDialogForScanVCard.setMessage(message);
		mProgressDialogForScanVCard.setProgressStyle(ProgressDialog.STYLE_SPINNER);
		mProgressDialogForScanVCard.setCanceledOnTouchOutside(false);

		mProgressDialogForScanVCard.setOnKeyListener(new DialogInterface.OnKeyListener() {
		...
		mProgressDialogForScanVCard.show();
	}
