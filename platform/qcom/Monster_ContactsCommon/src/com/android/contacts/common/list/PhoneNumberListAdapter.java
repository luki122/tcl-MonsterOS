...
//            Contacts.IS_SDN_CONTACT, // 11
        ...
        /// M: modify @{
        public static final int PHONE_INDICATE_PHONE_SIM_COLUMN_INDEX = 9;
        public static final int PHONE_IS_SDN_CONTACT = 10;
        /// @}
    }
    ...
    public void configureLoader(CursorLoader loader, long directoryId) {
        ...
        if (isExtendedDirectory(directoryId)) {
            ...
            Log.d(TAG,"configureLoader,uri:"+builder.build().toString()+" projection:"+Arrays.toString(PhoneQuery.PROJECTION_PRIMARY));
        ...
        label = Phone.getTypeLabel(getContext().getResources(), type, customLabel);
            ...
            label = ExtensionManager.getInstance().getAasExtension()
                    .getTypeLabel(type, (CharSequence) customLabel, (String) label,
                            cursor.getInt(PhoneQuery.PHONE_INDICATE_PHONE_SIM_COLUMN_INDEX));
        }
//        view.setLabel(label);
        final String text;
        if (displayNumber) {
    ...
    protected void bindPhoto(final ContactListItemView view, int partitionIndex, Cursor cursor) {
        ...
        if (photoId != 0) {
            ...
            if (photoUri == null) {
                ...
                /// M: Because of sim contacts, ImageRequest will need to pass subId also,
                /// instead of just new DefaultImageRequest @{
                request = ContactsCommonListUtils.getDefaultImageRequest(
                        cursor, displayName, lookupKey, getCircularPhotos());
                /// @}
            }
