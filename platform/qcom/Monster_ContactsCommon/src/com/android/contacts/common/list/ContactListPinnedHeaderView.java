...
public class ContactListPinnedHeaderView extends TextView {
    ...
    public ContactListPinnedHeaderView(Context context, AttributeSet attrs, View parent) {
       ...
       /* TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.ContactListItemView);
        ...
        //modify by liyang       
        int height=getContext().getResources().getDimensionPixelOffset(R.dimen.contact_listview_header_height);
//        int paddingLeft=getContext().getResources().getDimensionPixelOffset(R.dimen.contact_listview_header_padding_left);
        setBackgroundColor(getContext().getResources().getColor(R.color.contact_main_background));
        setTextAppearance(getContext(), R.style.mst_contacts_listview_header);
        setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, height));
        setLayoutDirection(parent.getLayoutDirection());
        setGravity(Gravity.CENTER_VERTICAL |
                (ViewUtil.isViewLayoutRtl(this) ? Gravity.RIGHT : Gravity.LEFT));
        setPadding(getResources().getDimensionPixelOffset(R.dimen.contact_listview_header_padding_left), 0, 0, 0);
    }
