...
public class ShareContactViaSDCardActivity extends Activity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        /**
         * Bug Fix for CR: ALPS00395378 @{ Original Code: mLookUpUris =
         * mIntent.getStringExtra("LOOKUPURIS");
         */
        mLookUpUris = null;
        ...
        if (userProfile != null && "true".equals(userProfile)) {
            Log.i(TAG, "[onCreate]user profile can not support sd card,return.");
            Toast.makeText(this.getApplicationContext(),
                    getString(R.string.user_profile_cannot_sd_card), Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
        ...
        Log.i(TAG, "[onCreate]mAction is " + mAction);
        // M: M: fixed CR ALPS02350357,get sFile value before calling checkSDCardAvaliable method
        // if not will be NullPointedException.
        sFile = getExternalStorageDirectory();

        if (!checkSDCardAvaliable()) {
            Log.w(TAG, "[onCreate]sd card is not avaliable.");
            showAlertDialog(R.string.no_sdcard_message, R.string.no_sdcard_title);
            return;
        }
        ...
        if (isSDCardFull()) {
            Log.w(TAG, "[onCreate]sd card is full.");
        ...
        if ((null != extraUri && extraUri.toString().startsWith("file") && mSingleContactId == -1)
            ...
            Log.i(TAG, "[onCreate]file is already on sd card,return.");
            Toast.makeText(this.getApplicationContext(),
    ...
    protected void onResume() {
        super.onResume();
        Log.i(TAG, "[onResume]mSdIsVisible = " + mSdIsVisible);
        if (mSdIsVisible) {
            if (Intent.ACTION_SEND.equals(mAction) && mIntent.hasExtra(Intent.EXTRA_STREAM)) {
                showProgressDialog();
            }
        }
    }
    ...
    private void shareViaSDCard(String lookUpUris) {
        ...
        if (mSingleContactId == -1) {
            ...
            /* dataUri */Contacts.CONTENT_URI, CONTACTS_PROJECTION, selection.toString(), null,
                    null);
            if (null != cursor) {
        ...
        Log.i(TAG, "[shareViaSDCard]exportselection:" + exportselection + ",callingActivity = "
                + callingActivity);
        ...
        it.putExtra("dest_path", sFile.getPath());
        it.putExtra(VCardCommonArguments.ARG_CALLING_ACTIVITY, callingActivity);
        Log.d(TAG,"exportselection:"+exportselection+" path:"+sFile.getPath()+" callingActivity:"+callingActivity);
    ...
    private  String getExternalStorageState() {
        /*try {
            IMountService mountService = IMountService.Stub.asInterface(ServiceManager
                    .getService("mount"));
            Log.d(TAG, "[getExternalStorageState] file : " + sFile.toString());
            return mountService.getVolumeState(sFile.toString());
        } catch (Exception rex) {
            Log.e(TAG, "[getExternalStorageState] Exception : " + rex);
            return Environment.MEDIA_REMOVED;
        }*/
        StorageManager storageManager = (StorageManager) getSystemService(Context.STORAGE_SERVICE);
        if (null == storageManager) {
            Log.w(TAG, "-----story manager is null----");
            return Environment.MEDIA_REMOVED;
        }
        String storageState = storageManager.getVolumeState(sFile.toString());
        Log.d(TAG, "[getExternalstorageState = " + storageState);
        return storageState;
    }
...
//        Log.d(TAG,"file2:"+file2+" path:"+file2.getPath());
        return file;
    }
    ...
    private boolean checkSDCardAvaliable() {
        ...
        // M: fixed CR ALPS02350357 using this.getExternalStorageState replace of Env
    ...
    private boolean isSDCardFull() {
        ...
        Log.d(TAG, "[isSDCardFull]state: " + state);
        if (Environment.MEDIA_MOUNTED.equals(state)) {
            String path = sFile.getPath();
            Log.i(TAG, "[isSDCardFull]storage path is " + path);
    ...
    private class CancelListener implements DialogInterface, DialogInterface.OnClickListener,
    ...
    private class SearchContactThread extends Thread implements OnCancelListener, OnClickListener {
        ...
        public void run() {
            ...
            Log.d(TAG, "[run]mDataUri is " + mDataUri + ",type :" + type);
