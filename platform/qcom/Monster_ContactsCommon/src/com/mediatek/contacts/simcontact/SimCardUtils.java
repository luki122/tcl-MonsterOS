...
public class SimCardUtils {
    ...
    public static boolean isFdnEnabed(int slotId) {
        ...
        if (v != null) {
            Log.w(TAG, "[isFdnEnabed]slotId:" + slotId + ",v:" + v);
            return v;
        }

//        final ITelephonyEx iTel = ITelephonyEx.Stub.asInterface(ServiceManager
//                .getService(TELEPHONY_SERVICE_EX));
...
//            }
//        } catch (RemoteException e) {
//            Log.e(TAG, "[isFdnEnabed]catch exception:");
//            e.printStackTrace();
//            isFdnEnabled = false;
//        }
//
//        Log.d(TAG, "[isFdnEnabed]slotId:" + slotId + ",isFdnEnabled:" + isFdnEnabled);
    ...
    public static boolean isSetRadioOn(ContentResolver resolver, int subId) {
        ...
        if (v != null) {
            Log.w(TAG, "[isSetRadioOn]subId:" + subId + ",v:" + v);
            return v;
        }

        boolean isRadioOn = false;
        if (SlotUtils.isGeminiEnabled()) {
            // /[Gemini+] dualSimSet rule: each bit stands for each slot radio
            // status
            // / e.g. 0101 means only slot 0 & slot 2 is set radio on
            final int flagAllSimOn = (1 << SlotUtils.getSlotCount()) - 1;
            int dualSimSet = Settings.System.getInt(resolver, "msim_mode_setting",
                    flagAllSimOn);
        ...
        Log.d(TAG, "[isSetRadioOn]subId:" + subId + ",isRadioOn:" + isRadioOn);
...
//        /**
//         * qinglei comment out for build pass try { if
//         * (SlotUtils.isGeminiEnabled()) { ///M:Lego API Refactoring
//         * simIndicatorState = iTelEx.getSimIndicatorState(slotId); } else {
//         * simIndicatorState = iTel.getSimIndicatorState(); } } catch
//         * (RemoteException e) { LogUtils.e(TAG,
//         * "[getSimIndicatorState] failed to get sim indicator state for slot "
//         * + slotId); e.printStackTrace(); }
//         */
//
...
//            Log.e(TAG, "[getSimTagBySlot]subId:" + subId + "is invalid!");
//            return null;
//        }
//        final ITelephonyEx iTel = ITelephonyEx.Stub.asInterface(ServiceManager
//                .getService(TELEPHONY_SERVICE_EX));
...
//        } catch (RemoteException e) {
//            Log.e(TAG, "catched exception. subId: " + subId);
//            e.printStackTrace();
//        }
    ...
    /**
     * M: [Gemini+] when a sim card was turned on or off in Settings, a
     * broadcast Intent.ACTION_MSIM_MODE_CHANGED would be sent, and carry
     * current Dual sim mode. This method is defined to parse the intent, and
     * check whether the slot is on or off.
     *
     * @param slotId
     *            slot to check
     * @param the
     *            mode retrived from the broadcast intent
    ...
    /**
     * M: Bug Fix for ALPS00566570 , get support email count some USIM cards
     * have no Email fields. if the slot is invalid, return -1, otherwise return
     * the email count
     *
     * @{
     */
    ...
    public static class ShowSimCardStorageInfoTask extends AsyncTask<Void, Void, Void> {
        ...
        public static void showSimCardStorageInfo(Context context, boolean needPopUp) {
            ...
            Log.i(TAG, "[showSimCardStorageInfo]_beg");
            if (sInstance != null) {
                sInstance.cancel();
                sInstance = null;
            }
            ...
            sInstance.execute();
            Log.i(TAG, "[showSimCardStorageInfo]_end");
        }
...
//            Log.i(
//                    TAG,
//                    "[ShowSimCardStorageInfoTask]: subInfos.size = "
//                            + SubInfoUtils.getActivatedSubInfoCount());
//            if (!mIsCancelled && (subscriptionInfoList != null)
...
//                                    + subscriptionInfo.getDisplayName() + "; simSlot = "
//                                    + subscriptionInfo.getSimSlotIndex() + "; subId = "
...
//                            storageInfos = phoneEx.getAdnStorageInfo(subscriptionInfo
//                                    .getSubscriptionId());
//                            if (storageInfos == null) {
//                                mIsException = true;
//                                Log.i(TAG, "[ShowSimCardStorageInfoTask]storageInfos  null.");
//                                return null;
//                            }
//                            Log.i(
//                                    TAG,
//                                    "[ShowSimCardStorageInfoTask] infos: "
...
//            Log.i(TAG, "[ShowSimCardStorageInfoTask]: doInBackground_end");
        ...
        public static int getSurplugCount(int subId) {
            if (null != sSurplugMap && sSurplugMap.containsKey(subId)) {
                ...
                Log.d(TAG, "[getSurplugCount] result : " + result + ",subId = " + subId);
                return result;
            } else {
