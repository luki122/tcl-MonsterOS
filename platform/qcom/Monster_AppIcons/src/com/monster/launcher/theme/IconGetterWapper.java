 ...
 */
public class IconGetterWapper implements IIconGetter {
    private static IIconGetter instance;
    private static IIconGetter instance2;
    private static BitmapMemoryCache bitmapCache;
     ...
     * @return
     */
    private IconGetterWapper(Context context, boolean usePkg) {
        if (usePkg) {
    ...
    public static  IIconGetter getInstance(Context context,boolean usePkg){
        synchronized (mLock){
            if(instance==null) {
                instance = new IconGetterWapper(context, usePkg);
    ...
    public IIconGetter setUseMemoryCache(boolean value) {
        if(value){
            bitmapCache = new BitmapMemoryCache(res);
        }else{
            bitmapCache = null;
        }
        return instance;
    ...
    public Drawable getIconDrawable(ActivityInfo info, UserHandle user) {
        ...
        if(bitmapCache!=null){
            drawable = bitmapCache.getDrawableFromMemCache(key);
        }
        if(drawable==null) {
            if(sdcarCache !=null){
