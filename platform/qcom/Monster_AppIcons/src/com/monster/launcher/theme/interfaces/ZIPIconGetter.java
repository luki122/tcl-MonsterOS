...
//                InputStream is = mPreContext.getClassLoader().getResourceAsStream(locaConfigPath);
    ...
    protected Drawable getResurceDrawable(String iconName) {
        ...
        Log.e("---lijun---","drawable : "+iconName+"," + drawable.getIntrinsicHeight()+","+drawable.getIntrinsicWidth());
        if(scale!=1.0f){
    ...
    private Drawable standardThirdPardIconBitmap(Bitmap source){
        ...
        mask_regular = getBitmapByName(Contents.MASK_REGULAR,null);
        bg = getBitmapByName(Contents.BACKGROUND,null);
        mask_unregular = getBitmapByName(Contents.MASK_UNREGULAR,null);
        Rect bound = new Rect();
