...
public class TrafficCorrectionWrapper {
	...
	public static TrafficCorrectionWrapper getInstance() {
		return sInstance;
	}
	...
	public void init(Context context) {
		if (context == null)
			return;
		mPreferences = context.getSharedPreferences("trafficInfo", Context.MODE_PRIVATE);

		mTcMgr = ManagerCreatorB.getManager(TrafficCorrectionManager.class);
		ArrayList<CodeName> allProvinces = mTcMgr.getAllProvinces();
		Log.v(TAG, "allProvinces>>" + allProvinces);
		mTcMgr.setTrafficCorrectionListener(new ITrafficCorrectionListener() {
			...
			public void onNeedSmsCorrection(int simIndex, String queryCode, String queryPort) {
				if (mTrafficCorrectionListener != null) {
					mTrafficCorrectionListener.onNeedSmsCorrection(simIndex, queryCode, queryPort);
				}
			}
			...
			public void onTrafficInfoNotify(int simIndex, int trafficClass, int subClass, int kBytes) {
				...
				if (mTrafficCorrectionListener != null) {
					mTrafficCorrectionListener.onTrafficInfoNotify(simIndex, trafficClass, subClass, kBytes);
				}
			}
	...
	public int setConfig(int simIndex, String provinceId, String cityId, String carryId, String brandId,
			int closingDay) {
		return mTcMgr.setConfig(simIndex, provinceId, cityId, carryId, brandId, closingDay);
	}
