...
public class BlackList extends InterceptionActivityBase {
	...
	private void delAllSelected() {
		...
		if (selectedCount == 1) {
			title = this.getResources().getString(R.string.remove_one_bn);
		} else {
			if (selectedCount < mAdapter.getCount()) {
				title = this.getResources().getString(R.string.remove_multi_bn,
						selectedCount);
			} else {
				title = this.getResources().getString(R.string.remove_all_bn);
			}
		}*/
			...
			if (!cursor.moveToFirst()) {
				...
				do {
					...
					String numberE164 = PhoneNumberUtils.formatNumberToE164(
							number, FormatUtils.getCurrentCountryIso(context));
