...
public class MarkList extends InterceptionActivityBase {
	...
	private void showEditDialogMenu(final int pos) {
		...
		final AlertDialog dialogs = new AlertDialog.Builder(mContext)
				.setTitle(
		...
		mark_content.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if(TextUtils.isEmpty(s)) {
                    dialogs.getButton(DialogInterface.BUTTON_POSITIVE).setEnabled(false);
                } else {
                    dialogs.getButton(DialogInterface.BUTTON_POSITIVE).setEnabled(true);
                }
            }
		...
		if (item.getItemId() == R.id.add) {
			...
			mMarkText
					.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
							15) });
			final AlertDialog dialogs = new AlertDialog.Builder(
					mContext)
					.setTitle(
							mContext.getResources().getString(
									R.string.add_mark))
			...
			dialogs.getWindow().setSoftInputMode(
					WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);
			mMarkText.addTextChangedListener(new TextWatcher() {
	            @Override
	            public void onTextChanged(CharSequence s, int start, int before, int count) {
	                // TODO Auto-generated method stub
	                if(TextUtils.isEmpty(s)) {
