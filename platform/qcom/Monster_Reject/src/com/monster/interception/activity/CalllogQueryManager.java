...
public class CalllogQueryManager {
	...
	void startQuery() {
		isQuery = true;
		//see CallLog.Calls.type,it has 6 type
		//tangyisen delete selection type in (1,3) and,if reject,it will be show
		mQueryHandler.startQuery(0, null, mCallsUri, null,
				"reject=?", new String[] { "1" }, "_id desc");
		callFlag = false;
	}
	...
	private Handler mCallHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			String[] tableCursor = new String[] { "_id", "number", "date",
					"area", "count", "reject", "name", "lable", "simId", "type" };
			...
			for (int i = 0; i < mCallLogEntityList.size(); i++) {
				callCursor
						.addRow(new Object[] { i,
								...
								mCallLogEntityList.get(i).getType(),});
			}
			if (mCallListAdapter == null) {
	...
	void setView(View v, boolean showEmpty){
        phone = v;
        mCalllogList=(MstListView) phone.findViewById(R.id.phone_list);
        mCallogEmpty=(TextView) phone.findViewById(R.id.phone_empty);
        if(showEmpty) {
	...
	void setView(View v, boolean showEmpty, PagerAdapter pagerAdapter){
        phone = v;
        mCalllogList=(MstListView) phone.findViewById(R.id.phone_list);
        mCallogEmpty=(TextView) phone.findViewById(R.id.phone_empty);
