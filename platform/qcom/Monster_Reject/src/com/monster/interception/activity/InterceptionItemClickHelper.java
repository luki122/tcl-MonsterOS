		...
		public void handleMessage(android.os.Message msg) {
			if (msg.arg1 == 0) {
				...
				if (msg.arg1 == 2) {
					...
					Toast.makeText(mActivity, mActivity.getString(R.string.delete_record_success_toast), Toast.LENGTH_SHORT).show();
				} else if (msg.arg1 == 1){
					smsBath = false;
					hideDialog();
                    ...
                    hideDialog();
                    mActivity.mSmsQueryManager.startQueryMms();
                    Toast.makeText(mActivity, mActivity.getString(R.string.recover_record_success_toast), Toast.LENGTH_SHORT).show();
				}
		...
		new Thread() {
			...
			public void run() {
                ...
                /*mContentResolver.delete(uriSms,
                        "address=? and type=1 and reject=1",
                        new String[] { smsNumbers.get(i) });*/
				    //begin tangyisen add for update thread
				    /*Uri uriThread = Uri.parse("content://mms-sms/mst-update-thread/" + smsIds.get(i));
				    mContentResolver.update(uriThread , null, null, null);*/
				    //end tangyisen
				    Uri conversationUpdateUri = Uri.parse("content://mms-sms/mst-update-thread");
				    threadValuesInsert = new ContentValues();
				    threadValuesInsert.put("threadId", smsIds.get(i));
				    mContentResolver.update(conversationUpdateUri, threadValuesInsert, null, null);
				}
				message = mDialogHandler.obtainMessage();
                message.arg1 = 3;
                message.sendToTarget();
