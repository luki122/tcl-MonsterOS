...
public class SettingsFragment extends PreferenceFragment  implements OnPreferenceChangeListener{
    //private CountAndNarrowPreference mBlack;
    private PreferenceScreen mBlack, mMark;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mBlack = (PreferenceScreen) findPreference("black_name");
        mMark = (PreferenceScreen) findPreference("mark");
        //mMark.setStatus("0个");
    ...
    public void onResume() {
        ...
        mQueryHandler.startQuery(1, null, BlackUtils.MARK_URI, null, "lable is not null"/*null*/ , null, null);
    }
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            if (cursor != null) {
                if (token == 0) {
                    ...
                    // true);
                    //mBlack.setCount(mBlackCount + getResources().getString(R.string.item));
                    mBlack.setStatus(mBlackCount + getResources().getString(R.string.item));
                } else {
                    mMarkCount = cursor.getCount();
                    // mark.SetArrowText(mark_list_size+getResources().getString(R.string.item),
                    // true);
                    mMark.setStatus(mMarkCount + getResources().getString(R.string.item));
                }
            }
            if (cursor != null) {
                cursor.close();
