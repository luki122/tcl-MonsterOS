...
public class CallLogDatabaseHelper {
	...
	public static List<CallLogEntity> queryCallLogs(Cursor cursor,
		...
		do {
			...
			//tangyisen fix exception
			subscriptionId = cursor.getString(cursor.getColumnIndex("subscription_id"));
			if(TextUtils.isEmpty(subscriptionId)) {
			    simId = -1;
			} else {
			    simId = Integer.valueOf(subscriptionId.trim());
			}
			//simId = Integer.valueOf(cursor.getString(cursor.getColumnIndex("subscription_id")));

			simId = SimUtils.getSlotbyId(context, simId);
			callLogEntity = temMap.get(number + "simId:" + simId);
			if (callLogEntity == null) {
				...
				temMap.put(number + "simId:" + simId, callLogEntity);
			}
