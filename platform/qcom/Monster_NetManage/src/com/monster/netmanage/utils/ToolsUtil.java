...
public class ToolsUtil {
	...
	public static ArrayList<PackageInfo>  getPackageInfos(Context context) {
		  if (mAllApps != null && mAllApps.size() > 0) {
			  ...
			  mAllApps = new ArrayList<PackageInfo>();
//		       NetworkPolicyManager policyManager = NetworkPolicyManager.from(context);
		       ArrayList<PackageInfo> netAppInfos = new ArrayList<PackageInfo>();
		       //除去uid相同的应用
		       ArrayList<PackageInfo> appInfos = new ArrayList<PackageInfo>();
...
//	                   	    			ApplicationInfo appInfo = packageInfo.applicationInfo;
	                   ...
	                   //除去uid相同的应用
	                  for (PackageInfo packageInfo : netAppInfos) {
		       		     if (appInfos.size() == 0) {
		       		    	 appInfos.add(packageInfo);
		       		     } else {
		       		    	 boolean exist = false;
		       		    	 for (int i = 0; i < appInfos.size(); i++) {
		       					if (packageInfo.applicationInfo.uid == appInfos.get(i).applicationInfo.uid) {
		       						exist = true;
		       						break;
		       					}
		       				}
	...
	public static String[] getIMSI(Context context) {
		...
		// 根据卡状态来创建卡imsi的数组  
		String[] imsis = new String[mSelectableSubInfos.size()];  
	...
	public static void updateIconReceiver() {
		...
		if (totalData == 0) {
			...
			if (reaminRate > 0 && reaminRate <= 20) {
				dataIntent.putExtra("remain_data_unit", StringUtil.formatFloatDataFlowSize(DataManagerApplication.getInstance(), remainData));
