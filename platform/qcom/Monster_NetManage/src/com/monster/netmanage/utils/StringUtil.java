...
public class StringUtil {
	...
	public static int getDaysToMonthEndDay(int monthEndDay) {
		...
		Log.e("StringUtil", "StringUtil--day>>>>" + currentDay);
		// 当前月最后一天
		...
		if (lastDay >= monthEndDay) {
			// 当前最后一天大于或等于月结日
			days = monthEndDay - currentDay;
			if (days < 0) {
				// 计算距下个月的月结日的时间
				days = (lastDay - currentDay) + monthEndDay;
			}
		} else {
			// 当前最后一天小于等于月结日
			days = lastDay - currentDay;
		}
		return days;
	}
	 ...
	 * @return  
	 */
	  public static String formatDataFlowSize(Context context, float kBytes) {
...
//	        long mult = 1;
     ...
     * @return
     */
	public static String formatFloatDataFlowSize(Context context, float kBytes) {
		Resources res = context.getResources();
		float result = kBytes;
		int suffix = R.string.kilobyte_short;
//	        long mult = 1;
