...
public class RecipientEditTextView extends MultiAutoCompleteTextView implements
    ...
    public void onFocusChanged(boolean hasFocus, int direction, Rect previous) {
        ...
        //lichao add for temp in 2016-10-22
        //commitDefault();
    }
...
//                height - backgroundPadding.top - backgroundPadding.bottom : 0;
    ...
    private boolean commitDefault() {
        ...
        if (mTokenizer == null) {
            //Log.d(TAG, "commitDefault(), return false 111");
            return false;
        }
        Editable editable = getText();
        int selectionEnd = getSelectionEnd();
        int whatStart = mTokenizer.findTokenStart(editable, selectionEnd);
        //Log.d(TAG, "commitDefault(), whatStart = "+whatStart);
        //Log.d(TAG, "commitDefault(), selectionEnd = "+selectionEnd);

        int whatEnd = mTokenizer.findTokenEnd(editable, whatStart);
        //Log.d(TAG, "commitDefault(), whatEnd = "+whatEnd);

        if (shouldCreateChip(whatStart, selectionEnd)) {
            // In the middle of chip; treat this as an edit
            // and commit the whole token.
            whatEnd = movePastTerminators(whatEnd);
            //Log.d(TAG, "commitDefault(), whatEnd = "+whatEnd);
            ...
            if (whatEnd != selectionEnd) {
                handleEdit(whatStart, whatEnd);
                //Log.d(TAG, "commitDefault(), return true 222");
                return true;
            }
            boolean result = commitChip(whatStart, selectionEnd , editable);
            //Log.d(TAG, "commitDefault(), return 333, result = "+result);
            return result;
        }
        //Log.d(TAG, "commitDefault(), return false 444");
        return false;
    }
