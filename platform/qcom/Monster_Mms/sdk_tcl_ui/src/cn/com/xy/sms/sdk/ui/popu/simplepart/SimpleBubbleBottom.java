...
public class SimpleBubbleBottom extends RelativeLayout {
    ...
    private UiPartInterface mUiPartAction;
    ...
    public SimpleBubbleBottom(Activity mContext, JSONArray jsonArray, HashMap<String, Object> extend) throws Exception {
        ...
        mUiPartAction = ViewManger.getUiPartInterface();
        int layoutId = R.layout.duoqu_simple_bubble_bottom_two;
        if(mUiPartAction != null){
            try{
                layoutId = (Integer) mUiPartAction.doUiAction(ViewManger.UIPART_ACTION_GET_SB_LY_ID, null);
            }catch (Throwable e) {
            }
        }
    ...
    public void setContent(JSONArray jsonArray, HashMap<String, Object> extend) {
        ...
        try {
            ...
            mSize = arrays.size();
            Boolean layoutClickFlag = (Boolean) mUiPartAction.doUiAction(ViewManger.UIPART_ACTION_SIMPLE_BOTTOM_LAYOUT_CLICK, null);
            if (mSize == 1) {
                ...
                if(layoutClickFlag != null && layoutClickFlag){
                    SimpleButtonUtil.setBotton(mContext, mBtn1, mTextView1, arrays.get(0), mDisLogo, mExtend);
                }else{
                ...
                if(layoutClickFlag != null && layoutClickFlag){
                    SimpleButtonUtil.setBotton(mContext, mBtn1, mTextView1, arrays.get(0), mDisLogo, mExtend);
                    SimpleButtonUtil.setBotton(mContext, mBtn2, mTextView2, arrays.get(1), mDisLogo, mExtend);
                }else{
                    SimpleButtonUtil.setBotton(mContext, mTextView1, mTextView1, arrays.get(0), mDisLogo, mExtend);
