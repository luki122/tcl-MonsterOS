...
public class WorkingMessage {
    private static final String TAG = "Mms/WorkingMessage";
    ...
    //tangyisen add for text too long to convert to mms
    public static final int MMS_TEXT = 6;

    // Current attachment type of the message; one of the above values.
    private int mAttachmentType;
    //tangyisen extra
    private int mAttachmentExtraWidth = 0;
    private int mAttachmentExtraHeight = 0;
    ...
    private WorkingMessage(ComposeMessageActivity activity) {
        ...
        mText = "";
        //begin tangyisen extra
        mMaxImageWidth = activity.getResources().getDimension(R.dimen.mms_inline_attachment_size);
        mMaxImageHeight = activity.getResources().getDimension(R.dimen.mms_inline_attachment_size);
        //end tangyisen
    }
    ...
    public void setWorkingMessageSub(int subId) {
        if(DEBUG) Log.e(TAG, "setWorkingMessageSub: subId = " + subId);
    ...
    public int setAttachment(int type, Uri dataUri, boolean append) {
        ...
        //tangyisen add VIDEO for extra
        if (mSlideshow != null && (type == IMAGE || type == VIDEO)) {
            ...
            if (numSlides > 0) {
                ...
                SlideModel slide = mSlideshow.get(numSlides - 1);
                if (slide.hasImage()) {
                    //mItemLoadedFuture = slide.getImage().loadThumbnailBitmap(mImageLoadedCallback);
                    cancelThumbnailLoading();
                    mItemLoadedFuture = slide.getImage().loadThumbnailBitmap(mImageLoadedCallback);
                } else if (slide.hasVideo()) {
                    //mItemLoadedFuture = slide.getVideo().loadThumbnailBitmap(mImageLoadedCallback);
                    cancelThumbnailLoading();
                    mItemLoadedFuture = slide.getVideo().loadThumbnailBitmap(mImageLoadedCallback);
                }
            }
    ...
    //begin tangyisen
    public boolean hasAttachmentNotText() {
        return (mAttachmentType > TEXT) && (mAttachmentType != MMS_TEXT);
    }
    //end tangyisen
        ...
        try {
            ...
            if (!mConversation.getRecipients().isEmpty()) {
                ...
                if(MmsApp.isCreateConversaitonIdBySim) {
                	mConversation.ensureThreadId(mCurrentConvSubId);
                } else {
            ...
            if (mMessageUri == null) {
                ...
                /*mMessageUri = createDraftMmsMessage(persister, sendReq, mSlideshow, null,
                        mActivity, null);*/
                mMessageUri = createDraftMmsMessage(persister, sendReq, mSlideshow, null,
                    mActivity, null, mConversation.getThreadId());
                //end tangyisen
            } else {
    ...
    public void saveDraft(final boolean isStopping) {
        ...
        if (requiresMms()) {
            ...
            //begin tangyisen add for null point
    ...
    public void setHasMultipleRecipients(boolean hasMultipleRecipients, boolean notify) {
        ...
        updateState(MULTIPLE_RECIPIENTS,
                hasMultipleRecipients &&
                    /*MessagingPreferenceActivity.getIsGroupMmsEnabled(mActivity),*/false,
                notify);
    }
    ...
    */
    ...
    private void updateState(int state, boolean on, boolean notify) {
        ...
        if (notify) {
            if (oldState == 0 && mMmsState != 0) {
                ...
                //begin tangyisen
                if((mMmsState & LENGTH_REQUIRES_MMS) > 0 && mAttachmentType == TEXT) {
                    mAttachmentType = MMS_TEXT;
                }
                //end tangyisen
            } else if (oldState != 0 && mMmsState == 0) {
                mStatusListener.onProtocolChanged(false);
                //begin tangyisen
                if((mMmsState & LENGTH_REQUIRES_MMS) > 0 && mAttachmentType == MMS_TEXT) {
                    mAttachmentType = TEXT;
                }
                //end tangyisen
            }
    ...
    private void preSendSmsWorker(Conversation conv, String msgText, String recipientsInUI) {
        ...
        long threadId;
        if(MmsApp.isCreateConversaitonIdBySim) {
          	threadId = conv.ensureThreadId(mCurrentConvSubId);
        } else {
        ...
        //tangyisen delete for can send msg when threadid is not same
       ...
       /* if (LogTag.SEVERE_WARNING && ((origThreadId != 0 && origThreadId != threadId) ||
            ...
            //ligy modify begin
            if(MmsApp.isCreateConversaitonIdBySim) {
                 deleteDraftSmsMessage(origThreadId);
            } else {
    ...
    private void sendMmsWorker(Conversation conv, Uri mmsUri, PduPersister persister,
        ...
        try {
            ...
            //tangyisen
            newMessage = (mmsUri == null) || forwardMessage;
            if (newMessage) {
                ...
                //begin tangyisen add extra
                //calcAttachmentExtraWidthAndHeight(slideshow);
                values.put("extra", mAttachmentType + MessageUtils.WIDTHPLUSHEIGHT + mAttachmentExtraWidth + MessageUtils.WIDTHPLUSHEIGHT + mAttachmentExtraHeight);
                //end tangyisen
                mmsUri = SqliteWrapper.insert(mActivity, mContentResolver, Mms.Outbox.CONTENT_URI,
                        values);
            } else {
                //tangyisen add for update threadId
                ContentValues values = new ContentValues();
                values.put(Mms.THREAD_ID, threadId);
                //begin tangyisen add extra
                //calcAttachmentExtraWidthAndHeight(slideshow);
    ...
    private void calcAttachmentExtraWidthAndHeight(SlideshowModel slideshow) {
        mAttachmentExtraWidth = 0;
        mAttachmentExtraHeight = 0;
        /*for(int i = 0; i < slideshow.size(); i ++) {
            SlideModel slideModel = slideshow.get(i);
            if(slideModel.getImage() != null) {
                ImageModel imageModel = slideModel.getImage();
                mAttachmentExtraWidth = Math.min(mMaxImageWidth , imageModel.getWidth());
                mAttachmentExtraHeight = Math.min(mMaxImageHeight , imageModel.getHeight());
                return;
            } else if(slideModel.getVideo() != null) {
                return;
            }
        }*/
    ...
    private ItemLoadedCallback<ImageLoaded> mImageLoadedCallback = new ItemLoadedCallback<ImageLoaded>() {
        public void onItemLoaded(ImageLoaded imageLoaded, Throwable exception) {
            if (exception == null) {
                ...
                if(imageLoaded.mBitmap != null) {
                    Bitmap src = imageLoaded.mBitmap;
                    if (imageLoaded.mIsVideo) {
                        mAttachmentExtraWidth = (int)mMaxImageWidth;
                        float scale = mMaxImageWidth / src.getWidth();
                        mAttachmentExtraHeight = (int)(src.getHeight() * scale);
                    } else {
                        mAttachmentExtraWidth = Math.min((int)mMaxImageWidth , src.getWidth());
                        mAttachmentExtraHeight = Math.min((int)mMaxImageHeight , src.getHeight());
                    }
                }
            }
        }
    ...
    private void markMmsMessageWithError(Uri mmsUri) {
        if(DEBUG) Log.d(TAG, "markMmsMessageWithError(), mmsUri = "+mmsUri);
        try {
            ...
            ContentValues values = new ContentValues(1);
            if(DEBUG) Log.d(TAG, "For Outbox Mms, >>>values.put(ERROR_TYPE, ERR_TYPE_GENERIC_PERMANENT(10))");
            //for Outbox Mms, not change ERR_TYPE_GENERIC_PERMANENT to ERR_TYPE_MMS_DOWNLOAD_FAILURE
    ...
    private static Uri createDraftMmsMessage(PduPersister persister, SendReq sendReq,
        SlideshowModel slideshow, Uri preUri, Context context,
        HashMap<Uri, InputStream> preOpenedFiles, long threadId) {
        if (slideshow == null) {
            return null;
        }
        try {
            PduBody pb = slideshow.toPduBody();
            sendReq.setBody(pb);
            //tangyisen modify tmp
            Uri res = persister.persist(sendReq, preUri == null ? Mms.Draft.CONTENT_URI : preUri,
                threadId, MessagingPreferenceActivity.getIsGroupMmsEnabled(context),
                    preOpenedFiles);
            slideshow.sync(pb);
            return res;
        } catch (MmsException e) {
            ...
            public void run() {
                try {
                    ...
                    if (mMessageUri == null) {
                        ...
                        mMessageUri = createDraftMmsMessage(persister, sendReq, mSlideshow, null,
                                mActivity, preOpenedFiles, conv.getThreadId());
                    } else {
    ...
    private static void updateDraftMmsMessage(Uri uri, PduPersister persister,
        ...
        persister.updateHeaders(uri, sendReq, false);
    ...
    private void ensureThreadIdIfNeeded(final Conversation conv, final boolean isStopping) {
        ...
        if (!conv.getRecipients().isEmpty()) {
		    //ligy modify begin
            if(MmsApp.isCreateConversaitonIdBySim) {
              	conv.ensureThreadId(mCurrentConvSubId);
            } else {
            	conv.ensureThreadId();
            }
			//ligy modify end
    ...
    public void updateMmsSizeIndicator(TextView view, int textSize) {
        //TextView sizeIndicator = (TextView) findViewById(R.id.mms_size_indicator);

        //ensureTextModelSlideshow();
        ensureSlideshow();
        syncTextToSlideshow();
        if(null != mSlideshow) {
            mSlideshow.updateTotalMessageSize();
            int totalTextSize = mSlideshow.getTotalTextMessageSize();
            int mediaSize = mSlideshow.getTotalMessageSize();
        //int mediaSize = mSlideshow.getCurrentMessageSize();
       // if (mediaSize == 0){
            //sizeIndicator.setVisibility(View.GONE);
        //} else {
            //sizeIndicator.setVisibility(View.VISIBLE);
            int currentSize = getSizeWithOverHead(mediaSize + mSlideshow.getSubjectSize() - totalTextSize + textSize);
            view.setText(mActivity.getString(R.string.mms_size_indicator,
    ...
    private void ensureTextModelSlideshow() {
        if (mSlideshow != null) {
            return;
        }
        SlideshowModel slideshow = SlideshowModel.createNew(mActivity);
        ...
        //add will add the size,and add will add too,so double here
        slide.add(text);
        slideshow.add(slide);
        text.setText(mText);
        mSlideshow = slideshow;
