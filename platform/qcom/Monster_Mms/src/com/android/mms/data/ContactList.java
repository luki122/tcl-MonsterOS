...
public class ContactList extends ArrayList<Contact>  {
    ...
    //tangyisen
    public String formatNames(String separator, int count) {
        if(count == 0){
            return null;
        }
    ...
    public long[] getDataIds(boolean includeEmail) {
        List<Long> dataIds = new ArrayList<Long>();
        long contactMethodId = -1;
        for (Contact c : this) {
            contactMethodId = c.getContactMethodId();
            if (contactMethodId != -1) {//Contact.CONTACT_METHOD_ID_UNKNOWN) {
                if(includeEmail) {
                    dataIds.add(contactMethodId);
                } else {
                   if (c.getContactMethodType() == Contact.CONTACT_METHOD_TYPE_EMAIL) {
                       continue;
                   }
                   dataIds.add(contactMethodId);
                }
        ...
        long[] rtn = new long[dataIds.size()];
        int count = 0;
