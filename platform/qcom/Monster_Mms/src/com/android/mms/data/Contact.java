...
public class Contact {
    ...
    //tangyisen add when the contacts name and number is same,it is duplicated
    @Override
    public boolean equals(Object o) {
        // TODO Auto-generated method stub
        if(o instanceof Contact) {
            if(this.getName().equals(((Contact)o).getName())
                && this.getNumber().equals(((Contact)o).getNumber())) {
                return true;
            }
        }
        return false;
    ...
    public int hashCode() {
        // TODO Auto-generated method stub
        return this.getName().hashCode() + 3 * this.getNumber().hashCode();
    }
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!TextUtils.isEmpty(name) && !name.equals(number)) {
            ...
            return name + " <" + formattedNumber.replaceAll(" ", "") + ">";
        } else {
    ...
    public synchronized void setNumber(String number) {
        //begin tangyisen add
        if (!TextUtils.isEmpty(number) && !Mms.isEmailAddress(number)) {
            String defaultCountryIso = MmsApp.getApplication().getCurrentCountryIso();
            //mNumberE164 function is to reset the value of defaultFormattingType
            mNumber = PhoneNumberUtils.formatNumber(number, mNumberE164, defaultCountryIso);
        } else {
            mNumber = number;
        }
    ...
    private static class ContactsCache {
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                //numberE164=+8615019467235 if number=15019467235
        ...
        private Contact internalGet(String numberOrEmail, boolean isMe) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = isMe || Mms.isEmailAddress(numberOrEmail) ||
                        ...
                        MessageUtils.isSpecialNumber(numberOrEmail);//tangyisen add tmp
    ...
    public void addToBlacklist(Context context){
        String number = getNumber();
        String name = getRealName();
        if(TextUtils.isEmpty(number)){
            Toast.makeText(context, R.string.get_recipient_number_failed, Toast.LENGTH_SHORT).show();
            return;
        }
        BlackUtils.addblack(context, number, name);
    }
