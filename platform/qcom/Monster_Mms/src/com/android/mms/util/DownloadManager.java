...
public class DownloadManager {
    private static final String TAG = LogTag.TAG+"/DownloadManager";
    ...
    public static final int STATE_UNSTARTED         = 0x80;//128
    public static final int STATE_DOWNLOADING       = 0x81;//129
    public static final int STATE_TRANSIENT_FAILURE = 0x82;//130
    public static final int STATE_PERMANENT_FAILURE = 0x87;//135
    public static final int STATE_PRE_DOWNLOADING   = 0x83;//131
    public static final int STATE_SKIP_RETRYING     = 0x89;//137
    ...
    public void markState(final Uri uri, int state) {
        ...
        if(DEBUG) Log.d(TAG,"markState(), uri = "+uri+", state="+state);
        try {
        ...
        if (state == STATE_PERMANENT_FAILURE) {
            mHandler.post(new Runnable() {
                public void run() {
                    try {
                        Toast.makeText(mContext, getToastMessage(uri),
                                Toast.LENGTH_LONG).show();
                    } catch (MmsException e) {
                        Log.e(TAG, e.getMessage(), e);
                    }
        ...
        if(DEBUG) Log.d(TAG,"markState(), values.put(Mms.STATUS, state), state="+state);
    ...
    private String getToastMessage(Uri uri) throws MmsException {
        NotificationInd ind = (NotificationInd) PduPersister
                .getPduPersister(mContext).load(uri);
