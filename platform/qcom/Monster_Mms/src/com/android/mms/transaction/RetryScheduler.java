...
public class RetryScheduler implements Observer {
    private static final String TAG = LogTag.TAG + "/RetryScheduler";
            ...
            try {
                if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    ...
                    if ((retryIndex < scheme.getRetryLimit()) && retry) {
                        ...
                        //retry Download case
                        if (isRetryDownloading) {
                            if(DEBUG) Log.w(TAG, "scheduleRetry(), retry, >>>markState(STATE_TRANSIENT_FAILURE)");
                        ...
                        if (isRetryDownloading) {
                            ...
                            if(DEBUG) Log.w(TAG, "scheduleRetry(), not retry>>>markState(STATE_PERMANENT_FAILURE)");
                            DownloadManager.getInstance().markState(
                    ...
                    //add isRetryDownloading for only Download case
                    if(isRetryDownloading && errorType == MmsSms.ERR_TYPE_GENERIC_PERMANENT){
                        if(DEBUG) Log.d(TAG, "scheduleRetry(), before put(), change errorType " +
                                "from GENERIC_PERMANENT to DOWNLOAD_FAILURE");
                        errorType = MmsSms.ERR_TYPE_MMS_DOWNLOAD_FAILURE;
                    }
