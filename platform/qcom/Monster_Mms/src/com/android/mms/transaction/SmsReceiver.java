...
public class SmsReceiver extends BroadcastReceiver {
    ...
    private static final String TAG = LogTag.TAG + "/SmsReceiver";
    ...
    protected void onReceiveWithPrivilege(Context context, Intent intent, boolean privileged) {
        ...
        if (!privileged && (Intents.SMS_DELIVER_ACTION.equals(action) ||
            ...
            if (DEBUG) Log.v(TAG, "onReceiveWithPrivilege, >>>return");
            return;
        }

        intent.setClass(context, SmsReceiverService.class);
        intent.putExtra("result", getResultCode());
        if (DEBUG) Log.v(TAG, "onReceiveWithPrivilege, >>>beginStartingService()");
