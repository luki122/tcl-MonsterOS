...
public class MmsMessageSender implements MessageSender {
    private static final String TAG = LogTag.TAG + "/MmsMessageSender";
    ...
    public boolean sendMessage(long token) throws MmsException {
        ...
        //begin tangyisen
        p.updateHeaders(mMessageUri, sendReq, false/*isNeedUpdateThreadId*/);
        //end tangyisen
        int readReport = sendReq.getReadReport();
        if(DEBUG) Log.v(TAG, "sendMessage, after updateHeaders, readReport = " + readReport);
    ...
    private void updatePreferencesHeaders(SendReq sendReq) throws MmsException {
        ...
        if(DEBUG) Log.v(TAG, "updatePreferencesHeaders DeliveryReport = " + dr);
        ...
        if(DEBUG) Log.v(TAG, "updatePreferencesHeaders ReadReport = " + rr);
        //public static final int VALUE_YES                       = 0x80;//128
        //public static final int VALUE_NO                        = 0x81;//129
    ...
    public static void sendReadRec(Context context, String to, String messageId, int subId,
            int status) {
        if(DEBUG) Log.d(TAG, "sendReadRec, to = "+to+", messageId = "+messageId
                +", subId = "+subId+", status = "+status);
        ...
        try {
            ...
            Uri uri;
            if(null != sender[0]){
                String recipient = sender[0].getString();
                if(DEBUG) Log.d(TAG, "sendReadRec, recipient = "+recipient);
                long threadId = Threads.getOrCreateThreadId(context, recipient, subId);
                uri = PduPersister.getPduPersister(context).persist(readRec, Mms.Outbox.CONTENT_URI,
                        threadId, MessagingPreferenceActivity.getIsGroupMmsEnabled(context), null);
            }else{
                uri = PduPersister.getPduPersister(context).persist(readRec, Mms.Outbox.CONTENT_URI,
                        true, MessagingPreferenceActivity.getIsGroupMmsEnabled(context), null);
            }
