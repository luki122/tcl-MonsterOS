...
public class NotificationTransaction extends Transaction implements Runnable {
    private static final String TAG = LogTag.TAG+"/NotificationTransaction";
    ...
    public NotificationTransaction(
        ...
        mUri = Uri.parse(uriString).buildUpon().appendQueryParameter("isIgnoreReject","true").build();
    ...
    public void run() {
        ...
        try {
            ...
            if (isMemoryFull || isTooLarge) {
                if(DEBUG) Log.w(TAG, "run(), (isMemoryFull || isTooLarge)>>>markState(STATE_PERMANENT_FAILURE)");
            ...
            if (retrieveConfData != null) {
                ...
                if ((pdu == null) || (pdu.getMessageType() != MESSAGE_TYPE_RETRIEVE_CONF)) {
                    ...
                    //tangyisen modify for download mms will be add in true subid
                    /*Uri uri = p.persist(pdu, Inbox.CONTENT_URI, true,
                            MessagingPreferenceActivity.getIsGroupMmsEnabled(mContext), null);*/
                    Uri uri = null;
                    if (mThreadId != 0) {
                        uri = p.persist(pdu, Inbox.CONTENT_URI, mThreadId,
                            MessagingPreferenceActivity.getIsGroupMmsEnabled(mContext), null);
                    } else {
                        uri = p.persist(pdu, Inbox.CONTENT_URI, true,
                            MessagingPreferenceActivity.getIsGroupMmsEnabled(mContext), null);
                    }
                    //tangyisen end
                    ...
                    if (c != null) {
                        try {
                            if (c.moveToFirst()) {
                                ...
                                //begin tangyisen modify the threadId
                                long threadId = c.getLong(c.getColumnIndex(Mms.THREAD_ID));
