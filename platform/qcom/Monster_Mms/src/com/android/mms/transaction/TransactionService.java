...
import mst.provider.Telephony.Mms.Sent;
...
public class TransactionService extends Service implements Observer {
    private static final String TAG = LogTag.TAG + "/TransactionService";
    ...
    private void onMmsPdpConnected(String subId, Network network) {
        Log.d(TAG, "onMmsPdpConnected subId = " + subId);
    ...
    private int getSubIdFromDb(Uri uri) {
        ...
        Cursor c = getApplicationContext().getContentResolver().query(uri,
                null, null, null, null);
        if(DEBUG) Log.d(TAG, "Cursor= " + DatabaseUtils.dumpCursorToString(c));
        if (c != null) {
            try {
                if (c.moveToFirst()) {
    ...
    public void onNewIntent(Intent intent, int serviceId) {
        ...
        if ((ACTION_ONALARM.equals(action) || ACTION_ENABLE_AUTO_RETRIEVE.equals(action) ||
            ...
            if(DEBUG) Log.d(TAG, "Cursor= " + DatabaseUtils.dumpCursorToString(cursor));
            if (cursor != null) {
                try {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        switch (transactionType) {
                            ...
                            case Transaction.RETRIEVE_TRANSACTION:
                                ...
                                if(failureType == MmsSms.ERR_TYPE_MMS_DOWNLOAD_FAILURE){
                                    if(DEBUG) Log.d(TAG, "scheduleRetry(), after get(), change errorType " +
                                            "from DOWNLOAD_FAILURE to GENERIC_PERMANENT");
                                    failureType = MmsSms.ERR_TYPE_GENERIC_PERMANENT;
                                }
                                ...
                                if (!(failureType == MmsSms.NO_ERROR || isTransientFailure(failureType))) {
                                //if ((failureType != MmsSms.NO_ERROR && !isTransientFailure(failureType))) {
                                //if (failureType < MmsSms.NO_ERROR || failureType >= MmsSms.ERR_TYPE_GENERIC_PERMANENT) {
                                //if (failureType < MmsSms.NO_ERROR || failureType >= MmsSms.ERR_TYPE_MMS_DOWNLOAD_FAILURE) {
                                    if (DEBUG) {
                            ...
                            default:
                                ...
                                if (DEBUG) Log.d(TAG, "onNewIntent: subId = " + subId);
                                if (subId < 0) {
    ...
    public static final int ERR_TYPE_GENERIC              = 1;//普通错误
    public static final int ERR_TYPE_SMS_PROTO_TRANSIENT  = 2;//短信协议临时错误，protocol缩写为PROTO
    public static final int ERR_TYPE_MMS_PROTO_TRANSIENT  = 3;//彩信协议临时错误
    public static final int ERR_TYPE_TRANSPORT_FAILURE    = 4;//传输错误
    public static final int ERR_TYPE_MMS_DOWNLOAD_FAILURE = 9;//lichao add for 下载出错
    public static final int ERR_TYPE_GENERIC_PERMANENT    = 10;//永久错误
    public static final int ERR_TYPE_SMS_PROTO_PERMANENT  = 11;//短信协议永久错误
    public static final int ERR_TYPE_MMS_PROTO_PERMANENT  = 12;//彩信协议永久错误
     */
    private static boolean isTransientFailure(int type) {
        return type >= MmsSms.NO_ERROR && type < MmsSms.ERR_TYPE_GENERIC_PERMANENT;
        //return type >= MmsSms.NO_ERROR && type < MmsSms.ERR_TYPE_MMS_DOWNLOAD_FAILURE;
    }
    ...
    private void onNetworkUnavailable(int serviceId, int transactionType,
        ...
        if (DEBUG) {
            Log.v(TAG, "onNetworkUnavailable: serviceId=" + serviceId
        ...
        try {
            ...
            if (cursor != null && (cursor.getCount() == 1) && cursor.moveToFirst()) {
                int index = cursor.getInt(cursor.getColumnIndexOrThrow(
                        PendingMessages.RETRY_INDEX));
                if(DEBUG) Log.v(TAG, "getRetryIndex:" + index) ;
                return index;
            }
            return 0;
        } finally {
    ...
    private void updateMsgErrorType(Uri mmsUri, int errorType) {
        if(DEBUG) Log.i(TAG,"updateMsgErrorType mmsUri="+mmsUri+", errorType="+errorType);
        //only input ERR_TYPE_MMS_PROTO_TRANSIENT, so no need change GENERIC_PERMANENT to DOWNLOAD_FAILURE
        ...
        try {
            ...
            switch (result) {
                ...
                case TransactionState.FAILED:
                    if (DEBUG) {
                        Log.v(TAG, "update(), case TransactionState.FAILED, serviceId: " + serviceId);
                    }
    ...
    private void acquireWakeLock() {
        ...
        if(DEBUG) Log.v(TAG, "mms acquireWakeLock");
        mWakeLock.acquire();
    }
    ...
    private void releaseWakeLock() {
        ...
        if (mWakeLock != null && mWakeLock.isHeld()) {
            if(DEBUG) Log.v(TAG, "mms releaseWakeLock");
            mWakeLock.release();
        }
    }
    ...
    */
    protected void beginMmsConnectivity(int subId) throws IOException {
        ...
        if(DEBUG) Log.v(TAG, "beginMmsConnectivity for subId = " + subId +" phoneId=" + phoneId);
    ...
    private final class ServiceHandler extends Handler {
                    ...
                    try {
                        ...
                        switch (transactionType) {
                            case Transaction.NOTIFICATION_TRANSACTION:
                                ...
                                if (uri != null) {
                                    ...
                                    if ((ind != null) && (ind.getMessageType() == type)) {
                                        ...
                                        //begin tangyisen
                                        transaction.setThreadId(args.getBundle().getLong("thread_id", 0));
                                        //end tangyisen
                                    } else {
                            ...
                            case Transaction.RETRIEVE_TRANSACTION:
                                ...
                                transaction.setThreadId(args.getBundle().getLong("thread_id", 0));
                                //end tangyisen
                                //begin tangyisen add for download mms
                                break;
                            case Transaction.SEND_TRANSACTION:
        ...
        public void markAllPendingTransactionsAsFailed() {
            Log.d(TAG, "markAllPendingTransactionAsFailed");
            synchronized (mProcessing) {
