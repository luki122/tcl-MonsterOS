...
import mst.provider.Telephony.Mms.Inbox;
//tangyisen add 
...
public class PushReceiver extends BroadcastReceiver {
    private static final String TAG = LogTag.TAG + "/PushReceiver";
            ...
            try {
                switch (type) {
                    ...
                    case MESSAGE_TYPE_READ_ORIG_IND: {
                        ...
                        if(DEBUG) Log.d(TAG, "doInBackground, findThreadId() = "+threadId);
                        if (threadId == -1) {
                        ...
                        //lichao modify for NullPointerException in 2016-12-19
                        /*
//E Mms/PushReceiver: java.lang.NullPointerException: Attempt to invoke virtual method 'long java.lang.Long.longValue()' on a null object reference
//E Mms/PushReceiver: 	at com.google.android.mms.pdu.PduPersister.persist(PduPersister.java:1705)
//E Mms/PushReceiver: 	at com.android.mms.transaction.PushReceiver$ReceivePushTask.doInBackground(PushReceiver.java:197)
                         */
                        //Uri uri = p.persist(pdu, Inbox.CONTENT_URI, true/*createThreadId*/,
                        //        MessagingPreferenceActivity.getIsGroupMmsEnabled(mContext), null);
						
                        //Uri uri = p.persist(pdu, Inbox.CONTENT_URI, true,
                        //        MessagingPreferenceActivity.getIsGroupMmsEnabled(mContext), null);
                        Uri uri = p.persist(pdu, Inbox.CONTENT_URI, threadId,
                                MessagingPreferenceActivity.getIsGroupMmsEnabled(mContext), null);

                        //if (null == uri) {
                        //    Log.w(TAG, "doInBackground, uri is null, break");
                        //    break;
                        //}
                        ...
                        if(DEBUG) Log.d(TAG, "doInBackground, SqliteWrapper.update(), threadId = "+threadId);
                    ...
                    case MESSAGE_TYPE_NOTIFICATION_IND: {
                        ...
                        if (!isDuplicateNotification(mContext, nInd)) {
                            ...
                            //begin tangyisen add
                            //long retVal = Threads.getOrCreateThreadId(mContext, recipients, subId);
                            long retVal = 0;
                            try {
                                /*PduHeaders header = pdu.getPduHeaders();
                                EncodedStringValue v = header.getEncodedStringValue(PduHeaders.FROM);*/
                                Method method = GenericPdu.class.getDeclaredMethod("getPduHeaders");
                                method.setAccessible(true);
                                PduHeaders header = (PduHeaders)method.invoke(pdu);
                                method = PduHeaders.class.getDeclaredMethod("getEncodedStringValue", int.class);
                                method.setAccessible(true);
                                EncodedStringValue v = (EncodedStringValue)method.invoke(header, PduHeaders.FROM);
                                if (v != null) {
                                    retVal = Threads.getOrCreateThreadId(mContext, v.getString(), subId);
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
    ...
    private static long findThreadId(Context context, GenericPdu pdu, int type) {
        ...
        if(DEBUG) Log.d(TAG, "findThreadId, messageId = "+messageId);
