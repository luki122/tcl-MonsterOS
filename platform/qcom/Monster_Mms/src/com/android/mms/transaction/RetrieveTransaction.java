...
public class RetrieveTransaction extends Transaction implements Runnable {
    private static final String TAG = LogTag.TAG+"/RetrieveTransaction";
    ...
    public void run() {
        try {
            ...
            if (isCancelMyself) {
                if(DEBUG) Log.w(TAG, "run(), isCancelMyself 111>>>markState(STATE_PERMANENT_FAILURE)");
            ...
            if (isCancelMyself) {
                if(DEBUG) Log.w(TAG, "run(), isCancelMyself 222>>>markState(STATE_PERMANENT_FAILURE)");
                DownloadManager.getInstance().markState(mUri,
                        DownloadManager.STATE_TRANSIENT_FAILURE);
                return;
            }
            ...
            if (isDuplicateMessage(mContext, retrieveConf, mContentLocation)) {
                ...
                if (mThreadId != 0) {
                    msgUri = persister.persist(retrieveConf, Inbox.CONTENT_URI, mThreadId,
