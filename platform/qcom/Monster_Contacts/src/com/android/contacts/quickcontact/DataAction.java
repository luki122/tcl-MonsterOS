...
public class DataAction implements Action {
    ...
    public DataAction(Context context, DataItem item, DataKind kind) {
        ...
        /// M: add for AAS
        mSubtitle = (String) ExtensionManager
            .getInstance().getAasExtension()
                .getTypeLabel(item.getContentValues().getAsInteger(Data.DATA2),
                    (CharSequence) item.getContentValues().getAsString(Data.DATA3),
                    (String) mSubtitle, IAasExtension.SIM_ID_DONT_KNOW_CURR);
        ...
        if (item instanceof PhoneDataItem) {
            if (PhoneCapabilityTester.isPhone(mContext)) {
                ...
                if (!TextUtils.isEmpty(number) && mSubtitle != null) {
