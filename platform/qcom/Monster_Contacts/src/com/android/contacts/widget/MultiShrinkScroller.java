...
public class MultiShrinkScroller extends FrameLayout {
     ...
     */
    private static final int PIXELS_PER_SECOND = 2000;
     ...
     */
    private static final float INTERMEDIATE_HEADER_HEIGHT_RATIO = 0.21f;
    ...
    private DetailTopAnimView mDragUpAnimView;
    private DragTopAnimView mDragDownAnimView;
    ...
    public void initialize(MultiShrinkScrollerListener listener, boolean isOpenContactSquare) {
        ...
        mDragUpAnimView = (DetailTopAnimView) findViewById(R.id.top_anim_view);
        mDragDownAnimView = (DragTopAnimView) findViewById(R.id.drag_anim_view);
        ...
        mTitleGradientView.setBackground(null);
...
//                            * INTERMEDIATE_HEADER_HEIGHT_RATIO);
                    mIntermediateHeaderHeight= 77 * 3;
                }
                ...
                mRealToolbar.setAlpha(0);
                mNoteView.setAlpha(0);
                mRealToolbar.setVisibility(View.GONE);
...
//        mTitleGradientView.setLayoutParams(titleGradientLayoutParams);
    ...
    public void setNote(String note) {
    	if (!TextUtils.isEmpty(note)) {
        	...
        	mNoteView.setText("");
        	mNoteView.setVisibility(View.INVISIBLE);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mIsTouchDisabledForDismissAnimation || mIsTouchDisabledForSuppressLayout || isUpAnim) return true;
        ...
        final float dragDelta =  event.getY() - mDragY;
        switch (action) {
            case MotionEvent.ACTION_MOVE:
...
//                    if(mNotMoveForEdgeEffectTop < 300
//                    		&& mScrollView.getScrollY() <= 0) {
//                        mEdgeGlowTop.onPull(dragDelta / getHeight(), event.getX() / getWidth());
//                    }
...
//                    }
                    ...
                    if(dragDelta > 0) {
                    	if(mScrollView.getScrollY() <= 0) { 
		                    mDragDownAnimView.setAssistPointX(event.getX());                    
//		                    mDragDownAnimView.setAssistPointY((float) Math.min((dragDelta < DRAG_DOWN_DELTA  ? 0 : dragDelta - DRAG_DOWN_DELTA), DragTopAnimView.height * 0.6));
		                    mDragDownAnimView.setAssistPointY((float) Math.min(dragDelta, DragTopAnimView.height * 0.6));
                    	} else {
                        	mDragDownAnimView.restore();
                    	}
	                    
	                   mDragUpAnimView.restore();
                    } else {
                    	mDragUpAnimView.setAssistPointX(event.getX());                    
                    	mDragUpAnimView.setAssistPointY((float) Math.max(dragDelta, -DragTopAnimView.height * 0.6));	    
                    	
                    	mDragDownAnimView.restore();
                    }
                    mDragUpAnimView.invalidate();
                    if(isUseOverlay) {
                        int[] location = new int[2];
                        mTopContainer.getLocationInWindow(location);
                        int y = location[1];
//                        int toolbarHeight =  mTopContainer.getLayoutParams().height;
                        int toolbarHeight =  50; // 144 + 66?
                    	mDragUpAnimView.layout(0, y - DetailTopAnimView.height - toolbarHeight , DetailTopAnimView.width, y - toolbarHeight);
                	}
                    mDragDownAnimView.invalidate();
...
//                	mDetailTopAnimView.restore();
//                	mDetailTopAnimView.invalidate();
                }
            	mDragUpAnimView.restore();
            	mDragUpAnimView.invalidate();
                break;
...
//        mEdgeGlowBottom.setColor((color & 0xffffff) | Color.argb(edgeEffectAlpha, 0, 0, 0));
        mEdgeGlowBottom.setColor(0x03000000);
    ...
    private void startDrag() {
        ...
        if(mDetailTopAnimContainer == null) {
    		mDetailTopAnimContainer = (ViewGroup) mDragUpAnimView.getParent()
    				.getParent();
    		if (isUseOverlay) {
    			mDetailTopAnimContainer.getOverlay().add(mDragUpAnimView);
    		}
        }

        mDragDownAnimView.setDrag(true);
        mDragUpAnimView.setDrag(true);
        mDragY = mLastEventPosition[1];
        mScroller.abortAnimation();
        
        boolean wasFullscreen = getScrollNeededToBeFullScreen() <= 0;
        if(wasFullscreen) {
        	mNotMoveForEdgeEffectTop  = 0;
        }
    }

    private void stopDrag(boolean cancelled) {
        mIsBeingDragged = false;
        if (!cancelled && getChildCount() > 0 && mHasEverTouchedTheTop) {
        ...
        mDragDownAnimView.setDrag(false);
        mDragUpAnimView.setDrag(false);
        mDragY = 0;
    }

    private void onDragFinished(int flingDelta) {
        if (getTransparentAndTopViewHeight() <= 0) {
            // Don't perform any snapping if quick contacts is full screen.
        	dragRestore(mDragDownAnimView.getAssistPointX(), mDragDownAnimView.getAssistPointY(), 250);
            return;
        }
...
//                smoothScrollBy(getTransparentAndTopViewHeight());
                scrollUpForEntranceAnimationMst(false);
                invalidate();                                                            
                return true;
            }
            return false;
        }
        if (getTransparentAndTopViewHeight() < mDismissDistanceOnRelease) {
            mScroller.forceFinished(true);
            smoothScrollBy(getTransparentAndTopViewHeight());
       	    dragRestore(mDragDownAnimView.getAssistPointX(), mDragDownAnimView.getAssistPointY(), 250);
            return true;
    ...
    public void scrollOffBottom() {
    	...
    	final int duration = this.topDuration;
        ...
        translateAnimation.setInterpolator(mTopInterpolator);
//        translateAnimation.setDuration(EXIT_FLING_ANIMATION_DURATION_MS);
        ...
        if(mDragDownAnimView.getDrag()) {
        	dragRestore(mDragDownAnimView.getAssistPointX(), mDragDownAnimView.getAssistPointY(), duration);
        } else {
        	scroll2Bottom(duration);
        }
//   
    }
...
//        animator.setDuration(topDuration);
...
//        Log.e("liumx", "delta:"+delta);
        ...
        if (!wasFullscreen && isFullscreen && !mHasEverTouchedTheTop) {
        	mDragUpAnimView.setVisibility(View.INVISIBLE);
        }
    ...
    public int getScroll() {
        return mTransparentStartHeight - getTransparentViewHeight() + mDragUpAnimView.height  - getTopAnimHeight()
...
//        return mIsOpenContactSquare ? mMaximumHeaderHeight : mIntermediateHeaderHeight;
    	return mIntermediateHeaderHeight;
    }
    ...
    public int getScrollNeededToBeFullScreen() {
        return getTransparentAndTopViewHeight();
    }
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mIsFullscreenDownwardsFling && getTransparentAndTopViewHeight() > 0) {
//            if(getTransparentAndTopViewHeight() > DRAG_DOWN_DELTA) {
                // Halt the fling once QuickContact's top is on screen.
                scrollTo(0, getScroll() + getTransparentAndTopViewHeight());
...
//        }
    ...
    private int getMaximumScrollUpwards() {
        if (!mIsTwoPanel) {
//            return mTransparentStartHeight + mDetailTopAnimView.height 
...
//                    + getFullyCompressedHeaderHeight());
        	//182 mScrollView paddingTop
            return mTransparentStartHeight + mDragUpAnimView.height 
                    + Math.max(0, mScrollViewChild.getHeight() - getHeight() + mTopContainer.getHeight() + mToolbar.getHeight() + 182);
        } else {
    ...
    private void scrollUp(int delta) {
        ...
        //add by lgy
        if (getTransparentViewHeight() <= 0 && getTopAnimHeight() != 0) {
            final int originalValue = getTopAnimHeight();
            setTopAnimHeight(getTopAnimHeight() - delta);
            setTopAnimHeight(Math.max(0, getTopAnimHeight()));
            delta -= originalValue - getTopAnimHeight();
        }
...
//        final int originalValueEffectTop = mNotMoveForEdgeEffectTop;
//		if (mNotMoveForEdgeEffectTop < DRAG_DOWN_DELTA) {
//			mNotMoveForEdgeEffectTop -= delta;
//			mNotMoveForEdgeEffectTop = Math.min(DRAG_DOWN_DELTA,
//					mNotMoveForEdgeEffectTop);
//			delta -= originalValueEffectTop - mNotMoveForEdgeEffectTop;
//		}
...
//        }        
        ...
        //add by lgy
        if(getTopAnimHeight() < mDragUpAnimView.height) {
            final ViewGroup.LayoutParams topAnimLayoutParams = mDragUpAnimView.getLayoutParams();
            final int originalValue = topAnimLayoutParams.height;
            topAnimLayoutParams.height -= delta;
            topAnimLayoutParams.height = Math.min(topAnimLayoutParams.height,
            		DetailTopAnimView.height);
            mDragUpAnimView.setLayoutParams(topAnimLayoutParams);
            delta -= originalValue - topAnimLayoutParams.height;
        }
...
//            mPhotoViewContainer.setElevation(0);//(mToolbarElevation);
//        } else {
...
//        mPhotoView.setImageAlpha((int) ((1-rate) * 255));
//    	if(getTransparentAndTopViewHeight() < (mTransparentStartHeight + mDetailTopAnimView.height + 0.01f) /2) {
//        	float rate = getTransparentAndTopViewHeight() / (mTransparentStartHeight + mDetailTopAnimView.height + 0.01f);
//        	mPhotoView.setAlpha(1-rate);
//    	} else {
//        	mPhotoView.setAlpha(0);
//    	}
//    	
        if(getTransparentAndTopViewHeight() < 350) {
        	  float rate = getTransparentAndTopViewHeight() /350f;
        	  mPhotoView.setImageAlpha((int) ((1-rate) * 255));
        } else {
        	  mPhotoView.setImageAlpha(0);
        }
...
//        }
	...
	final int topDuration = 270;
    private View mTopContainer;
    final private boolean isUseOverlay = false;
    private float mDragY = 0; 
    private ViewGroup mDetailTopAnimContainer;
    private final Interpolator mTopInterpolator =  new LinearInterpolator();
    private AnimatorUpdateListener mTopUpdateListener = new AnimatorUpdateListener() {
        @Override
        public void onAnimationUpdate(ValueAnimator animation) {
        	   // Permanently set photo width and height.
        	if(isUseOverlay) {
                ...
                int y = location[1];
//                int toolbarHeight =  mTopContainer.getLayoutParams().height;
                int toolbarHeight =  18; // 144 + 66?
            	Log.i("scrollUpForEntranceAnimation", "mTopContainer.y = "+ y + " toolbarHeight = " + toolbarHeight);
            	mDragUpAnimView.layout(0, y - DetailTopAnimView.height - toolbarHeight , DetailTopAnimView.width, y - toolbarHeight);
        	}
            mDragUpAnimView.invalidate();
        }
    ...
    private AnimatorUpdateListener mTopDownUpdateListener = new AnimatorUpdateListener() {
        @Override
        public void onAnimationUpdate(ValueAnimator animation) {
        	mDragDownAnimView.invalidate();
        }
	...
	private void scroll2Bottom(int duration) {
		 Log.i("scroll2Bottom", "start");

		mDragDownAnimView.setRestore(true);
		mDragDownAnimView.setAssistPointX(DragTopAnimView.width/2);
		mDragDownAnimView.setAssistPointY(0);
		PropertyValuesHolder pvhY1 = PropertyValuesHolder.ofFloat("assistPointY",
				0, 100);
		ObjectAnimator topAnimator1 = ObjectAnimator.ofPropertyValuesHolder(
				mDragDownAnimView, pvhY1);
		topAnimator1.setInterpolator(mTopInterpolator);
		topAnimator1.addUpdateListener(mTopDownUpdateListener);
		topAnimator1.setDuration(duration * 4 / 22);
		PropertyValuesHolder pvhY2 = PropertyValuesHolder.ofFloat("assistPointY",
				100, 150);
		ObjectAnimator topAnimator2 = ObjectAnimator.ofPropertyValuesHolder(
				mDragDownAnimView, pvhY2);
		topAnimator2.setInterpolator(mTopInterpolator);
		topAnimator2.addUpdateListener(mTopDownUpdateListener);
		topAnimator2.setDuration(duration * 4 / 22);
		PropertyValuesHolder pvhY3 = PropertyValuesHolder.ofFloat("assistPointY",
				150, 0);
		ObjectAnimator topAnimator3 = ObjectAnimator.ofPropertyValuesHolder(
				mDragDownAnimView, pvhY3);
		topAnimator3.setInterpolator(mTopInterpolator);
		topAnimator3.addUpdateListener(mTopDownUpdateListener);
		topAnimator3.setDuration(duration * 14 / 22);
		topAnimator3.addListener(new AnimatorListenerAdapter() {
			@Override
			public void onAnimationEnd(Animator animation) {
				mDragDownAnimView.setRestore(false);
			}
		});

		AnimatorSet bouncer = new AnimatorSet();
		bouncer.playSequentially(topAnimator1, topAnimator2, topAnimator3);
		bouncer.start();
	}
	
	private void dragRestore(float x , float y, int duration) {

		mDragDownAnimView.setRestore(true);
		mDragDownAnimView.setAssistPointX(x);
		mDragDownAnimView.setAssistPointY(y);
		PropertyValuesHolder pvhX = PropertyValuesHolder.ofFloat(
				"assistPointX", x, DragTopAnimView.width / 2);
		PropertyValuesHolder pvhY = PropertyValuesHolder.ofFloat(
				"assistPointY", y, 0);
		ObjectAnimator topAnimator = ObjectAnimator.ofPropertyValuesHolder(
				mDragDownAnimView, pvhX, pvhY);
		topAnimator.setInterpolator(mTopInterpolator);
		topAnimator.addUpdateListener(mTopDownUpdateListener);
    ...
    public void scrollUpForEntranceAnimationMst(boolean scrollToCurrentPosition) {    
    	
    	float rate = getTransparentAndTopViewHeight() / (mTransparentStartHeight + mDragUpAnimView.height + 0.01f);
    	int duration = (int)(topDuration * rate);
    	
    	isUpAnim = true;
        final int currentPosition = getScroll();
        final int bottomScrollPosition = currentPosition;
        final int desiredValue = currentPosition + (scrollToCurrentPosition ? currentPosition
                : getTransparentAndTopViewHeight());
        final ObjectAnimator animator = ObjectAnimator.ofInt(this, "scroll", bottomScrollPosition,
                desiredValue);
        animator.setInterpolator(mTopInterpolator);
        ...
        animator.start();
        scroll2TopAnim(duration);
    }
    
	private void scroll2TopAnim(int duration) {
		mDragUpAnimView.setAssistPointX(mDragUpAnimView.getAssistPointX());
		mDragUpAnimView.setAssistPointY(mDragUpAnimView.getAssistPointY());
		mDragUpAnimView.setDirection(true);
		PropertyValuesHolder pvhX = PropertyValuesHolder.ofFloat("assistPointX",
				mDragUpAnimView.getAssistPointX(), DetailTopAnimView.width/2);
		PropertyValuesHolder pvhY = PropertyValuesHolder.ofFloat("assistPointY",
				mDragUpAnimView.getAssistPointY(), 0);
		ObjectAnimator topAnimator = ObjectAnimator.ofPropertyValuesHolder(
				mDragUpAnimView, pvhX, pvhY);
		topAnimator.setInterpolator(mTopInterpolator);
		topAnimator.addUpdateListener(mTopUpdateListener);
		topAnimator.setDuration(duration);
		topAnimator.addListener(new AnimatorListenerAdapter() {
			@Override
			public void onAnimationEnd(Animator animation) {
    ...
    private int getTopAnimHeight() {
        return mDragUpAnimView.getLayoutParams().height;
    }
    
    private void setTopAnimHeight(int height) {
    	mDragUpAnimView.getLayoutParams().height = height;
    	mDragUpAnimView.setLayoutParams(mDragUpAnimView.getLayoutParams());
    }
    
    private int getTransparentAndTopViewHeight() {
        return getTransparentViewHeight() + getTopAnimHeight();
    }
    
    private void setNameTextSize() {
    	float rate = getTransparentAndTopViewHeight() / (mTransparentStartHeight + mDragUpAnimView.height + 0.01f);
    	if(rate > 1) rate = 1;
        float nameSize = (float) ((33.3-15) * rate) + 15;
        if(getTransparentAndTopViewHeight() < 100) {
      	    float alphaRate = getTransparentAndTopViewHeight() /100f;
            mRealToolbar.setAlpha(1- alphaRate);
            mNoteView.setAlpha(1- alphaRate);
            mRealToolbar.setVisibility(View.VISIBLE);
        } else {
            mRealToolbar.setAlpha(0);
            mNoteView.setAlpha(0);
            mRealToolbar.setVisibility(View.GONE);
        ...
        if(!mHasEverTouchedTheTop) {
        	mNameView.setTextSize(nameSize);
        }
    }
    
    private void setNameMargin() {
      	float rate = getTransparentAndTopViewHeight() / (mTransparentStartHeight + mDragUpAnimView.height + 0.01f);
    	if(rate > 1) rate = 1;
        final FrameLayout.LayoutParams nameLayoutParams
        = (FrameLayout.LayoutParams) mNameContainer.getLayoutParams();
        // 33dp->147dp
        // 33 = 42 -9 , 147 = 77 + 70
        nameLayoutParams.topMargin =   (int) (getTransparentAndTopViewHeight() + (35 + 111.5 * (1- rate)) * 3);
        //32.7dp->40dp
        nameLayoutParams.leftMargin =   (int) (32.7 + 7.3 * (1- rate)) * 3;
        mNameContainer.setLayoutParams(nameLayoutParams);
        // -59.1px->182px,控制下方电话号码card和mNameContainer的距离
        final LinearLayout.LayoutParams toobarParentLayoutParams = (LinearLayout.LayoutParams) mToolbar.getLayoutParams();
//        toobarParentLayoutParams.bottomMargin =   (int) (-59.1 + (182+59.1) *(1- rate));
        toobarParentLayoutParams.bottomMargin =   (int) (-131.1 * rate);
        mToolbar.setLayoutParams(toobarParentLayoutParams);
        //0->182
        final FrameLayout.LayoutParams mScrollViewChildLayoutParams = (FrameLayout.LayoutParams) mScrollViewChild.getLayoutParams();
        mScrollViewChildLayoutParams.leftMargin =   (int) (-7.3 + 7.3 * (1- rate)) * 3;
        mScrollViewChild.setLayoutParams(mScrollViewChildLayoutParams);
        mScrollView.setPadding(0, (int) (167 * (1- rate)), 0, 0);
    }
    
    private View mRealToolbar;

    final static int DRAG_DOWN_DELTA = 300;
    private int mNotMoveForEdgeEffectTop = DRAG_DOWN_DELTA;
    
    public void setTransparentViewHeightMst(int height) {
        mTransparentStartHeight = height;
        mTransparentView.getLayoutParams().height = height;
        mTransparentView.setLayoutParams(mTransparentView.getLayoutParams());
    }
