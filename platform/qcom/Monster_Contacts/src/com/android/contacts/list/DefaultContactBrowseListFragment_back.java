...
import com.mst.contacts.FragmentCallbacks;
 ...
 *//*
public class DefaultContactBrowseListFragment_back extends ContactBrowseListFragment {
	...
	private FrameLayout mProfileHeaderContainer,mGroupHeaderContainer;
	...
	public DefaultContactBrowseListFragment_back() {
	...
	public void configureHeaderDisplay(boolean isSearchMode){
		if(isSearchMode){
			if(mProfileHeaderContainer!=null){
				getListView().removeHeaderView(mProfileHeaderContainer);
			}
			if(mSearchLayout!=null){
				getListView().removeHeaderView(mSearchLayout);
			}
			if(mGroupHeaderContainer!=null){
				getListView().removeHeaderView(mGroupHeaderContainer);
			}
		}else{
			if(mSearchLayout!=null){
				getListView().addHeaderView(mSearchLayout, null, false);
			}
			
			if(mProfileHeaderContainer!=null){
				getListView().addHeaderView(mProfileHeaderContainer, null, false);
			}
			
			if(mGroupHeaderContainer!=null){
				getListView().addHeaderView(mGroupHeaderContainer, null, false);
			}
		}
	}
	...
	protected void onCreateView(LayoutInflater inflater, ViewGroup container) {
		...
		*//** M: Bug Fix for ALPS00115673 Descriptions: add wait cursor *//*
		...
		//add by liyang begin 
		mSearchLayout = (RelativeLayout) inflater.inflate(R.layout.mst_goto_search_mode, null);
		mSearchLayout.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				if(mCallbacks==null) return;
				mCallbacks.onFragmentCallback(FragmentCallbacks.SWITCH_TO_SEARCH_MODE, null);
			}
		});
		getListView().addHeaderView(mSearchLayout, null, false);
...
//		showEmptyUserProfile(false);
		...
		View mGotoGroupView = inflater.inflate(R.layout.mst_goto_group_header, null, false);
		mGroupHeaderContainer = new FrameLayout(inflater.getContext());
		mGroupHeaderContainer.addView(mGotoGroupView);
		getListView().addHeaderView(mGroupHeaderContainer, null, false);

		// Add a button with a message inviting the user to create a local profile
		Button mGroupHeaderMessage = (Button) mGotoGroupView.findViewById(R.id.group_header_button);
		mGroupHeaderMessage.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				startActivity(new Intent(getActivity(), GroupBrowseActivity.class));
			}
		});
