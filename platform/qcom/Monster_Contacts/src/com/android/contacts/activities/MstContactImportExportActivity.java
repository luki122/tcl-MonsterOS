...
public class MstContactImportExportActivity extends PreferenceActivity implements OnPreferenceClickListener {
	...
	private static String emptyNameString=null;
	/*
	...
	public boolean isSimContactsReady;////是否可以读取sim卡联系人了
	private boolean isSim0Ready,isSim1Ready;

	SharedPreferences prefs;
	private void querySimState(){
		if(prefs==null){
			prefs = PreferenceManager.getDefaultSharedPreferences(MstContactImportExportActivity.this);
		}

		isMultiSimEnabled=prefs.getBoolean("isMultiSimEnabled", false);
		isSimContactsReady=prefs.getBoolean("isSimContactsReady", false);
		isSim0Ready=prefs.getBoolean("isSim0Ready", false);
		isSim1Ready=prefs.getBoolean("isSim1Ready", false);
	}
	Context mActivity;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Log.d(TAG,"onCreate1:"+MstContactImportExportActivity.this+" firstload:"+firstLoad);
		...
		emptyNameString=getResources().getString(R.string.mst_contact_no_name);
		...
		//		intentFilter.addAction(ACTION_SIM_READAY);
		intentFilter.addAction(MST_ACTION_SIM_STATE_CHANGED);
	...
	public class ContactsExportCompletedReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			...
			}else if(TextUtils.equals(action,MST_ACTION_SIM_STATE_CHANGED)){
				Log.d(TAG,"MST_ACTION_SIM_STATE_CHANGED:");
	...
	private void updateSimPreference(){
		Log.d(TAG, "updateSimPreference:"+isSimContactsReady);
		mSimToPhonePref.setEnabled(isSimContactsReady);
		mSimToPhonePref.setSelectable(isSimContactsReady);

		mPhoneToSimPref.setEnabled(isSimContactsReady);
		mPhoneToSimPref.setSelectable(isSimContactsReady);		

		mDeleteSimContacts.setEnabled(isSimContactsReady);
		mDeleteSimContacts.setSelectable(isSimContactsReady);

		mPartDeleteSimContacts.setEnabled(isSimContactsReady);
		mPartDeleteSimContacts.setSelectable(isSimContactsReady);
	}
	...
	public boolean onPreferenceClick(Preference preference) {
		...
		if(preference==mSimToPhonePref){//从sim导入到手机
			if(isMultiSimEnabled){
				...
				int slot0Status=prefs.getInt("slot0Status", -5);
				int slot1Status=prefs.getInt("slot1Status", -5);
		...
		}else if(preference==mSDToPhonePref){//内部存储器导入到手机
		...
		}else if(preference==mPhoneToSDPref){//手机导出到内部存储器
			//add by lgy for 3321097
			if (VCardService.isProcessing(VCardService.TYPE_IMPORT)
					|| VCardService.isProcessing(VCardService.TYPE_EXPORT)) {
				Toast.makeText(this, R.string.contact_import_export_tips, Toast.LENGTH_SHORT)
	...
	private void processPickResultMst(Intent data) {
		...
		Log.d(TAG,"slotId:"+slotId+" subId:"+getSubIdbySlot(mActivity, slotId));
	...
	private static class NamePhoneTypePair {
		...
		public NamePhoneTypePair(String nameWithPhoneType) {
			...
			//			Log.d(TAG,"nameWithPhoneType:"+nameWithPhoneType);
			if(!TextUtils.isEmpty(nameWithPhoneType)){
	...
	protected class QueryHandler extends AsyncQueryHandler {
		...
		protected void onQueryComplete(int token, Object cookie, Cursor c) {
			...
			if(mProgressDialog!=null&&isRunningForeground()) {
				try{
					mProgressDialog.dismiss();
				}catch(Exception e){
