...
public class KindSectionView extends LinearLayout implements EditorListener {
    ...
    public void onDeleteRequested(Editor editor) {
        if (mShowOneEmptyEditor
                && (getEditorCount() == 1 || ExtensionManager.getInstance().getAasExtension()
                        .updateView(mState, null, null,
                                IAasExtension.VIEW_UPDATE_DELETE_EDITOR))) {
            ...
            if (mListener != null) {
                ...
                if (mShowOneEmptyEditor && !hasEmptyEditor()) {
    ...
    private void rebuildFromState() {
        ...
        ///M:
         ArrayList<ValuesDelta> values = ExtensionManager.getInstance().getAasExtension().
    ...
    private View createEditorView(ValuesDelta entry) {
        ...
        if (view instanceof LabeledEditorView) {
        	if (getEditorCount() > 0) {
        		((LabeledEditorView) view).setHideTypeInitially(false);
        	} else {
    ...
    public void updateEmptyEditors(boolean shouldAnimate) {
        ...
        int max = 0;
        if (mKind != null) {
            max = ExtensionManager.getInstance().getAasExtension()
                .getMaxEmptyEditors(mState, mKind.mimeType);
        }
    ...
    private Drawable getMimeTypeDrawable(String mimeType) {
        switch (mimeType) {
            ...
            /// M: Add for IMS call support.
            case ImsCall.CONTENT_ITEM_TYPE:
                return getResources().getDrawable(R.drawable.ic_dialer_ims_black);
            default:
                return null;
        }
    }
