...
public class RawContactReadOnlyEditorView extends BaseRawContactEditorView
    ...
    public void setState(RawContactDelta state, AccountType type, ViewIdGenerator vig,
        ...
        if (isProfile) {
            ...
            if (!TextUtils.isEmpty(mAccountName)
                    && !type.accountType.equals(AccountTypeUtils.ACCOUNT_TYPE_LOCAL_PHONE)) {
            ...
            /// M: Bug fix ALPS00453091, if local phone, set string to "Phone contact". @{
            if (type.accountType.equals(AccountTypeUtils.ACCOUNT_TYPE_LOCAL_PHONE)) {
                mAccountHeaderTypeTextView.setText(accountType);
            } else {
            /// @}
                mAccountHeaderTypeTextView.setText(getContext().getString(
                        R.string.account_type_format, accountType));
            }
        }
