...
public class CompactRawContactsEditorView extends LinearLayout implements View.OnClickListener {
    ...
    private static final String TAG = "CompactRawContactsEditorView";
    ...
    public void setState(RawContactDeltaList rawContactDeltas,
            ...
            boolean isUserProfile, SubscriberAccount subscriberAccount) {
        ...
        if (subscriberAccount.isAccountTypeIccCard(rawContactDeltas)
                && SimCardUtils.getIccCardEmailCount(subscriberAccount.getSubId()) <= 0) {
            Log.d(TAG, "[setState] No Email field in subId: " + subscriberAccount.getSubId());
            mEmails.removeAllViews();
        }
        /// @}
    }
    ...
    private void addAccountInfo(RawContactDeltaList rawContactDeltas, boolean hasNewContact,
        ...
        vlog("Account info loaded,accountInfo.first = " + accountInfo.first
                + ",accountInfo.second =" + accountInfo.second);
    ...
    private void addPhotoView(RawContactDeltaList rawContactDeltas,
        ...
        for (RawContactDelta rawContactDelta : rawContactDeltas) {
            ...
            if (dataKind != null && dataKind.editable) {
                ...
                /* M: if account type is IccCard account, don't show add phone icon @{ */
                boolean noNeedShowPotoIcon = false;
                if (accountType.isIccCardAccount()) {
                    noNeedShowPotoIcon = true;
                }
                /* @}*/
                if (valuesDelta != null) {
                    if (readOnlyContact) {
                        mPhoto.setPhoto(valuesDelta);
                    /* M: if account type is IccCard account, don't show add phone icon @{ */
                    } else if (noNeedShowPotoIcon) {
                        mPhotoRawContactId = rawContactDelta.getRawContactId();
                        mPhoto.setValues(dataKind, valuesDelta, rawContactDelta,
                                noNeedShowPotoIcon, mMaterialPalette, viewIdGenerator);
                    /* @}*/
                    } else {
                        mPhotoRawContactId = rawContactDelta.getRawContactId();
                        mPhoto.setValues(dataKind, valuesDelta, rawContactDelta,
