...
import mst.view.menu.BottomWidePopupMenu.OnMenuItemClickListener;
...
public class PhotoActionPopup {
    ...
    public static BottomWidePopupMenu createDialogMenu(Context context,
            final Listener listener, int mode) {
        final ArrayList<ChoiceListItem> choices = getChoices(context, mode);

        final BottomWidePopupMenu listPopupWindow = new BottomWidePopupMenu(context);
		for (int i = 0; i < choices.size(); i++) {
			ChoiceListItem choice = choices.get(i);
			listPopupWindow.addItem(choice.mId, choice.mCaption, null);
		}
        listPopupWindow.setOnMenuItemClickedListener(new OnMenuItemClickListener() {
			
			@Override
			public boolean onItemClicked(MenuItem mMenuItem) {
