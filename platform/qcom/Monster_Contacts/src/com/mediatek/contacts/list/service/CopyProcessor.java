...
public class CopyProcessor extends ProcessorBase {
    ...
    private static final String[] DATA_ALLCOLUMNS = new String[] { Data._ID, Data.MIMETYPE,
            ...
            Data.SYNC4, Data.IS_ADDITIONAL_NUMBER };
    ...
    private void copyContactsToSimWithRadioStateCheck() {
        ...
        Log.d(TAG, "[copyContactsToSimWithRadioCheck]AccountName: " + account.name
    ...
    private void copyContactsToSim() {
        ...
        AccountWithDataSetEx account = (AccountWithDataSetEx) mAccountDst;
        Log.d(TAG, "[copyContactsToSim]AccountName:" + account.name + "|accountType:"
                + account.type);
        int dstSubId = account.getSubId();
        Log.d(TAG, "[copyContactsToSim]dstSlotId:" + "|dstSubId:" + dstSubId);
        ...
        for (MultiChoiceRequest request : this.mRequests) {
            ...
            final String[] projection = new String[] { Contacts._ID, Contacts.Data.MIMETYPE,
                    Contacts.Data.DATA1, Contacts.Data.IS_ADDITIONAL_NUMBER,
            ...
            for (int i = 0; i < maxCount; i++) {
                ...
                Log.d(TAG, "[copyContactsToSim]sim tag is " + simTag);
                simNum = null;
                simAnrNum = null;
                simEmail = null;
                ...
                if (isTargetUsim) {
                    ...
                    ExtensionManager.getInstance().getAasExtension()
                            .updateValuesforCopy(dataUri, dstSubId, mAccountDst.type, values);
                ...
                Log.i(TAG, "[copyContactsToSim]Before insert Sim card. values=" + values);
                ...
                if (retUri != null) {
                    ...
                    if ("error".equals(checkUriPathSegs.get(0))) {
                        ...
                        if ("-3".equals(checkUriPathSegs.get(1))) {
                            ...
                            if (retUri != null
                                ...
                                ExtensionManager
                                        .getInstance()
                                        .getSneExtension()
                                        .copySimSneToAccount(operationList, mAccountDst, dataUri,
                                                backRef);
                                subContact++;
                            }
                        }
                        ...
                        SubContactsUtils.buildInsertOperation(operationList, mAccountDst, simTag,
                                ...
                                indexInSim, null);
                        ExtensionManager.getInstance().getSneExtension()
                                .copySimSneToAccount(operationList, mAccountDst, dataUri, backRef);
                        subContact++;
                        // successfulItems++;
                    }
                } else {
    ...
    private void generateDataBuilder(Cursor dataCursor, Builder builder, String[] columnNames,
        ...
        for (int i = 1; i < columnNames.length; i++) {
            ...
            if (ExtensionManager
                    .getInstance()
                    .getAasExtension()
                    .cursorColumnToBuilder(dataCursor, builder, srcAccountType, mimeType,
                            ((AccountWithDataSetEx) mAccountDst).getSubId(), i)) {
                continue;
            }
