...
public class MultiChoiceService extends Service {
    ...
    public synchronized void mstHandleDeleteRequest(List<MultiChoiceRequest> requests,
    		MstDeleteRequestListener mstDeleteRequestListener,MultiChoiceHandlerListener multiChoiceHandlerListener) {
        sCurrentJobId++;
        Log.i(TAG, "[mstHandleDeleteRequest]sCurrentJobId:" + sCurrentJobId);
        if (tryExecute(new DeleteProcessor(this, multiChoiceHandlerListener, requests, sCurrentJobId,mstDeleteRequestListener))) {
            if (mstDeleteRequestListener != null) {
            	mstDeleteRequestListener.onProcessed(TYPE_DELETE, sCurrentJobId, 0, -1,
                        requests.get(0).mContactName);
            }
        }
    }
    
    public interface MstDeleteRequestListener{
    	public void onProcessed(final int requestType, final int jobId, final int currentCount,
