...
import com.mediatek.contacts.list.service.MultiChoiceService.MstDeleteRequestListener;
...
public class DeleteProcessor extends ProcessorBase {
	...
	private MstDeleteRequestListener mstDeleteRequestListener=null;
	...
	public DeleteProcessor(final MultiChoiceService service,
			final MultiChoiceHandlerListener listener, final List<MultiChoiceRequest> requests,
			final int jobId,final MstDeleteRequestListener mstDeleteRequestListener) {
		Log.i(TAG, "[DeleteProcessor]new.");
		mService = service;
		mResolver = mService.getContentResolver();
		mListener = listener;
		this.mstDeleteRequestListener=mstDeleteRequestListener;
	...
	private void runInternal() {
		...
		for (MultiChoiceRequest request : mRequests) {
			...
			long contactId=request.mContactId;
			if(preContactId!=contactId){
				currentCount++;
				/*mListener*/mstDeleteRequestListener.onProcessed(MultiChoiceService.TYPE_DELETE, mJobId, currentCount, totalItems,
			...
			if (request.mIndicator > 0) {
				...
				if (!isReadyForDelete(subId)) {
					Log.d(TAG, "[runInternal] run: isReadyForDelete(" + subId + ") = false");
					succeessful = false;
					continue;
				}

				// / M: change for SIM Service refactoring
				if (simServiceStarted || !simServiceStarted
						&& SIMServiceUtils.isServiceRunning(subId)) {
					Log.d(TAG,
				...
				if (deleteCount <= 0) {
					Log.d(TAG, "[runInternal] run: delete the sim contact failed");
					succeessful = false;
				} else {
					successfulItems++;
