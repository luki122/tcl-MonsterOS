...
/**
 * Fragment used for displaying a Recycler View.
 * It has the RecyclerView Adapter, and the LayoutManager.
 * You can also specify ItemDecorations for your recyclerView.
 *
 * @author nordhal
 */
public abstract class RecyclerViewFragment extends Fragment {
    ...
    public static final String ARG_MUTI_SELCET = "muti_selcet";
    public static final String ARG_FIRST_SONGID = "first_songid";
    ...
    //TODO Manage Standard EmptyView
    ...
    protected Set<ItemDecoration> mItemDecorations = new HashSet<ItemDecoration>();


    public interface ItemClickFromFragmentListener {
        boolean onRecyclerItemClick(RecyclerViewFragment fragment, final ViewHolder viewHolder, final int position, View v);
    }
    ...
    private RecyclerViewAdapter.OnItemClickListener mOnItemClickListener = new RecyclerViewAdapter.OnItemClickListener() {
        ...
        public void onItemClick(final ViewHolder viewHolder, final int position, View v) {
            if (v.getId() == com.mixvibes.mvlib.R.id.item_menu_image_button || v.getId() == cn.tcl.music.R.id.play_view) {
                NLog.d(TAG, "mOnItemClickListener onItemClick item_menu_image_button");
                ...
                if (mItemClickFromFragmentListener != null) {
                    alreadyHandled = mItemClickFromFragmentListener.onRecyclerItemClick(RecyclerViewFragment.this, viewHolder, position, v);
                }
                if (!alreadyHandled) {
    ...
    private void setRecyclerViewShown(boolean shown, boolean animate) {
        ...
        if (mProgressContainer == null) {
            NLog.e(TAG, "Can't be used without progressContainer");
            return;
        }
...
//            ((ActivityAddSong) getActivity()).refreshSelect(isRecyclerViewEmpty); // MODIFIED by beibei.yang, 2016-07-05,BUG-2459696
    ...
    private boolean ensureRecyclerView() {
        ...
        View root = getView();
        if (root == null) {
            NLog.e(TAG, "Content View not yet created.");
            return false;
        }
        ...
        if (root instanceof RecyclerView) {
            ...
            mRecyclerView = (RecyclerView) root.findViewById(com.mixvibes.mvlib.R.id.recycler_view);
            if (mRecyclerView == null) {
            ...
            mProgressContainer = (ViewGroup) root.findViewById(com.mixvibes.mvlib.R.id.progress_container);
            mRecyclerViewContainer = (ViewGroup) root.findViewById(com.mixvibes.mvlib.R.id.recycler_container);
