...
import cn.tcl.music.fragments.PortaitPlayLyricFragment;

/**
 * Created by yanping.cao on 6/30/16.
 */

public class LrcView_v2 extends View implements ILrcView{


    public final static String TAG = PortaitPlayLyricFragment.TAG;
    ...
    public LrcView_v2(Context context,AttributeSet attr){
        super(context,attr);
        TypedArray a = context.obtainStyledAttributes(attr, R.styleable.Lyric);
        normaltextSize = a.getDimensionPixelSize(R.styleable.Lyric_normaltextSize, 45);
        mLrcFontSize = normaltextSize;
        hltextSize = a.getDimensionPixelSize(R.styleable.Lyric_hltextSize, 45);
        ...
        mLiricMargin = a.getDimensionPixelSize(R.styleable.Lyric_liricMargin, 16);
        a.recycle();
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int highlightRowY = mScreenHeight- mLrcFontSize ;
        ...
        rowY = highlightRowY - mLiricMargin - mLrcFontSize;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                NLog.d(TAG, "down,mLastMotionY:" + mLastMotionY);
    ...
    private void doScale(MotionEvent event) {
        if(mDisplayMode == DISPLAY_MODE_SEEK){
            ...
            NLog.d(TAG, "two move but teaking ...change mode");
            return;
        }
        ...
        NLog.d(TAG,"scaleSize:" + scaleSize);
        if(scaleSize != 0){
    ...
    private void doSeek(MotionEvent event) {
        ...
        NLog.d(TAG, "move new hightlightrow : " + mHignlightRow + " offsetY: " + offsetY + " rowOffset:" + rowOffset);
        if(offsetY < 0){
            // finger move up
    ...
    private int getScale(MotionEvent event){
        ...
        NLog.d(TAG,"scaleSize maxOffset:" + maxOffset);
