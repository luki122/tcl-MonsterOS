...
public class TUncaughtExceptionHandler implements UncaughtExceptionHandler {
	...
	public static  final String TAG = "TUncaughtExceptionHandler";
	...
	private void collectDeviceInfo ()
	{
		...
		deviceInfos.append("androidVersion=").append(Build.VERSION.RELEASE);
	...
	private boolean saveCrashInfo2File (Thread thread, Throwable ex)
	{
		...
		if (TextUtils.isEmpty(mCrashDirPath)) {
			dirForCrashFile = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + 
					"crashes";
		}
