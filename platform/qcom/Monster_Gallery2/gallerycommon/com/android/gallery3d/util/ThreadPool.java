...
public class ThreadPool {
    ...
    private final Executor mForgroundExecutor;
    ...
    public ThreadPool(int initPoolSize, int maxPoolSize) {
        ...
        // TCL ShenQianfeng Begin on 2016.11.07
        mForgroundExecutor = new ThreadPoolExecutor(
                initPoolSize, maxPoolSize, KEEP_ALIVE_TIME,
                TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
                new PriorityThreadFactory("thread-pool-urgent",
    ...
    // TCL ShenQianfeng Begin on 2016.11.07
    public <T> Future<T> submitImmediately(Job<T> job, FutureListener<T> listener) {
        ImmediateWorker<T> w = new ImmediateWorker<T>(job, listener);
        mForgroundExecutor.execute(w);
        return w;
    }
    
    private class ImmediateWorker<T>  implements Runnable, Future<T>, JobContext {
        ...
        public ImmediateWorker(Job<T> job, FutureListener<T> listener) {
            mJob = job;
            mListener = listener;
        }
