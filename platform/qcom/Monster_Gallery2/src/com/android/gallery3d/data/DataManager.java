...
public class DataManager implements StitchingChangeListener {
    ...
    public static final int INCLUDE_MST_LOCAL_ALL = 8;
    ...
    private static final String MST_TOP_SET_PATH = "/combo/item/{/local/all/" + MediaSetUtils.CAMERA_BUCKET_ID
                                                                                                                               + ",/local/all/" +  MediaSetUtils.SCREENSHOTS_BUCKET_ID  + "}";
    
    private static final String MST_TOP_SET_PATH_IMAGE = "/combo/item/{/local/image/" + MediaSetUtils.CAMERA_BUCKET_ID
            + ",/local/image/" +  MediaSetUtils.SCREENSHOTS_BUCKET_ID  + "}";
    
    private static final String MST_TOP_SET_PATH_VIDEO = "/combo/item/{/local/video/" + MediaSetUtils.CAMERA_BUCKET_ID
            + ",/local/video/" +  MediaSetUtils.SCREENSHOTS_BUCKET_ID  + "}";

    /*
    private static final String MST_TOP_SET_PATH = "/combo/item/{/local/all/" + MediaSetUtils.CAMERA_BUCKET_ID
            + ",/local/all/" +  MediaSetUtils.SNAPSHOT_BUCKET_ID  
            + ",/local/all/" +  MediaSetUtils.MST_TEST_BUCKET_ID  + "}";
    */
    //TCL ShenQianfeng End  on 2016.06.12
    ...
    //TCL ShenQianfeng Begin on 2016.06.12
    public String mst_getTopSetPath() {
        return MST_TOP_SET_PATH;
    }
    
    public String mst_getTopSetPathByTypeBits(int typeBits) {
        switch (typeBits) {
        case INCLUDE_IMAGE: return MST_TOP_SET_PATH_IMAGE;
        case INCLUDE_VIDEO: return MST_TOP_SET_PATH_VIDEO;
        case INCLUDE_ALL: return MST_TOP_SET_PATH;
        case INCLUDE_LOCAL_IMAGE_ONLY: return MST_TOP_SET_PATH_IMAGE;
        case INCLUDE_LOCAL_VIDEO_ONLY: return MST_TOP_SET_PATH_VIDEO;
        case INCLUDE_LOCAL_ALL_ONLY: return MST_TOP_SET_PATH;
    ...
    public MediaObject getMediaObject(Path path) {
        synchronized (LOCK) {
            ...
            try {
                ...
                //LogUtil.i2(TAG, "getMediaObject path :" + path);
                if (object == null) {
    ...
    public void registerChangeNotifier(Uri uri, ChangeNotifier notifier) {
        ...
        //LogUtil.d(TAG, " mNotifierMap: size:" + mNotifierMap.size());
