...
public class StringTexture extends CanvasTexture {
    ...
    /*
    public static TextPaint getDefaultPaint(float textSize, int color, int typefaceId) {
        TextPaint paint = new TextPaint();
        paint.setTextSize(textSize);
        paint.setAntiAlias(true);
        paint.setColor(color);
        paint.setShadowLayer(2f, 0f, 0f, Color.BLACK);
        paint.setTypeface(TypefaceManager.get(typefaceId));
        return paint;
    }
    
    public static StringTexture newInstance(
            String text, float textSize, int color, int typefaceId) {
        return newInstance(text, getDefaultPaint(textSize, color, typefaceId));
    }
    */
    ...
    public static StringTexture newCustomerInstance(Context context, String text, int textSize, int color) {
        return newCustomerInstance(context, text, textSize, color, TypefaceManager.obtain(textSize));
    }
    
    public static StringTexture newCustomerInstance(Context context, String text, int textSize, int color, Typeface typeface) {
        TextPaint paint = new TextPaint();
        DisplayMetrics metrics =  context.getResources().getDisplayMetrics();
        paint.setTextSize(metrics.density * textSize);
        paint.setAntiAlias(true);
        paint.setColor(color);
