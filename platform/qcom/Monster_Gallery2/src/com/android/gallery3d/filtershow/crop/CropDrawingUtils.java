...
public abstract class CropDrawingUtils {
	...
	private static boolean mForceFixedAspect = true;
	//modify end
    ...
    public static void drawIndicators(Canvas canvas, Drawable []cropIndicator, int indicatorSize,
		...
		//modify begin by liaoanhua		
        if (fixedAspect || mForceFixedAspect) {
		//modify end
            if ((selection == CropObject.TOP_LEFT) || notMoving) {
                drawIndicator(canvas, cropIndicator[0], indicatorSize, bounds.left + indicatorSize/2, bounds.top + indicatorSize/2);
            }
            if ((selection == CropObject.TOP_RIGHT) || notMoving) {
                drawIndicator(canvas, cropIndicator[1], indicatorSize, bounds.right - indicatorSize/2, bounds.top + indicatorSize/2);
            }
            if ((selection == CropObject.BOTTOM_LEFT) || notMoving) {
                drawIndicator(canvas, cropIndicator[2], indicatorSize, bounds.left + indicatorSize/2, bounds.bottom - indicatorSize/2);
            }
            if ((selection == CropObject.BOTTOM_RIGHT) || notMoving) {
                drawIndicator(canvas, cropIndicator[3], indicatorSize, bounds.right - indicatorSize/2, bounds.bottom - indicatorSize/2);
            }
        } else {
            if (((selection & CropObject.MOVE_TOP) != 0) || notMoving) {
                drawIndicator(canvas, cropIndicator[0], indicatorSize, bounds.centerX(), bounds.top);
            }
            if (((selection & CropObject.MOVE_BOTTOM) != 0) || notMoving) {
                drawIndicator(canvas, cropIndicator[1], indicatorSize, bounds.centerX(), bounds.bottom);
            }
            if (((selection & CropObject.MOVE_LEFT) != 0) || notMoving) {
                drawIndicator(canvas, cropIndicator[2], indicatorSize, bounds.left, bounds.centerY());
            }
            if (((selection & CropObject.MOVE_RIGHT) != 0) || notMoving) {
                drawIndicator(canvas, cropIndicator[3], indicatorSize, bounds.right, bounds.centerY());
            }
        }
    }
