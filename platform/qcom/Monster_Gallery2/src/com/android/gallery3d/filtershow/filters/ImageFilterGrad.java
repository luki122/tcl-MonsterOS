...
public class ImageFilterGrad extends ImageFilterRS {
    ...
    protected void createFilter(android.content.res.Resources res, float scaleFactor,
        ...
        //mScript = new ScriptC_grad(rsCtx, res, R.raw.grad);//ShenQianfeng sync begin on 2016.08.17, line below is original
    ...
    private void runSelectiveAdjust(Allocation in, Allocation out) {
        ...
        for (ty = 0; ty < height; ty += STRIP_SIZE) {
            ...
            /* MODIFIED-BEGIN by Yaoyu.Yang, 2016-06-20,BUG-2323999*/
            if (null != mScript) {
                mScript.forEach_selectiveAdjust(in, out, options);
            }
            /* MODIFIED-END by Yaoyu.Yang,BUG-2323999*/
            if (checkStop()) {
                return;
            }
        }
