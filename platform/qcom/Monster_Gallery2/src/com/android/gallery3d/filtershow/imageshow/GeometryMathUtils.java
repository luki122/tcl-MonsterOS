...
public final class GeometryMathUtils {
    ...
    public static final float SHOW_SCALE = .93f;
    ...
    public static final class GeometryHolder {
        ...
        private ArrayList<FilterRepresentation> mGeometryFilters = new ArrayList<FilterRepresentation>();
        ...
        // Modify To:
        public int rotation = 0; // may be 0, 90, 180, 270
        // TCL ShenQianfeng End on 2016.08.25
        ...
        public void set(GeometryHolder h) {
            ...
            // TCL ShenQianfeng Begin on 2016.08.25
            mGeometryFilters.clear();
            for(int i=0; i < h.mGeometryFilters.size(); i++) {
                mGeometryFilters.add(h.mGeometryFilters.get(i));
            }
            // TCL ShenQianfeng End on 2016.08.25
        }
        ...
        public boolean equals(Object o) {
            ...
            return rotation == h.rotation && straighten == h.straighten &&
                    ...
                    mirror == h.mirror; */
            // Modify To:
            if(h.mGeometryFilters.size() != this.mGeometryFilters.size()) return false;
            for(int i=0; i<mGeometryFilters.size(); i++) {
                FilterRepresentation thisOne = mGeometryFilters.get(i);
                FilterRepresentation theOther = h.mGeometryFilters.get(i);
                if( ! thisOne.equals(theOther)) {
                    return false;
                }
            }
            return rotation == h.rotation && straighten == h.straighten &&
        ...
        public String toString() {
             ...
             */
            // Modify To:
            return getClass().getSimpleName() + "[" + "rotation:" + rotation
                    + ",straighten:" + straighten + ",crop:" + crop.toString()
                    + ",mirror:" + mirror.value() + "]" + mGeometryFilters.toString();
            // TCL ShenQianfeng End on 2016.08.25
        }
    }
    ...
    private static void concatMirrorMatrix(Matrix m, GeometryHolder holder) {
        ...
        */
        ...
        if (type == Mirror.HORIZONTAL) {
            if (holder.rotation.value() == 90
                    || holder.rotation.value() == 270) {
                type = Mirror.HORIZONTAL;//type = Mirror.VERTICAL;
            }
        } else if (type == Mirror.VERTICAL) {
            if (holder.rotation.value() == 90
                    || holder.rotation.value() == 270) {
    ...
    public static void unpackGeometry(GeometryHolder out,
        ...
        for (FilterRepresentation r : geometry) {
            ...
            if (r.getSerializationName() == FilterRotateRepresentation.SERIALIZATION_NAME) {
                ...
                // Modify To:
                FilterRotateRepresentation rep = (FilterRotateRepresentation)r;
                if(rep.getRotation() == Rotation.NINETY) {
                    out.rotation += 90;
                } else if(rep.getRotation() == Rotation.NEGATIVE_NINETY) {
                    out.rotation -= 90;
                } else {
                    throw new IllegalArgumentException("not illegal rotation");
                }
                // TCL ShenQianfeng End on 2016.08.25
                ...
                // Modify To:
                FilterStraightenRepresentation rep = (FilterStraightenRepresentation)r;
                out.straighten += rep.getStraighten();
                // TCL ShenQianfeng End on 2016.08.25
    ...
    public static Matrix getImageToScreenMatrix(Collection<FilterRepresentation> geometry,
        ...
        // [BUGFIX]-Add by TCTNJ,hao.yin, 2016-03-05,PR1717524 begin
        int width = bmapDimens != null ? bmapDimens.width():0;
        int height = bmapDimens != null ? bmapDimens.height():0;
        return GeometryMathUtils.getOriginalToScreen(h, reflectRotation, width,
                 height, viewWidth, viewHeight);
        // [BUGFIX]-Add by TCTNJ,hao.yin, 2016-03-05,PR1717524 end
    }
    ...
    public static Matrix getOriginalToScreen(GeometryHolder holder, boolean rotate,
        ...
        int orientation = MasterImage.getImage().getZoomOrientation();
        int rotation = getRotationForOrientation(orientation);
        int prev = holder.rotation;
        rotation = (rotation + prev) % 360;
     ...
     */
    // Modify To:
    public static boolean needsDimensionSwap(int rotation) {
    ...
    private static Matrix getFullGeometryMatrix(GeometryHolder holder, int bitmapWidth, int bitmapHeight) {
        ...
        Matrix m = new Matrix();
        m.setTranslate(-centerX, -centerY);
        for(int i=0; i<holder.mGeometryFilters.size(); i++) {
            FilterRepresentation rep = holder.mGeometryFilters.get(i);
            String repSerializationName = rep.getSerializationName();
            if(repSerializationName.equals(FilterRotateRepresentation.SERIALIZATION_NAME)) {
                m.postRotate(((FilterRotateRepresentation)rep).getRotation().value());
            } else if(repSerializationName.equals(FilterMirrorRepresentation.SERIALIZATION_NAME)) {
                FilterMirrorRepresentation mirror = (FilterMirrorRepresentation)rep;
                if(mirror.getIsHorizontal()) {
                    m.postScale(-1, 1);
                } else {
                    m.postScale(1, -1);
                }
            } else if(repSerializationName.equals(FilterStraightenRepresentation.SERIALIZATION_NAME)) {
                FilterStraightenRepresentation straighten = (FilterStraightenRepresentation)rep;
                m.postRotate(straighten.getStraighten());
            }
        }
        //concatMirrorMatrix(m, holder);
        return m;
