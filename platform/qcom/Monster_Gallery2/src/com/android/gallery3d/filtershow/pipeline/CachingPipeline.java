    ...
    public synchronized void render(RenderingRequest request) {
        ...
        synchronized (CachingPipeline.class) {
            ...
            if (request.getType() == RenderingRequest.FULL_RENDERING
                ...
                //[BUGFIX]-Modify by TCTNJ, dongliang.feng, 2015-05-05, PR988861 begin
                if ( ! mEnvironment.needsStop()) {
                    Bitmap bmp = preset.apply(bitmap, mEnvironment);
                //[BUGFIX]-Modify by TCTNJ, dongliang.feng, 2015-05-05, PR988861 end
                    request.setBitmap(bmp);
                }
                mFiltersManager.freeFilterResources(preset);
            }
