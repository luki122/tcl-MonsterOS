...
public class ImageStraighten extends ImageShow {
    ...
    public void setFilterStraightenRepresentation(FilterStraightenRepresentation rep) {
        mLocalRep = (rep == null) ? new FilterStraightenRepresentation(mContext.getString(R.string.straighten)) : rep; // MODIFIED by caihong.gu-nb, 2016-05-10,BUG-2105073
    ...
    public Collection<FilterRepresentation> getFinalRepresentation() {
        ...
        if (mInitialAngle != mLocalRep.getStraighten()) {
            reps.add(new FilterCropRepresentation(mCrop,mContext.getString(R.string.crop))); // MODIFIED by caihong.gu-nb, 2016-05-10,BUG-2105073
        }
        return reps;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-03-20,PR953723 begin
        if(event.getPointerCount() >= 2) {
            return true;
        }
        //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-03-20,PR953723 end
        float x = event.getX();
        float y = event.getY();
        ...
        switch (event.getActionMasked()) {
            ...
            case (MotionEvent.ACTION_UP):
                if (mState == MODES.MOVE) {
                    ...
                    mCurrentX = x;
                    mCurrentY = y;
//                    computeValue();//[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-03-20,PR953723
            ...
            case (MotionEvent.ACTION_MOVE):
                if (mState == MODES.MOVE) {
                    ...
                    mCurrentY = y;
                    //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-03-20,PR953723 begin
                    if(isValidateMove(mCurrentX, mCurrentY, mTouchCenterX, mTouchCenterY, getWidth() >> 1, getHeight() >> 1)) {
                        computeValue();
                    }
                    //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-03-20,PR953723 end
                }
    ...
    //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-03-20,PR953723 begin
    private boolean isValidateMove(float mCurrentX, float mCurrentY, float mTouchCenterX, float mTouchCenterY, int centerX, int centerY) {
        if((mTouchCenterX > centerX && mTouchCenterY < centerY) && (mCurrentX < centerX && mCurrentY > centerY)) {
            return false;
        } else if((mTouchCenterX < centerX && mTouchCenterY < centerY) && (mCurrentX > centerX && mCurrentY > centerY)) {
            return false;
        } else if((mTouchCenterX < centerX && mTouchCenterY > centerY) && (mCurrentX > centerX && mCurrentY < centerY)) {
            return false;
        } else if((mTouchCenterX > centerX && mTouchCenterY > centerY) && (mCurrentX < centerX && mCurrentY < centerY)) {
