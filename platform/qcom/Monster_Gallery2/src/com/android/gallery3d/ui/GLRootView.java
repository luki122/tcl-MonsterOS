...
public class GLRootView extends GLSurfaceView
    ...
    public void requestRender() {
        if (DEBUG_INVALIDATE) {
            ...
            //LogUtil.d(TAG, "invalidate: " + caller);
        }
    ...
    private void layoutContentPane() {
        ...
        /*
        LogUtil.i(TAG, "layout content pane " + w + "x" + h + " (compensation " + mCompensation + ")"); */
    ...
    public void onSurfaceCreated(GL10 gl1, EGLConfig config) {
        ...
        if (mGL != null) {
            ...
            //LogUtil.i(TAG, "GLObject has changed from " + mGL + " to " + gl);
        }
        mRenderLock.lock();
        try {
            mGL = gl;
    ...
    public void onSurfaceChanged(GL10 gl1, int width, int height) {
        /*
        LogUtil.i(TAG, "onSurfaceChanged: " + width + "x" + height
                + ", gl10: " + gl1.toString());*/
        Process.setThreadPriority(Process.THREAD_PRIORITY_DISPLAY);
        GalleryUtils.setRenderThread();
        if (DEBUG_PROFILE) {
            LogUtil.d(TAG, "Start profiling");
    ...
    private void outputFps() {
        ...
        if (mFrameCountingStart == 0) {
            ...
            LogUtil.d(TAG, "fps: " + (double) mFrameCount
    ...
    public void onDrawFrame(GL10 gl) {
        //LogUtil.d(TAG, "GLRootView onDrawFrame .................");
        ...
        while (mFreeze) {
            LogUtil.d(TAG, "onDrawFrame waiting....");
            mFreezeCondition.awaitUninterruptibly();
            LogUtil.d(TAG, "onDrawFrame stop waiting and continue....");
        }
        ...
        if (DEBUG_PROFILE_SLOW_ONLY) {
            ...
            if (durationInMs > 34) {  // 34ms -> we skipped at least 2 frames
                LogUtil.v(TAG, "----- SLOW (" + durationDrawInMs + "/" + durationInMs + ") -----");
                Profile.commit();
            } else {
                Profile.drop();
            }
    ...
    public void setLightsOutMode(boolean enabled) {
        /*
        //LogUtil.i(TAG, "setLightsOutMode enabled:" + enabled);
        if (!ApiHelper.HAS_SET_SYSTEM_UI_VISIBILITY) return;
        int flags = 0;
        int displayRotation = mOrientationSource.getDisplayRotation();
        boolean isPortrait = displayRotation == 0 || displayRotation == 180;
        if(isPortrait) {
    ...
    @Override
    public void applySystemUIVisibility(int flags) {
        setSystemUiVisibility(flags);
    }
    ...
    protected boolean fitSystemWindows(Rect insets) {
        ...
        if( ! mWindowInsets.equals(insets)) {
            ...
            mContentView.onWindowsInsetsChanged(mWindowInsets);
            //Log.e(TAG, "fitSystemWindows rect " + insets);
        }
        return true;
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        //LogUtil.d(TAG, "GLRootView::surfaceChanged will call unfreeze()");
        unfreeze();
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        //LogUtil.d(TAG, "GLRootView::surfaceCreated will call unfreeze()");
        unfreeze();
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        //LogUtil.d(TAG, "GLRootView::surfaceDestroyed will call unfreeze()");
        unfreeze();
    ...
    protected void onDetachedFromWindow() {
        //LogUtil.d(TAG, "GLRootView::onDetachedFromWindow will call unfreeze()");
        unfreeze();
    ...
    protected void finalize() throws Throwable {
        try {
            //LogUtil.d(TAG, "GLRootView::finalize will call unfreeze()");
            unfreeze();
        } finally {
            super.finalize();
        }
