...
public class TileImageView extends GLView {
    ...
    //ShenQianfeng modify mCenterX, mCenterY from int to float.
    ...
    public boolean hasValidScreenNailTexture() {
        TiledScreenNail tsn = null;
        if(mScreenNail != null && mScreenNail instanceof TiledScreenNail) {
            tsn = (TiledScreenNail)mScreenNail;
            if(null != tsn && tsn.hasValidTexture()) {
                return true;
            }
        }
        return false;
    ...
    //ShenQianfeng modify centerX, centerY from int to float.
    ...
    //ShenQianfeng modify cX, cY from int to float.
    ...
    protected void render(GLCanvas canvas) {
        //LogUtil.d(TAG, "TileImageView render----------");
        ...
        try {
            if (level != mLevelCount && !isScreenNailAnimating()) {
                if (mScreenNail != null) {
                    LogUtil.d(TAG, "TileImageView::render mScreenNail.noDraw");
                    mScreenNail.noDraw();
                }
    ...
    public boolean isScreenNailAnimating() {
    ...
    private class TileUploader implements GLRoot.OnGLIdleListener {
        ...
        public boolean onGLIdle(GLCanvas canvas, boolean renderRequested) {
            //LogUtil.d(TAG, "TileUploader::onGLIdle");
    ...
    public void drawTile(GLCanvas canvas, int tx, int ty, int level, float x, float y, float length) {
        //LogUtil.d(TAG, "drawTile canvas tx ty  level x y length");
    ...
    static boolean drawTile(Tile tile, GLCanvas canvas, RectF source, RectF target) {
        //LogUtil.d(TAG, "drawTile tile canvas source target");
        while (true) {
            if (tile.isContentValid()) {
