...
class PositionController {
    ...
    public static final float SCALE_LIMIT = 10;//ShenQianfeng modify this limit to 10, original is 4
    ...
    public void setViewSize(int viewW, int viewH) {
        ...
        if (wasMinimal) {
            ...
            //LogUtil.d(TAG, "setViewSize : b.mCurrentScale" + b.mCurrentScale);
        }
    ...
    private boolean setBoxSize(int i, int width, int height, boolean isViewSize) {
        ...
        if ((wasViewSize && !isViewSize) || !mFilmMode) {
            ...
            //LogUtil.d(TAG, "setBoxSize 111111: b.mCurrentScale" + b.mCurrentScale);
            b.mAnimationStartTime = NO_ANIMATION;
        } else {
            b.mCurrentScale *= ratio;
            //LogUtil.d(TAG, "setBoxSize 222222: b.mCurrentScale" + b.mCurrentScale);
            b.mFromScale *= ratio;
            b.mToScale *= ratio;
        }
    ...
    private void convertBoxToRect(int i) {
        ...
        if (i == 0) {
            ...
            r.right = r.left + w;
            /*
            int wwwwwww = (int) (b.mImageW * b.mCurrentScale + 0.5f);
            LogUtil.i(TAG, "convertBoxToRect i:" + i + " mPlatform.mCurrentX:" + mPlatform.mCurrentX + " mViewW:" + mViewW + 
                    " x:" + x + " w:" + w + " r.left" + r.left + " r.right:" + r.right + " b.mImageW:" + b.mImageW + " b.mCurrentScale:" + b.mCurrentScale);
            */
        } else if (i > 0) {
    ...
    private void initBox(int index) {
        ...
        b.mCurrentY = 0;
        b.mCurrentScale = b.mScaleMin;
        //LogUtil.d(TAG, "initBox: 2222  index:" + index + " b.mCurrentScale: " + b.mCurrentScale);
        b.mAnimationStartTime = NO_ANIMATION;
    ...
    private void initBox(int index, Size size) {
        ...
        //LogUtil.d(TAG, "initBox index:" + index + " b.mImageW: " + b.mImageW + " b.mImageH:" + b.mImageH);
        b.mUseViewSize = false;
        b.mScaleMin = getMinimalScale(b);
        b.mScaleMax = getMaximalScale(b);
        b.mCurrentY = 0;
        b.mCurrentScale = b.mScaleMin;
        //LogUtil.d(TAG, "initBox: index:" + index + " b.mCurrentScale: " + b.mCurrentScale + " size:" + size.width + " & " + size.height);
        b.mAnimationStartTime = NO_ANIMATION;
        b.mAnimationKind = ANIM_KIND_NONE;
    }
    ...
    public void moveBox(int fromIndex[], boolean hasPrev, boolean hasNext, boolean constrained, Size[] sizes) {
    ...
    private float getMinimalScale(Box b) {
        ...
        if (!mFilmMode && mConstrained && !mConstrainedFrame.isEmpty()
            ...
            //LogUtil.d(TAG, "--------- getMinimalScale 1111111");

        } else {
            viewW = mViewW;
            viewH = mViewH;
            //LogUtil.d(TAG, "--------- getMinimalScale 2222222");
        }

        if (mFilmMode) {
            //LogUtil.d(TAG, "--------- getMinimalScale 33333333");
        ...
        /*
        LogUtil.i2(TAG, "--------- getMinimalScale 444444444 wFactor:" + wFactor + 
                " viewW:" + viewW + 
                " b.mImageW:" +  b.mImageW + 
                " hFactor:" + hFactor +
                " viewH:" + viewH +
                " b.mImageH:" + b.mImageH + 
                " data 1:" + (wFactor * viewW / b.mImageW) + 
                " data 2:" + (hFactor * viewH / b.mImageH) + 
                " s:" + s); */
    ...
    private class Box extends Animatable {
        ...
        public boolean startSnapback() {
            ...
            if (mAnimationKind == ANIM_KIND_DELETE && mListener.isHoldingDelete()) return false;
            ...
            if (this == mBoxes.get(0)) {
                float scaleMin = mExtraScalingRange ? mScaleMin * SCALE_MIN_EXTRA : mScaleMin;
                float scaleMax = mExtraScalingRange ? mScaleMax * SCALE_MAX_EXTRA : mScaleMax;
