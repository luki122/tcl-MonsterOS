...
import com.android.gallery3d.data.UnlockImage;
...
public abstract class AbstractSlotRenderer implements SlotView.SlotRenderer {
    ...
    //private final NinePatchTexture mFrameSelected;//ShenQianfeng annotated on 2016.08.19
    
    // TCL ShenQianfeng Begin on 2016.08.19
    private final ColorTexture mTextureSelectedMask;
    private final ResourceTexture mTextureSelected;
    private int mSelectedTextureMargin;
    ...
    protected AbstractSlotRenderer(Context context) {
        ...
        //mFrameSelected = new NinePatchTexture(context, R.drawable.grid_selected);//ShenQianfeng annotated on 2016.08.19
        
        // TCL ShenQianfeng Begin on 2016.08.19
        mTextureSelectedMask = new ColorTexture(Color.parseColor("#4D000000"));//30% transparency
        mTextureSelected = new ResourceTexture(context, R.drawable.ic_multi_selected );//
        mSelectedTextureMargin = context.getResources().getDimensionPixelSize(R.dimen.mst_selected_texture_margin);
    ...
    protected void drawSelectedFrame(GLCanvas canvas, int width, int height) {
        ...
        // Modify To:
        mTextureSelectedMask.draw(canvas, 0, 0, width, height);
        int w = (int)(mTextureSelected.getWidth() / mDensity);
        int h = (int)(mTextureSelected.getHeight() / mDensity);
        int x = width - w - mSelectedTextureMargin;
        int y = height - h - mSelectedTextureMargin;
        
        mTextureSelected.draw(canvas, x, y, w, h);
        // TCL ShenQianfeng End on 2016.08.19
