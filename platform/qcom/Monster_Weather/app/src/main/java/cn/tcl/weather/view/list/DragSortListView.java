...
package cn.tcl.weather.view.list;
...
import cn.tcl.weather.view.IInterceptyTouchEvent;
...
public class DragSortListView extends ListView {
     ...
     *
     * @return The ListAdapter set as the argument of {@link setAdapter(BaseAdapter)}
     */
...
//            final int alpha = (int) (255f * mCurrFloatAlpha * alphaMod);
            ...
            final int alpha = (int) (255f * mCurrFloatAlpha * mFloatView.getAlpha());
    ...
    private boolean updatePositions() {
        ...
        int firstExpPos = mFirstExpPos;
        int secondExpPos = mSecondExpPos;
        float slideFrac = mSlideFrac;

        int oldFirstExpPos = firstExpPos;
        int oldSecondExpPos = secondExpPos;
        float oldSlideFrac = slideFrac;
        ...
        if (mAnimate) {
            ...
            if (mFloatViewMid < slideEdgeTop) {
                firstExpPos = itemPos - 1;
                secondExpPos = itemPos;
                slideFrac = 0.5f * ((float) (slideEdgeTop - mFloatViewMid)) / slideRgnHeightF;
                // Log.d("mobeta",
                // "firstExp="+firstExpPos+" secExp="+secondExpPos+" slideFrac="+slideFrac);
            } else if (mFloatViewMid < slideEdgeBottom) {
                firstExpPos = itemPos;
                secondExpPos = itemPos;
            } else {
                firstExpPos = itemPos;
                secondExpPos = itemPos + 1;
                slideFrac = 0.5f * (1.0f + ((float) (edgeBottom - mFloatViewMid))
                        / slideRgnHeightF);
                // Log.d("mobeta",
                // "firstExp="+firstExpPos+" secExp="+secondExpPos+" slideFrac="+slideFrac);
            }
        ...
        if (canDraging(firstExpPos)) {
            mFirstExpPos = firstExpPos;
            mSecondExpPos = secondExpPos;
            mSlideFrac = slideFrac;
        }
    ...
    private boolean canDraging(int position) {
        if (null != mDragListener) {
            return mDragListener.canDraging(position);
        }
        return true;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean fvmIntercept = false;
        if (null != mFloatViewManager) {
            fvmIntercept = mFloatViewManager.onInterceptTouchEvent(ev);
        }
        ...
        if (!(intercept | fvmIntercept)) {
            intercept = mVerticalInterceptTouchEvent.onInterceptTouchEvent(ev);
            if (intercept) {
                mVerticalInterceptTouchEvent.resetMotionEvent();
            }
        }

        return intercept | fvmIntercept;
    }
    ...
    private void continueDrag(int x, int y) {
        ...
        if (!canDraging(pointToPosition(x, y))) {
            return;
        }
    ...
    public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
        if (mDragState != IDLE || !mInTouchEvent || mFloatView != null || floatView == null
                || !mDragEnabled || !canDraging(position)) {
            return false;
        }
     ...
     * draggable, where the drag int mode determines how
    ...
    public interface DragListener {
        ...
        boolean canDraging(int position);
    }
