...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    private float mInterceptPx, mInterceptPy;
     ...
     * and ON_DOWN drag int. By default, sorting is enabled, and
     * removal is disabled.
     *
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (mDragInitMode == ON_LONG_PRESS) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                mInterceptPx = ev.getX();
                mInterceptPy = ev.getY();
                mDslv.postDelayed(mLongPress, TAP_TIMEOUT + LONGPRESS_TIMEOUT);
            } else if (action == MotionEvent.ACTION_MOVE) {
                float px = ev.getX() - mInterceptPx;
                float py = ev.getY() - mInterceptPy;
                if (px * px + py * py > mTouchSlopSquare) {
                    resetLongPress();
                }
            } else {
                if (isLongPressed && isLongPressDealed) {
                    mDslv.onTouchEvent(ev);
                }
                resetLongPress();
            }
    ...
    private boolean isLongPressDealed = true;
    ...
    private void resetLongPress() {
        ...
        mInterceptPy = 0;
        isLongPressed = false;
        mDslv.removeCallbacks(mLongPress);
    }
     ...
     * @return The list position to drag if a drag-int gesture is
    ...
    public boolean onDown(MotionEvent ev) {
        ...
        if (mHitPos != MISS) {
            if (mDragInitMode == ON_DOWN) {
                startDrag(mHitPos, (int) ev.getX() - mItemX, (int) ev.getY() - mItemY);
            } else if (isLongPressed && !isLongPressDealed) {
                isLongPressDealed = true;
    ...
    public void onLongPress(MotionEvent e) {
        ...
        if (mHitPos != MISS && mDragInitMode == ON_LONG_PRESS && !isLongPressDealed) {
            isLongPressDealed = true;
            mDslv.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
    ...
    private Runnable mLongPress = new Runnable() {
        ...
        public void run() {
            isLongPressed = true;
            isLongPressDealed = false;
            onTouch(mDslv, mDownEvent);
        }
    };
