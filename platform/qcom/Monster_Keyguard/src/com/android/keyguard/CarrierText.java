...
public class CarrierText extends TextView {
    ...
    */
    ...
    ///TCL Monster: kth mod for getCarrierName() show english rather than chinese 20161115 start
    public String tranferName(String name ,String operatorNumeric)
    {
        if ("46001".equals(operatorNumeric)
                || "UNICOM".equals(name)
                || "CHN-UNICOM".equals(name)) {
            name = getContext().getString(com.android.internal.R.string.China_Unicom);
        }
        if ("46000".equals(operatorNumeric)
                || "46002".equals(operatorNumeric)
                || "46007".equals(operatorNumeric)
                || "46008".equals(operatorNumeric)
                || "CMCC".equals(name)
                || "CHINA MOBILE".equals(name)) {
            name = getContext().getString(com.android.internal.R.string.China_Mobile);
        }
        if ("46003".equals(operatorNumeric)
                || "46011".equals(operatorNumeric)
                || "CHINA TELECOM".equals(name)){
            name = getContext().getString(com.android.internal.R.string.China_Telecom);

        }
        return name;
    }
    ///TCL Monster: kth mod for getCarrierName() show english rather than chinese 20161115 end
    ...
    protected void updateCarrierText() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            boolean showRat = SubscriptionManager.getResourcesForSubId(mContext,
                    subId).getBoolean(R.bool.kg_config_display_rat);//TCL Monster: kth add for keyguard carrier text show Rat 20161019
            if (showRat) {
                ...
                if (ss != null && (ss.getDataRegState() == ServiceState.STATE_IN_SERVICE
                    ...
                    ///TCL Monster: kth mod for keyguard carrier text show 4G when (sim insert & sim disabled & connected wifi) 20161025 start @{
            ...
            carrierName = tranferName(carrierName.toString(), null);    ///TCL Monster: kth mod for getCarrierName() show english rather than chinese 20161115
            if (showLocale || showRat) {
                ...
                if (DEBUG) Log.d(TAG, "updateCarrierText: name.length=" + names.length + "subId=" + subId + " getDefaultDataSubscriptionId()=" + SubscriptionManager.getDefaultDataSubscriptionId());
                for (int j = 0; j < names.length; j++) {
                    ...
                    if (!TextUtils.isEmpty(names[j])) {
                        ///TCL Monster: kth add for keyguard carrier text show Rat 20161019 start
                        ...
                        ///TCL Monster: kth add for keyguard carrier text show Rat 20161019 end
