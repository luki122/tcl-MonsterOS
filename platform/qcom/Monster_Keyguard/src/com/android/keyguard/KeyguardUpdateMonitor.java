...
import android.content.ContentResolver; // MODIFIED by jianguang.sun, 2016-08-31,BUG-2740734
...
import android.net.Uri; // MODIFIED by jianguang.sun, 2016-08-31,BUG-2740734
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private boolean mAllowAlarm;
    private boolean mBlock = false;

    private boolean shouldListenForFingerError = true;// TCL Monster: kth add for stop listen finger when finger too many attempts 20161122
    ...
    /* MODIFIED-BEGIN by jianguang.sun, 2016-08-25,BUG-2740734*/
    private void handleFingerprintAuthenticated(AuthenticationResult result) {
        /* MODIFIED-BEGIN by jianguang.sun, 2016-10-12,BUG-2987595*/
        int fpid = 0;
        int tag = 0;
        ...
        try {
            ...
            }else if(tag == FingerprintManager.FP_TAG_PRIVATE_MODE) {
                //to be done
                /* MODIFIED-BEGIN by jianguang.sun, 2016-10-12,BUG-2987595*/
                //jianguangadd
            }else if(tag == FingerprintManager.FP_TAG_ALARM) {
                onAlarmFingerprintAuthenticated();
                if (!mAllowAlarm) {
                    startListeningForFingerprint();
                    return;
                }
                mBlock = true;
                Log.d(TAG, "authen mode : ALARM");
                Settings.System.putInt(mContext.getContentResolver(),
                        Settings.System.TCT_IS_ALARMFP, 1);
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.d(TAG, "delay start authen after get alarm fp ");
                        startListeningForFingerprint();
                        mBlock = false;
                        Settings.System.putInt(mContext.getContentResolver(),
                                Settings.System.TCT_IS_ALARMFP, 0);
                    }
                }, 1500);
            }
            /* MODIFIED-END by jianguang.sun,BUG-2740734*/
        } finally {
            if (!(tag == FingerprintManager.FP_TAG_ALARM && !mAllowAlarm)) {
                setFingerprintRunningState(FINGERPRINT_STATE_STOPPED);
            }
        }
    }

    private void onAlarmFingerprintAuthenticated() {
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onFingerprintIsAlarm();
            }
            /* MODIFIED-END by jianguang.sun,BUG-2987595*/
        }
    }
    ...
    private void handleFingerprintError(int msgId, String errString) {
        if (msgId == FingerprintManager.FINGERPRINT_ERROR_CANCELED
            ...
            setFingerprintRunningState(FINGERPRINT_STATE_STOPPED);
            if (DEBUG) Log.d(TAG, "handleFingerprintError: stop listen for fingerprint until called reset");
            shouldListenForFingerError = false;// TCL Monster: kth add for stop listen finger when finger too many attempts 20161122
        }
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    private void handleFingerprintLockoutReset() {
        shouldListenForFingerError = true;// TCL Monster: kth add for stop listen finger when finger too many attempts 20161122
    ...
    private FingerprintManager.AuthenticationCallback mAuthenticationCallback
        ...
        public void onAuthenticationSucceeded(AuthenticationResult result) {
            handleFingerprintAuthenticated(result); // MODIFIED by jianguang.sun, 2016-08-25,BUG-2740734
        }
    ...
    private class SettingsObserver extends ContentObserver{
        private final Uri UNLOCK_URI=Settings.System.getUriFor(Settings.System.TCT_UNLOCK_SCREEN);
        /* MODIFIED-BEGIN by jianguang.sun, 2016-10-12,BUG-2987595*/
        private final Uri ALARM_URI=Settings.System.getUriFor(Settings.System.TCT_ONE_KEY_ALARM);
        ...
        void observer(){
            ...
            contentResolver.registerContentObserver(ALARM_URI, false, this, UserHandle.USER_ALL);
            /* MODIFIED-END by jianguang.sun,BUG-2987595*/
            contentResolver.registerContentObserver(UNLOCK_URI, false, this, UserHandle.USER_ALL);
            update(null);
        }
        ...
        public void update(Uri uri){
            ...
            if(UNLOCK_URI.equals(uri) || uri == null){
                mAllowUnlock = Settings.System.getInt(resolver,Settings.System.TCT_UNLOCK_SCREEN,0) == 1;
                mAllowAlarm = Settings.System.getInt(resolver,Settings.System.TCT_ONE_KEY_ALARM,0) == 1; // MODIFIED by jianguang.sun, 2016-10-12,BUG-2987595
            }
         }
    }
    /* MODIFIED-END by jianguang.sun,BUG-2740734*/
    ...
    private void updateFingerprintListeningState() {
        ...
        if (DEBUG) Log.d(TAG, "updateFingerprintListeningState: shouldListenForFingerprint=" + shouldListenForFingerprint);
    ...
    private boolean shouldListenForFingerprint() {
        if (DEBUG)
            Log.d(TAG, "shouldListenForFingerprint: mKeyguardIsVisible=" + mKeyguardIsVisible
            ...
            + " mBlock= " + mBlock
            + " shouldListen=" + shouldListenForFingerError);
        return (mKeyguardIsVisible || !mDeviceInteractive || mBouncer || mGoingToSleep)
                && !mSwitchingUser && !mFingerprintAlreadyAuthenticated
                && !isFingerprintDisabled(getCurrentUser()) && !mBlock && shouldListenForFingerError; // MODIFIED by jianguang.sun, 2016-10-12,BUG-2987595
    }
    ...
    public boolean isUnlockWithFingerprintPossible(int userId) {
        return mFpm != null && mFpm.isHardwareDetected() && !isFingerprintDisabled(userId)
                && mFpm.getEnrolledFingerprints(userId).size() > 0 && (mAllowUnlock|| mAllowAlarm); // MODIFIED by jianguang.sun, 2016-08-31,BUG-2740734
    }
