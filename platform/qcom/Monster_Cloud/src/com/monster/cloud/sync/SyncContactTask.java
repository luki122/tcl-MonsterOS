...
public class SyncContactTask extends BaseSyncTask {

    private final WeakReference<Context> mWeakContext;
    private final WeakReference<TCLSyncManager.UIHandler> mWeakHandler;

    private SyncContact task;

    SyncContactTask(Context context, TCLSyncManager.UIHandler handler){
        super();
    ...
    protected boolean onPrepare() {
        ...
        if (null == context || null == handler)
            return false;
        task = new SyncContact(context, new SyncProcessorObserver(this, handler));
        return true;
    }
