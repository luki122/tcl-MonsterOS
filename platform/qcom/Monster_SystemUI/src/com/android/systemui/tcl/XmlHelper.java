...
public class XmlHelper {
     ...
     * 写xml
     */
    public boolean writeXML(List<WdjNotification> notifications) {
        ...
        try {
            ...
            for (WdjNotification notification : notifications) {
                serializer.startTag(null, "wdj_notification");

                serializer.startTag(null, "notify_type");
                serializer.text(Utils.serialize(notification.getNotifyType()));
                serializer.endTag(null, "notify_type");

                serializer.startTag(null, "notifications");
                serializer.text(Utils.serialize(notification.getNotifications()));
     ...
     * 解析xml
     */
    public List<WdjNotification> pullPhraseXml(Context context) {
        List<WdjNotification> mList = new ArrayList<WdjNotification>();
        WdjNotification notification = null;
        InputStream is;
        try {
                ...
                switch (eventType) {
                    ...
                    case XmlPullParser.START_TAG:
                        if (xpp.getName().equals("wdj_notification")) { // 判断开始标签元素是否是wdj_notification
                            notification = new WdjNotification();
                        } else if (xpp.getName().equals("notify_type")) {
                            eventType = xpp.next();
                            notification.setNotifyType(Utils.deSerialize(xpp.getText()));
                        } else if (xpp.getName().equals("notifications")) {
                            eventType = xpp.next();
                            notification.setNotifications(Utils.deSerialize(xpp.getText()));
                        }
                        break;
