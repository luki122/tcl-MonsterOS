...
public class AppNotifySetting extends BaseActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle arg0) {
        ...
        Log.e("kebelzc24", "pkg = " + mPackageName + " -- mUid " + mUid + " -- user id " + UserHandle.getUserId(mUid));
        //find view
    ...
    private void loadAppInfo() {
        ...
        mAppRow = mBackend.loadAppRow(AppNotifySetting.this, pm, getApplicationInfo(this,mPackageName,mUid));
        mIsSystemPackage = Utils.isSystemPackage(pm, info);
    }

    private ApplicationInfo getApplicationInfo(Context context, String pkg, int uid) {
        ...
        for (UserInfo user : mUm.getProfiles(UserHandle.myUserId())) {
            if (user.id == UserHandle.getUserId(uid)) {
                try {
    ...
    private View addItemWithDescView(int iconRes, int name, int desc, OnClickListener callback, boolean ischecked, int categoryID) {
        ...
        if (R.string.notify_show_detail == name) {
            ...
            mShowDetailLockScreen.setEnabled(mAppRow.lockscreennotify);
        }
        return view;
    }
