                ...
                if (!mCancelled && mLoadQueue.isEmpty()) {
                    synchronized(mLoadQueue) {
                        try {
                            ...
                            //[SOLUTION]-Add-BEGIN by TCTNB.(Yubin.Ying), 08/25/2016, SOLUTION-2481973
...
public class RecentsTaskLoader {
    ...
    /**Mst: add by tangjunfor rotate bitmap begin*/
    public Bitmap rotate(Bitmap b, int degrees) {
        if (degrees != 0 && b != null) {
            ...
            if (b != b2) {
            	//Mst: tangjun TODO need to do begin
            	b.recycle();
            	//Mst: tangjun TODO need to do end
            	b = b2;
            }
        }
        return b;
    }
    /**Mst: add by tangjunfor rotate bitmap end*/
    
    /**
     * Acquires the task resource data directly from the cache, loading if necessary.
     */
    public void loadTaskData(Task t, Context context) {
        Drawable icon = mIconCache.getAndInvalidateIfModified(t.key);
        Bitmap thumbnail = null;
        ActivityManager.TaskThumbnailInfo thumbnailInfo = null;
        ThumbnailData thumbnailData = mThumbnailCache.getAndInvalidateIfModified(t.key);
    	...
    	if(thumbnail != null && !thumbnail.isRecycled()) {
    		if(thumbnail.getHeight() > thumbnail.getWidth() && context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
    			thumbnail = rotate(thumbnail, -90);
    		} else if (thumbnail.getHeight() <= thumbnail.getWidth() && context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
    			thumbnail = rotate(thumbnail, 90);
    		}
    ...
    Bitmap getAndUpdateThumbnail(Task.TaskKey taskKey, boolean loadIfNotCached) {
        ...
        if (loadIfNotCached) {
            ...
            if (config.svelteLevel < RecentsConfiguration.SVELTE_DISABLE_LOADING) {
                ...
                thumbnailData = ssp.getTaskThumbnail(taskKey);
...
//        return activityInfo;
        ...
        if (!SmartContainerConfig.WITH_OUT_APP_CLONE) {
            ...
            try {
                ci = SmartContainerManagerNative.get().getContainerInfo(taskKey.userId);
            } catch (Exception exception) {
            }
            if (ci == null || !ci.isVirtualBox()) {
                activityInfo = mActivityInfoCache.get(cn);
            }
            if (activityInfo == null) {
                activityInfo = ssp.getActivityInfo(cn, taskKey.userId);
                if (cn == null || activityInfo == null) {
