...
public class RecentsConfiguration {
    ...
    /**Mst: tangjun define final var to fix requirement begin*/
    private static final String PREF_RECENTS_APP_LOCK_INFO= "recents_lock_info";
    /**Mst: tangjun define final var to fix requirement end*/
    ...
    /**Mst: tangjun add read lock state from sharedpreference to fix requirement begin*/
    public static boolean readRecentsAppsLockState(Context c,String pkgName){
        SharedPreferences sharedPreferences = c.getSharedPreferences(
        		PREF_RECENTS_APP_LOCK_INFO, 0);
        return sharedPreferences.getBoolean(pkgName, false);
    }
    /**Mst: tangjun add read lock state from sharedpreference to fix requirement end*/
    
    /**Mst: tangjun add write lock state from sharedpreference to fix requirement begin*/
    public static void writeRecentsAppLockState(Context c,String pkgName,boolean val){
        SharedPreferences sharedPreferences = c.getSharedPreferences(
        		PREF_RECENTS_APP_LOCK_INFO, 0);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putBoolean(pkgName, val);
        editor.commit();
    }
    /**Mst: tangjun add write lock state from sharedpreference to fix requirement end*/
    
    /**Mst: tangjun getRecentsAppsKey to fix requirement start*/
    public static String getRecentsAppsKey(Task t){
    	return t.key.baseIntent.getComponent().getPackageName()+"$"+ t.key.baseIntent.getComponent().getClassName();
    }
    /**Mst: tangjun getRecentsAppsKey to fix requirement end*/
    
	/**Mst: tangjun add for get Integer value from dimens list begin*/
	public static int getIntegerValueFromResourcesDimens(Resources res,int id){
		return res.getInteger(id);
	}
	/**Mst: tangjun add for get Integer value from dimens list end*/
}
