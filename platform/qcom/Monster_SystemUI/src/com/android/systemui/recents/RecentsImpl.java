...
public class RecentsImpl implements ActivityOptions.OnAnimationFinishedListener {
     ...
     * Creates the activity options for a home->recents transition.
     */
    protected ActivityOptions getHomeTransitionActivityOptions(TaskStack stack) {
    	if(stack.getTaskCount() > 1 && !"com.monster.launcher".equals(Recents.getSystemServices().getRunningTask().topActivity.getPackageName())) {
	        return ActivityOptions.makeCustomAnimation(mContext,
	                R.anim.recents_from_launcher_enter,
	                R.anim.recents_from_launcher_exit,
	                mHandler, null);
	        ...
	        return ActivityOptions.makeCustomAnimation(mContext,
	                R.anim.recents_from_launcher_enter_ex,
	                R.anim.recents_from_launcher_exit,
	                mHandler, null);
    	}
    }
    ...
    protected void startRecentsActivity(ActivityManager.RunningTaskInfo runningTask,
    	...
    	/**Mst: tangjun mod for don't need useThumbnailTransition begin*/
        //if (useThumbnailTransition) {
        if (false) {
    	/**Mst: tangjun mod for don't need useThumbnailTransition end*/
            ...
            opts = hasRecentTasks
                ...
                */
            opts = hasRecentTasks
                    ? getHomeTransitionActivityOptions(stack)
                    : getUnknownTransitionActivityOptions();
        }
