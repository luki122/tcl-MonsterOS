...
public class RecentsTaskLoadPlan {
    ...
    public synchronized void preloadRawTasks(boolean includeFrontMostExcludedTask) {
        ...
        /**Mst: tangjun mod don't need to reverse begin*/
        //Collections.reverse(mRawTasks);
        /**Mst: tangjun mod don't need to reverse end*/
    }
        ...
        for (int i = 0; i < taskCount; i++) {
            ...
            if(i == 0) {
            	if(Recents.getSystemServices().hasDockedTask()) {
            		//Log.d("111111", "RecentsTaskLoadPlan add");
            		if(!SystemServicesProxy.mDockPackageNames.contains(t.baseIntent.getComponent().getPackageName())) {
            			SystemServicesProxy.mDockPackageNames.add(t.baseIntent.getComponent().getPackageName());
            		}
            	} else {
            		//Log.d("111111", "RecentsTaskLoadPlan remove");
            		SystemServicesProxy.mDockPackageNames.remove(t.baseIntent.getComponent().getPackageName());
            		//add for app fenshen
            		SystemServicesProxy.mDockRunningTaskUserId = t.userId;
            	}
            }
            ...
            //Log.d("111111", "RecentsTaskLoadPlan preloadPlan--t.baseIntent.getComponent().getPackageName() = -" + t.baseIntent.getComponent().getPackageName());
            //Log.d("111111", "RecentsTaskLoadPlan preloadPlan--t.realActivity.getPackageName() = -" + t.realActivity.getPackageName());
