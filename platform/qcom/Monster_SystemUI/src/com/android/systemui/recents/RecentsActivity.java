    ...
    class  SplitscreenGridViewAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null){
				...
				Drawable d = null;
				try {
					d = Utilities.createIconBitmapDrawable(PKGIcongetter.getInstance(mContext).getIconDrawable(mGridLauncherActivityInfoList.get(position).getComponentName()), mContext);
				} catch (Exception e) {
					// TODO: handle exception
					d = Utilities.createIconBitmapDrawable(mGridLauncherActivityInfoList.get(position).getBadgedIcon(0), mContext);
				}
...
//                        //modify by liuzhicang end
    ...
    /**Mst: tangjun add begin*/
    private void inflateSplitScreenRecents() {
    	...
    	//Log.d("111111", "---hasDockedTask = " + Recents.getSystemServices().hasDockedTask());
        if(mSplitViewPagerFrame != null) {
	        if(hasDockedTask) {
        ...
        //Mst: tangjun add begin
        if (mSplitViewPagerFrame == null && hasDockedTask) {
        	mSplitViewPagerFrame = Utilities.findViewStubById(this,
                    R.id.splitscreen_recents).inflate();
        ...
        if(mSplitViewPagerFrame != null && hasDockedTask) {
        	...
        	if(mLauncherActivityInfoList != null) {
        		mLauncherActivityInfoList.clear();
        	} else {
        		mLauncherActivityInfoList = new ArrayList<LauncherActivityInfo>();
        	}
        	
        	if(mLauncherActivityInfoUserList != null) {
        		mLauncherActivityInfoUserList.clear();
        	} else {
        		mLauncherActivityInfoUserList = new ArrayList<UserHandle>();
        	}
        	
        	//Log.d("111111", "---android.os.Process.myUserHandle() = " + android.os.Process.myUserHandle());
        	int index = 0;
        	for(int i = 0; i < userHandleList.size(); i++) {
        		//Log.d("111111", "---i = " + i + ", userHandleList = " + userHandleList.get(i));
        		...
        		for(int j = index; j < count; j++) {
        			...
        			//Log.d("111111", "----launcherActivityInfo = " + launcherActivityInfo.getComponentName().getPackageName());
        	...
        	if(size > 0) {
        		mSplitViewPager.setVisibility(View.VISIBLE);
        		mPageIndicatorLinear.setVisibility(View.VISIBLE);
        		this.pageTotal = pageTotal;
        		mPageGridViews.clear();
        		...
        		mSplitViewPager.setVisibility(View.GONE);
        		mPageIndicatorLinear.setVisibility(View.GONE);
        	}
        }
    }
    /**Mst: tangjun add end*/
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        RecentsImpl.setRecentActivityFocuse(hasFocus);
