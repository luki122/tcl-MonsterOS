...
public class QSTileBaseView extends LinearLayout {
    ...
    public void onStateChangedEditMode(QSTile.State state) {
        mHandler.obtainMessage(H.STATE_CHANGED_EDIT_MODE, state).sendToTarget();
    }

    protected void handleStateChangedEditMode(QSTile.State state) {
        handleStateChanged(state, true);
    }

    protected void handleStateChanged(QSTile.State state, boolean isEditMode) {
        if (!isEditMode) {
            mIcon.setIcon(state);
        }
    ...
    private class H extends Handler {
        ...
        private static final int STATE_CHANGED_EDIT_MODE = 2;// TCL Monster: kth add for edit qstile icon 20161125
        public H() {
        ...
        public void handleMessage(Message msg) {
            ...
            } else if (msg.what == STATE_CHANGED_EDIT_MODE) {
                handleStateChangedEditMode((QSTile.State) msg.obj);// TCL Monster: kth mod for edit qstile icon 20161125
            }
        }
    }
    public void setLabelClick(OnClickListener onclick){} //add by chenhl
}
