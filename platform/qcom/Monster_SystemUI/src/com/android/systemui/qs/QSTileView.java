...
public class QSTileView extends QSTileBaseView {
    ...
    protected void createLabel() {
        ...
        mLabelLayout = view;//add by chenhl
    ...
    protected void handleStateChanged(QSTile.State state) {
        ...
        mPadLock.setVisibility(state.showMore ? View.VISIBLE : View.GONE);
    }

    // TCL Monster: kth add for edit qstile icon 20161125 start
    @Override
    protected void handleStateChangedEditMode(QSTile.State state) {
        super.handleStateChangedEditMode(state);
        if (!Objects.equal(mLabel.getText(), state.label)) {
            mLabel.setText(state.label);
        }
        mLabel.setEnabled(!state.disabledByPolicy);
    ...
    // TCL Monster: kth add for edit qstile icon 20161125 end

    //add by chenhl start
    private View mLabelLayout;
    public void setLabelClick(OnClickListener onclick){
        mLabelLayout.setOnClickListener(onclick);
    }
    //add by chenhl end
}
