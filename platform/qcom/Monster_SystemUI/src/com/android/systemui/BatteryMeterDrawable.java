...
public class BatteryMeterDrawable extends Drawable implements
    ...
    private float mFrameBtnWidth;
    private float mFrameBtnHeight;
    private float mFrameStroke;
    private float mFramebetwen;
    private Drawable mBoltDrawable,mBoltDrawable1;
    private final Paint mButtonframePaint;
    //add by chenhl end
    ...
    public BatteryMeterDrawable(Context context, Handler handler, int frameColor) {
        ...
        mFramePaint.setStrokeWidth(2.4f);//modify by chenhl for 0 to 2.4f
        mFramePaint.setStyle(Paint.Style.STROKE); //modify by chenhl for FILL_AND_STROKE to STROKE
        ...
        //add by chenhl start
        mButtonframePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mButtonframePaint.setDither(true);
        mButtonframePaint.setStrokeWidth(0);
        mButtonframePaint.setColor(frameColor);
        mButtonframePaint.setStyle(Paint.Style.FILL_AND_STROKE);
        //add by chenhl end
        ...
        //add by chenhl start
        mFrameBtnWidth = context.getResources().getDimension(R.dimen.tcl_battery_btn_width);
        mFrameBtnHeight = context.getResources().getDimension(R.dimen.tcl_battery_btn_height);
        mFrameStroke = context.getResources().getDimension(R.dimen.tcl_battery_frame_stroke);
        mFramebetwen = context.getResources().getDimension(R.dimen.tcl_battery_frame_betwen);
        mBoltDrawable=context.getResources().getDrawable(R.drawable.tcl_stat_sys_charge);
        mBoltDrawable1=context.getResources().getDrawable(R.drawable.tcl_stat_sys_charge_white);
        mBoltDrawable.setTint(mDarkModeFillColor);
        //add by chenhl end
    ...
    public void setDarkIntensity(float darkIntensity) {
        ...
        mButtonframePaint.setColor(fillColor);
    ...
    //add by chenhl start
    private void tclDraw(Canvas canvas){
        final boolean showChargingAnim
                = mContext.getResources().getBoolean(R.bool.config_show_battery_charging_anim);
        final int level = showChargingAnim
                ? updateChargingAnimLevel()
        ...
        final float buttonHeight =mFrameBtnWidth;
        mFrame.set(mFrameStroke/2, mFrameStroke/2, width-mFrameStroke/2, height-mFrameStroke/2);

        float df = (mFrame.height()-mFrameBtnHeight)/2;
        mButtonFrame.set(
                mFrame.right-buttonHeight+mFrameStroke/2 ,
                mFrame.top+ df,
                mFrame.right+mFrameStroke/2,
                mFrame.bottom - df);
        mFrame.right -= buttonHeight;

        float batterystart = mFramebetwen+mFrameStroke;
        mBatteryFrame.set(batterystart,batterystart,width-buttonHeight-batterystart,height-batterystart);

        // set the battery charging color
        mBatteryPaint.setColor(mPluggedIn ? mChargeColor : getColorForLevel(level));
        final float levelTop = mBatteryFrame.width()*drawFrac+batterystart;
        mBatteryFrame.right = levelTop;
        canvas.drawRoundRect(mFrame,1,1,mFramePaint);
        canvas.drawRect(mButtonFrame,mButtonframePaint);

        if (mPluggedIn) {
            // define the bolt shape
            Drawable drawable = mOldDarkIntensity>0.5?mBoltDrawable:mBoltDrawable1;
            Bitmap bitmap = drawableToBitmap(drawable);
            canvas.drawBitmap(bitmap, mFrame.width()/2-bitmap.getWidth()/2, mFrame.height()/2
                    -bitmap.getHeight()/2, new Paint());
        } else{
            canvas.drawRect(mBatteryFrame,mBatteryPaint);
        }
