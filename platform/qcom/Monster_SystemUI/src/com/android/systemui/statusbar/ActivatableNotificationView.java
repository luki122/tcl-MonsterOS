...
public abstract class ActivatableNotificationView extends ExpandableOutlineView {
    ...
    protected void onFinishInflate() {
        ...
        mLineView = findViewById(R.id.tcl_keyguard_line);//add by chenhl
        updateBackground();
    ...
    public void setDimmed(boolean dimmed, boolean fade) {
        if (mDimmed != dimmed) {
            ...
            //add by chenhl start
            mBackgroundNormal.setCustomBackground(mDimmed?R.color.tcl_notification_keyguard_bg:
                R.drawable.notification_material_bg);
            mLineView.setVisibility((!mIsChild && mDimmed)?View.VISIBLE:View.GONE);
            //add by chenhl end
        }
    }
    ...
    public void setActualHeight(int actualHeight, boolean notifyListeners) {
        ...
        //add by chenhl start
        if(mLineView != null) {
        	mLineView.setTranslationY(actualHeight-1);
        }
        //add by chenhl end
    }
    ...
    private int calculateBgColor(boolean withTint) {
        if (withTint && mBgTint != 0) {
            return mNormalColor;   //modify by chenhl
        } else if (mShowingLegacyBackground) {
            return mNormalColor;//mLegacyColor;  //modify by chenhl
        } else if (mIsBelowSpeedBump) {
            return mNormalColor;//mLowPriorityColor; //modify by chenhl
        } else {
            return mNormalColor;
        }
    }
    ...
    @Override
    public void setShowAsChildren(boolean is) {
        mIsChild = is;
    }
    //add by chenhl end
}
