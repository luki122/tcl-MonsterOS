...
import com.android.systemui.tcl.TclNotificationCleanCallback;
import com.android.systemui.tuner.TunerService;

public class QuickStatusBarHeader extends BaseStatusBarHeader implements
        NextAlarmChangeCallback, OnClickListener, OnUserInfoChangedListener,
        TclNotificationCleanCallback {
    ...
    private TextView mNotificationCounts; //add by zhicang.liu
    ...
    protected void onFinishInflate() {
        ...
        //add by chenhl start
        findViewById(R.id.id_notification_settings).setOnClickListener(this);
        //add by chenhl end
        // add by zhicang.liu begin
        mNotificationCounts = (TextView)findViewById(R.id.id_notification_counts);
    ...
    private void updateResources() {
        ...
        mSecondHalfAnimator = new TouchAnimator.Builder()
                .addFloat(mAlarmStatus/*mShowFullAlarm ? mAlarmStatus : findViewById(R.id.date)*/, "alpha", 0, 1)
...
//                ? View.VISIBLE : View.INVISIBLE);
        mEmergencyOnly.setVisibility(mExpanded ? View.VISIBLE : View.INVISIBLE);
        //modify by chenhl end
    ...
    public void onClick(View v) {
        ...
        //add by chenhl start
        else if(v.getId()==R.id.id_notification_settings){
            Intent intent = new Intent(this.mContext, NotifyInfoActivity.class);
    ...
    // add by zhicang.liu begin
    private int DISPLAY_NOTIFY_CLEANED_NUM = 1;
    private Handler mHandler = new Handler(){
        @Override
        public void handleMessage(Message msg)  {
            if( DISPLAY_NOTIFY_CLEANED_NUM == msg.what) {
                if (null != mNotificationCounts) {
                    if (SpamNotifyHandle.getInstance(mContext).getNotifyNum() > 0) {
                        mNotificationCounts.setVisibility(VISIBLE);
                        mNotificationCounts.setText(SpamNotifyHandle.getInstance(mContext).getNotifyNum() + "");
                    } else {
                        mNotificationCounts.setVisibility(INVISIBLE);
                    }
    ...
    public void cleanNotify() {
        ...
        mHandler.sendEmptyMessage(DISPLAY_NOTIFY_CLEANED_NUM);
    }
    //add by zhicang.liu end
}
