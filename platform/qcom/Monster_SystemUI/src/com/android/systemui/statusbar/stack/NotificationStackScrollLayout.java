...
import com.android.systemui.tcl.SpamNotifyHandle;
...
public class NotificationStackScrollLayout extends ViewGroup
    ...
    //add by chenhl start
    private float mLastMaxStackHeight;
    //add by chenhl end
    ...
    private int mMaxHeight;//add by chenhl
    /**
    ...
    public void onGearTouched(ExpandableNotificationRow row, int x, int y) {
        if (mLongPressListener != null) {
            ...
            mLongPressListener.onManagerClicked(row);
    ...
    protected void onDraw(Canvas canvas) {
        //canvas.drawRect(0, mCurrentBounds.top, getWidth(), mCurrentBounds.bottom, mBackgroundPaint);//delete by chenhl
        if (DEBUG) {
            int y = mTopPadding;
    ...
    private void initView(Context context) {
        ...
        mPaddingBetweenElements = 0;//add by chenhl
        ...
        //add by chenhl start
        mMaxHeight= getResources().getDimensionPixelOffset(R.dimen.tcl_max_statuswind_height_h);
        //add by chenhl end
    }
    ...
    private void updateChildren() {
        ...
        //add by chenhl start
        mStackScrollAlgorithm.setHeadsUpShow(mIsShowHeadsUP);
        //add by chenhl end
    ...
    private void setTopPadding(int topPadding, boolean animate) {
        if (mTopPadding != topPadding) {
            ...
            //add by chenhl start
            int contenth=mCollapsedSize*3+mTopPadding;
            int height = getHeight();
            if(isAntiFalsingNeeded()){
                height=mMaxHeight;
            }
            int bottom=height-contenth;
            int top = mTopPadding;

            if(bottom<mBottomPadding||!isAntiFalsingNeeded()){
                bottom = mBottomPadding;
            }
            if(!isAntiFalsingNeeded()){
                top = 0;
            }
            if(getPaddingBottom()!=bottom){
                setPadding(0, getPaddingTop(), 0, bottom);
            }
            //add by chenhl end
            requestChildrenUpdate();
     ...
     */
    public void setStackHeight(float height,float maxHeigt) {
        mLastSetStackHeight = height;
        mLastMaxStackHeight = maxHeigt;
        ...
        float translationScaleY = (maxHeigt==0)?-1:(height/maxHeigt)-1;
        translationScaleY = Math.min(0,translationScaleY);
        float tanst=translationScaleY*bottom;
        if(trackingHeadsUp){
            tanst=0;
        }
        setTranslationY(tanst);
        mMstDismissView.setTranslationY(tanst);
//        mScrimController.setTranslation(tanst);/// TCL Monster: kth mod scrim alpha instead of making it gone 20161114
        //add by chenhl end
    ...
    private void handleChildDismissed(View v) {
        ...
        if (v instanceof ExpandableNotificationRow) {
            ...
            if (true  == row.getStatusBarNotification().getNotification().extras.getBoolean("spamNotifyClean")) {
                SpamNotifyHandle.getInstance(null).resetCountToShow();
            }
    ...
    public ExpandableView getChildAtPosition(float touchX, float touchY) {
        ...
        //add by chenhl start
        if(rawIntercepter(touchX,touchY)){
            return null;
        }
        //add by chenhl end
        final int count = getChildCount();
    ...
    private boolean onScrollTouch(MotionEvent ev) {
		...
		//modify by chenhl start
        if (ev.getY() < mQsContainer.getBottom()&&(ev.getActionMasked()==MotionEvent.ACTION_DOWN)) {
            return false;
        }
		//modify by chenhl end
        mForcedScroll = null;
        ...
        //add by chenhl start
        if (getChildAtPosition(ev.getX(), ev.getY()) == null
    ...
    private int getScrollRange() {
        ...
        int scrollRange=Math.max(0,contentHeight-bottom);
        //modify by chenhl end
        return scrollRange;
    }
    ...
    public void updateTopPadding(float qsHeight, boolean animate,
        ...
        if (stackHeight <= minStackHeight) {
            ...
            //start = getHeight() - stackHeight; //delete by chenhl
        ...
        setTopPadding(ignoreIntrinsicPadding ? (int) start : clampPadding((int) start),
                animate);
        setStackHeight(mLastSetStackHeight,mLastMaxStackHeight);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean intercept=rawIntercepter(ev.getX(),ev.getY())
                &&(ev.getActionMasked()==MotionEvent.ACTION_DOWN);//add by chenhl
        return intercept||swipeWantsIt || scrollWantsIt || expandWantsIt || super.onInterceptTouchEvent(ev);
    }
    ...
    public void updateDismissView(boolean visible) {
        //add by chenhl start
        boolean trackingHeadsUp = mTrackingHeadsUp || mHeadsUpManager.hasPinnedHeadsUp();
        mShowDismiss = visible;
        mMstDismissView.setVisibility(!trackingHeadsUp&&visible?View.VISIBLE:View.GONE);
        //add by chenhl end
        int oldVisibility = mDismissView.willBeGone() ? GONE : mDismissView.getVisibility();
        int newVisibility = GONE;//visible ? VISIBLE : GONE;//modify by chenhl
        if (oldVisibility != newVisibility) {
            if (newVisibility != GONE) {
    ...
    public boolean isBelowLastNotification(float touchX, float touchY) {
       return ! isIsInShadowNotificationArea(touchY);//modify by chenhl
        /*int childCount = getChildCount();
        for (int i = childCount - 1; i >= 0; i--) {
            ExpandableView child = (ExpandableView) getChildAt(i);
            if (child.getVisibility() != View.GONE) {
    ...
    private class NotificationSwipeHelper extends SwipeHelper {
            ...
            if (mCurrIconRow != null) {
                ...
                if (locationChange) {
                    ...
                    if (!mHandler.hasCallbacks(mCheckForDrag)) {
                        ...
                        //mCurrIconRow.setIconLocation(translation > 0 /* onLeft */); //delete by chenhl
                    }
                }
				//modify by chenhl 
        ...
        public boolean handleUpEvent(MotionEvent ev, View animView, float velocity,
            ...
            //add by chenhl start
            if(!mCurrIconRow.isIconOnLeft()){
                if(swipedEnoughToShowGear(animView)){
        ...
        private boolean swipedEnoughToShowGear(View animView) {
		     //modify by chenhl
            if (mTranslatingParentView == null || mHeadsUpManager.hasPinnedHeadsUp()) {
                return false;
            }
            ...
            //modify by chenhl
            return (translation < -snapBackThreshold);
            ...
            public void run() {
                ...
                if ((mCurrIconRow != null && (!mCurrIconRow.isVisible()
                    ...
                    /*mCurrIconRow.fadeInSettings(translation > 0 *//* fromLeft *//*, translation,
                            notiThreshold);*/ //delete by chenhl
                }
            }
        }
    ...
    private View mMstDismissView;
    private boolean mShowDismiss=false;
    public void setHeadsUpShow(boolean is){
        if(mIsShowHeadsUP==is){
            return;
        }
        mIsShowHeadsUP = is;
        if (mPhoneStatusBar.getBarState() == StatusBarState.KEYGUARD) {
        ...
        setBackgroundColor(is?Color.TRANSPARENT:getResources().getColor(R.color.tcl_notification_bg));
    }

    public void setMstBottomPadding(int padding){
        if(!isAntiFalsingNeeded()){
    ...
    private boolean isIsInShadowNotificationArea(float touchY){
        float bottom = (getBottom()-getPaddingBottom()+getTranslationY());
        return touchY>mTopPadding&&touchY<bottom;
    }

    public void setMstDismissView(View dismissView) {
        mMstDismissView = dismissView;
    }

    public boolean getmEmptyShadeView(){
        if(mEmptyShadeView==null){
            return false;
        }
        return mEmptyShadeView.getVisibility()==View.VISIBLE;
    }

    public boolean rawIntercepter(float touchX, float touchY){
        int bottomBt = getBottom()-getPaddingBottom();
        boolean trackingHeadsUp = mTrackingHeadsUp || mHeadsUpManager.hasPinnedHeadsUp();
        if((touchY>bottomBt || touchY<getTopPadding())&&!trackingHeadsUp){
            return true;
        }
        return false;
    }
