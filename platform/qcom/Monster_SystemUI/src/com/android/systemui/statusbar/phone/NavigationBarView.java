...
public class NavigationBarView extends LinearLayout {
    ...
    private void updateCurrentView() {
        ...
        LinearLayout linearLayout = (LinearLayout) mCurrentView.findViewById(R.id.ends_group);
        int backButtonID = linearLayout.findViewById(R.id.back).getId();
        int recentButtonID = linearLayout.findViewById(R.id.recent_apps).getId();
        int num = linearLayout.getChildCount();
        int x=0,y=0;
        for (int i = 0; i < num; i++) {
            if(linearLayout.getChildAt(i).getId() == backButtonID) {
                x++;
                if(y != 0) {
                    y++;
                }
            }
            if(linearLayout.getChildAt(i).getId() == recentButtonID) {
                y++;
                if(x != 0) {
                    x++;
                }
        ...
        //if(mSoftKeyMode) {
            if(rot == 0||rot == 2) {
                if((softkeyold == 0 && (x<y)) || (softkeyold == 1 && (x>y))) {
                    swapChildrenOrderIfVertical(mCurrentView.findViewById(R.id.ends_group));
                }
            }else{
                if((softkeyold == 0 && (x>y)) || (softkeyold == 1 && (x<y))) {
                    swapChildrenOrderIfVertical2(mCurrentView.findViewById(R.id.ends_group));
                }
            }
        //}
        /* MODIFIED-END by liang.zhang1-nb,BUG-2827969*/
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        if (mDockedStackExists||uiCarModeChanged || mConfiguration.densityDpi != newConfig.densityDpi) {
    ...
    */
    ...
    public void setIconsDark(boolean dark) {
        ...
        KeyButtonView backButton = (KeyButtonView)getBackButton().getCurrentView();
        KeyButtonView homeButton = (KeyButtonView)getHomeButton().getCurrentView();
        KeyButtonView recnetButton = (KeyButtonView)getRecentsButton().getCurrentView();
        KeyButtonView imeButton = (KeyButtonView)getImeSwitchButton().getCurrentView();

        //getBackButton().setImageDrawable(dark?mBackAltIcon:mBackIcon);
        getBackButton().setImageTint(tint);
        getHomeButton().setImageTint(tint);
        getRecentsButton().setImageTint(tint);
        getImeSwitchButton().setImageTint(tint);
        //modify ripple color
        //KeyButtonRipple backripple= (KeyButtonRipple)backButton.getBackground();
        //backripple.updateRippleColor(tint);
        ...
        recnetButton.setBackground(new KeyButtonRipple(recnetButton.getContext(),recnetButton,tint));
        imeButton.setBackground(new KeyButtonRipple(imeButton.getContext(),imeButton,tint));
        /*KeyButtonRipple homeripple= (KeyButtonRipple)homeButton.getBackground();
        homeripple.updateRippleColor(tint);
        KeyButtonRipple recentripple= (KeyButtonRipple)recnetButton.getBackground();
        recentripple.updateRippleColor(tint);*/
    }
    ...
    //add by chenhl end
	/* MODIFIED-BEGIN by liang.zhang1-nb, 2016-09-29,BUG-2827969*/
    public void setSoftKeyMode(boolean mode) {
        mSoftKeyMode = mode;
    }
    ...
    private void swapChildrenOrderIfVertical(View group) {
        if (group instanceof LinearLayout) {
            ...
            ArrayList<View> mychildList = new ArrayList<>(2);
            int backButtonID = linearLayout.findViewById(R.id.back).getId();
            int recentButtonID = linearLayout.findViewById(R.id.recent_apps).getId();
            int k = 1;
            for (int i = 0; i < childCount; i++) {
                ...
                if(linearLayout.getChildAt(i).getId() == backButtonID) {
                    mychildList.add(linearLayout.getChildAt(i));
                }
                if(linearLayout.getChildAt(i).getId() == recentButtonID) {
                    mychildList.add(linearLayout.getChildAt(i));
                }
            ...
            /*for (int i = childCount - 1; i >= 0; i--) {
                ...
                if(i == 0) linearLayout.addView(childList.get(childCount -1));
            }*/
            for (int i = 0; i < childCount; i++) {
                if(childList.get(i).getId() == backButtonID) {
                    if((k>=0)&&(k<2)) {
                        linearLayout.addView(mychildList.get(k));
                        k--;
                    }
                }else if(childList.get(i).getId() == recentButtonID) {
                    if((k>=0)&&(k<2)) {
                        linearLayout.addView(mychildList.get(k));
                        k--;
                    }
    ...
    private void swapChildrenOrderIfVertical2(View group) {
        if(group instanceof ReverseLinearLayout) {
            ReverseLinearLayout linearLayout = (ReverseLinearLayout) group;
            int childCount = linearLayout.getChildCount();
            ArrayList<View> childList = new ArrayList<>(childCount);
            ArrayList<View> mychildList = new ArrayList<>(2);
            int backButtonID = linearLayout.findViewById(R.id.back).getId();
            ...
            for (int i = 0; i < childCount; i++) {
                if(childList.get(i).getId() == backButtonID) {
                    if((k>=0)&&(k<2)) {
                        linearLayout.tsAddView(mychildList.get(k));
                        k--;
                    }
                }else if(childList.get(i).getId() == recentButtonID) {
                    if((k>=0)&&(k<2)) {
                    ...
                    linearLayout.tsAddView(childList.get(i));
                }
            }
        }
    }
