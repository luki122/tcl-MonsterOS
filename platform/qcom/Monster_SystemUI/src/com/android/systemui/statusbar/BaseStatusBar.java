...
import com.android.systemui.tcl.NotificationBackend;
import com.android.systemui.tcl.NotificationListener;
...
import com.cmx.cmplus.SmartContainerManagerNative;
            ...
            if (Intent.ACTION_USER_SWITCHED.equals(action)) {
                ...
                if (recentTask != null && recentTask.size() > 0) {
                    ...
                    if (user != null && user.isManagedProfile()) {
                        //patch clone app delete begin
//                        Toast toast = Toast.makeText(mContext,
//                                R.string.managed_profile_foreground_toast,
//                                Toast.LENGTH_SHORT);
                        //patch clone app delete end
                        //patch clone app add begin
                        /* MODIFIED-BEGIN by Wang Xiongke, 2016-09-28,BUG-3005175*/
                        //SmartContainer modified begin
                        int resId_toast = R.string.managed_profile_foreground_toast;
                        ContainerInfo ci = null;
                        if (!SmartContainerConfig.WITH_OUT_VIRTUAL_BOX) {
                            try {
                                ci = SmartContainerManagerNative.get().getContainerInfo(user.id);
                                if (ci != null && ci.isVirtualBox()) {
                                    resId_toast = R.string.clone_box_foreground_toast;
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                        Toast toast = Toast.makeText(mContext, resId_toast,
                                Toast.LENGTH_SHORT);
                        //patch clone app add end
                        ...
                        //patch clone app add begin
                        if (!SmartContainerConfig.WITH_OUT_VIRTUAL_BOX && ci != null) {
                            if (ci != null) {
                                try {
                                    Drawable draw = new BitmapDrawable(SmartContainerManagerNative.get().getIcon(ci.id));
                                    text.setCompoundDrawablesRelativeWithIntrinsicBounds(
                                            draw, null, null, null);
                                } catch (Exception e) {
                                    e.printStackTrace();
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (isUpdate) {
                            ...
                            //add by zhciang.liu begin
                            calculateNotifyCount(sbn);
    ...
    //统计应用总通知数
    private void calculateNotifyCount(StatusBarNotification sbn) {
        //只统计动态通知
        if(sbn.isOngoing()) return;
        new Thread(new Runnable() {
            @Override
            public void run() {
                boolean notifyInClean = SpamNotifyHandle.getInstance(mContext).isNotifyInCleaned(sbn);
                if (false == notifyInClean) {
                    Utils.saveNotifyCount(mContext, sbn.getPackageName(), true);
                }
            }
        }).start();
    }
    ...
    /**Mst: tangjun add for begin*/
    private void setAppOpsMode() {
    	try {
        	PackageManager pm = mContext.getPackageManager();
            ApplicationInfo ai = pm.getApplicationInfo("com.monster.appmanager", PackageManager.GET_ACTIVITIES);
            int appmanagerUid = ai.uid;
            ai = pm.getApplicationInfo("com.monster.netmanage", PackageManager.GET_ACTIVITIES);
            int netmanagerUid = ai.uid;
            AppOpsManager  mAppOpsManager = (AppOpsManager) mContext.getSystemService(Context.APP_OPS_SERVICE);
            mAppOpsManager.setMode(AppOpsManager.OP_GET_USAGE_STATS, appmanagerUid,
            		"com.monster.appmanager", AppOpsManager.MODE_ALLOWED);
            mAppOpsManager.setMode(AppOpsManager.OP_GET_USAGE_STATS, netmanagerUid,
            		"com.monster.netmanage", AppOpsManager.MODE_ALLOWED);
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }
    }
    /**Mst: tangjun add for end*/
    ...
    public void start() {
        ...
        SpamNotifyHandle.getInstance(mContext);
    ...
    private void startAppNotificationSettingsActivity(String packageName, final int appUid) {
        ...
        final Intent intent = new Intent("android.intent.action.SHOW_APP_NOTIFY_SETTING_ACTIVITY");
        //modify by chenhl end
        intent.putExtra(Settings.EXTRA_APP_PACKAGE, packageName);
        intent.putExtra(Settings.EXTRA_APP_UID, appUid);
        intent.putExtra("caller",mContext.getPackageName());
    ...
    protected SwipeHelper.LongPressListener getNotificationLongClicker() {
        return new SwipeHelper.LongPressListener() {
            ...
            public boolean onLongPress(View v, final int x, final int y) {
                ...
                startAppDetailActivity(pkg, appUid);
            ...
            @Override
            public void onManagerClicked(View v) {
                             if (!(v instanceof ExpandableNotificationRow)) {
                    return ;
                }
                if (v.getWindowToken() == null) {
                ...
                //modify by chenhl start for longpress
                final StatusBarNotification sbn = row.getStatusBarNotification();
                PackageManager pmUser = getPackageManagerForUser(mContext,
                        sbn.getUser().getIdentifier());
                String pkg = sbn.getPackageName();
    ...
    protected void updateRowStates() {
        ...
        //mStackScroller.changeViewPosition(mDismissView, mStackScroller.getChildCount() - 1); //delete by chenhl
    ...
    public boolean shouldShowOnKeyguard(StatusBarNotification sbn) {
        ...
        PackageInfo info = findPackageInfo(pm, sbn.getPackageName(), sbn.getUid());
        if (info != null) {
            NotificationBackend.AppRow appRow = mBackend.loadAppRow(mContext, pm, info.applicationInfo);
            if (false == appRow.lockscreennotify) {
                return false;
            }
        }
        // end add by zhicang.liu
        return mShowLockscreenNotifications /*&& !mNotificationData.isAmbient(sbn.getKey())*/;//delete by chenhl
    }
    ...
    //add by chenhl start for start app details
    private void startAppDetailActivity(String packageName, final int appUid) {
