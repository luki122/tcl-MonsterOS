...
public class StatusBarIconController extends StatusBarIconList implements Tunable {
    ...
    private SignalClusterView mSignalCluster, mKeyguardSignalCluster;/// kth mod for keyguard inversion 20161224
    ...
    private TextView mBatteryPercent, mKeyguardBatteryPercent; //add by chenhl

    private int mIconSize;
    private int mIconHPadding;

    private int mIconTint = DEFAULT_ICON_TINT;
    /// kth mod for keyguard inversion 20161224 start
    private int mKeyguardIconTint = DEFAULT_ICON_TINT;
    private float mKeyguardDarkIntensity;
    /// kth mod for keyguard inversion 20161224 end
    ...
    public StatusBarIconController(Context context, View statusBar, View keyguardStatusBar,
        ...
        mKeyguardSignalCluster = (SignalClusterView) keyguardStatusBar.findViewById(R.id.signal_cluster);/// kth mod for keyguard inversion 20161224
        ...
        mClock = mNotificationIconAreaController.getClock();
        ...
        //add by chenhl start
        mBatteryPercent = (TextView) statusBar.findViewById(R.id.battery_percent);
        mBatteryMeterView.setPercentView(mBatteryPercent);
        //add by chenhl end
        mKeyguardBatteryPercent = (TextView) keyguardStatusBar.findViewById(R.id.battery_percent);/// kth mod for keyguard inversion 20161224
    ...
    public void setKgSignalCluster(SignalClusterView signalCluster) {
        mKeyguardSignalCluster = signalCluster;
    }
    ...
    private void applyIconTint() {
        ...
        //add by chenhl start
        mBatteryPercent.setTextColor(getTint(mTintArea,mBatteryPercent,mIconTint));
        //add by chenhl end
    }

    /// TCL Monster: kth add for change keyguard inversion color according lockscreen wallpaper 20161014 start @{
    public void applyKeyguardIconTint() {
        Log.d("0917", "applyKeyguardIconTint: mStatusIcons.getChildCount()=" + mStatusIcons.getChildCount());
        for (int i = 0; i < mStatusIcons.getChildCount(); i++) {
            StatusBarIconView v_keyguard = (StatusBarIconView) mStatusIconsKeyguard.getChildAt(i);
            v_keyguard.setImageTintList(ColorStateList.valueOf(getTint(mTintArea, v_keyguard, mKeyguardIconTint)));
        }
        mKeyguardSignalCluster.setIconTint(mKeyguardIconTint, mKeyguardDarkIntensity, mTintArea);
        mBatteryMeterViewKeyguard.setDarkIntensity(
                isInArea(mTintArea, mBatteryMeterViewKeyguard) ? mKeyguardDarkIntensity : 0);
        mKeyguardBatteryPercent.setTextColor(mKeyguardIconTint);
    }

    private void setKeyguardIconTintInternal(float darkIntensity) {
        mKeyguardDarkIntensity = darkIntensity;
        mKeyguardIconTint = (int) ArgbEvaluator.getInstance().evaluate(darkIntensity,
                mLightModeIconColorSingleTone, mDarkModeIconColorSingleTone);
        applyKeyguardIconTint();
    }

    public void setKeyguardIconsDark(boolean dark) {
        setKeyguardIconTintInternal(dark ? 1.0f : 0.0f);
    }
    /// end @}
