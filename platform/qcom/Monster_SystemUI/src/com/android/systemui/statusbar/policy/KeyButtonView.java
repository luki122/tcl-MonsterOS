...
public class KeyButtonView extends ImageView {
    ...
    private boolean interceptKeyEvent(MotionEvent event) {
        if (mCode != KeyEvent.KEYCODE_HOME && mCode != KeyEvent.KEYCODE_BACK) {
            return false;
        }
        ...
        if (dockSide == WindowManager.DOCKED_INVALID) {
            ...
            if (mCode == KeyEvent.KEYCODE_HOME) {
                if (SystemServicesProxy.getInstance(mContext).isDockStackFocused()) {
                    //焦点在DockStack上，不拦截事件
                    ...
                    if (event.getAction() == MotionEvent.ACTION_CANCEL || event.getAction() == MotionEvent.ACTION_UP) {
                        if (SystemServicesProxy.getInstance(mContext).isRecentsActivityVisible()) {
                            EventBus.getDefault().send(new UndockingTaskEvent());
                        } else {
            ...
            } else if (mCode == KeyEvent.KEYCODE_BACK) {
                if (SystemServicesProxy.getInstance(mContext).isRecentsActivityVisible() && !SystemServicesProxy.getInstance(mContext).isDockStackFocused()) {
                    //RecentsActivity可见并且焦点不在dockstack上
                    if (event.getAction() == MotionEvent.ACTION_CANCEL || event.getAction() == MotionEvent.ACTION_UP) {
                        EventBus.getDefault().send(new UndockingTaskEvent());
                    }
                    return true;
