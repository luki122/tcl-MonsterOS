...
public class StackScrollAlgorithm {
    ...
    public void getStackScrollState(AmbientState ambientState, StackScrollState resultState) {
        ...
        updatePositionsForState2(resultState, algorithmState, ambientState);//modify by chenhl
    ...
    //add by chenhl start for modify notification position
    private void updatePositionsForState2(StackScrollState resultState,
            StackScrollAlgorithmState algorithmState, AmbientState ambientState) {
     ...
     // The starting position of the bottom stack peek
        ...
        //Log.d("chenhl","mIsExpanded:"+mIsExpanded);
        for (int i = 0; i < childCount; i++) {
            ExpandableView child = algorithmState.visibleChildren.get(i);
            StackViewState childViewState = resultState.getViewStateForView(child);
            childViewState.location = StackViewState.LOCATION_MAIN_AREA;
            ...
            childViewState.yTranslation = currentYPosition;
            currentYPosition = childViewState.yTranslation + childHeight + paddingAfterChild; 
            childViewState.yTranslation += ambientState.getTopPadding()- algorithmState.scrollY;
            if (currentYPosition <= 0) {
                childViewState.location = StackViewState.LOCATION_HIDDEN_TOP;
            }
            childViewState.height = childHeight;
            if(mIsHeadsUpShow){
                childViewState.alpha=0;
            }else{
                childViewState.alpha=1;
            }
        }
    }
    //add by chenhl end
    ...
    private void updateHeadsUpStates(StackScrollState resultState,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //add by chenhl start
            childState.alpha=1;
            childState.yTranslation+=ambientState.getStackTranslation();
            //add by chenhl end
    ...
    //add by chenhl start
    private boolean mIsHeadsUpShow;
