...
public class LockIcon extends KeyguardAffordanceView {
    ...
    public void update(boolean force) {
        ...
        if (state != mLastState || mDeviceInteractive != mLastDeviceInteractive
            ...
            boolean isAnim = true;
            if (DEBUG) Log.d("ktt", "LockIcon update() mLastState=" + mLastState + " state=" + state + " mLastDeviceInteractive="
                + mLastDeviceInteractive + " mDeviceInteractive=" + mDeviceInteractive + " mLastScreenOn=" 
                + mLastScreenOn + " mScreenOn=" + mScreenOn);
            ///kth modify for removing fingerprint icon in Keyguard, replace by arrow icon 20160810 start @{
            /*old: int iconRes = getAnimationResForTransition(mLastState, state, mLastDeviceInteractive,
    ...
    private int getIconForState(int state, boolean screenOn, boolean deviceInteractive) {
        ///TCL Monster: kth modify for removing fingerprint icon in Keyguard, replace by arrow icon 20160810 start @{					
        /* switch (state) {
        ...
        return R.drawable.ic_lock_24dp_custom;
        /// end@}
    }
