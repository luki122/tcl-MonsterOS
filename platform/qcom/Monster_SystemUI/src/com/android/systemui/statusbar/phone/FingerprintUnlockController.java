...
public class FingerprintUnlockController extends KeyguardUpdateMonitorCallback {
    ...
    public void onFingerprintAuthenticated(int userId) {
        ...
        mMode = calculateMode();
        if (DEBUG_FP)Log.d(TAG, "fp Authenticated: calculateMode=" + mMode);// TCL Monster: kth add for fp debug 20161130
    ...
    public void onFingerprintAuthFailed() {
        //TCL Monster: kth add for jump to bouncer when finger not allowed 20161129 start @{
        if (!mUpdateMonitor.isUnlockingWithFingerprintAllowed()) {
            Log.d(TAG, "Unlocking With Fingerprint not Allowed, and Fingerprint AuthFailed: animateCollapsePanels");
            animateCollapsPanel();
        }///end @}
        cleanup();
    }

    private void animateCollapsPanel(){
        //TCL Monster: kth add for jump to bouncer when finger too many attempts 20161122 start
        boolean wasDeviceInteractive = mUpdateMonitor.isDeviceInteractive();
        if (!wasDeviceInteractive) {
            if (DEBUG_FP_WAKELOCK) {
                Log.i(TAG, "fp wakelock: Authenticated, waking up...");
        ...
        //TCL Monster: kth add for jump to bouncer when finger too many attempts 20161122 end
    }
    ...
    ///TCL Monster: kth add for NB 20161027 start
    /* MODIFIED-BEGIN by jianguang.sun, 2016-10-12,BUG-2987595*/
    @Override
    public void onFingerprintIsAlarm() {
        cleanup();
    }
    /* MODIFIED-END by jianguang.sun,BUG-2987595*/
    ///TCL Monster: kth add for NB 20161027 end
