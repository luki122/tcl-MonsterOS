...
public abstract class PanelView extends FrameLayout {
    ...
    private void endMotionEvent(MotionEvent event, float x, float y, boolean forceCancel) {
        mTrackingPointer = -1;
        //add by chenhl start
        boolean ismove= mExpandedHeight>0&&mExpandedHeight!=getMaxPanelHeight();
        //add by chenhl end
    ...
    protected boolean flingExpands(float vel, float vectorVel, float x, float y) {
        //add by chenhl start for headsup pull
        if(isExpandingFromHeadsUp()){
            return false;
        }
        //add by chenhl end
        if (isFalseTouch(x, y)) {
     ...
     */
    public boolean isFalseTouch(float x, float y) {
    ...
    protected void fling(float vel, boolean expand, float collapseSpeedUpFactor,
        ...
        flingToHeight(vel, expand, target, collapseSpeedUpFactor, expandBecauseOfFalsing, null);
        /**Mst: tangjun mod end*/
    }
    
    /**Mst: tangjun add begin*/
    protected void fling(float vel, boolean expand, boolean expandBecauseOfFalsing, AnimFinishListener animFinishListener) {
        fling(vel, expand, 1.0f /* collapseSpeedUpFactor */, expandBecauseOfFalsing, animFinishListener);
    }

    protected void fling(float vel, boolean expand, float collapseSpeedUpFactor,
            boolean expandBecauseOfFalsing, AnimFinishListener animFinishListener) {
        cancelPeek();
        float target = expand ? getMaxPanelHeight() : 0.0f;
        if (!expand) {
            mClosing = true;
        }
        flingToHeight(vel, expand, target, collapseSpeedUpFactor, expandBecauseOfFalsing, animFinishListener);
    }
    /**Mst: tangjun add end*/
   ...
   /**Mst: tangjun mod end*/
        ...
        if (clearAllExpandHack) {
            target = getMaxPanelHeight() /*- getClearAllHeight()*/; //modify by chenhl
        }
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationCancel(Animator animation) {
                ...
                /**Mst: tangjun mod begin*/
                if(expand && animFinishListener != null) {
                	animFinishListener.playFinishLister();
                }
                /**Mst: tangjun mod end*/
            }
