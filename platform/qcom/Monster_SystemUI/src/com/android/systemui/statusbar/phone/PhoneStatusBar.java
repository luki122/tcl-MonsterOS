...
import com.android.systemui.statusbar.phone.LockscreenWallpaper.LockscreenWallpaperListener;
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    ///TCL Monster: kth add for keyguard inversion 20161026 start @{
    private static final float MASK_DROPBACK_ALPHA_DEFAULT = 0.3f;
    private static final long MASK_NORMAL_ANIMATION_DURATION = 200;
    ///TCL Monster: kth add for keyguard inversion 20161026 end @}
    ...
    private boolean mSoftKeyMode = false; // MODIFIED by liang.zhang1-nb, 2016-09-29,BUG-2827969
    ...
    private Chronometer mCallStatusTime;
    private TextView mCallStatusText;
    //add by chenhl end

    ///TCL Monster: kth add for keyguard inversion 20161022 start
    private LockscreenWallpaperListener lockscreenWpListener;
        ...
        public void onChange(boolean selfChange) {
            ...
            if (userSetup != mUserSetup) {
                ...
                if (mNotificationPanel != null) {
                    mNotificationPanel.setUserSetupComplete(mUserSetup);// TCL Monster: kth add for update camera visible notify setup complete or not 20161124
                }
                if (mLockscreenWallpaper != null) {
                    mLockscreenWallpaper.setUserSetupComplete(mUserSetup);// TCL Monster: kth add for update lockscreen wallpaper notify setup completed 20161124
                }
            }
            if (mIconPolicy != null) {
    ...
    private ImageView mBackdropFront, mBackdropBack, mBackdropMask;///TCL Monster: kth add for keyguard inversion 20161022
    ...
    public void start() {
        ...
        //add by chenhl start
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor("tcl_sys_hide_navigationbar"), false, mHideNavigationbarObserver);
        //add by chenhl end
    }
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        try {
            ...
            //if (showNav) {
            if (showNav&&!shoudHideNavigationbar()) {
        ...
        mBackdropMask = (ImageView) mBackdrop.findViewById(R.id.backdrop_mask);
        ...
        mBatteryController.addStateChangedCallback(new BatteryStateChangeCallback() {
            ...
            public void onPowerSaveChanged(boolean isPowerSave) {
                ...
                //add by chenhl start
                if(mLightStatusBarController!=null){
                    //mLightStatusBarController.updateByPowerSaveChange(isPowerSave);
                }
                //add by chenhl end
            }
            @Override
        ...
        mNetworkController.setUserSetupComplete(mUserSetup);
        /// TCL Monster: kth add for update Keyguard CarrierText 20161017 start
        /*        if(mNetworkController != null) {
            mNetworkController.setKgStatusBarView(mKeyguardStatusBar);
        }*/
        /// TCL Monster: kth add for update Keyguard CarrierText 20161017 end
        ...
        filter.addAction("android.intent.action.softkey_change"); // MODIFIED by liang.zhang1-nb, 2016-09-29,BUG-2827969
        //add by chenhl start
        filter.addAction("monster.intent.action.show.callstatus");
        //add by chenhl end
        //add by liuzhicang begin
        filter.addAction("action.launcher_dock_change");
        //add by liuzhicang end
        ...
        /**Mst: tangjun add for launch camera begin*/
        mNotificationPanel.setKeyguardBottomAreaView(mKeyguardBottomArea);
        /**Mst: tangjun add launch camera end*/
        ...
        //add by chenhl start for call statusbar
        mCallStatusBar = mStatusBarView.findViewById(R.id.id_tcl_call_statusbar);
        mCallStatusTime =(Chronometer)mStatusBarView.findViewById(R.id.id_tcl_call_status_time);
        mCallStatusText =(TextView) mStatusBarView.findViewById(R.id.id_tcl_call_status_info);
        //add by chenhl end

        ///TCL Monster: kth add for keyguard inversion 20161022 start
        mKeyguardStatusBar.setPhoneStatusBar(this);
        mBackdropMask.setImageDrawable(new ColorDrawable(Color.parseColor("#000000")));
        ///TCL Monster: kth add for keyguard inversion 20161022 end
    ...
    private void inflateSignalClusters() {
        ...
        //TCL Monster: kth mod for keyguard signal cluster inversion 20161214 start
        SignalClusterView keyguardSignalClusterView = reinflateSignalCluster(mKeyguardStatusBar);
        mIconController.setKgSignalCluster(keyguardSignalClusterView);
        //TCL Monster: kth mod for keyguard signal cluster inversion 20161214 end
    }
    ...
    private void inflateDismissView() {
        ...
        //add by chenhl start
        View tclDisMissView=mStatusBarWindow.findViewById(R.id.id_tcl_notification_dismiss);
        tclDisMissView.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v) {
                MetricsLogger.action(mContext, MetricsEvent.ACTION_DISMISS_ALL_NOTES);
                clearAllNotifications();
        ...
        mStackScroller.setMstDismissView(tclDisMissView);
        //add by chenhl end
    }
    ...
    private void performDismissAllAnimations(ArrayList<View> hideAnimatedList) {
        ...
        //modify by chenhl start for animation from top to down
        ...
        //modify by chenhl end for animation from top to down
    }
    ...
    protected void startKeyguard() {
        ...
        /**Mst: tangjun add for KeyguardBouncer show anim begin*/
        mNotificationPanel.setStatusBarKeyguardViewManager(
                mStatusBarKeyguardViewManager);
        /**Mst: tangjun add for KeyguardBouncer show anim begin*/

        /**Mst: tangjun add for KeyguardBouncer hide begin*/
        mStatusBarWindow.setStatusBarKeyguardViewManager(
                mStatusBarKeyguardViewManager);
//        mStatusBarKeyguardViewManager.setPanelView(mNotificationPanel);/// TCL Monster: kth add for notificationpanelview fling action when bouncer showing 20160926
        /**Mst: tangjun add for KeyguardBouncer hide end*/
    }
    ...
    //add by liuzhicang begin
    public final void onBusEvent(SplitScreenModeEvent event) {
        mContext.sendBroadcast(new Intent(BaseActivity.ACTION_FINISH_ACTIVITY));
        if (event.showRecents) {
            showRecents(false, false);
        } else {
            toggleRecentApps();
        }
    }
    ...
    protected void toggleSplitScreenMode(int metricsDockAction, int metricsUndockAction) {
        ...
        //add by liuzhicang begin
        //systemui相关界面下不进入分屏
        if(SystemServicesProxy.getInstance(mContext).isSystemUiActivityVisible()){
    ...
    public void addNotification(StatusBarNotification notification, RankingMap ranking,
        ...
        boolean isHeadsUped = shouldPeek(shadeEntry)&&!mExpandedVisible;//modify by chenhl for headsup show
        if (isHeadsUped) {
    ...
    private void updateNotificationShade() {
        ...
        for (int i = 0; i < mStackScroller.getChildCount(); i++) {
            ...
            j++;
            //add by chenhl start
            targetChild.updateExpandButton(mState == StatusBarState.KEYGUARD);
            //add by chenhl end
    ...
    protected void updateNotifications() {
        ...
        //updateCallStatusbar2();//add by chenhl
    }
    ...
    public void updateMediaMetaData(boolean metaDataChanged, boolean allowEnterAnimation) {
                ...
                if (mFingerprintUnlockController.getMode()
                    ...
                    if (mKeyguardFadingAway) {
                        mBackdrop.animate()
                                ...
                                // behind.
                                .setDuration(mKeyguardFadingAwayDuration / 4)// kth mod for UX performance 20161121
        ...
        notifyKgBackgroundChanged();///TCL Monster: kth add for keyguard inversion 20161022
    }

    ///TCL Monster: kth add for keyguard inversion 20161022 start
    public void notifyKgBackgroundChanged() {
        if (mStatusBarKeyguardViewManager != null && mStatusBarKeyguardViewManager.isBouncerShowing()) {
            if (DEBUG) Log.d(TAG, "notifyKgBackgroundChanged: bouncer is showing");
            return;
        }
        //TCL Monster: kth add for update lockscreen wallpaper notify setup completed 20161124 start
        if (mLockscreenWallpaper == null) {
            return;
        }
        Bitmap lockScreenBitmap = mLockscreenWallpaper.getBitmap();
        boolean isDark;
        boolean showMask;
        if (lockScreenBitmap == null || mBackdropBack.getDrawable() == null) {
            if (DEBUG)
                Log.w(TAG, "notifyKgBackgroundChanged: Keyguard inversion failed as lockScreenBitmap or mBackdropBack.getDrawable is null");
            showMask = false;
            isDark = true;
        } else if (mStackScroller.getNotGoneChildCount() != 0 && !KeyguardUpdateMonitor.getInstance(mContext).isSimPinSecure()) {
            showMask = true;
            isDark = false;
        } else {
            showMask = false;
            isDark = MstMainColorHelper.isTextColorDark(lockScreenBitmap, mContext);
        }
        if (DEBUG)
            Log.d(TAG, "notifyKgBackgroundChanged: showMask=" + showMask + " isDark=" + isDark + " Notification getNotGoneChildCount=" + mStackScroller.getNotGoneChildCount());
        int textRGB = isDark ? Color.BLACK : Color.WHITE;
        if (showMask) {
            animatorBackdropMask(mBackdropMask, MASK_DROPBACK_ALPHA_DEFAULT, true, 300, Interpolators.ALPHA_IN);
        } else {
            animatorBackdropMask(mBackdropMask, 0, true, 300, Interpolators.ALPHA_OUT);
        }

        // first boot? some view may not be inited when Boot Wizard
        mKeyguardStatusView.onWallpaperChanged(textRGB);
        mKeyguardBottomArea.onWallpaperChanged(textRGB);
        if (mIconController != null) {
            mIconController.setKeyguardIconsDark(isDark);
        }
        if (mStatusBarKeyguardViewManager != null) {
            mStatusBarKeyguardViewManager.getBouncer().onWallpaperChanged(textRGB, isDark ? .8f : 1f);
        }
        if (lockscreenWpListener != null) {
            lockscreenWpListener.notifyWallpaperChanged(textRGB, isDark ? .6f : 1f);
        }
        //TCL Monster: kth add for update lockscreen wallpaper notify setup completed 20161124 end
    }

    private void animatorBackdropMask(final ImageView img, float alpha, boolean animate, long duration, Interpolator interpolator) {
        float endAlpha = alpha;
        if (!animate) {
            img.setAlpha(endAlpha);
        } else {
            float currentAlpha = img.getAlpha();
            ValueAnimator animator = ValueAnimator.ofFloat(currentAlpha, endAlpha);
            ...
            if (duration == -1) {
                float durationFactor = Math.abs(currentAlpha - endAlpha);
                durationFactor = Math.min(1.0f, durationFactor);
                duration = (long) (MASK_NORMAL_ANIMATION_DURATION * durationFactor);
            }
    ...
    //add by chenhl start
    private final String HIDE_NAVIGATIONBAR = "systemui_hide_navigatonbar";
    private void hideNavigationbar(){
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if(hide==0) {
                    showHideConfirem();
                    Settings.System.putInt(mContext.getContentResolver(),HIDE_NAVIGATIONBAR,1);
                }
            }
        },200);
    }
    ...
    private void showHideConfirem(){

        AlertDialog.Builder builder = new AlertDialog.Builder(mContext)
                .setTitle(R.string.tcl_hide_navigastionbar_tile)
                .setMessage(R.string.tcl_hide_navigastionbar_message)
                .setPositiveButton(R.string.dialog_btn_positive,null);
    ...
    public void setWindowState(int window, int state) {
        ...
        //add by chenhl start
        if(window == StatusBarManager.WINDOW_NAVIGATION_BAR&&state>3){
            if(state==4){
                mNavigationBarWindowState = StatusBarManager.WINDOW_STATE_SHOWING;
                showNavigationbar();
                Settings.System.putInt(mContext.getContentResolver(),"tcl_sys_hide_navigationbar",0);
            }else if(state==5){
                //mNavigationBarWindowState = StatusBarManager.WINDOW_STATE_HIDDEN;
                //hideNavigationbar();
            }
            return;
        }
    ...
    public void setSystemUiVisibility(int vis, int fullscreenStackVis, int dockedStackVis,
        ...
        if (diff != 0) {
            ...
            if(mNavigationBarView!=null){
                boolean light = (vis & View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR) != 0;
                if(!light){
                    light = (fullscreenStackVis & View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR)!=0;
                }
                mNavigationBarView.setIconsDark2(light);
            }
            //add by chenhl end
    ...
    private void checkBarModes() {
        ...
        if (mNavigationBarView != null) {
            //modify by chenhl
            checkBarMode((mState!=StatusBarState.KEYGUARD&&mExpandedVisible)?MODE_LIGHTS_OUT_TRANSPARENT:mNavigationBarMode,
    ...
    private void checkBarMode(int mode, int windowState, BarTransitions transitions,
        ...
        if (powerSave && getBarState() == StatusBarState.SHADE) {
            //mode = MODE_WARNING; //delete by chenhl
        }
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            /* MODIFIED-BEGIN by liang.zhang1-nb, 2016-09-29,BUG-2827969*/
            else if("android.intent.action.softkey_change".equals(action)) {
                int softkeynew = intent.getIntExtra("softkey",0);
                int softkeyold = Settings.System.getInt(mContext.getContentResolver(),"softkeymode",0);
                if(softkeynew != softkeyold) {
                    mSoftKeyMode = true;
                } else {
                    mSoftKeyMode = false;
                }
                try {
                    Settings.System.putInt(mContext.getContentResolver(),"softkeymode",softkeynew);
                } catch (Exception e) {
                    Log.e(TAG,"set softkeymode error.");
                }
                if (mNavigationBarView == null || !mNavigationBarView.isAttachedToWindow()) return;
                if(mSoftKeyMode) {
            ...
            //add by chenhl start
            else if("monster.intent.action.show.callstatus".equals(action)){
                int show = intent.getIntExtra("show",0);
                long time = intent.getLongExtra("time",0);
                boolean showtime = intent.getBooleanExtra("showTime",false);
                String showText = intent.getStringExtra("callstate");
                updateCallStatusbar3(show==1,time,showtime,showText);
            }
            //add by chenhl end
            //add by liuzhicang begin
            else if ("action.launcher_dock_change".equals(action)){
                //先不用这个方案
                //针对按back键挂后台回到launcher的应用
                //launcher onresume时发出来的广播，防止第三方应用强制回到launcher
                //在分屏创建过程中launcher也会先执行onresume，导致进入分屏时会马上退出来，造成无法进入分屏的现象
                //使用isRecentActivityOnCreating，在recentactivity没有加载出来之前忽略掉该广播
                //int dockSide = WindowManagerProxy.getInstance().getDockSide();
                //if (dockSide != WindowManager.DOCKED_INVALID && !RecentsImpl.isRecentActivityOnCreating) {
    ...
    private void dismissKeyguardThenExecute(OnDismissAction action, Runnable cancelAction,
        ...
        if (mStatusBarKeyguardViewManager.isShowing()) {
//            mNotificationPanel.fling(4000, false);//kth mod swipe up NotificationPanel and show bouncer when Notifications double-click 20160826
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        repositionNavigationBar();
        updateRowStates();
        //mIconController.defineSlots(); //delete by chenhl for two same icon in statusbar
    ...
    public boolean hideKeyguard() {
        mKeyguardGoingAway = false;//add by chenhl
    ...
    protected void updateKeyguardState(boolean goingToFullShade, boolean fromShadeLocked) {
        ...
        /// TCL Monster: kth add for bug, disable CameraImage when Horizontal screen 20160918 start @{
        mNotificationPanel.getCamerImageView().setVisibility(mState != StatusBarState.SHADE ? View.VISIBLE : View.GONE);/// end @}
    ...
    public void updateStackScrollerState(boolean goingToFullShade, boolean fromShadeLocked) {
        ...
        //modify by zhicang.liu begin
        //滑动锁也能隐藏通知内容功能
        //mStackScroller.setHideSensitive(isLockscreenPublicMode(), goingToFullShade);
        mStackScroller.setHideSensitive(mStatusBarKeyguardViewManager.isShowing(), goingToFullShade);
        //modify by zhicang.liu end
        ...
        //add by chenhl start
        if(onKeyguard){
            mStackScroller.setBackgroundColor(Color.TRANSPARENT);
        }else {
            mStackScroller.setBackgroundColor(mContext.getResources().getColor(R.color.tcl_notification_bg));
        }
        //add by chenhl end
    }
...
//            mKeyguardIndicationController.hideTransientIndication();
            mKeyguardIndicationController.setDefaultIndication();// kth mod for always show default indication text 20161108
            mStackScroller.setActivatedChild(null);
        }
    }
    ...
    //TCL Monster: kth translate lockscreen bg 20160912 start
    public ImageView getmBackdropBack() {
        return mBackdropBack;
    }

    public ImageView getmBackdropMask() {
        return mBackdropMask;
    }
    //TCL Monster: kth translate lockscreen bg 20160912 end

    //add by chenhl start
    private void updateCallStatusbar() {
        if (mState == StatusBarState.KEYGUARD || mExpandedVisible || (!mShowCallStatus&&!mShowPay)) {
            mCallStatusBar.setVisibility(View.GONE);
            return;
        }
        mCallStatusBar.setVisibility(View.VISIBLE);
    }

    private float mInitialTouchY;
    private float mInitialTouchX;
    private boolean mIsClick;
    private Notification mShowNotification;
    private boolean mShowCallStatus;
    private boolean mShowTime;
    private String mShowText;
    private long mTime;
    private void updateCallStatusbar2(){
        ArrayList<Entry> activeNotifications=mNotificationData.getActiveNotifications();
        int N = activeNotifications.size();
        ArrayList<Notification> showCall = new ArrayList<Notification>();
        for (int i = 0; i < N; i++) {
            Entry entry=activeNotifications.get(i);
            StatusBarNotification bn=entry.notification;
            Notification notify=bn.getNotification();
            if(notify.showStatusbar){
                showCall.add(notify);
            }
        }
        mShowNotification = null;
        mShowCallStatus=false;
        if(showCall.size()==0){
            mCallStatusBar.setOnTouchListener(null);
        }else {
            for(int i=0;i<showCall.size();i++){
                Notification no=showCall.get(i);
                if(mShowNotification==null){
                    mShowNotification = no;
                    mShowCallStatus = true;
                }else{
                    if(mShowNotification.statusPriority<no.statusPriority){
                        mShowNotification=no;
                    }
                }
            }
            if(mShowNotification.backgroundColor!=0){
                mCallStatusBar.setBackgroundColor(mShowNotification.backgroundColor);
            }
            if(mShowNotification.statusTitle!=null){
                //mCallStatusBar.setText(mShowNotification.statusTitle);
            }
            final ViewConfiguration configuration = ViewConfiguration.get(mCallStatusBar.getContext());
            final float mTouchSlop = configuration.getScaledTouchSlop();
            mCallStatusBar.setOnTouchListener(new View.OnTouchListener() {
                @Override
                public boolean onTouch(View v, MotionEvent event) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        float w=event.getX()-mInitialTouchX;
                        float h=event.getY()-mInitialTouchY;
                        if(Math.abs(h) > mTouchSlop||Math.abs(w) > mTouchSlop){
                            mIsClick=false;
                        }
                        break;
                    case MotionEvent.ACTION_UP:
                        if(mIsClick){
                            if(mShowNotification!=null&&mShowNotification.clickIntent!=null){
                                new Thread() {
                                    @Override
                                    public void run() {
                                        try {
                                        ...
                                        try {
                                            mShowNotification.clickIntent.send();
                                        } catch (PendingIntent.CanceledException e) {
                                            Log.w(TAG, "Sending contentIntent failed: " + e);
                                        }
                                        if (mShowNotification.clickIntent.isActivity()) {
                                            mAssistManager.hideAssist();
                                        }
                                    }
                                }.start();
                    ...
                    return mStatusBarView.onTouchEvent(event);
                }
            });
        }
        updateCallStatusbar();
    ...
    private void changeNavigationBarMode(){
        if(mState==StatusBarState.KEYGUARD){
            return;
        }
        if (mNavigationBarView != null) {
            if(mExpandedVisible){
                mNavigationBarView.getBarTransitions().transitionTo(MODE_LIGHTS_OUT_TRANSPARENT, true);
            }else{
                mNavigationBarView.getBarTransitions().transitionTo(mNavigationBarMode, true);
            }
        }
    }
     ...
     * @param show
     */
    private void updateCallStatusbar3(boolean show,long time,boolean showtime,String showtext){
        mShowCallStatus = show;
        mShowTime = showtime;
        mShowText = showtext;
        mTime = time;
        if(!show){
            mCallStatusBar.setOnTouchListener(null);
            mCallStatusTime.stop();
        }else {
            if(mShowPay){
                return;
            }
            mCallStatusTime.setVisibility(showtime?View.VISIBLE:View.GONE);
            mCallStatusBar.setBackgroundColor(0xff22b4ba);
            if(showtime) {
                mCallStatusTime.setBase(SystemClock.elapsedRealtime() - time);
                mCallStatusTime.start();
            }
            if(showtext!=null){
                mCallStatusText.setText(showtext);
            }
            final ViewConfiguration configuration = ViewConfiguration.get(mCallStatusBar.getContext());
            final float mTouchSlop = configuration.getScaledTouchSlop();
            mCallStatusBar.setOnTouchListener(new View.OnTouchListener() {
                                ...
                                AsyncTask.execute(new Runnable() {
                                    public void run() {
                                        try {
                                            ...
                                            intent.setComponent(new ComponentName("com.android.incallui",
    ...
    public void showPayfor(boolean show,String showText,int color){
        mShowPay = show;
        mCallStatusTime.setVisibility(View.GONE);
        mCallStatusBar.setVisibility(mShowPay?View.VISIBLE:View.GONE);
        mCallStatusBar.setOnTouchListener(null);
        mCallStatusText.setText(showText);
        if(color!=0){
            mCallStatusBar.setBackgroundColor(color);
        }
        if(!mShowPay&&mShowCallStatus){
            updateCallStatusbar3(mShowCallStatus,mTime,mShowTime,mShowText);
        }
    }
    @Override
    public void updatePayState(boolean show,String showText,int color){
        showPayfor(show,showText,color);
    }

    private final ContentObserver mHideNavigationbarObserver = new ContentObserver(mHandler) {
        @Override
        public void onChange(boolean selfChange) {

            if(shoudHideNavigationbar()){
                mNavigationBarWindowState = StatusBarManager.WINDOW_STATE_HIDDEN;
                hideNavigationbar();
            }else{
                mNavigationBarWindowState = StatusBarManager.WINDOW_STATE_SHOWING;
    ...
    private boolean shoudHideNavigationbar(){
        int hide=Settings.System.getInt(mContext.getContentResolver(),"tcl_sys_hide_navigationbar",0);
        return hide==1;
    }
    //add by chenhl end

    // kth add for Keyguard inversion test temp 20161013 start
    public void addLockScreenWpListener(LockscreenWallpaper.LockscreenWallpaperListener lockscreenWpListener){
        this.lockscreenWpListener = lockscreenWpListener;
    }
    // kth add for Keyguard inversion test temp 20161013 end

    // TCL Monster: kth mod for optimizing launch accordance camera 20161207 start
    public void resetWindowFrameX(boolean launched) {
        mNotificationPanel.resetWindowFrameX(launched);
    }
    // TCL Monster: kth mod for optimizing launch accordance camera 20161207 end
