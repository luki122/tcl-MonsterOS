...
import com.android.keyguard.utils.MstMainColorHelper;///TCL Monster: kth mod for sim pin puk inversion 20161117
...
//        mStatusBarKeyguardViewManager.setPanelView(mNotificationPanel);/// TCL Monster: kth add for notificationpanelview fling action when bouncer showing 20160926
    ...
    public void notifyKgBackgroundChanged() {
        ...
        if (lockscreenWpListener != null && mLockscreenWallpaper.getBitmap() != null && mBackdropBack.getDrawable() != null &&  mStatusBarKeyguardViewManager != null) {
            boolean isDark = MstMainColorHelper.isTextColorDark(mLockscreenWallpaper.getBitmap(), mContext);
            ///TCL Monster: kth mod for sim pin puk inversion 20161117 start
            if (DEBUG) Log.d(TAG, "notifyKgBackgroundChanged: " + " isSimPinSecure=" + KeyguardUpdateMonitor.getInstance(mContext).isSimPinSecure());
            if (mStackScroller.getNotGoneChildCount() != 0 && !KeyguardUpdateMonitor.getInstance(mContext).isSimPinSecure()) {
            ///TCL Monster: kth mod for sim pin puk inversion 20161117 end
                ...
                lockscreenWpListener.notifyWallpaperChanged(Color.WHITE, 1f);
                mKeyguardStatusView.onWallpaperChanged(Color.WHITE);
                mStatusBarKeyguardViewManager.getBouncer().onWallpaperChanged(Color.WHITE, 1f);
                mIconController.setKeyguardIconsDark(false);
                mKeyguardBottomArea.onWallpaperChanged(Color.WHITE);
            } else {
                int textRGB = isDark ? Color.BLACK : Color.WHITE;
                animatorBackdropMask(mBackdropMask,0, true, 300, Interpolators.ALPHA_OUT);
                lockscreenWpListener.notifyWallpaperChanged(textRGB, .6f);
                mKeyguardStatusView.onWallpaperChanged(textRGB);
                mStatusBarKeyguardViewManager.getBouncer().onWallpaperChanged(textRGB, .8f);
                mKeyguardBottomArea.onWallpaperChanged(textRGB);
                mIconController.setKeyguardIconsDark(isDark ? true : false);
            }
        }
    }
...
//            mKeyguardIndicationController.hideTransientIndication();
    ...
    private void updateCallStatusbar() {
        if (mState == StatusBarState.KEYGUARD || mExpandedVisible || !mShowCallStatus) {
