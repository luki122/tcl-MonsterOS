...
import com.android.systemui.statusbar.phone.StatusBarWindowView.AnimFinishListener;
...
public class NotificationPanelView extends PanelView implements
    ...
    public static final boolean DEBUG_LOG = KeyguardConstants.DEBUG;// kth add for debug 20160816
    ...
    private static final int INIT_CLOCK_POSITION = 180;//kth add for reset Clock And Notification position: based on marginTop 270px 20160816
    ...
    //add by chenhl start
    private int mKeyguardNotificationPaddingtop;
    //add by chenhl end
    /**
    ...
    private boolean mUserSetupComplete;// TCL Monster: kth add for update camera visible notify setup complete or not 20161124
    ...
    protected void onFinishInflate() {
		...
		/**Mst: tangjun add for camera show begin*/
        cameraImage = (ImageView)findViewById(R.id.cameraimage);
        windowFrame = (FrameLayout)findViewById(R.id.windowframe);
        pagingTouchSlop = ViewConfiguration.get(getContext()).getScaledTouchSlop();
        
    	WindowManager wm = (WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE);
    	windowWidth = wm.getDefaultDisplay().getWidth();
    	windowHeight = wm.getDefaultDisplay().getHeight();
    	
    	cameraImage.setTranslationX(windowWidth);
        /**Mst: tangjun add for camera show end*/
    }
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
    	...
    	//Log.e("101010", "---NotificationPanelView onWindowFocusChanged--");
    }
    /**Mst: tangjun add for camera show end*/
    ...
    protected void loadDimens() {
        ...
        //add by chenhl start
        mKeyguardNotificationPaddingtop=getResources().getDimensionPixelSize(
                R.dimen.tcl_keyguard_toppadding);
        //add by chenhl end
    }
    ...
    private void positionClockAndNotifications() {
        if (DEBUG_LOG) Log.d("ktt", "---------NotificationPanelView positionClockAndNotifications---------");
        //Thread.dumpStack();
        ...
        if (mStatusBarState != StatusBarState.KEYGUARD) {
            ...
            if (animate || mClockAnimator != null) {
                startClockAnimation(INIT_CLOCK_POSITION);//kth mod for reset Clock And Notification position: based on marginTop 270px 20160816
            } else {
                mKeyguardStatusView.setY(INIT_CLOCK_POSITION);
            }
            updateClock(mClockPositionResult.clockAlpha, mClockPositionResult.clockScale);
            //modify by chenhl start
            //stackScrollerPadding = mClockPositionResult.stackScrollerPadding;
            stackScrollerPadding = mKeyguardNotificationPaddingtop;
            //modify by chenhl end
            mTopPaddingAdjustment = mClockPositionResult.stackScrollerPaddingAdjustment;

            ///TCL Monster: kth translate lockscreen bg 20160912 start @{
            if (DEBUG_LOG) Log.d("ktt", "NotificationPanelView positionClockAndNotification"
                    + " getExpandedHeight()=" + getExpandedHeight()
                    + " getMaxPanelHeight()=" + getMaxPanelHeight()
                    + " getHeight()=" + getHeight()
                    + " getKeyguardViewInitTranslationY()=" + mStatusBarKeyguardViewManager.getBouncer().getKeyguardViewInitTranslationY());
            if(!UnlockMethodCache.getInstance(getContext()).canSkipBouncer()) {
                float keyguardViewY = getExpandedHeight() / getMaxPanelHeight()
                        * mStatusBarKeyguardViewManager.getBouncer().getKeyguardViewInitTranslationY();
                if (DEBUG_LOG) Log.d("ktt", "positionClockAndNotifications: keyguardViewY=" + keyguardViewY);
//                mStatusBarKeyguardViewManager.getBouncer().setKeyguardViewTranslationY(keyguardViewY);//bouncer swipe up with Clock and Notification // kth remove it for pm acquire 20161112
            } else {
//                mStatusBar.getmBackdropBack().setY((getExpandedHeight() - getMaxPanelHeight()) / getMaxPanelHeight() * getHeight());
            }
            ///TCL Monster: kth translate lockscreen bg end @}
        }
        mNotificationStackScroller.setIntrinsicPadding(stackScrollerPadding);
        requestScrollerTopPaddingUpdate(animate);
    //Mst: tangjun  translate clock begin
        if(mNotificationStackScroller.getNotGoneChildCount()>0|| Math.abs(mClockPositionAlgorithm.getClockYExpansionAdjustment2())>0) {
            if (DEBUG_LOG) Log.d("ktt", "NotificationPanelView positionClockAndNotification" + " getExpandedHeight()=" + getExpandedHeight() + " mKeyguardStatusView.getY()=" + mKeyguardStatusView.getY());
            mKeyguardStatusView.setY(getExpandedHeight()-getMaxPanelHeight() + INIT_CLOCK_POSITION);
        }
        //Mst: tangjun  translate clock end
    }
    ...
    private void updateClock(float alpha, float scale) {
    	//TCL Monster: kth mod for clock layout 20161026 start @{
...
//        mKeyguardStatusView.setScaleY(scale);
    ...
    public void expandWithQs() {
        if (mQsExpansionEnabled) {
            //mQsExpandImmediate = true; //delete by chenhl
        }
        expand(true /* animate */);
    }
    ...
    */
    ...
    @Override
    protected void flingToHeight(float vel, boolean expand, float target,
            float collapseSpeedUpFactor, boolean expandBecauseOfFalsing, final AnimFinishListener animFinishListener) {
        mHeadsUpTouchHelper.notifyFling(!expand);
        setClosingWithAlphaFadeout(!expand && getFadeoutAlpha() == 1.0f);
        super.flingToHeight(vel, expand, target, collapseSpeedUpFactor, expandBecauseOfFalsing, animFinishListener);
    }
    /**Mst: tangjun mod end*/
    ...
    private float x = 0;
    private float y = 0;
    private int isShowCameraImage;
    private boolean isShowBouncer;
    ...
    public void resetWindowFrameX(final boolean launched) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                Log.d(TAG, "run: resetWindowFrameX as camera affordance launched =" + launched);
                windowFrame.setX(0);
                cameraImage.setX(windowWidth);
                mStatusBar.getmBackdropBack().setY(0f);// kth add for [BUG 3411787] 20161223
            }
        }, 1500);
    }
    ...
    private void startAnimationForShow() {
        ObjectAnimator cameraImageAnim = ObjectAnimator.ofFloat(cameraImage, "x", cameraImage.getX(), 0).setDuration(300);
        ObjectAnimator windowFrameAnim = ObjectAnimator.ofFloat(windowFrame, "x", windowFrame.getX(), -windowWidth).setDuration(300);
        AnimatorSet animSet = new AnimatorSet();
        animSet.play(cameraImageAnim).with(windowFrameAnim);
        animSet.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                // TODO Auto-generated method stub
                // kth mod for [BUG 3411787] 20161223 start
                mBlockTouches = true;
                mStatusBar.getmBackdropBack().setY(-getMaxPanelHeight());
                launchCamera(true/*animate*/, 2 /*source affordance*/);// TCL Monster: kth mod for optimizing launch accordance camera 20161207
                // kth mod for [BUG 3411787] 20161223 end
            }
    ...
    private void startAnimationForBack() {

        ObjectAnimator cameraImageAnim = ObjectAnimator.ofFloat(cameraImage, "x", cameraImage.getX(), windowWidth).setDuration(300);
        ObjectAnimator windowFrameAnim = ObjectAnimator.ofFloat(windowFrame, "x", windowFrame.getX(), 0).setDuration(300);
        AnimatorSet animSet = new AnimatorSet();
        animSet.addListener(new AnimatorListener() {
    ...
    private void leftScrollForcamera(MotionEvent event) {
        final int action = event.getAction();
        if (DEBUG_LOG) Log.d("ktt_fre", "NotificationPanelView onTouchEvent leftScrollForcamera Action=" + event.getAction());
        acquireVelocityTracker(event);
        final VelocityTracker verTracker = mVelocityTracker_Mst;
        /**
         * isShowCameraImage 0:represent don't know  slide to upwards or slide to the left
         *                                  1:represent slide to upwards
         *                                  2:represent slide to left*/
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                isShowCameraImage = 0;
                x = event.getX();
        ...
        case MotionEvent.ACTION_MOVE:
            // TODO: 16-10-11 {link NotificationStackScrollLayout.NotificationSwipeHelper}
            if (isAnimating) {
                break;
            }
            if (event.getX() - x < 0 && isShowCameraImage != 1 && !isShowBouncer) {
                // kth mod 20160825 Note:The slope angle of 30: moveX > (moveY/tan30)
                if ((Math.abs(event.getX() - x) > pagingTouchSlop && Math.abs(event.getX() - x) > (1.73 * Math.abs(event.getY() - y))) || isShowCameraImage == 2) {
                    cameraImage.setX(windowWidth + event.getX() - x);
                    windowFrame.setX(event.getX() - x);
                    isShowCameraImage = 2;
                } else if (Math.abs(event.getY() - y) > pagingTouchSlop) {
                    isShowCameraImage = 1;
                }
            } else {
                isShowCameraImage = 1;
                startAnimationForBack();// kth add for Temporary solution 20161011
            }
            break;
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                //Log.e("101010", "----NotificationPanelView ACTION_UP velocityX = ----~~" + velocityX);
                //Log.e("101010", "----NotificationPanelView ACTION_UP velocityY = ----~~" + velocityY);
                releaseVelocityTracker();
                if (isShowCameraImage == 2) {
                    if (event.getX() - x < -windowWidth / 2 || (velocityX < mFlingAnimationUtils.getMinVelocityPxPerSecond() && event.getX() - x < -windowWidth / 4)) {
                        startAnimationForShow();
                    } else {
                        startAnimationForBack();
                    }
                }
                isShowCameraImage = 0;
                break;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //modify by chenhl start
        if(mHeadsUpTouchHelper.onTouchEvent(event)){
            return true;
        }
        //modify by chenhl end
        ...
        /**Mst: tangjun add for camera show begin*/
        /// kth add for disable ScrollForCamera feature while bouncer is showing 20160716 start @{
        final int action = event.getAction(); 
        switch (action) { 
            case MotionEvent.ACTION_DOWN: 
                if (DEBUG_LOG) Log.d("ktt_fre", "NotificationPanelView onTouchEvent getExpandedHeight=" + getExpandedHeight());
                isShowBouncer = (getExpandedHeight()==0) ? true : false;
                break; 
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
                isShowBouncer = false;
        ...
        if (DEBUG_LOG) Log.d("ktt_fre", "NotificationPanelView onTouchEvent isShowBouncer=" + isShowBouncer + " mUserSetupComplete=" + mUserSetupComplete);/// end@}
        if (mStatusBar.getBarState() != StatusBarState.SHADE && !isShowBouncer && mUserSetupComplete) {
            leftScrollForcamera(event);
        }
        /**Mst: tangjun add for camera show end*/
        /**Mst: tangjun add for camera show begin*/
    ...
    private boolean isInQsArea(float x, float y) {
        return (x >= mQsAutoReinflateContainer.getX()
                ...
                || y <= mQsContainer.getY() + mQsContainer.getHeight() || y <= getMaxPanelHeight()/*TCL Monster: kth add for keyguard panel 20161028*/);
    }
    ...
    private boolean isOpenQsEvent(MotionEvent event) {
        ...
        //modify by chenhl start for empty notify pull qsArea
        final boolean noNotify = mNotificationStackScroller.getmEmptyShadeView();
        return twoFingerDrag || stylusButtonClickDrag || mouseButtonClickDrag ||noNotify;
        //modify by chenhl end
    }
    ...
    private void updateQsState() {
        ...
        /*mQsNavbarScrim.setVisibility(mStatusBarState == StatusBarState.SHADE && mQsExpanded
                && !mStackScrollerOverscrolling && mQsScrimEnabled
                        ? View.VISIBLE
                        : View.INVISIBLE);*/
        mQsNavbarScrim.setVisibility(View.GONE);
        //modify by chenhl end
    ...
    protected void updateQsExpansion() {
        ...
        if(mKeyguardShowing){
            ...
            int tt=(int)(255*getQsExpansionFraction());
            StringBuilder builder = new StringBuilder("#");
            if(tt<16){
                builder.append("0");
            }
            builder.append(Integer.toHexString(tt));
            builder.append("374147");
            mNotificationStackScroller.setBackgroundColor(Color.parseColor(builder.toString()));
        }
        //add by chenhl end
    }
    ...
    private float calculateQsTopPadding() {
        if (mKeyguardShowing
            ...
            //modify by chenhl start
            if(mQsExpansionHeight<=mNotificationStackScroller.getIntrinsicPadding()){
                return mNotificationStackScroller.getIntrinsicPadding();
            }else{
    ...
    protected int getMaxPanelHeight() {
        ...
        //modify by chenhl start
        int maxHeight=mNotificationStackScroller.getHeight();
    ...
    private void updateNotificationTranslucency() {
        ...
        // TCL Monster: kth add for notification layout change alpha same with clock 20161027 start
        if (mKeyguardShowing) {
            alpha = mClockPositionResult.clockAlpha;
        }
        if (DEBUG_LOG) Log.d(TAG, "updateNotificationTranslucency: alpha=" + alpha + " mKeyguardShowing=" + mKeyguardShowing);
        // TCL Monster: kth add for notification layout change alpha same with clock 20161027 end
...
/*                lockIcon.setImageScale(LOCK_ICON_ACTIVE_SCALE, true, 150,
                        Interpolators.FAST_OUT_LINEAR_IN); ///TCL Monster: kth remove for keyguard lockicon layout 20161026*/
...
/*                lockIcon.setImageScale(1.0f, true, 150,
    ...
    private float getKeyguardContentsAlpha() {
        float alpha;
        /// TCL Monster: kth mod for keyguard hint 20161025 start @{
/*        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
        ...
        alpha = (float) Math.pow(alpha, 0.75);*/
        float moveHeight = Math.abs(getMaxPanelHeight() - getExpandedHeight()) - mHintDistance;
        float alphaChangedDistance = getResources().getDimension(R.dimen.swipe_out_alpha_change_distance);
        alpha = moveHeight > 0 ? (moveHeight < alphaChangedDistance ? (1 - (moveHeight / alphaChangedDistance)) : 0) : 1;
        /// end @}
        return alpha;
    }
...
//            lockIcon.setImageScale(2.0f, true, 100, Interpolators.FAST_OUT_LINEAR_IN);///TCL Monster: kth remove for keyguard lockicon layout 20161026
        }
    }
    ...
    private void handleDoubleTap() {
        if (!getContext().getResources().getBoolean(com.android.keyguard.R.bool.config_enable_double_tap_sleep)) {
            return;
        }
        int timeout = getContext().getResources().getInteger(com.android.keyguard.R.integer.config_double_tap_sleep_timeout);
        ...
        if (count == 1) {
            ...
            if (lastClick - firstClick < timeout) {
                if (DEBUG_LOG) Log.d(TAG, "handleDoubleTap and go to sleep");
...
//                    startUnlockHintAnimation();// TCL Monster: kth mod for remove hint click 20161102
                    handleDoubleTap();///TCL Monster: kth add for double tap to go sleep 20161122
                }
                return true;
    ...
    protected void updateStackHeight(float stackHeight) {
        mNotificationStackScroller.setStackHeight(stackHeight,getMaxPanelHeight());//modify by chenhl
        updateKeyguardBottomAreaAlpha();
    }
    ...
    /**Mst: tangjun add for lunch camera begin*/
    ...
    /**Mst: tangjun add for lunch camera end*/
    ...
    /// TCL Monster: kth add for bug, disable CameraImage when Horizontal screen 20160918
    public View getCamerImageView(){
        return cameraImage;
    }
    //add by chenhl start
    ...
    // TCL Monster: kth add for update camera visible notify setup complete or not 20161124 start
    public void setUserSetupComplete(boolean userSetupComplete) {
        if (DEBUG_LOG) Log.d(TAG, "setUserSetupComplete: userSetupComplete=" + userSetupComplete);
        mUserSetupComplete = userSetupComplete;
    }
    // TCL Monster: kth add for update camera visible notify setup complete or not 20161124 end
}
