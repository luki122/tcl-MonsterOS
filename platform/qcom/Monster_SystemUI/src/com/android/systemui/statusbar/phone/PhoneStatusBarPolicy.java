...
public class PhoneStatusBarPolicy implements Callback, RotationLockController.RotationLockControllerCallback, DataSaverController.Listener {
    ...
    /* MODIFIED-BEGIN by Wang Xiongke, 2016-09-28,BUG-3005175*/
    //SmartContainer modified begin
    private final String mSlotCloneBox;
    //SmartContainer modified end
    /* MODIFIED-END by Wang Xiongke,BUG-3005175*/
    //patch clone app add end
    private final String mSlotVibrate="tclvibrate";//add by chenhl
    ...
    private boolean mCloneBoxFocused = false;
    private boolean mCloneBoxIconVisible = false;
    private boolean mCloneBoxInQuietMode = false;
    ...
    public PhoneStatusBarPolicy(Context context, StatusBarIconController iconController,
        ...
        /* MODIFIED-BEGIN by Wang Xiongke, 2016-09-28,BUG-3005175*/
        //SmartContainer modify begin
        mSlotCloneBox = context.getString(com.android.internal.R.string.status_bar_clone_box);
        //SmartContainer modify end
        /* MODIFIED-END by Wang Xiongke,BUG-3005175*/
        //patch clone app add end
        ...
        /* MODIFIED-BEGIN by Wang Xiongke, 2016-09-28,BUG-3005175*/
        //SmartContainer modify begin
        mIconController.setIcon(mSlotCloneBox, R.drawable.stat_sys_clone_box_status,
                mContext.getString(R.string.accessibility_clone_box));
        mIconController.setIconVisibility(mSlotCloneBox, mCloneBoxIconVisible);
        //SmartContainer modify end
        /* MODIFIED-END by Wang Xiongke,BUG-3005175*/
        //patch clone app add end
        // data saver
        ...
        //add by chenhl start
        SettingObserver observer = new SettingObserver(mHandler);
        observer.setListening(true);
        updateVibrate();
        //add by chenhl end
    }
    ...
    private final void updateVolumeZen() {
        ...
        //modify by chenhl start for show silent icon
        ...
        } */else if (mZen == Global.ZEN_MODE_OFF
                && audioManager.getRingerModeInternal() == AudioManager.RINGER_MODE_SILENT) {
             volumeVisible = true;
             volumeIconId = R.drawable.stat_sys_ringer_silent;
             volumeDescription = mContext.getString(R.string.accessibility_ringer_silent);
    ...
    private void profileChanged(int userId) {
        ...
        if (!SmartContainerConfig.WITH_OUT_VIRTUAL_BOX) {
            mCloneBoxFocused = false;
            if (user != null) {
                try {
                    ContainerInfo ci = SmartContainerManagerNative.get().getContainerInfo(user.id);
                    if (ci != null && ci.isVirtualBox()) {
                        mCloneBoxFocused = true;
                        mManagedProfileFocused = false; //force reset if virtualbox focused
                    }
                } catch (RemoteException e) {
                }
            }
            if (DEBUG) Log.v(TAG, "profileChanged: mCloneBoxFocused: " + mCloneBoxFocused);
        }
        //SmartContainer modified end
        /* MODIFIED-END by Wang Xiongke,BUG-3005175*/
        //patch clone app add end
    ...
    private void updateManagedProfile() {
        ...
        //SmartContainer modified begin
        if (!SmartContainerConfig.WITH_OUT_VIRTUAL_BOX) {
            if (DEBUG) Log.v(TAG, "updateManagedProfile: mCloneBoxFocused: "
                    + mCloneBoxFocused);
            final boolean showCloneIcon;
            if (mCloneBoxFocused && !mStatusBarKeyguardViewManager.isShowing()) {
                showCloneIcon = true;
                mIconController.setIcon(mSlotCloneBox,
                        R.drawable.stat_sys_clone_box_status,
                        mContext.getString(R.string.accessibility_clone_box));
            } else if (mCloneBoxInQuietMode) {
                showCloneIcon = true;
                mIconController.setIcon(mSlotCloneBox,
                        R.drawable.stat_sys_clone_box_status_off,
                        mContext.getString(R.string.accessibility_clone_box));
            } else {
                showCloneIcon = false;
            }
            if (mCloneBoxIconVisible != showCloneIcon) {
                mIconController.setIconVisibility(mSlotCloneBox, showIcon);
                mCloneBoxIconVisible = showIcon;
            }
        }
        //SmartContainer modified end
        /* MODIFIED-END by Wang Xiongke,BUG-3005175*/
    ...
    //add by chenhl start
     private void updateVibrate(){
         ...
         mIconController.setIcon(mSlotVibrate,R.drawable.stat_sys_ringer_vibrate, null);
         mIconController.setIconVisibility(mSlotVibrate, show);
     }
