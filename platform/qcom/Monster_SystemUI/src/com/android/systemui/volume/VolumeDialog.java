...
public class VolumeDialog implements TunerService.Tunable {
    ...
    public VolumeDialog(Context context, int windowType, VolumeDialogController controller,
		...
		//modify by liuzhicang mActiveSliderTint = loadColorStateList(R.color.system_accent_color);
		//按效果图不区分颜色
        mActiveSliderTint = loadColorStateList(R.color.volume_slider_active);
        mInactiveSliderTint = loadColorStateList(R.color.volume_slider_active);
    ...
    private void updateWindowWidthH() {
		...
		// modify by liuzhicang int w = dm.widthPixels;
        int w = dm.widthPixels - 2*mContext.getResources()
                .getDimensionPixelSize(R.dimen.volume_dialog_margin);
    ...
    protected void dismissH(int reason) {
        ...
        //add by liuzhicang begin
        findRow(AudioManager.STREAM_VOICE_CALL).important = false;
        findRow(AudioManager.STREAM_ALARM).important = false;
        //add by liuzhicang end
        mHandler.removeMessages(H.DISMISS);
        mHandler.removeMessages(H.SHOW);
        if (!mShowing) return;
    ...
    private void updateRowsH() {
        ...
        //add by liuzhicang begin
        //通话或者闹钟界面调节音量时保持该进度条不消失
        if (activeRow.stream == AudioManager.STREAM_VOICE_CALL || activeRow.stream == AudioManager.STREAM_ALARM) {
            activeRow.important = true;
        }
        //add by liuzhicang end
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        final int iconRes =
                ...
                : isRingSilent || zenMuted ? R.drawable.ic_volume_ringer_mute
        ...
        //modify by liuzhicang begin /*|| isRingSilent*/
        final int vlevel = row.ss.muted && (isRingVibrate || isRingSilent || !isRingStream && !zenMuted) ? 0
                : row.ss.level;
