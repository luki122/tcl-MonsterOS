...
import com.tct.camera.robotium.By;
import com.tct.camera.robotium.Condition;
import com.tct.camera.robotium.Illustration;
import com.tct.camera.robotium.SystemUtils;
import com.tct.camera.robotium.Timeout;
import com.tct.camera.robotium.WebElement;
...
public class Solo {
	...
	*/
	...
	public View getView(String id, boolean allowNotFound){
		if(config.commandLogging){
			Log.d(config.commandLoggingTag, "getView(\""+id+"\")");
		}

		return getView(id, 0, allowNotFound);
	}
	...
	public View getView(String id, int index, boolean allowNotFound){
		if(config.commandLogging){
			Log.d(config.commandLoggingTag, "getView(\""+id+"\", "+index+")");
		}
		...
		if(viewToReturn == null && !allowNotFound) {
			int match = index + 1;
			if(match > 1){
