...
import com.android.external.plantform.ExtBuild; // MODIFIED by shunyin.zhang, 2016-06-29,BUG-2421416
...
public class CaptureLayoutHelper implements CameraAppUI.NonDecorWindowSizeChangedListener,
    ...
    private int mMinTopOptionHeight;
    private int mMaxTopOptionheight;
    private int mSpecialBottomBarHeight;
    private boolean mIsCaptureIntent;
    private boolean mIsBottomBarHeightReset;
    ...
    public CaptureLayoutHelper(CameraServices cameraService,int bottomBarMinHeight, int bottomBarMaxHeight,
            int bottomBarOptimalHeight, int specialBottomBarHeight, int minTopOptionHeight,
                               int maxTopOptionheight, boolean isCaptureIntent) {
        ...
        mSpecialBottomBarHeight = specialBottomBarHeight;
        mMinTopOptionHeight = minTopOptionHeight;
        mMaxTopOptionheight = maxTopOptionheight;
    ...
    public RectF getBottomBarRect() {
        ...
        RectF rect = new RectF(mPositionConfiguration.mBottomBarRect);
        if (ExtBuild.buildDevice() == ExtBuild.IDOL4_MINI) { // MODIFIED by shunyin.zhang, 2016-07-07,BUG-2479909
            rect.set(rect.left, rect.top , rect.right, rect.bottom + 96);
        }
        return rect;
    ...
    public RectF getUncoveredPreviewRect() {
        ...
        if (mWindowHeight > mWindowWidth) {
            // Portrait.
            if (mRotation >= 180&&!mService.isReversibleEnabled()) {
...
//        config.mPreviewRect.set(0, 0, width, height);
        ...
        if (landscape) {
            config.mBottomBarRect.set(barStartCoord, 0, width, height);
        } else {
            config.mBottomBarRect.set(0, barStartCoord, width, height);
        }
...
//                            width, height / 2 + previewShorterEdge / 2);
                    ...
                    config.mBottomBarRect.set(0, barStartCoord, width, mDisplayHeight);
...
//                            width / 2 + previewShorterEdge / 2, height);
                ...
                config.mBottomBarOverlay = false;
                if (landscape) {
                    if (mIsCaptureIntent && barSize > mSpecialBottomBarHeight) {
                        float tempLeft = width - barSize - previewLongerEdge;
                        if (tempLeft > mMinTopOptionHeight && tempLeft < mMaxTopOptionheight) {
                            barSize = mSpecialBottomBarHeight;
                            // third-party bottombar, hide mode name and mode scroll indicator
                            config.mBottomBarRect.set(width - barSize, 0, width, height);
                            mIsBottomBarHeightReset = true;
                        }
                    }
...
//                    config.mBottomBarRect.set(width - barSize, 0, width, height);
                    ...
                    barSize = config.mBottomBarRect.bottom - config.mBottomBarRect.top;
                    if (mIsCaptureIntent && barSize > (mSpecialBottomBarHeight + navigationBarHeight)) {
                        float tempTop = height - barSize - previewLongerEdge;
                        if (tempTop > mMinTopOptionHeight && tempTop < mMaxTopOptionheight) {
                            barSize = mSpecialBottomBarHeight + navigationBarHeight;
                            // third-party bottombar, hide mode name and mode scroll indicator
                            config.mBottomBarRect.set(0, height - barSize, width, height);
                            mIsBottomBarHeightReset = true;
                        }
...
//                    config.mBottomBarRect.set(0, height - barSize, width, height);
    ...
    public boolean isBottomBarHeightReset() {
        return mIsBottomBarHeightReset;
    }

    public boolean isFullscreen(){
        return mAspectRatio ==  TextureViewHelper.MATCH_SCREEN;
    }
}
