    ...
    public static synchronized CameraAgent getAndroidCameraAgent(Context context, CameraApi api) {
        ...
        if (api == CameraApi.API_1) {
            ...
            if (sAndroidCameraAgent == null) {
                sAndroidCameraAgent = new AndroidCameraAgentImpl(context); // MODIFIED by sichao.hu, 2016-08-16,BUG-2743263
                sAndroidCameraAgentClientCount = 1;
                Log.v(TAG, "create androidCameraAgent instance");
            } else {
                Log.v(TAG,"androidCameraAgent instance is recycled ?"+((AndroidCameraAgentImpl)sAndroidCameraAgent).isRecycled());
