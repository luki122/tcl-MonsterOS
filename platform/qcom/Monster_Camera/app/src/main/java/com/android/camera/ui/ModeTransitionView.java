...
public class ModeTransitionView extends View {
    ...
    private static final int FADE_IN_DURATION_MS=350;
    ...
    private int mAnimationType = IDLE;
    ...
    /*MODIFIED-BEGIN by xuan.zhou, 2016-04-12,BUG-1939380*/
    private BitmapPackager mCoveredBitmap;
    private RectF mCoveredRect;
    /*MODIFIED-END by xuan.zhou,BUG-1939380*/
    ...
    public void onDraw(Canvas canvas) {
        if (mAnimationType == PEEP_HOLE_ANIMATION) {
            ...
            Rect dstRect=null;
            if(mRect!=null) {
                dstRect=new Rect((int) mRect.left, (int) mRect.top, (int) mRect.right, (int) mRect.bottom);
            }else {
                dstRect = new Rect(0, 0, canvas.getWidth(), canvas.getHeight());
            }
            Log.w(TAG,"dst rect is "+dstRect);

            Rect srcRect=new Rect(0,0,mBackgroundBitmap.get().getWidth(),mBackgroundBitmap.get().getHeight());

            canvas.drawBitmap(mBackgroundBitmap.get(),srcRect,dstRect, null);

            if(mRect!=null) {
                ColorDrawable[] marginDrawables = new ColorDrawable[4];

                Rect[] marginRects = new Rect[4];
                marginRects[0] = new Rect(0, 0, (int) mRect.left, canvas.getHeight());
                marginRects[1] = new Rect(0, 0, canvas.getWidth(), (int) mRect.top);
                marginRects[2] = new Rect((int) mRect.right, 0, canvas.getWidth(), canvas.getHeight());
                marginRects[3] = new Rect(0, (int) mRect.bottom, canvas.getWidth(), canvas.getHeight());

                for (int i = 0; i < 4; i++) {
                    marginDrawables[i] = new ColorDrawable(Color.BLACK);
                    marginDrawables[i].setBounds(marginRects[i]);
                    marginDrawables[i].draw(canvas);
                }
            }
            super.onDraw(canvas);
            return;
    ...
    public void setupModeCover(Bitmap screenShot) {
        ...
        mModeTransState=STATE_ANIMATING;
        mAnimationType = SHOW_STATIC_IMAGE;
        mBackgroundBitmap =new BitmapPackager(screenShot);
        setVisibility(VISIBLE);
    }

    public static interface OnTransAnimationListener{
        public void onAnimationDone();
    }
    public void setupModeCoverTileAnimationDone(Bitmap screenShot,final OnTransAnimationListener ... listeners){
...
//        }
        ...
        if(mShowAnimator!=null) {
            Log.w(TAG, "animation running ?" + mShowAnimator.isRunning() + " alpha is " + this.getAlpha());
        }
        if (mShowAnimator!=null&&(mShowAnimator.isRunning()||this.getVisibility()==VISIBLE)) {
        ...
        /*MODIFIED-BEGIN by xuan.zhou, 2016-04-12,BUG-1939380*/
        // mBackgroundBitmap may set null after mHideCoverAnimator
        mCoveredBitmap = new BitmapPackager(screenShot);
        mCoveredRect = calculateRectF();
        /*MODIFIED-END by xuan.zhou,BUG-1939380*/
        setAlpha(0);
        setVisibility(VISIBLE);
        if(mShowAnimator==null){
        ...
        if(duration>=0){
            mShowAnimator.setDuration(duration);
            mHideCoverAnimator.setDuration(duration);
        }else{
            mShowAnimator.setDuration(FADE_IN_DURATION_MS);
            mHideCoverAnimator.setDuration(FADE_OUT_DURATION_MS);
        }
        ...
        mShowAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                ...
                Log.w(TAG,"blur animation start end");
                mShowAnimator.removeListener(this);
            }
            ...
            public void onAnimationCancel(Animator animator) {
                ...
                Log.w(TAG,"blur animation start cancel");
                mShowAnimator.removeListener(this);
            }
    ...
    private void initializeBlurAnimator(){
        mShowAnimator=ValueAnimator.ofFloat(0.2f,1.0f);
        mShowAnimator.setDuration(FADE_IN_DURATION_MS);
        ...
        mHideCoverAnimator=ValueAnimator.ofFloat(1.0f,0);
        mHideCoverAnimator.setDuration(FADE_OUT_DURATION_MS);
        mHideCoverAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                setAlpha((float) valueAnimator.getAnimatedValue());
                invalidate();
    ...
    private int mModeTransState=STATE_STOP;
    /**
    ...
    public void hideImageCover() {
        ...
        if(this.getVisibility()==GONE/*||!AppController.ENABLE_BLUR_TRANS*/){
            mBackgroundBitmap = null;
            setVisibility(GONE);
            mModeTransState=STATE_STOP;
            mAnimationType = IDLE;
            if(mPendingRect!=null) {
                mRect = mPendingRect;
                mPendingRect=null;
            }
            return;
        }
        if(mAnimationType==SHOW_STATIC_IMAGE){
            Log.w(TAG,"try hide mode cover");
            if(mHideCoverAnimator==null){
                initializeBlurAnimator();
            }
            if(mHideCoverAnimator.isRunning()){
                return;
            }
            ...
            mHideCoverAnimator.addListener(new Animator.AnimatorListener() {
                ...
                @Override
                public void onAnimationEnd(Animator animator) {
                    Log.w(TAG,"mode cover hidden");
                    mBackgroundBitmap = null;
                    setVisibility(GONE);
                    mModeTransState=STATE_STOP;
                    mAnimationType = IDLE;
                    mHideCoverAnimator.removeListener(this);
                    if(mPendingRect!=null) {
                        mRect = mPendingRect;
                        mPendingRect=null;
                    }
    ...
    public void mapPreviewRect(RectF rectF){
        if(mModeTransState==STATE_ANIMATING){
            mPendingRect =new RectF(rectF);
        }else {
    ...
    public Bitmap getBackgroundBitmap() {
        /*MODIFIED-BEGIN by xuan.zhou, 2016-04-12,BUG-1939380*/
        return mCoveredBitmap == null ? null :
                mCoveredBitmap.get();
    }
    ...
    private RectF calculateRectF() {
        // After mBackgroundBitmap set, I'm not sure whether hideImageCover() will be invoked or not,
        // so get the rectF according to the ratio
        if (mCoveredBitmap == null || mPendingRect == null ||
                Math.abs(mPendingRect.right - mPendingRect.left) == 0) {
            return mRect;
        }
        final double ASPECT_TOLERANCE = 0.01;
        double ratio = mCoveredBitmap.get().getHeight() / mCoveredBitmap.get().getWidth();
        double pendingRatio = Math.abs(mPendingRect.bottom - mPendingRect.top) /
                Math.abs(mPendingRect.right - mPendingRect.left);
        if (Math.abs(ratio - pendingRatio) <= ASPECT_TOLERANCE) {
            return mPendingRect;
        } else {
            return mRect;
        }
