...
public class IndicatorIconController
    implements SettingsManager.OnSettingChangedListener,
        ...
        TopMenus.TopMenusButtonStatusListener{
...
//            syncFlashIndicator();
    ...
    public void onMenusVisibilityChanged(TopMenus topMenus, int buttonId) {
        syncIndicatorWithMenu(buttonId);
    }
     ...
     * Sync the icon and visibility of the flash indicator.
     */
    private void syncFlashButtonIndicator() {
        TopMenus topMenus = mController.getCameraAppUI().getTopMenus();
        // If flash isn't an enabled and visible option,
        // do not show the indicator.
        if (topMenus.isButtonEnabled(TopMenus.BUTTON_FLASH)
                && topMenus.isButtonVisible(TopMenus.BUTTON_FLASH)) {
            setIndicatorState(mController.getCameraScope(),
                    Keys.KEY_FLASH_MODE, mFlashIndicator,
                    mFlashIndicatorPhotoIcons, false);
        } else if (topMenus.isButtonEnabled(TopMenus.BUTTON_TORCH)
                && topMenus.isButtonVisible(TopMenus.BUTTON_TORCH)) {
            setIndicatorState(mController.getCameraScope(),
                    Keys.KEY_VIDEOCAMERA_FLASH_MODE, mFlashIndicator,
                    mFlashIndicatorVideoIcons, false);
        } else {
