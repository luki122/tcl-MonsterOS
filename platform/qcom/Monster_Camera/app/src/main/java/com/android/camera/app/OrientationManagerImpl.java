...
public class OrientationManagerImpl implements OrientationManager {
    ...
    public void lockOrientation() {
        ...
        if (ApiHelper.HAS_ORIENTATION_LOCK) {
            ...
            ((CameraActivity)mActivity).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LOCKED);
        } else {
            ((CameraActivity)mActivity).setRequestedOrientation(calculateCurrentScreenOrientation());
        }
    }
    ...
    public void unlockOrientation() {
        ...
        ((CameraActivity)mActivity).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
    }
    ...
    private int roundOrientation(int orientation, int orientationHistory) {
        ...
        if (orientationHistory == OrientationEventListener.ORIENTATION_UNKNOWN) {
            ...
            changeOrientation = (dist >= 45 + getOrientationHysteresis());
        }
    ...
    private int getOrientationHysteresis() {
        if (mActivity == null) {
            return ORIENTATION_HYSTERESIS;
        }
        if (DeviceInfo.isReversibleOn(mActivity.getContentResolver())) {
            return ORIENTATION_HYSTERESIS_REVERSIBLEON;
        } else {
            return ORIENTATION_HYSTERESIS;
        }
    }
