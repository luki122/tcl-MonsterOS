...
public class AppUpgrader extends SettingsUpgrader {
     ...
     * With this version hdr options change to {auto, hdr_auto, hdr} from {0, 1}
     */
    private static final int CAMERA_SETTINGS_HDR_UPGRADE = 7;
    ...
    protected static final String OLD_SETTINGS_HDR_ON = "1";
    protected static final String OLD_SETTINGS_HDR_OFF = "0";
...
//                    Keys.KEY_STARTUP_MODULE_INDEX);
        ...
        /* MODIFIED-BEGIN by xuyang.liu, 2016-10-13,BUG-3110198*/
        if (lastVersion < CAMERA_SETTINGS_HDR_UPGRADE && Keys.isAutoHdrEnable()) {
            upgradeHdrOptionValue(settingsManager, context);
        }
        /* MODIFIED-END by xuyang.liu,BUG-3110198*/
    }
    ...
    private void upgradeTypesToStrings(SettingsManager settingsManager) {
        ...
        if (defaultPreferences.contains(Keys.KEY_STARTUP_MODULE_INDEX)) {
            int startupModuleIndex = removeInteger(defaultPreferences,
                    Keys.KEY_STARTUP_MODULE_INDEX, settingsManager.getIntegerDefault(Keys.KEY_STARTUP_MODULE_INDEX));
        ...
        if (defaultPreferences.contains(Keys.KEY_CAMERA_MODULE_LAST_USED)) {
            int lastCameraUsedModuleIndex = removeInteger(defaultPreferences,
                    Keys.KEY_CAMERA_MODULE_LAST_USED, settingsManager.getIntegerDefault(Keys.KEY_CAMERA_MODULE_LAST_USED));
    ...
    /* MODIFIED-BEGIN by xuyang.liu, 2016-10-13,BUG-3110198*/
    private void upgradeHdrOptionValue(SettingsManager settingsManager, Context context) {
        SharedPreferences defaultPreferences = settingsManager.getDefaultPreferences();
        if (defaultPreferences.contains(Keys.KEY_CAMERA_HDR)) {
            String hdr = removeString(defaultPreferences, Keys.KEY_CAMERA_HDR);
            String hdrAuto = context.getResources().getString(R.string.pref_camera_hdr_auto);
            String hdrOff = context.getResources().getString(R.string.pref_camera_hdr_off);
            if (OLD_SETTINGS_HDR_ON.equals(hdr)) {
                settingsManager.set(SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_HDR, hdrAuto);
            } else if (OLD_SETTINGS_HDR_OFF.equals(hdr)){
                settingsManager.set(SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_HDR, hdrOff);
            }
        }
    }
    /* MODIFIED-END by xuyang.liu,BUG-3110198*/
