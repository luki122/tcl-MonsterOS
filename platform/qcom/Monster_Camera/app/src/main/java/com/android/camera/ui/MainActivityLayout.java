...
public class MainActivityLayout extends FrameLayout {
    ...
    private boolean mTouchObstruct = false;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getActionMasked() == MotionEvent.ACTION_DOWN) {
            ...
            if(mHelpTipsManager != null && mHelpTipsManager.isHelpTipShowExist()) {
                mTouchReceiver = mHelpTipsManager.getHelpTipView();
            }else {
                mTouchReceiver = null;
            }
...
//                if (mModeStrip.isLocked()) {
//                    return false;
//                }
                // Intercept right swipe
                if (deltaX >= Math.abs(deltaY) * 2) {
//                    mTouchReceiver = mModeList;
                    if(mHelpTipsManager != null && mHelpTipsManager.isHelpTipShowExist() && mHelpTipsManager.helpTipCheckToIntercept()){
                        mTouchReceiver = mHelpTipsManager.getHelpTipView();
                    }else{
                        mTouchReceiver = mModeStrip;
                    }
...
//            mTouchReceiver.setVisibility(VISIBLE);
    ...
    public void onFinishInflate() {
        ...
        mModeStrip=(StereoModeStripView)findViewById(R.id.mode_strip_view);
    }
    ...
    public void setTouchObstruct(boolean value) {
        mTouchObstruct = value;
    }
    ...
    public void setHelpTipManager(HelpTipsManager helpTipManager) {
        mHelpTipsManager = helpTipManager;
    }

    /* MODIFIED-BEGIN by jianying.zhang, 2016-10-27,BUG-3212745*/
    private boolean isScaleBarVisible() {
        ZoomBar zoomBar = (ZoomBar) findViewById(R.id.zoom_bar);
        if(zoomBar != null && zoomBar.getVisibility() == View.VISIBLE) {
            return true;
        }
        return false;
    }
