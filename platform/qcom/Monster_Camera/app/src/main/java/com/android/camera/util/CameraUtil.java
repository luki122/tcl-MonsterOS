...
import android.app.Dialog; // MODIFIED by xuan.zhou, 2016-07-11,BUG-2481700
...
import android.content.res.Configuration; //MODIFIED by nie.lei, 2016-04-01,BUG-1899903
...
import android.os.Build; // MODIFIED by xuan.zhou, 2016-04-26,BUG-1998180
...
import android.view.ViewGroup; // MODIFIED by xuan.zhou, 2016-07-11,BUG-2481700
...
import com.android.camera.Storage; // MODIFIED by nie.lei, 2016-05-06,BUG-2104974
import com.android.camera.debug.Log;
import com.android.camera.filmstrip.ImageData;
import com.android.camera.permission.PermissionUtil;
import com.android.camera.settings.Keys;
import com.android.camera.settings.SettingsManager;
import com.android.camera.widget.LocationDialogLayout; // MODIFIED by xuan.zhou, 2016-07-11,BUG-2481700
...
public class CameraUtil {
    ...
    public final static int  SETGPS = 1000;
    ...
    public static final int BOOM_KEY = 276;//Boom Key define
    public static final String BOOM_EFFECT_SETTINGS = "boom_key_unlock_enable";//Boom Key define
    public static final String CAMERA_KEY_SETTINGS = "boom_key_action";//Boom Key define
    public static final int BOOM_EFFECT_ON = 1;
    public static final int BOOM_EFFECT_OFF = 0;
    public static final int CAMERA_KEY_ON = 0;//0 means camer key is on
    ...
    public static AlertDialog showStorageLowAndFinish(final Activity activity, String msgStr
            , final boolean storageLow,final int btnCxtId, final Runnable callback) { // MODIFIED by nie.lei, 2016-05-26,BUG-2208223
...
//            new AlertDialog.Builder(activity)
            ...
            if(storageLow){
                DialogInterface.OnClickListener changeStorageListener =
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                dialog.dismiss(); // MODIFIED by nie.lei, 2016-05-26,BUG-2208223
                                if(btnCxtId == R.string.storage_low_phone_full_sdcard_ok_button){
                                    ((CameraActivity)activity).getSettingsManager().
                                            set(SettingsManager.SCOPE_GLOBAL,
                                                    Keys.KEY_CAMERA_SAVEPATH,Storage.SDCARD_STORAGE);
                                    Storage.setSavePath(Storage.SDCARD_STORAGE);
                                }else if(btnCxtId == R.string.storage_low_storage_full_phone_ok_button){
                                    ((CameraActivity)activity).getSettingsManager().
                                            set(SettingsManager.SCOPE_GLOBAL,
                                                    Keys.KEY_CAMERA_SAVEPATH,Storage.PHONE_STORAGE);
                ...
                builder.setNegativeButton(btnCxtId, changeStorageListener);
            }
            /* MODIFIED-END by nie.lei,BUG-2104974*/
    ...
    public static int getOptimalPreviewSizeIndex(Context context,
        ...
        Log.w(TAG,"default size is "+defaultDisplaySize);
        ...
        for (int i = 0; i < sizes.size(); i++) {
            ...
            if (heightDiff < minDiff) {
                optimalSizeIndex = i;
//                minDiff = heightDiff;//removed by sichao.hu , remove restriction of choosing smaller-than-display size , the higher resolution is preferred
    ...
    public static Camera.Size getOptimalExifSize(List<Camera.Size> supportedExifSizes, com.android.ex.camera2.portability.Size photoSize){
        if(photoSize==null){
            return null;
        }
        ...
        double tollerance=0.01d;
        double photoSizeRatio=photoSize.width()/((double)photoSize.height());
        for(Camera.Size size:supportedExifSizes){
            double ratio=size.width/((double)size.height);
            if(Math.abs(photoSizeRatio-ratio)<tollerance){
    ...
    public static String getProximateResolution(Set set, int width, int height) {
        ...
        for(Iterator iter= set.iterator();iter.hasNext();){
            ...
            int pos = key.indexOf('x');
            if (pos != -1) {
                int w = Integer.parseInt(key.substring(0, pos));
                int h = Integer.parseInt(key.substring(pos + 1));
                int error = Math.abs(w * h - size);
                if (error < minError) {
                    minError = error;
    ...
    public static com.android.ex.camera2.portability.Size getProximateSize(
            List<com.android.ex.camera2.portability.Size> sizes, int width, int height) {

        int maxPixels = 0;
        com.android.ex.camera2.portability.Size maxSize = null;
        for (com.android.ex.camera2.portability.Size size : sizes) {
            int w = size.width();
            int h = size.height();
            if (w <= width && h <= height) {
    ...
    public static void prepareMatrix(Matrix matrix, boolean mirror, int displayOrientation,
        ...
        // This is the value for android.hardware.Camera.setPostGestureRotation.
        ...
        if (dir != null && dir.exists() && dir.isDirectory()) {
            for (File item : dir.listFiles()) {
                ...
                if (index != -1) {
                    ...
                    Log.d(TAG,"cleanSharedPreference  item: "+path+",  "+preferenceName);
                    SharedPreferences preferences = context.getSharedPreferences(
                            preferenceName, Context.MODE_PRIVATE);
                    if(CustomUtil.getInstance().getBoolean(CustomFields.DEF_CAMERA_SUPPORT_HELP_TIP_TUTORIAL, false)&&
                            preferences.equals(PreferenceManager.getDefaultSharedPreferences(context))){
    ...
    private static void filterClearHelpTipsPreference(SharedPreferences preferences){
        Set<String> allOfKeys = preferences.getAll().keySet();
        for(String key : allOfKeys){
            switch (key){
                case Keys.KEY_HELP_TIP_GESTURE_FINISHED:
                case Keys.KEY_HELP_TIP_MANUAL_FINISHED:
                case Keys.KEY_HELP_TIP_PANO_FINISHED:
                case Keys.KEY_HELP_TIP_WELCOME_FINISHED:
                case Keys.KEY_HELP_TIP_WELCOME_STEP:
                    continue;
                default:
    ...
    public static Bitmap downSample(final byte[] data, int downSampleFactor) {
        try {
            ...
            Log.e(TAG, "OutOfMemoryError when down-sample the jpeg" + e);
            return null;
        }
    }
    ...
    public static void playVideo(Activity activity, Uri uri, String title) {
        try {
            ...
            ToastUtil.showToast(activity, activity.getString(R.string.video_err), Toast.LENGTH_SHORT);
        }
    }
    ...
    public static int getCameraShutterNormalStateIconId(int modeIndex, Context context) {
        // Find the camera mode normal state icon using id
        TypedArray shutterIcons = context.getResources()
                .obtainTypedArray(R.array.camera_mode_shutter_normal_state_icon);
        if (modeIndex < 0 || modeIndex >= shutterIcons.length()) {
            Log.e(TAG, "Invalid mode index: " + modeIndex);
            throw new IllegalStateException("Invalid mode index: " + modeIndex);
        }
    ...
    public static String serializeToJson(Map<String,Object> jsonEntities){
        ...
        Log.w(TAG,"jsonString is "+jsonString);
        return jsonString;
    }
    ...
    public static List<String> getCompensatedFaceRects(Camera.Face[] faces, com.android.ex.camera2.portability.Size previewSize,
                                                     com.android.ex.camera2.portability.Size  photoSize){
        ...
        for(Camera.Face face : faces){
            // Camera driver coordinates range from (-1000, -1000) to (1000, 1000).
            //No need to do second time map for original signal
            if(face.score<50){
                continue;
            }
            Rect faceRectInPreview=face.rect;
//            int left=faceRectInPreview.left;
//            int right=faceRectInPreview.right;
//            int top=faceRectInPreview.top;
//            int bottom=faceRectInPreview.bottom;
//
//
//            int mappedLeft=left*photoSize.width()/previewSize.width();
//            int mappedRight=right*photoSize.width()/previewSize.width();
//            int mappedTop=top*photoSize.height()/previewSize.height();
//            int mappedBottom=bottom*photoSize.height()/previewSize.height();
//
//            Rect mappedFaceRect=new Rect(mappedLeft,mappedTop,mappedRight,mappedBottom);
            StringBuilder rectStrBuilder=new StringBuilder();
            rectStrBuilder.append(""+faceRectInPreview.left);
            rectStrBuilder.append(SPLITTER);
            rectStrBuilder.append(""+faceRectInPreview.top);
            rectStrBuilder.append(SPLITTER);
            rectStrBuilder.append(""+faceRectInPreview.right);
            rectStrBuilder.append(SPLITTER);
            rectStrBuilder.append(""+faceRectInPreview.bottom);
    ...
    public static List<String> getCompensatedFaceRects(ArrayList<RectF> faces){
        ...
        for(RectF face : faces){
            ...
            rectStrBuilder.append(""+faceRectInPreview.bottom);
            rects.add(rectStrBuilder.toString());
            Log.d(TAG, "getCompensatedFaceRects:" + face+", "+faceRectInPreview+", "+rectStrBuilder.toString());
        }
    ...
    public static AlertDialog showBatteryInfoDialog(final Activity activity, boolean cancelable,
                                                int titleId, int msgId, int buttonId,
    ...
    public static boolean gotoGpsSetting(final Activity activity, SettingsManager settingsManager, int resId) {
        boolean isGpsSaved = settingsManager.getBoolean(SettingsManager.SCOPE_GLOBAL, Keys.KEY_RECORD_LOCATION);
        if (!isGpsSaved) {
            return false;
        }
        if (!checkGpsEnable(activity)) {
            // if system settings->location is not on, do not remember location
            ...
            alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, activity.getResources().getString(R.string.location_service__dialog_setting),
                    new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int arg1) {
                            Intent intent = new Intent("android.settings.LOCATION_SOURCE_SETTINGS");
                            activity.startActivityForResult(intent, SETGPS);
                        }
                    });
            alertDialog.setButton(AlertDialog.BUTTON_NEGATIVE, activity.getResources().getString(R.string.location_service_dialog_cancel),
                    new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int arg1) {
                            dialog.cancel();
            ...
            alertDialog.getButton(AlertDialog.BUTTON_POSITIVE).setTextColor(activity.getColor(R.color.gps_dialog_button_color));
            alertDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(activity.getColor(R.color.gps_dialog_button_color));
            return true;
        }
        return false;
    }
    ...
    public static boolean checkGpsEnable(Context context) {
        ...
        boolean networkEnabled = Settings.Secure.isLocationProviderEnabled(context.getContentResolver(),
                android.location.LocationManager.NETWORK_PROVIDER);

        Log.w(TAG, "gpsEnabled = " + gpsEnabled + " ; networkEnabled = " + networkEnabled);
        return gpsEnabled || networkEnabled;
    }

    // For the battery saver.
    ...
    public static boolean isBatterySaverEnabled(Context context) {
        ...
        final int lowPowerMode = Settings.Global.getInt(context.getContentResolver(),
                LOW_POWER_MODE, LOW_POWER_MODE_OFF);
        Log.d(TAG, "LowPowerMode is " + lowPowerMode);
        return (lowPowerMode == LOW_POWER_MODE_ON);
    }
    ...
    public static boolean isScreenOff(Context context) {
        if (ApiHelper.isLOrHigher()) { // Display.getState() requires API level 20.
            WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
            Display dp = wm.getDefaultDisplay();
            int state = dp.getState();
            Log.w(TAG, "Current display state is " + state);
            return state == Display.STATE_OFF;
        } else {
            return (getCurrentBrightness() == 0);
        }
    }

    public static int getCurrentBrightness() {
        final String BRIGHTNESS_PATH = "/sys/class/leds/lcd-backlight/brightness";
        int brightness = -1;
        BufferedReader reader = null;
        try {
            FileReader fr = new FileReader(BRIGHTNESS_PATH);
            reader = new BufferedReader(fr);
            brightness = Integer.parseInt(reader.readLine().trim());
        } catch (IOException e) {
            Log.e(TAG, "IOException " + e + " happens when reading " + BRIGHTNESS_PATH);
        } finally {
            if (reader != null) {
                try {
                    reader.close();
                } catch (IOException e) {
                    Log.e(TAG, "IOException " + e + " happens when close reader ");
                }
            }
        }
        Log.w(TAG, "Current brightness is " + brightness);
        return brightness;
    }

    // Copy from android.provider.Settings.Secure.CAMERA_DOUBLE_TAP_POWER_GESTURE_DISABLED
    public static final String CAMERA_DOUBLE_TAP_POWER_GESTURE_DISABLED =
            "camera_double_tap_power_gesture_disabled";
    public static boolean isCameraDoubleTapPowerGestureDisbled(Context context) {
        int value = Settings.Secure.getInt(context.getContentResolver(),
                CAMERA_DOUBLE_TAP_POWER_GESTURE_DISABLED, -1);
        Log.w(TAG, "camera_double_tap_power_gesture_disabled is " + value);
        return (value == 1);
    }
    /* MODIFIED-END by xuan.zhou,BUG-1998180*/

    /* MODIFIED-BEGIN by xuan.zhou, 2016-07-11,BUG-2481700*/
    public static Dialog getLocationPrompt(final CameraActivity activity,
                                   final CameraActivity.LocationDialogCallback locationCallback) {
        // Copy from PhotoUI.java, I thinks it looks better than the FirstTimeLocationAlert.
        final Dialog mDialog = new Dialog(activity,
                android.R.style.Theme_Black_NoTitleBar_Fullscreen);
        final LocationDialogLayout locationDialogLayout = (LocationDialogLayout) activity
    ...
    public static ComponentName getComponentNameByOrder(Context context, int order) {
        ActivityManager am = (ActivityManager) context
                .getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = null;
        List<ActivityManager.RunningTaskInfo> list = am.getRunningTasks(order + 1);
        for (ActivityManager.RunningTaskInfo info : list) {
            Log.d(TAG, "RunningTaskInfo: " + info.baseActivity);
        }
        if (list != null && order < list.size()) {
            cn = list.get(order).baseActivity;
        }
        return cn;
    }

    public static boolean isLocationEnable(Context context){
        return PermissionUtil.isNoncriticalPermissionGranted(context) &&
                CameraUtil.checkGpsEnable(context);
    }
    ...
    public static float getDistance(PointF a, PointF b) {
        ...
        return (float) Math.hypot(Math.abs(a.x - b.x), Math.abs(a.y - b.y));
    }
