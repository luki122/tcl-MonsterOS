...
public class Thumbnail {
    ...
    private static Media getLastVideoThumbnail(ContentResolver resolver) {
        ...
        Uri query = baseUri.buildUpon().appendQueryParameter("limit", "1").build();
        String[] projection = new String[] {Video.VideoColumns._ID, MediaStore.MediaColumns.DATA,
                Video.VideoColumns.DATE_TAKEN};
        String selection = Video.VideoColumns.BUCKET_ID + '=' + Storage.BUCKET_ID;
        String order = Video.VideoColumns.DATE_TAKEN + " DESC," + Video.VideoColumns._ID + " DESC";
    ...
    public static Bitmap getBitmapFromUri(ContentResolver resolver, Uri uri, int orientation) {
        try {
            Bitmap bitmap = Images.Media.getBitmap(resolver, uri);
