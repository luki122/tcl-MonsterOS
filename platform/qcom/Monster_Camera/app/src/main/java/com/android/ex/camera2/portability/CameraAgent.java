...
public abstract class CameraAgent {
    ...
    public static final int INDEX_NONE_FILTER = 8; // MODIFIED by sichao.hu, 2016-08-16,BUG-2743263
    ...
    public static class CameraOpenCallbackForward implements CameraOpenCallback {
        ...
        private CameraSettings.BoostParameters mBoostParameters;
        ...
        public static CameraOpenCallbackForward getNewInstance(Handler handler,CameraOpenCallback cb,CameraSettings.BoostParameters parameters){
            if (handler == null || cb == null) {
                return null;
            }
        ...
        private CameraOpenCallbackForward(Handler h, CameraOpenCallback cb ,CameraSettings.BoostParameters parameters) {
            // Given that we are using the main thread handler, we can create it
            // here instead of holding onto the PhotoModule objects. In this
            // way, we can avoid memory leak.
            mHandler = new Handler(Looper.getMainLooper());
            mCallback = cb;
            mBoostParameters=parameters;
        ...
        public void onCameraOpened(final CameraProxy camera) {
            Log.w(TAG,"KPI post open E");
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Log.w(TAG, "KPI post open X");
        ...
        public boolean isBoostPreview() {
            return mCallback.isBoostPreview();
        }

        @Override
        public Context getCallbackContext() {
            return mCallback.getCallbackContext();
        }

        @Override
        public CameraSettings.BoostParameters getBoostParam() {
            return mBoostParameters;
        }

        @Override
        public boolean isReleased() {
            return (mCallback != null) ? mCallback.isReleased() : false;
        }
    }
    ...
    public static interface CameraGDCallBack {
        public void onGesture();
    }

    public static interface CameraPanoramaCallback {
        public void onCapture(byte[] jpegData);
    }

    public static interface CameraPanoramaMoveCallback {
    ...
    public static interface CameraOpenCallback {
        ...
        /**
         * Called when requestCamera to mark camera release state false.
         */
        public void onCameraRequested();

        /**
         * Called when closeCamera to mark camera release state true.
         */
        public void onCameraClosed();

        /**
         * Get whether Camera.release() is called, if so, onCameraOpened will
         * not be called in AndroidCameraAgentImpl.
         */
        public boolean isReleased();
        ...
        try {
            getDispatchThread().runJob(new Runnable() {
                ...
                public void run() {
                    ...
                    getCameraHandler().obtainMessage(CameraActions.OPEN_CAMERA, cameraId, -1,
                            CameraOpenCallbackForward.getNewInstance(handler, callback)).sendToTarget();
                }
            });
        } catch (final RuntimeException ex) {
            Log.e(TAG,"open camera post to handler failed :"+ex.getMessage());
            getCameraExceptionHandler().onDispatchThreadException(ex);
        }
    }

    public void openCamera(final Handler handler, final int[] cameraIds,
                           final CameraOpenCallback callback) {
        try {
            getDispatchThread().runJob(new Runnable() {
                @Override
                public void run() {
                    getCameraHandler().obtainMessage(CameraActions.OPEN_CAMERA, cameraIds[0], cameraIds.length>1?cameraIds[1]:-1,
    ...
    public void openCameraBoost(final Handler handler, final int cameraId,final CameraOpenCallback callback, final CameraSettings.BoostParameters param){
        try {
            getDispatchThread().runJob(new Runnable() {
                @Override
                public void run() {
                    getCameraHandler().obtainMessage(CameraActions.OPEN_CAMERA, cameraId, 0,
                            CameraOpenCallbackForward.getNewInstance(handler, callback, param)).sendToTarget();
                }
            });
        } catch (final RuntimeException ex) {
            Log.e(TAG,"open camera post to handler failed :"+ex.getMessage());
    ...
    public boolean applySettings(final CameraSettings.BoostParameters settings){
        if (settings == null) {
            Log.v(TAG, "null argument in applySettings()");
            return false;
        }

        final int statesToAwait=CAMERA_IDLE|CAMERA_UNLOCKED;
        try {
            getDispatchThread().runJob(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        Log.w(TAG, "wait for state " + statesToAwait + " currentState is " + cameraState.getState());
                        cameraState.waitForStates(statesToAwait);
                        Log.w(TAG, "wait for state done");
                        getCameraHandler().obtainMessage(CameraActions.APPLY_PREVIEW_RELATED_SETTINGS, settings)
                                .sendToTarget();
                    }
                });
        } catch (final RuntimeException ex) {
            ...
            getDispatchThread().runJob(new Runnable() {
                ...
                public void run() {
                    getCameraHandler().obtainMessage(CameraActions.START_PREVIEW_ASYNC,
                            CameraStartPreviewCallbackForward.getNewInstance(handler, callback))
                            .sendToTarget();
                }});
        } catch (final RuntimeException ex) {
            getCameraExceptionHandler().onDispatchThreadException(ex);
    ...
    public abstract static class CameraProxy {
        ...
        public void fakeStartPreview() {
            try {
                getDispatchThread().runJob(new Runnable() {
                    ...
                    public void run() {
                        getCameraHandler()
                                .obtainMessage(CameraActions.FAKE_START_PREVIEW_ASYNC, null)
                                .sendToTarget();
                    }
                });
            } catch (final RuntimeException ex) {
        ...
        public void fakeSetPreviewTexture() {
            try {
                getDispatchThread().runJob(new Runnable() {
                    ...
                    public void run() {
                        getCameraHandler()
                                .obtainMessage(CameraActions.FAKE_SET_PREVIEW_TEXTURE_ASYNC, null)
                                .sendToTarget();
                    }
                });
            } catch (final RuntimeException ex) {
        ...
        public void fakeStopPreview() {
            ...
            try {
                getDispatchThread().runJobSync(new Runnable() {
                    @Override
                    public void run() {
                        getCameraHandler().obtainMessage(CameraActions.FAKE_STOP_PREVIEW, bundle)
                                .sendToTarget();
                    }
        ...
        public void waitPreviewWithCallback(final Handler h,final CameraStartPreviewCallback cb){
            try {
                getDispatchThread().runJob(new Runnable() {
                    @Override
                    public void run() {
                        getCameraHandler().obtainMessage(CameraActions.WAIT_PREVIEW_ASYNC,
                                CameraStartPreviewCallbackForward.getNewInstance(h, cb))
                                .sendToTarget();
                    }
                });
        ...
        public void stopPreview() {
            ...
            // It seems that STOP_PREVIEW not need to be synchronous now.
            // final WaitDoneBundle bundle = new WaitDoneBundle();
            try {
                // getDispatchThread().runJobSync(new Runnable() {
                //     @Override
        ...
        public void setMetadataCb(Handler handler, CameraMetaDataCallback cb) {} // MODIFIED by xuyang.liu, 2016-10-13,BUG-3110198
        /**
        ...
        /* MODIFIED-BEGIN by sichao.hu, 2016-08-16,BUG-2743263*/
        public abstract void takePicture(Handler handler,CameraShutterCallback shutter,
                                         CameraPictureCallback raw,
                                         CameraPictureCallback postview,
                                         CameraPictureCallback jpeg,int filterIndex);
                                               /* MODIFIED-END by sichao.hu,BUG-2743263*/


        /**
         * take picture without waiting for previous picture jpeg compressed done
         *
         * @param handler   The handler in which the callback will be invoked.
         * @param shutter   The callback for shutter action, may be null.
         * @param raw       The callback for uncompressed data, may be null.
         ...
         *         android.hardware.Camera.PictureCallback)
         */
        public abstract void takePictureWithoutWaiting(
                Handler handler,
                CameraShutterCallback shutter,
                CameraPictureCallback raw,
                CameraPictureCallback postview,
        ...
        public abstract void burstShot(
                Handler handler,
                CameraShutterCallback shutter,
                CameraPictureCallback raw,
                CameraPictureCallback postview,
        ...
        public abstract void setGestureCallback(final Handler handler, final CameraGDCallBack cb);
        public abstract void startGestureDetection();
        public abstract void stopGestureDetection();

        public abstract void startRama(final Handler handler, int num);
        public abstract void stopRama(final Handler handler, int isMerge);
        public abstract void setRamaCallback(final Handler handler, final CameraPanoramaCallback cb);
        public abstract void setRamaMoveCallback(final Handler handler, final CameraPanoramaMoveCallback cb);


        /* MODIFIED-BEGIN by sichao.hu, 2016-08-16,BUG-2743263*/
        public abstract static class WaitDoneCallback{
            private Handler mHandler;
            public WaitDoneCallback(Handler handler){
                mHandler=handler;
            }
        ...
        public void waitDone(final WaitDoneCallback cb){
            try {
                getDispatchThread().runJob(new Runnable() {
                    @Override
                    public void run() {
                        getCameraHandler()
                                .obtainMessage(CameraActions.WAIT_DONE_DUMMY,cb)
                                .sendToTarget();
                    }});
            } catch (final RuntimeException ex) {
                getAgent().getCameraExceptionHandler().onDispatchThreadException(ex);
