...
import com.android.external.plantform.ExtStorage;
...
public class Storage {
    ...
    public static final long LOW_STORAGE_THRESHOLD_BYTES = 25*1024*1024L; // MODIFIED by fei.hui, 2016-09-09,BUG-2868515
    ...
    // If there is storage item in device settings, directory may
    // be changed, it should be got from the Environment.
    // Now use sdcard0/1 temporarily.
    private static void setupRootDirectory() {
        ...
        if (ExtBuild.device() == ExtBuild.MTK_MT6755) {
            //sdcardRootDirectory = "/storage/7EA1-11F8";
             /*MODIFIED-BEGIN by bin.zhang2-nb, 2016-03-29,BUG-1871416*/
            //sdcardRootDirectory = "/mnt/m_external_sd";
            sdcardRootDirectory = ExtStorage.getExternalStoragePath();
             /*MODIFIED-END by bin.zhang2-nb,BUG-1871416*/
            Log.d(TAG, "BZDB: [setupRootDirectory] sdcardRootDirectory=" + sdcardRootDirectory);
        }
    }

    public static void resetSDCardRootDirectory() {
        sdcardRootDirectory = null;
    ...
    private static void setupSDCardDirectory() {
        ...
        sdcardDirectory = sdcardRootDirectory + "/DCIM/Camera";
    }

    public static void setSavePath(String path) {
        isSDCardStorage = false;
        if (path != null && path.equals(SDCARD_STORAGE)) {
    ...
    public static boolean isSDCardAvailable() {
        return isSDCardMounted() && isSDDirectoryWriteable();
    }
    ...
    /**
     * get available storage according path value
     *
     * @param path ,if path is SDCARD_STORAGE , you should call isSDCardAvailable first.
     */
    public static long getSpecialAvailableSpace(String path) {
        if(path == null){
           return UNKNOWN_SIZE;
        }
        String DCIMPath = null;
        String DirectoryPath = null;
        if(path.equals(PHONE_STORAGE)){
            DCIMPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM).toString();
            DirectoryPath = DCIMPath + "/Camera";
        }else if(path.equals(SDCARD_STORAGE)){
            if (isSDCardAvailable()) {
                if(sdcardRootDirectory == null){
                    setupSDCardDirectory();
                }

                DCIMPath = sdcardRootDirectory + "/DCIM";
                DirectoryPath = DCIMPath + "/Camera";
            }else {
                return UNKNOWN_SIZE;
            }
        ...
        Log.e(TAG,"path = "+ path +",DCIMPath = " + DCIMPath + ",DirectoryPath =" + DirectoryPath);
