...
public class  FocusOverlayManager implements PreviewStatusListener.PreviewAreaChangedListener,
    ...
    public static final int RESET_TOUCH_FOCUS_DELAY_MILLIS = Settings3A.getFocusHoldMillis();
    // Hide focus ui 500ms after auto focus failed.
    public static final int RESET_TOUCH_FOCUS_FAILED_DELAY_MILLIS = 500;
    ...
    private static final int STATE_FAIL = 5; // Focus finishes and fails.
    ...
    public  interface FocusUI {
        ...
        void showMetering();
        void hideMetering();
        boolean isMeteringShowing();
        Point getMeteringPosition();
        void setMeteringPosition(int x, int y);
    }
...
//        mUI
    ...
    public void onPreviewAreaChanged(RectF previewArea) {
        ...
        mUI.setPreviewRect(mPreviewRect);
    }
    ...
    public void onShutterUp(CameraCapabilities.FocusMode currentFocusMode) {
        ...
        if (needAutoFocusCall(currentFocusMode)) {
            ...
            if (mState == STATE_FOCUSING || mState == STATE_SUCCESS||mState==STATE_SUCCESS_AND_STAY
                    || mState == STATE_FAIL) {
                cancelAutoFocus();
            }
        }
    ...
    public void focusAndCapture(CameraCapabilities.FocusMode currentFocusMode) {
        ...
        Log.w(TAG, "mState is " + mState);
        if (!needAutoFocusCall(currentFocusMode)) {
            // Focus is not needed.
            capture();
        } else if (mState == STATE_SUCCESS||mState==STATE_SUCCESS_AND_STAY || mState == STATE_FAIL||mState==STATE_LOCKED) {
    ...
    public static int ACTION_RESTORE_CAF_LATER =1<<1;
    public static int ACTION_KEEP_FOCUS_FRAME =1<<2;

    public void onAutoFocus(boolean focused , int action){
        boolean shutterButtonPressed=((action& ACTION_PRESS_SHUTTER)!=0);
        boolean needResetFocusLater=((action& ACTION_RESTORE_CAF_LATER)!=0);
        boolean needKeepFocusFrame=((action& ACTION_KEEP_FOCUS_FRAME)!=0);
        if (mState == STATE_FOCUSING_SNAP_ON_FINISH) {
            // Take the picture no matter focus succeeds or fails. No need
            // to play the AF sound if we're about to play the shutter
            // sound.
            if (focused) {
                mState = needKeepFocusFrame?STATE_SUCCESS_AND_STAY:STATE_SUCCESS;
            } else {
                mState = STATE_FAIL;
            }
            updateFocusUI();
            ...
            if (mFocusArea != null) {
                mFocusLocked = true;
                if(needResetFocusLater) {
                    mHandler.sendEmptyMessageDelayed(RESET_TOUCH_FOCUS, focused ?
                            RESET_TOUCH_FOCUS_DELAY_MILLIS : RESET_TOUCH_FOCUS_FAILED_DELAY_MILLIS);
                }
            }
    ...
    public void onSingleTapUp(int x, int y) {
        ...
        // touch position is not in the preview range
        if (x < mPreviewRect.left || x > mPreviewRect.right || y < mPreviewRect.top || y > mPreviewRect.bottom) {
            return;
        }

        // Let users be able to cancel previous touch focus.
        if ((mFocusArea != null) && (mState == STATE_FOCUSING ||
                    mState == STATE_SUCCESS||mState==STATE_SUCCESS_AND_STAY || mState == STATE_FAIL)) {
            cancelAutoFocus();
        }
        if (mPreviewRect.width() == 0 || mPreviewRect.height() == 0) {
            return;
    ...
    public void onMoving() {
        // When camera is AE/AF locked, cancelAutoFocus doesn't work.
        // So call onDeviceMoving first to reset the state.
        if (mListener != null) {
            mListener.onDeviceMoving();
        }
         if (mFocusLocked) {
    ...
    private void autoFocus(int focusingState) {
        ...
        Log.w(TAG,"update focus state to "+mState);
        mListener.autoFocus();
    ...
    public void updateFocusUI() {
        ...
        if (mState == STATE_IDLE) {
            ...
            if (mFocusMode == CameraCapabilities.FocusMode.CONTINUOUS_PICTURE) {
                ...
                mUI.onFocusSucceeded();
            } else if (mState == STATE_SUCCESS_AND_STAY) {
//                mUI.onFocusSucceeded();
                mUI.onFocusSucceededAndStay();
            }
    ...
    public boolean isFocusCompleted() {
        return mState == STATE_SUCCESS ||mState==STATE_SUCCESS_AND_STAY|| mState == STATE_FAIL;
    }
    ...
    private boolean needAutoFocusCall(CameraCapabilities.FocusMode focusMode) {
        return !(focusMode == CameraCapabilities.FocusMode.INFINITY
                ...
                || focusMode == CameraCapabilities.FocusMode.MANUAL);
    }


    // Update the metering area after dragging.
    public void setMeteringArea(Point pos) {
        if (pos == null) {
            mMeteringArea = null;
            return;
        }
        initializeMeteringAreas(pos.x, pos.y);
    }
}
