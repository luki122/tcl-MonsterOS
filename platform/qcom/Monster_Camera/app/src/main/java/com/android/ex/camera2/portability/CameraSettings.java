...
public abstract class CameraSettings {
    ...
    protected int mManualFocusPosition;

    protected int mContinuousIso;
    protected int mRotation; // MODIFIED by peixin, 2016-04-26,BUG-2000931
    protected String mIso;

    protected String mExposureTime;
    ...
    protected CameraSettings(CameraSettings src) {
        ...
        mManualFocusPosition = src.mManualFocusPosition;
        mContinuousIso = src.mContinuousIso;
        mRotation = src.mRotation; // MODIFIED by peixin, 2016-04-26,BUG-2000931
        mIso = src.mIso;
        mExposureTime = src.mExposureTime;
        isZslOn=src.isZslOn;
        isInstantAEC=src.isInstantAEC;
        mHsr = src.mHsr;
        mIsFaceBeauty=src.mIsFaceBeauty;
        /* MODIFIED-BEGIN by bin.zhang2-nb, 2016-04-26,BUG-1996450*/
        if (ExtBuild.device() != ExtBuild.MTK_MT6755) {
            mFaceBeautySkinSmoothing=src.mFaceBeautySkinSmoothing;
        } else {
            mFaceBeautySkinSmoothing=src.mFaceBeautySkinSmoothing;
            mFaceBeautySkinWhitening=src.mFaceBeautySkinWhitening;
        }
        /* MODIFIED-END by bin.zhang2-nb,BUG-1996450*/
        mIsLowLight=src.mIsLowLight;
        mIsSuperResolution = src.mIsSuperResolution;
        aec=src.aec;
        mAntibanding = src.mAntibanding;
        mIsMirrorSelfieOn = src.mIsMirrorSelfieOn;
        mCameraAisEnabled = src.mCameraAisEnabled; // MODIFIED by yuanxing.tan, 2016-05-10,BUG-2125215
    }
    ...
    /**
     * @param videoSize The size to use for preview.
     * @return Whether the operation was allowed (i.e. the sizes are unlocked).
     */
    public boolean setVideoSize(Size videoSize) {
        ...
        mCurrentVideoSize = new Size(videoSize);
        return true;
    }
    ...
    public void setSceneMode(CameraCapabilities.SceneMode sceneMode) {
        if(sceneMode!= CameraCapabilities.SceneMode.AUTO){ // MODIFIED by xuyang.liu, 2016-10-13,BUG-3110198
            mIsFaceBeauty=false;
            mIsLowLight=false;
        }
    ...
    public void setCameraAisEnabled(boolean enabled) {
        mCameraAisEnabled = enabled;
    }

    public boolean isCameraAisEnabled() {
        return mCameraAisEnabled;
    }
    ...
    public void setContinuousIso(int continuousIso) {
        mContinuousIso = continuousIso;
    }

    public int getContinuousIso() {
        return mContinuousIso;
    }
    ...
    public void setManualFocusPosition(int focusMode) {
        mManualFocusPosition  = focusMode;
    }

    public int getManualFocusPosition() {
        return mManualFocusPosition;
    }
    ...
    public boolean isInstantAEC=false;
    ...
    private int mFaceBeautySkinWhitening; // MODIFIED by bin.zhang2-nb, 2016-04-26,BUG-1996450
    ...
    public boolean getFaceBeauty(){
        return mIsFaceBeauty;
    }

    public int getFaceBeautySkinSmoothing(){
        return mFaceBeautySkinSmoothing;
    }

    /* MODIFIED-BEGIN by bin.zhang2-nb, 2016-04-26,BUG-1996450*/
    public int getFaceBeautySkinWhitening() {
        return mFaceBeautySkinWhitening;
    }
    /* MODIFIED-END by bin.zhang2-nb,BUG-1996450*/
    ...
    public void setLowLight(boolean isLowLightOn){
        ...
        mIsLowLight=isLowLightOn;

    }

    public void setFaceBeauty(boolean isFaceBeautyOn, int skinSmoothing){
        if(isFaceBeautyOn){
            mFaceBeautySkinSmoothing = skinSmoothing;
            mIsLowLight=false;
            setSceneMode(CameraCapabilities.SceneMode.AUTO);
        }
        mIsFaceBeauty=isFaceBeautyOn;
    }

    /* MODIFIED-BEGIN by bin.zhang2-nb, 2016-04-26,BUG-1996450*/
    public void setFaceBeauty(boolean isFaceBeautyOn, int skinSmoothing, int skinWhitening) {
        if(isFaceBeautyOn){
            mFaceBeautySkinSmoothing = skinSmoothing;
            mFaceBeautySkinWhitening = skinWhitening;
            mIsLowLight=false;
            setSceneMode(CameraCapabilities.SceneMode.AUTO);
        }
        mIsFaceBeauty=isFaceBeautyOn;
    ...
    public void setFaceBeautySmoothing(boolean isFaceBeautyOn, int skinSmoothing) {
        if(isFaceBeautyOn){
            mFaceBeautySkinSmoothing = skinSmoothing;
            mIsLowLight=false;
            setSceneMode(CameraCapabilities.SceneMode.AUTO);
    ...
    public void setFaceBeautyWhitening(boolean isFaceBeautyOn, int skinWhitening) {
        if(isFaceBeautyOn){
            mFaceBeautySkinWhitening = skinWhitening;
            System.out.println("BZDB: "+this+"+++++> visidon: mFaceBeautySkinWhitening = " + mFaceBeautySkinWhitening);
            mIsLowLight=false;
            setSceneMode(CameraCapabilities.SceneMode.AUTO);
        }
        mIsFaceBeauty=isFaceBeautyOn;
    ...
    public void setSuperResolutionOn(boolean isSuperResolutionOn) {
        mIsSuperResolution=isSuperResolutionOn;
    }
    public boolean isSuperResolutionOn() {
        return mIsSuperResolution;
    }
    //customize mode off
    public int aec=0;
    ...
    public boolean getMirrorSelfieOn(){
        return mIsMirrorSelfieOn;
    }

    public void setMirrorSelfieOn(boolean isMirror){
        mIsMirrorSelfieOn = isMirror;
    }
