...
public class Call implements CreateConnectionResponse {
    ...
    private class QueryHandler extends AsyncQueryHandler {
        ...
        public QueryHandler(Context context, String phoneNumber) {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            while (cursor.moveToNext()) {
                ...
                if (mPhoneNumber.equals(mNumber)) {
                    mCallerInfo.name = mName;
                    mFdnMissedCallName = mName;
                    mDitor.putString(mLastFdnMissedCallName,mName);
                    mDitor.putString(mLastFdnMissedCallNumber,mNumber);
                    mDitor.commit();
                    break;
                }
            }
    ...
    private String mFdnMissedCallName;
    private String mFdnMissedCallNumber;
    //[SOLUTION]-Add-END by TCTNB.(Caixia Chen)
    ...
    public void setState(int newState, String tag) {
        if (mState != newState) {
            ...
            if (mState == CallState.ACTIVE || mState == CallState.ON_HOLD) {
                ...
                //add by lgy for 3322284
                mDisconnectClockTimeMillis = 0;
            } else if (mState == CallState.DISCONNECTED) {
                mDisconnectTimeMillis = System.currentTimeMillis();
                //add by lgy for 3322284
                mDisconnectClockTimeMillis = SystemClock.elapsedRealtime();
    ...
    //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/11/2016, SOLUTION-2464116 And TASk-2706864
    ...
    public long getAgeMillis() {
        if (mState == CallState.DISCONNECTED &&
            ...
            //modify by lgy for 3322284
//            return System.currentTimeMillis() - mConnectTimeMillis;
            return SystemClock.elapsedRealtime() - mConnectClockTimeMillis;
        }

        //modify by lgy for 3322284
//        return mDisconnectTimeMillis - mConnectTimeMillis;
        return mDisconnectClockTimeMillis - mConnectClockTimeMillis;
    }
    ...
    private void setCallerInfo(Uri handle, CallerInfo callerInfo) {
        ...
        //[SOLUTION]-Add-BEGIN by TCTNB.(Caixia Chen), 08/23/2016, SOLUTION-2504488
        //Add by jie.cheng for Defect 3243890 begin
        if ((mCallerInfo!=null)&&mCallerInfo.name == null && mCallerInfo.phoneNumber != null) {
        //Add by jie.cheng for Defect 3243890 end
            final TelephonyManager telephonyManager = (TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE);
            final SubscriptionManager subscriptionManager = SubscriptionManager.from(mContext);
            boolean  startThePhone = msharedPreferences.getBoolean(isStartThePhone,false);
            mDitor.putBoolean(isStartThePhone,false).commit();
            Uri mFdnUri = null;
            String mPhoneNumber = mCallerInfo.phoneNumber.replace(" ", "").replace("-", "").replace("(", "").replace(")", "");
            if (telephonyManager.getDefault().isMultiSimEnabled()) {
                final TelecomManager mTelecomManager = TelecomManager.from(mContext);
                PhoneAccount mPhoneAccount = mTelecomManager.getPhoneAccount(mTargetPhoneAccountHandle);
                int subId = telephonyManager.getSubIdForPhoneAccount(mPhoneAccount);

                int phoneId = subscriptionManager.getPhoneId(subId);
                boolean isFdnEnable = telephonyManager.getIccFdnEnabled(phoneId);
                if (isFdnEnable && !startThePhone) {
                    mFdnUri = Uri.withAppendedPath(Uri.parse("content://icc/fdn"), "subId/"+ subId);
                    new QueryHandler(mContext,mPhoneNumber).startQuery(1, null, mFdnUri, null, null, null, null);
                } else {
                    mFdnMissedCallName = msharedPreferences.getString(mLastFdnMissedCallName,null);
                    mFdnMissedCallNumber = msharedPreferences.getString(mLastFdnMissedCallNumber,null);
                    if (mCallerInfo.phoneNumber.equals(mFdnMissedCallNumber) && mFdnMissedCallName != null) {
                        mCallerInfo.name = mFdnMissedCallName;
                    }
                    for (Listener l : mListeners) {
                ...
                int mDefualtPhoneId = subscriptionManager.getPhoneId(subscriptionManager.getDefaultSubscriptionId());
                if (telephonyManager.getIccFdnEnabled(mDefualtPhoneId) && !startThePhone ) {
                    mFdnUri = Uri.parse("content://icc/fdn");
                    new QueryHandler(mContext,mPhoneNumber).startQuery(1, null, mFdnUri, null, null, null, null);
                } else {
                   mFdnMissedCallName = msharedPreferences.getString(mLastFdnMissedCallName,null);
                   mFdnMissedCallNumber = msharedPreferences.getString(mLastFdnMissedCallNumber,null);
    ...
    //add by lgy for 3322284
    private long mConnectClockTimeMillis = 0;
    private long mDisconnectClockTimeMillis = 0;
    private void setConnectClockTime() {
        Bundle extras = new Bundle();
        mConnectClockTimeMillis = SystemClock.elapsedRealtime();
        extras.putLong("connect_clock_time", SystemClock.elapsedRealtime());
        putExtras(Call.SOURCE_CONNECTION_SERVICE, extras);
    }
}
