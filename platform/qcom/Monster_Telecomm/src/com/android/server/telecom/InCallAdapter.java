...
class InCallAdapter extends IInCallAdapter.Stub {
    ...
    private static final int MSG_ANSWER_CALL_END_ACTIVE_WHEN_TWO_CALLS = 20;
    private static final int MSG_ANSWER_CALL_END_HOLD_WHEN_THREE_CALLS = 21;
    private static final int MSG_ANSWER_CALL_END_ACTIVE_WHEN_THREE_CALLS = 22;
    ...
    //[BUGFIX]-Mod-BEGIN by TCTNB.yubin.ying,09/01/2016,2828163,
    public void answerAndEndActiveWhenTwoCalls(String callId, int videoState) {
        Log.d(this, "answerAndEndActiveWhenTwoCalls(%s,%d)", callId, videoState);
        if (callId != null) {
            SomeArgs args = SomeArgs.obtain();
            args.arg1 = callId;
            args.arg2 = videoState;
            mHandler.obtainMessage(MSG_ANSWER_CALL_END_ACTIVE_WHEN_TWO_CALLS, args).sendToTarget();
        }
    }

    public void answerAndEndHoldWhenThreeCalls(String callId, int videoState) {
        Log.d(this, "answerAndEndHoldWhenThreeCalls(%s,%d)", callId, videoState);
        if (callId != null) {
            SomeArgs args = SomeArgs.obtain();
            args.arg1 = callId;
            args.arg2 = videoState;
            mHandler.obtainMessage(MSG_ANSWER_CALL_END_HOLD_WHEN_THREE_CALLS, args).sendToTarget();
        }
    }

    public void answerAndEndActiveWhenThreeCalls(String callId, int videoState) {
        Log.d(this, "answerAndEndActiveWhenThreeCalls(%s,%d)", callId, videoState);
        if (callId != null) {
            SomeArgs args = SomeArgs.obtain();
            args.arg1 = callId;
            args.arg2 = videoState;
            mHandler.obtainMessage(MSG_ANSWER_CALL_END_ACTIVE_WHEN_THREE_CALLS, args).sendToTarget();
        }
    }
    private final class InCallAdapterHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            Call call;
            switch (msg.what) {
                // liujia for dialer ergo RR890085 14-12-30 sta
                case MSG_ANSWER_CALL_END_ACTIVE_WHEN_TWO_CALLS: {
                    SomeArgs args = (SomeArgs) msg.obj;
                    try {
                        call = mCallIdMapper.getCall(args.arg1);
                        int videoState = (int) args.arg2;
                        if (call != null) {
                            mCallsManager.answerAndEndActiveWhenTwoCalls(call, videoState);
                        } else {
                            Log.w(this, "answerAndEndActiveWhenTwoCalls, unknown call id: %s", msg.obj);
                        }
                    } finally {
                        args.recycle();
                    }
                ...
                case MSG_ANSWER_CALL_END_HOLD_WHEN_THREE_CALLS: {
                    SomeArgs args = (SomeArgs) msg.obj;
                    try {
                        call = mCallIdMapper.getCall(args.arg1);
                        int videoState = (int) args.arg2;
                        if (call != null) {
                            mCallsManager.answerAndEndHoldWhenThreeCalls(call, videoState);
                        } else {
                            Log.w(this, "answerAndEndHoldWhenThreeCalls, unknown call id: %s", msg.obj);
                        }
                    } finally {
                        args.recycle();
                    }
                ...
                case MSG_ANSWER_CALL_END_ACTIVE_WHEN_THREE_CALLS: {
                    SomeArgs args = (SomeArgs) msg.obj;
                    try {
                        call = mCallIdMapper.getCall(args.arg1);
                        int videoState = (int) args.arg2;
                        if (call != null) {
                            mCallsManager.answerAndEndActiveWhenThreeCalls(call, videoState);
                        } else {
                            Log.w(this, "answerAndEndActiveWhenThreeCalls, unknown call id: %s", msg.obj);
                        }
                    } finally {
                        args.recycle();
                    }
