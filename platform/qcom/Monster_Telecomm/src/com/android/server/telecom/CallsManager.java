...
import com.mst.privacy.ManagePrivacy;
import com.mst.privacy.PrivacyMissedCallNotifierImpl;
...
//[SOLUTION]-Add-BEGIN by TCTNB.(Caixia Chen), 08/10/2016, SOLUTION-2487026
import android.widget.Toast;
...
public class CallsManager extends Call.ListenerBase
    ...
    private static final int MAXIMUM_CONFERENCE_CHILDREN_CALLS = 5;//[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/11/2016, SOLUTION-2461449
    ...
    //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/17/2016, SOLUTION- 2470880 And TASk-2720022
    //Porting Dialer not presented when calling using #31#
    private static boolean isPotentialCLIRMMICode = false;
    ...
    CallsManager(
        ...
        //add by lgy 
        mPrivacyMissedCallNotifier = new PrivacyMissedCallNotifierImpl(mContext, mPhoneAccountRegistrar);
    ...
    public void onSuccessfulOutgoingCall(Call call, int callState) {
        ...
        //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/17/2016, SOLUTION- 2470880 And TASk-2720022
        //Porting Dialer not presented when calling using #31#
        if(isPotentialCLIRMMICode){
            for (CallsManagerListener listener : mListeners) {
                listener.onCallAdded(call);
            }
            updateCallsManagerState();
            isPotentialCLIRMMICode = false;
        }
        //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
    ...
    Call startOutgoingCall(Uri handle, PhoneAccountHandle phoneAccountHandle, Bundle extras,
            UserHandle initiatingUser, Intent intent) {
		...
		if(accounts.size() > 1) {
	        ...
	        Log.i(this, "slot = " + slot);
            if(slot > -1) {
                PhoneAccountHandle mstPah = getPhoneAccountHandleBySlot(mContext, slot);
                Log.i(this, "PhoneAccountHandle = " + mstPah);
                if(mstPah != null) {
                    phoneAccountHandle = mstPah;
                }
            } else {
                if(!call.isEmergencyCall()) {
                    phoneAccountHandle = null;
        ...
        if (!isPotentialInCallMMICode && (!isReusedCall &&
            ...
            //[SOLUTION]-Add-BEGIN by TCTNB.(Caixia Chen), 08/10/2016, SOLUTION-2487026
            Toast.makeText(mContext,R.string.incall_error_call_failed,Toast.LENGTH_SHORT).show();
            //[SOLUTION]-Add-END by TCTNB.(Caixia Chen)
            return null;
        }
        ...
        //[SOLUTION]-Add-BEGIN by TCTNB.(Yubin.Ying), 08/08/2016, SOLUTION-2496835
        boolean isCTSCall = false;
        if (phoneAccountHandle != null &&
                phoneAccountHandle.getComponentName() != null &&
                "android.telecom.cts.CtsConnectionService".equals(phoneAccountHandle.getComponentName().getClassName())) {
            isCTSCall = true;
            android.util.Log.i("CallsManager", "startOutgoingCall, isCTSCall: " + isCTSCall);
        }
        //[SOLUTION]-Add-END by TCTNB.(Yubin.Ying)

        // Do not add the call if it is a potential MMI code.
        if (!isCTSCall && (isPotentialMMICode(handle) || isPotentialInCallMMICode) && !needsAccountSelection) {
            //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/17/2016, SOLUTION- 2470880 And TASk-2720022
            //Porting Dialer not presented when calling using #31#
            if(isPotentialMMICodeCLIR(handle)){
                 isPotentialCLIRMMICode = true;
            } else {
                 isPotentialCLIRMMICode = false;
            }
            //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
    ...
    public void placeOutgoingCall(Call call, Uri handle, GatewayInfo gatewayInfo,
        ...
        if (call.isEmergencyCall()) {
            ...
            //call.setTargetPhoneAccount(null); // MODIFIED by qili.zhang, 2016-09-09,Porting BUG-2044385
    ...
    //[BUGFIX]-Mod-BEGIN by TCTNB.yubin.ying,09/01/2016,2828163,
    /**
     * When there are two calls, one active call and one waiting call,
     * end the active one and answer the waiting one.
     *
     * @param call The call to answer.
     * @param videoState The video state in which to answer the call.
     */
    void answerAndEndActiveWhenTwoCalls(Call call, int videoState) {
        if (!mCalls.contains(call)) {
            Log.i(this, "Request to answer a non-existent call %s", call);
        } else {
              Log.i(this, "end active and answer waiting when two calls");
              Call activeCall = getFirstCallWithState(call.getTargetPhoneAccount()
                      .getId(), CallState.ACTIVE, CallState.DIALING);
              Call holdCall = getFirstCallWithState(call
                      .getTargetPhoneAccount().getId(), CallState.ON_HOLD);
    ...
    /**
     * When there are three calls, one active call, one hold call and one waiting call
     * end the hold one, hold the active one and answer the waiting one.
     *
     * @param call The call to answer.
     * @param videoState The video state in which to answer the call.
     */
    public void answerAndEndHoldWhenThreeCalls(Call call, int videoState) {
        if (!mCalls.contains(call)) {
            Log.i(this, "Request to answer a non-existent call %s", call);
        } else {
              Log.i(this, "end hold and answer waiting when three calls");
              ...
              Call holdCall = getFirstCallWithState(call
               ...
               //Check for existence of active call & held
              if(activeCall != null && holdCall != null) {
                  Log.i(this, "Disconnect hold call");
                  holdCall.disconnect();
                  holdCall = null;
              }
              ...
              if (activeCall != null && activeCall != call &&
                      (activeCall.isActive() ||
                       activeCall.getState() == CallState.DIALING)) {
                  if (0 == (activeCall.getConnectionCapabilities() & android.telecom.Call.Details.CAPABILITY_HOLD)) {
                      // This call does not support hold.  If it is from a different connection
                      // service, then disconnect it, otherwise allow the connection service to
                      // figure out the right states.
                      if (activeCall.getConnectionService() != call.getConnectionService()) {
                      ...
                      Log.i(this, "before answering incoming call %s.",
                              call);
                      activeCall.hold();
                  }
                  // TODO: Wait until we get confirmation of the active call being
    ...
    /**
     * When there are three calls, one active call, one hold call and one waiting call
     * end the the active one and answer the waiting one.
     *
     * @param call The call to answer.
     * @param videoState The video state in which to answer the call.
     */
    public void answerAndEndActiveWhenThreeCalls(Call call, int videoState) {
        if (!mCalls.contains(call)) {
            Log.i(this, "Request to answer a non-existent call %s", call);
        } else {
            Call activeCall = getFirstCallWithState(call.getTargetPhoneAccount()
                    .getId(), CallState.ACTIVE, CallState.DIALING);
            //Check for existence of active call & held
            if ((activeCall != null) && (getFirstCallWithState(call
    ...
    boolean onMediaButton(int type) {
        if (hasAnyCalls()) {
            ...
            } else if (HeadsetMediaButton.DOUBLE_PRESS == type&&mCalls.size()>1) {
                Log.d(this, "handleHeadsetHook: double click -> SWITCH CALL");
    ...
    boolean canAddCall() {
        ...
        for (Call call : mCalls) {
            ...
            //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/11/2016, SOLUTION-2461449
            if (call.isConference() && (call.getChildCalls().size() >= MAXIMUM_CONFERENCE_CHILDREN_CALLS)
                    && (count >= MAXIMUM_TOP_LEVEL_CALLS)) {
                Log.d(this, "canAddCall(): have reached the maximum conference calls. Cannot add call");
                return false;
            }
            //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
        }
    ...
    Call createConferenceCall(
        ...
        setCallState(call, Call.getStateFromConnectionState(parcelableConference.getState()),
                "new conference call");
        //[SOLUTION]-Add-BEGIN by TCTNB.(Caixia Chen), 08/09/2016, SOLUTION-2487113
        if (call.getState() == CallState.ACTIVE) {
            long tempConnectTimeMillis = Long.MAX_VALUE;
            for (Call tempCall : mCalls) {
                if (tempCall.getConnectTimeMillis() > 0) {
                    connectTime = Math.min(tempCall.getConnectTimeMillis(), tempConnectTimeMillis);
                    tempConnectTimeMillis = connectTime;
                }
            }
        }
        call.setConnectTimeMillis(connectTime);
        //[SOLUTION]-Add-END by TCTNB.(Caixia Chen)
    ...
    //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/17/2016, SOLUTION- 2470880 And TASk-2720022
    //Porting Dialer not presented when calling using #31#
    private boolean isPotentialMMICodeCLIR(Uri handle) {
        return (handle != null && handle.getSchemeSpecificPart() != null
                && (handle.getSchemeSpecificPart().contains("#31#")
                        || handle.getSchemeSpecificPart().contains("*31#")));
    }
    //[SOLUTION]-Add-END by TCTNB.(Chuanjun Chen)
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 08/12/2016, SOLUTION-2481326
//GPR][UK-WIND][CALL][USSD]22 Send is being recognized as a USSD code
            if("22".equals(dialedNumber)
                    && mContext.getResources().getBoolean(com.android.internal.R.bool.def_phone_judge_22_mmicode)){
                return false;
            }
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
        ...
        if (hasMaximumLiveCalls()) {
            ...
            if (hasMaximumOutgoingCalls()) {
                ...
                if (isEmergency && !outgoingCall.isEmergencyCall()) {
                    ...
                    //add by lgy for 3472510
                    handleEmergencyCallAbnormal();
                    return true;
                }
    ...
    private void maybeMoveToEarpiece(Call call) {
        if (!call.getStartWithSpeakerphoneOn() && !mWiredHeadsetManager.isPluggedIn() &&
                !mBluetoothManager.isBluetoothAvailable()) {
            //[SOLUTION]-Add-BEGIN by TCTNB.(Chuanjun Chen), 08/23/2016, SOLUTION- 2496815 And TASk-2781054
            //The activated speaker turn off when add a new call via shortcut,contacts,phone,messages and so on
            boolean isSpeakerOn = false;
            if(getAudioState() != null){
                isSpeakerOn = getAudioState().getRoute() == CallAudioState.ROUTE_SPEAKER;
            }
            if(!isSpeakerOn){
    ...
    void onUserSwitch(UserHandle userHandle) {
        ...
        //add by lgy 
        mPrivacyMissedCallNotifier.setCurrentUserHandle(userHandle);
        final UserManager userManager = UserManager.get(mContext);
    ...
    */
    ...
    //add by lgy
    private PhoneAccountHandle getPhoneAccountHandleBySlot(Context context, int phoneId) {
        if (!SubscriptionManager.isValidPhoneId(phoneId)) {
            return null;
        }
        String subId = String.valueOf(getSubIdbySlot(context, phoneId));
    ...
    private int getSubIdbySlot(Context ctx, int slot) {  
    ...
    //add by lgy for 3472510
    private void handleEmergencyCallAbnormal() {
        mLchHandler.postDelayed(new java.lang.Runnable() {
            public void run() {
                if(mPendingMOEmerCall != null) {
                    clearPendingMOEmergencyCall();
                } 
    ...
    public PrivacyMissedCallNotifierImpl mPrivacyMissedCallNotifier;
    
    public void reloadPrivacyMissedCallsOfUser(UserHandle userHandle) {
        mPrivacyMissedCallNotifier.reloadFromDatabase(
                mLock, this, mContactsAsyncHelper, mCallerInfoAsyncQueryFactory, userHandle);
    }
