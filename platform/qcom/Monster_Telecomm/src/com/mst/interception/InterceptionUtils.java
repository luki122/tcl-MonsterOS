...
public class InterceptionUtils {  
    ...
    public static boolean isToAddBlack(Context context, String number) {
		...
		privateData = PrivacyDataUtils.getPrivateData(number);
    ...
    public static boolean isNeedHangup(Context context, String number) {
...
//    	}
		...
		if(privateId != currentPrivateId && privateId > 0 && private_noti_type == 1) {
	    	Log.v(TAG, "isNeedHangup true "); 
	        ManagePrivacy.getInstance().notificationMgr.notifyHangupPrivateRingingCallFake(privateId);
	    	result = true;
		} else if(privateId == 0 && InterceptionUtils.isBlackNumber(context, number)) {            
...
//		mCache.put(number, result);
		Log.v("isNeedHangup", "result = " + result );
		return result;
    }
