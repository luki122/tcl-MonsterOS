...
//		checkButtonEnable(bean, baseHolder);
		checkButtonEnableDelay(bean, baseHolder);	// 直接在getView设置会失效,因此延时一点时间在Handler执行
		...
		case TYPE_DOWNLOADED:
			if (bean.getDownloadData() == null) {
				...
				if (installedAppInfo != null
					...
					if (bean.getDownloadStatus() == AppDownloader.STATUS_INSTALLING) {
						...
						String id = downloadedHolder.tv_status.getTag() == null ?
								null : (String) downloadedHolder.tv_status.getTag();

						if (id != null && id.equals(bean.getDownloadData().getTaskId()) &&
								bean.getDownloadStatus() == AppDownloader.STATUS_INSTALLED
								&& downloadedHolder.tv_status.getText().equals(managerActivity
								.getString(R.string.downloadman_installing))) {
			...
			downloadedHolder.tv_status.setTag(bean.getDownloadData().getTaskId());
			break;
		}
	...
	/**
	 * 直接在getView设置会失效,因此延时一点时间在Handler执行
	 * @param bean
	 * @param holder
     */
	private void checkButtonEnableDelay(final DownloadManagerBean bean, final BaseHolder holder) {
		...
		if (managerActivity != null) {
			managerActivity.postHandlerDelay(runnable, 50);
		}
	}
