...
import com.monster.launcher.util.UiThreadCircularReveal;
...
public class LauncherStateTransitionAnimation {
    ...
    boolean beingAnimaFromOverViewToWorkspace = false;
    boolean beingAnimaBetweenOverViewAndWidgets = false;
    ...
    public void startAnimationToAllApps(final Workspace.State fromWorkspaceState,
        ...
        //modify by xiangzx
         View buttonView = mLauncher.getAllAppsButton();
        if(LauncherAppState.getInstance().isShowAllAppsInWorkspace()){
            buttonView = mLauncher.getAppSearchView();
        }
...
//        final WidgetsContainerView toView = mLauncher.getWidgetsView();
        final View toView = mLauncher.getWidgetsPanel();
...
//                toView.getRevealView(), null, animated, cb);
        mCurrentAnimation = startAnimationToOverlay(fromWorkspaceState,
                Workspace.State.OVERVIEW_HIDDEN, buttonView, toView, toView,
                toView, null, animated, cb);
    }
    /**
    ...
    public void startAnimationToWorkspace(final Launcher.State fromState,
        ...
        if (toWorkspaceState != Workspace.State.NORMAL &&
                ...
                toWorkspaceState != Workspace.State.OVERVIEW && toWorkspaceState!= Workspace.State.NORMAL_DRAG) {
            Log.e(TAG, "Unexpected call to startAnimationToWorkspace");
        }
        if (fromState == Launcher.State.APPS || fromState == Launcher.State.APPS_SPRING_LOADED) {
            startAnimationToWorkspaceFromAllApps(fromWorkspaceState, toWorkspaceState, toWorkspacePage, animated,
                     onCompleteRunnable);
        } else {
    ...
    private void startAnimationToWorkspaceFromAllApps(final Workspace.State fromWorkspaceState,
        ...
        mCurrentAnimation = startAnimationToWorkspaceFromOverlay(fromWorkspaceState, toWorkspaceState,
                                //modify by xiangzx
                toWorkspacePage,  buttonView//mLauncher.getAllAppsButton()
                , appsView, appsView.getContentView(),
    ...
    private void startAnimationToWorkspaceFromWidgets(final Workspace.State fromWorkspaceState,
                                                      final Workspace.State toWorkspaceState, final int toWorkspacePage,
                                                      final boolean animated, final Runnable onCompleteRunnable) {
//        final WidgetsContainerView widgetsView = mLauncher.getWidgetsView();//lijun modify
        final View widgetsView = mLauncher.getWidgetsPanel();
        PrivateTransitionCallbacks cb = new PrivateTransitionCallbacks() {
            @Override
            float getMaterialRevealViewFinalAlpha(View revealView) {
                return 0.3f;
...
//                onCompleteRunnable, cb);
        mCurrentAnimation = startAnimationToWorkspaceFromOverlay(fromWorkspaceState,
                toWorkspaceState, toWorkspacePage, mLauncher.getWidgetsButton(), widgetsView,
                widgetsView, widgetsView, null, animated,
                onCompleteRunnable, cb);
    }
     ...
     */
    public AnimatorSet startAnimationBetweenOverviewAndWidgets(final Workspace.State toWorkspaceState,final boolean animated) {
        final AnimatorSet animation = LauncherAnimUtils.createAnimatorSet();
        final boolean toWidgets = (toWorkspaceState == Workspace.State.OVERVIEW_HIDDEN);

        final View fromView = toWidgets?mLauncher.getOverviewPanel():mLauncher.getWidgetsPanel();
        final View toView = toWidgets?mLauncher.getWidgetsPanel():mLauncher.getOverviewPanel();
        final HashMap<View, Integer> layerViews = new HashMap<>();
        final int finalFromViewTranslationY = mLauncher.getWidgetsPanel().getHeight()/2;
        final int finalToViewTranslationY = 0;
        // If for some reason our views aren't initialized, don't animate
        boolean initialized = mLauncher.getWidgetsButton() != null;
        ...
        toViewAnima.translationY(finalToViewTranslationY);

        fromViewAnima.addListener(new AlphaUpdateListener(fromView,false));
        toViewAnima.addListener(new AlphaUpdateListener(toView,false));
        ...
        if (layerViews != null) {
            ...
            fromViewAnima.withLayer();
            toViewAnima.withLayer();
        }
        ...
        if (animated && initialized) {
            ...
            if(toWidgets){
                toView.setTranslationY(mLauncher.getWidgetsPanel().getHeight()/2);
            ...
            toView.setTranslationX(0.0f);
            toView.setTranslationY(finalToViewTranslationY);
            toView.setScaleX(1.0f);
            toView.setScaleY(1.0f);
            toView.setVisibility(View.VISIBLE);
            toView.bringToFront();
...
//            pCb.onTransitionComplete();
    ...
    private AnimatorSet startAnimationToWorkspaceFromOverlay(final Workspace.State fromWorkspaceState,
        ...
        if (animated && initialized) {
            beingAnimaFromOverViewToWorkspace = true;
        }
        ...
        if (animated && initialized) {
            ...
            animation.addListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    beingAnimaFromOverViewToWorkspace = false;
