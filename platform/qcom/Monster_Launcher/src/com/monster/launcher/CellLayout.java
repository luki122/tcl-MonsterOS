...
import com.monster.launcher.BubbleTextView.BubbleTextShadowHandler;
import com.monster.launcher.FolderIcon.FolderRingAnimator;
import com.monster.launcher.accessibility.DragAndDropAccessibilityDelegate;
import com.monster.launcher.accessibility.FolderAccessibilityHelper;
import com.monster.launcher.accessibility.WorkspaceAccessibilityHelper;
...
import com.monster.launcher.Hotseat.HotseatDragState;

public class CellLayout extends ViewGroup implements BubbleTextShadowHandler , IChangeColors.IItemsColorChange {
    ...
    //add by xiangzx
    ArrayList<CellAndSpan> widgetCellAndSpanList = new ArrayList<CellAndSpan>();
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        isVertical = mLauncher.getDeviceProfile().isVerticalBarLayout();
...
/*        if (mFolderLeaveBehindCell[0] >= 0 && mFolderLeaveBehindCell[1] >= 0) {
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (lp.cellX >= 0 && lp.cellX <= mCountX - 1 && lp.cellY >= 0 && lp.cellY <= mCountY - 1) {
            ...
            WindowGlobalValue windowGlobalValue = appState.getWindowGlobalVaule();
            int color = -1;
            if (tag != null&&tag instanceof ItemInfo) {
                ItemInfo info = (ItemInfo)tag;
                if (this.getParent() instanceof FolderPagedView) {
                    color = windowGlobalValue.getTextExtraColor();
                } else {
                    color = windowGlobalValue.getTextColor();
                }
            }
            if(child instanceof BubbleTextView){
                ((BubbleTextView)child).setTextColor(color);
            }
    ...
    void cellToPoint(int cellX, int cellY, int[] result) {
        ...
        if(isVerticalBarLayout()){
            ...
            result[1] = vStartPadding + cellY * (mCellHeight + mHeightGap)+mHeightGap;
        }else{
            result[0] = hStartPadding + cellX * (mCellWidth + mWidthGap)+mWidthGap;
     ...
     */
     void regionToRect(int cellX, int cellY, int spanX, int spanY, Rect result) {
        ...
        final int vStartPadding = getPaddingTop();
         ...
         int top = 0;
         if(isVerticalBarLayout()){
              left = hStartPadding + cellX * (mCellWidth + mWidthGap);
              top = vStartPadding + cellY * (mCellHeight + mHeightGap)+mHeightGap;
         }else{
              left = hStartPadding + cellX * (mCellWidth + mWidthGap)+mWidthGap;
              top = vStartPadding + cellY * (mCellHeight + mHeightGap);
         }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isHotseat()&&LauncherAppState.getInstance().isShowAllAppsInWorkspace()) {
            mWidthGap = mDynamicWidthGap;
            mHeightGap =mDynamicHeightGap;
            mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight, mWidthGap,
                    mHeightGap, mCountX, mCountY);
        } else {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        /*mShortcutsAndWidgets.layout(left, top,
                left + r - l,
                top + b - t);*/
        final LayoutParams lp =  (LayoutParams) mShortcutsAndWidgets.getLayoutParams();
        if (lp.customPosition) {
            mShortcutsAndWidgets.layout(lp.x, lp.y, lp.x + lp.width, lp.y + lp.height);
        }else {
    ...
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration,
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            Log.v("reorder", "info.title="+info.title+", lp.cellX="+lp.cellX+", lp.cellY="+lp.cellY+", ---> cellX="+cellX+", cellY="+cellY);
            ...
            if (oldX == newX && oldY == newY) {
                Log.v("reorder", "Exit early----------newX="+newX+", newY="+newY);
                lp.isLockedToGrid = true;
                return true;
            }
            ...
            va.addUpdateListener(new AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    Log.v("reorder", "onAnimationUpdate--------");
                    child.requestLayout();
                }
            });
            va.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    // place just yet.
                    Log.v("reorder", "onAnimationEnd--------cancelled="+cancelled+",   lp.cellX="+lp.cellX+",lp.cellY="+lp.cellY);
                    if (!cancelled) {
                        lp.isLockedToGrid = true;
                        child.requestLayout();
                    }
                ...
                public void onAnimationCancel(Animator animation) {
                    cancelled = true;
                    Log.v("reorder", "onAnimationCancel");
                }
            });
    ...
    private int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX,
        ...
        if (mLauncher.isHotseatLayout(this)) {
                ...
                if(HotseatDragState.IN.equals(mLauncher.getHotseat().getHotseatDragState())) {
                    childcount=mVisibleCount;
                }
                if(childcount==0) {
                    childcount+=1;
                }
                if (countX > childcount&&childcount!=0) {
                    if(isVerticalBarLayout()){
                        countY = childcount;
                    }else{
                        countX = childcount;
        ...
        Log.i(Workspace.TAG,"countX = "+countX);
    ...
    private boolean pushViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
        ...
        int countX = mCountX;
        int countY = mCountY;
        if(mLauncher.isHotseatLayout(this)&&HotseatDragState.NONE.equals(mDragPushState)) {
            if(isVerticalBarLayout()){
                countX = mCountX;
                countY=getShortcutsAndWidgets().getChildCount();
            }else{
                countX = getShortcutsAndWidgets().getChildCount();
                countY=mCountY;
            }
        }
        ...
        //add by xiangzx to push icon by row
     if(!isWidget()) {
         if (whichEdge == ViewCluster.LEFT) {
             //order by row desc
             Collections.sort(currentState.sortedViews, new Comparator<View>() {
                 @Override
        ...
        if (!fail && clusterRect.left >= 0 && clusterRect.right <= countX && clusterRect.top >= 0 &&
                clusterRect.bottom <= countY) {
            foundSolution = true;
        } else {
    ...
    private void pushIconByRow(CellAndSpan c, int countX, int countY, int whichEdge){
        if (whichEdge == ViewCluster.LEFT) {
            ...
            if (c.x == 0 && c.y - c.spanY >= 0) {
                c.y = c.y - c.spanY;
                c.x = countX;
            }
            int result = isCellInLauncherAppWidget(c.x - 1, c.y, ViewCluster.LEFT);
            if (result != -1) {
                c.x = result;
                while(c.x == 0 && c.y - c.spanY >= 0) {
                    c.y = c.y - c.spanY;
                    c.x = countX;
                    int cellX = isCellInLauncherAppWidget(c.x - 1, c.y, ViewCluster.LEFT);
                    if (cellX != -1) {
                        c.x = cellX;
                    }
                }
            }
            Log.v("edge", "left---addView: x=" + c.x + ", y=" + c.y);

        } else if (whichEdge == ViewCluster.RIGHT) {
            if (c.x == countX - 1 && c.y + c.spanY <= countY - 1) {
                c.y = c.y + c.spanY;
                c.x = -1;
            }
            int result = isCellInLauncherAppWidget(c.x + 1, c.y, ViewCluster.RIGHT);
            if (result != -1) {
                c.x = result;
                while(c.x == countX - 1 && c.y + c.spanY <= countY - 1) {
                    c.y = c.y + c.spanY;
                    c.x = -1;
                    int cellX = isCellInLauncherAppWidget(c.x + 1, c.y, ViewCluster.RIGHT);
                    if (cellX != -1) {
                        c.x = cellX;
                    }
                }
            }
            Log.v("edge", "right---addView: x=" + c.x + ", y=" + c.y);
        }
    }

    //add by xiangzx
    private int isCellInLauncherAppWidget(int x, int y, int whichEdge){
        for(CellAndSpan c : widgetCellAndSpanList){
            if(x >= c.x && x < c.x + c.spanX && y >= c.y && y < c.y + c.spanY){
                if(whichEdge == ViewCluster.LEFT) {
                    return c.x;
                }else if(whichEdge == ViewCluster.RIGHT){
                    return c.x + c.spanX - 1;
                }
            }
        }
        return -1;
    ...
    private boolean rearrangementExists(int cellX, int cellY, int spanX, int spanY, int[] direction,
        ...
        for (View child: solution.map.keySet()) {
            ...
            if (Rect.intersects(r0, r1)) {
                ...
                //add by xiangzx to prevent push LauncherAppWidget using BubbleTextView
    ...
    private ItemConfiguration findReorderSolution(int pixelX, int pixelY, int minSpanX, int minSpanY,
            ...
            ItemConfiguration solution,int[] result) {
        ...
        if(pixelX!=EXTRA_EMPTY_POSTION&& pixelY!=EXTRA_EMPTY_POSTION) {
        ...
        if (!success) {
            ...
            if (spanX > minSpanX && (minSpanY == spanY || decX)) {
                return findReorderSolution(pixelX, pixelY, minSpanX, minSpanY, spanX - 1, spanY,
                        direction, dragView, false, solution,result);
            } else if (spanY > minSpanY) {
                return findReorderSolution(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY - 1,
                        direction, dragView, true, solution,result);
            }
    ...
    private void copyCurrentStateToSolution(ItemConfiguration solution, boolean temp) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //add by xiangzx to collect all widget in current cellLayout
            if(child instanceof LauncherAppWidgetHostView){
                widgetCellAndSpanList.add(c);
            }
    ...
    */
    ...
    boolean createAreaForResize(int cellX, int cellY, int spanX, int spanY,
        ...
        ItemConfiguration swapSolution = findReorderSolution(pixelXY[0], pixelXY[1], spanX, spanY,
                 spanX,  spanY, direction, dragView,  true,  new ItemConfiguration(),null);
    ...
    int[] performReorder(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        // First we determine if things have moved enough to cause a different layout
        if(pixelX!=CellLayout.EXTRA_EMPTY_POSTION&&pixelY!=CellLayout.EXTRA_EMPTY_POSTION){
            result = findNearestArea(pixelX, pixelY, spanX, spanY, result);
        }
        if (resultSpan == null) {
        ...
        ItemConfiguration swapSolution = findReorderSolution(pixelX, pixelY, minSpanX, minSpanY,
                 spanX,  spanY, mDirectionVector, dragView,  true,  new ItemConfiguration(),result);
    ...
    @Thunk class ItemConfiguration {
        ...
        public String toString() {
            ...
            /*for(CellAndSpan sp : map.values()){
                s+=sp.toString()+" , ";
            }*/
            for(Map.Entry<View, CellAndSpan> a:map.entrySet()){
                s+=a.toString();
            }
            return s;
        }
    ...
    private class CellAndSpan {
        ...
        public CellAndSpan(int x, int y, int spanX, int spanY,int index) {
            this.x = x;
            this.y = y;
            this.spanX = spanX;
            this.spanY = spanY;
    ...
    void onDropChild(View child) {
        ...
        if (mLauncher.isHotseatLayout(this)) {
            mDragPushState  = HotseatDragState.NONE;
        }
    }
    ...
    void markCellsForView(int cellX, int cellY, int spanX, int spanY, boolean[][] occupied,
            boolean value) {
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public void setup(int cellWidth, int cellHeight, int widthGap, int heightGap,
            ...
            if (isLockedToGrid) {
                ...
                if(isVertical){
                    x = (int) (myCellX * (cellWidth + widthGap) + leftMargin);
                    y = (int) (myCellY * (cellHeight + heightGap) + heightGap+ topMargin);
                }else{
    ...
    public static final class CellInfo {
        ...
        public String toString() {
            return "Cell[view=" + (cell == null ? "null" : cell.getClass())
                    + ", x=" + cellX + ", y=" + cellY +", screenId="+screenId+", container"+container+ "]";
        }
    }
     ...
     */
    boolean getEmptyLayPlace(int minSpanX, int minSpanY, int spanX, int spanY, int[] result, int resultSpan[]) {
        for(int i = spanX ; i >= minSpanX ; i --){
            for(int j = spanY ; j >= minSpanY ; j --){
                if(findCellForSpan(result,i,j)){
                    if(result!=null){
                        resultSpan[0] = i;
                        resultSpan[1] = j;
                    }
    ...
    /**
     * lijun add
     * 获取CellLayout的框架缩略图View，用于Pageindicator显示
     * isActive:是否是当前页  cell:拖拽当前坐标 source:拖拽源坐标（为空则不在拖拽页） useTemp:正在挤压状态
     * @return
     */
    public Bitmap getFrameNailBitmap(Boolean isActive,int[] targetCell,int[] source,boolean useTemp) {
        DeviceProfile grid = mLauncher.getDeviceProfile();
        Bitmap newb = Bitmap.createBitmap(grid.pageIndicatorCubeCellWidthPx, grid.pageIndicatorCubeCellHeightPx, Bitmap.Config.ARGB_4444);
        int targetColor;
        int bgColor;
        ...
        FrameCanvas cv = new FrameCanvas(newb , mCountX ,mCountY,grid);
//        cv.drawBackground(bgColor);
//        cv.drawOutline(outlineColor);
        int childCount = mShortcutsAndWidgets.getChildCount();
        int cx,cy;
        for (int i = 0; i < childCount; i++) {
            View child = mShortcutsAndWidgets.getChildAt(i);
            ...
            if (targetCell != null && cx == targetCell[0] && cy == targetCell[1]) continue;
            if (source != null && lp.cellX == source[0] && lp.cellY == source[1]) continue;
            cv.drawCell(cx,cy,lp.cellHSpan,lp.cellVSpan,bgColor);
        }
        if(targetCell != null){
            cv.drawCell(targetCell[0],targetCell[1],targetCell[2],targetCell[3],targetColor);
        }
        cv.save();
        cv.restore();
        return newb;
    ...
    /**
     * 用于绘制CellLayout的骨架缩略图
     */
    class FrameCanvas extends Canvas {
        Bitmap sBitmap;
        int sColumns, sRows;
        float leftPadding, rigthPadding, topPadding, bottomPadding;
        float xGap, yGap;
        float sWidth, sHeight;
        float sCellWidth, sCellHeight;
        Paint sPaint = new Paint();
        Paint sBgPaint = new Paint();
        Paint sOutlinePaint = new Paint();
        RectF bgRect;
        RectF outLineRect;
        float paintWith = 3;//描边画笔宽度
        public FrameCanvas(Bitmap bitmap, int columns, int rows, DeviceProfile grid) {
            super(bitmap);
            sColumns = columns;
            sRows = rows;
            sWidth = grid.pageIndicatorCubeCellWidthPx;
            sHeight = grid.pageIndicatorCubeCellHeightPx;
            bgRect = new RectF(paintWith,paintWith,sWidth-paintWith,sHeight-paintWith);
            outLineRect = new RectF(paintWith/2,paintWith/2,sWidth-paintWith/2,sHeight-paintWith/2);

            if(sWidth/sColumns < sHeight/sRows){//30%的空间用于显示padding和gap,且padding是gap的2倍
                sCellWidth = sCellHeight = sWidth/sColumns*0.7f;
            }else{
                sCellWidth = sCellHeight = sHeight/sRows*0.7f;
            }
            xGap = (sWidth - sColumns*sCellWidth)/(sColumns+3);
            leftPadding = rigthPadding = xGap*2;
            yGap = (sHeight - sRows*sCellHeight)/(sRows+3);
            topPadding = bottomPadding = yGap*2;

            sBitmap = bitmap;
            sPaint.setStyle(Paint.Style.FILL);
            sPaint.setAntiAlias(true);
            sOutlinePaint.setStyle(Paint.Style.STROKE);
            sOutlinePaint.setStrokeWidth(paintWith);
        }

        public void drawCell(int cellX, int cellY, int hSpan, int vSpan, int color) {
            sPaint.setColor(color);
            RectF rect = new RectF();
            rect.left = leftPadding + cellX * (sCellWidth + xGap);
            rect.top = topPadding + cellY * (sCellHeight + yGap);
            rect.right = rect.left + (sCellWidth + xGap) * hSpan - xGap;
            rect.bottom = rect.top + (sCellHeight + yGap) * vSpan - yGap;
            this.drawRoundRect(rect, sCellWidth / 7, sCellHeight / 7, sPaint);
        }

        public void drawBackground(int bgColor){
            sBgPaint.setColor(bgColor);
            this.drawRoundRect(bgRect, sCellWidth / 7, sCellHeight / 7, sBgPaint);
        }

        public void drawOutline(int color){
            sOutlinePaint.setColor(color);
            this.drawRoundRect(outLineRect, sCellWidth / 7, sCellHeight / 7, sOutlinePaint);
        }
    }

 int mVisibleCount=0;
    private HotseatDragState mDragPushState =HotseatDragState.NONE;
    List<View> mSortLists=new ArrayList<View>();
    public void revertCellLayout(int dragmode,HotseatDragState state,View dragItemView, Hotseat hotseat){
        ItemConfiguration solution = new ItemConfiguration();
        List<View> list = new ArrayList<View>();
        mVisibleCount = revertCurrentStateToSolution(solution,false,dragItemView,
                state,list);
        mDragPushState = (dragmode==MODE_ON_DROP)||(dragmode==MODE_ON_DROP_EXTERNAL)?HotseatDragState.NONE:state;
        reLayout(mVisibleCount);
        if (!DESTRUCTIVE_REORDER) {
            setUseTempCoords(true);
        }
        if (HotseatDragState.OUT.equals(state) ) {
            if (dragmode == MODE_DRAG_OVER || dragmode == MODE_ON_DROP || dragmode == MODE_ON_DROP_EXTERNAL) {
                if (!DESTRUCTIVE_REORDER) {
                    copySolutionToTempState(solution, null);
                }
                setItemPlacementDirty(true);
                animateItemsToSolution(solution, null, dragmode == MODE_ON_DROP);
                if (!DESTRUCTIVE_REORDER &&
                        (dragmode == MODE_ON_DROP || dragmode == MODE_ON_DROP_EXTERNAL||dragmode == MODE_DRAG_OVER)) {
                    commitTempPlacement();
                    completeAndClearReorderPreviewAnimations();
                    setItemPlacementDirty(false);
                } else {
                    beginOrAdjustReorderPreviewAnimations(solution, null,
                            REORDER_ANIMATION_DURATION,ReorderPreviewAnimation.MODE_PREVIEW);
                }
            }
        }
    ...
    public void restoreItems(int index,View cell){
        final int count = mShortcutsAndWidgets.getChildCount();
        boolean isExitTheSomeToIndex = false;
        for(int i = 0; i < count; i++){
            View child = mShortcutsAndWidgets.getChildAt(i);
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            if(!(cell!=null&&cell.equals(child))) {
                if (lp.tmpCellX == index) {
                    isExitTheSomeToIndex = true;
                }
            }
        }
        for (int i = 0; i < count; i++) {
            View child = mShortcutsAndWidgets.getChildAt(i);
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            Log.i("fuck","before : child = "+child+"  , lp.temp = ("+lp.tmpCellX+" , "+lp.tmpCellY+")  , lp.cell = ("+lp.cellX +" ,  "+lp.cellY+" ), index = "+index+" isExitTheSomeToIndex = "+isExitTheSomeToIndex);
            if(!(cell!=null&&cell.equals(child))){
                if(isExitTheSomeToIndex){
                    if(lp.tmpCellX>=index){
                        lp.tmpCellX++;
            ...
            Log.i("fuck","after : child = "+child+"  , lp = "+lp.tmpCellX+" , "+lp.tmpCellY+" , lp.cellx = "+lp.cellX +" , lp.celly = "+lp.cellY+" , index = "+index);
        }
    }

    private int revertCurrentStateToSolution(ItemConfiguration solution, boolean temp, View dragItemView, HotseatDragState state, List<View> list) {
        int childCount = mShortcutsAndWidgets.getChildCount();
        int index=0;
        mSortLists.clear();
        resort(mSortLists, mShortcutsAndWidgets);
        boolean isDragView = false;
        for(int i=0;i<childCount;i++){
            View child = mSortLists.get(i);
            LayoutParams lp =(LayoutParams)child.getLayoutParams();
            CellAndSpan c;
            if((child == dragItemView)){
                continue;
            }
            if(temp){
                if(isVertical){
                    lp.tmpCellY = index;
                    c = new CellAndSpan(lp.tmpCellX,lp.tmpCellY,lp.cellHSpan,lp.cellVSpan,lp.tmpCellY*mCountX+lp.tmpCellX);
                }else{
                    lp.tmpCellX = index;
                    c = new CellAndSpan(lp.tmpCellX,lp.tmpCellY,lp.cellHSpan,lp.cellVSpan,lp.tmpCellY*mCountX+lp.tmpCellX);
                }
                ...
                if(isVertical){
                    lp.tmpCellY = lp.cellY;
                    lp.cellY = index;
                    c = new CellAndSpan(lp.cellX,lp.cellY,lp.cellHSpan,lp.cellVSpan,lp.cellY*mCountX+lp.cellX);
                }else{
                    lp.tmpCellX = lp.cellX;
                    lp.cellX = index;
                    c = new CellAndSpan(lp.cellX,lp.cellY,lp.cellHSpan,lp.cellVSpan,lp.cellY*mCountX+lp.cellX);
        ...
        if(HotseatDragState.IN.equals(state)){
            index++;
        }
        return index;
    }

    private void resort(List<View> list,ShortcutAndWidgetContainer shortcut) {
        for(int i=0;i<shortcut.getChildCount();i++) {
            View child = shortcut.getChildAt(i);
            list.add(child);
        }
        Collections.sort(list, new Comparator<View>() {
            ...
            public int compare(View lhs, View rhs) {
                LayoutParams lplhs = (LayoutParams) lhs.getLayoutParams();
                LayoutParams lprhs = (LayoutParams) rhs.getLayoutParams();
                return (int) (lplhs.cellX - lprhs.cellX);
            }
        });
    }
    private int mDynamicWidthGap = 0;
    private int mDynamicHeightGap = 0;
    public void relayout(){
        reLayout(this.getShortcutsAndWidgets().getChildCount());
    }
    public void reLayout(int visibleCount) {
        if (visibleCount > 0) {
            int availableWidth =getRight() - getLeft() - getPaddingLeft() - getPaddingRight();
            int availableHeight = getBottom() - getTop() -getPaddingTop() - getPaddingBottom();
            Log.i("reLayout","this w = "+this.getWidth()+" , h = "+this.getHeight());
            ...
            int wGap =isVerticalBarLayout()? mOriginalWidthGap :(availableWidth - cellWidth * visibleCount) / (visibleCount+1);
            int hGap =isVerticalBarLayout()? (availableHeight - cellHeight * visibleCount) / (visibleCount+1):mHeightGap;
            mWidthGap = mDynamicWidthGap = wGap;
            mHeightGap = mDynamicHeightGap =hGap;
            Log.i("Hotseat","wGap = "+wGap);
            getShortcutsAndWidgets().setCellDimensions(mCellWidth, mCellHeight, mWidthGap, mHeightGap, visibleCount, 1);
        }
    }
    private static boolean isVertical=false;
    private static boolean isVerticalBarLayout(){
        return isVertical;
    }
    public final static int EXTRA_EMPTY_POSTION=-101;
    //liuzuo begin
    public void setCellDimensions(int width, int height,int widthGap,int originalWidthGap) {
        mFixedCellWidth = mCellWidth = width;
        mFixedCellHeight = mCellHeight = height;
        mWidthGap=widthGap;
        mOriginalWidthGap=originalWidthGap;
        mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight, mWidthGap, mHeightGap,
                mCountX, mCountY);
    }
    //liuzuo end
    ...
    public void changeItemColors(int[] colors){
        ShortcutAndWidgetContainer shortcutAndWidgetContainer = getShortcutsAndWidgets();
        if(shortcutAndWidgetContainer!=null){
            int count = shortcutAndWidgetContainer.getChildCount();
            for(int i=0;i<count;i++){
                View view = shortcutAndWidgetContainer.getChildAt(i);
                if(this.getParent() instanceof WidgetsContainerPageView)Log.i("cbb","3");
                if(view instanceof IChangeColors.IItemColorChange){
                    if(this.getParent() instanceof WidgetsContainerPageView)Log.i("cbb","4");
                    ((IChangeColors.IItemColorChange)view).changeColors(colors);
                }
            }
        }
    }
