...
import com.monster.launcher.unread.UnreadSupportShortcut;
...
class AllAppsList {
     ...
     * Boring constructor.
     */
    public AllAppsList(Context c,IconCache iconCache, AppFilter appFilter) {
        //lijun add for unread start
        mContext = c;
        //lijun add end
    ...
    public void updateUnread(){
        ArrayList<UnreadSupportShortcut> unreadShortcuts =  new ArrayList<UnreadSupportShortcut>();
        Object[] datas = data.toArray();
        for(Object inf : datas){
            //外发时需要
            AppInfo info = (AppInfo)inf;
//            Bitmap icon = info.iconBitmap;
//            if(icon == null){
//                icon = Utilities.getAppIcon(mContext,info.componentName.getPackageName(),info.user);
//            }
//            if(icon == null){
//                icon = PhotoUtils.drawable2bitmap(mContext.getResources().getDrawable(android.R.drawable.sym_def_app_icon));
//            }
            Bitmap icon = null ;
            int type = getUnreadType(info.componentName);
            UserHandle user = null;
            if(info.user!=null){
                user = info.user.getUser();
            }else {
                user = UserHandleCompat.myUserHandle().getUser();
            }
            unreadShortcuts.add(new UnreadSupportShortcut(info.componentName, icon, type,user));
        }
        MonsterUnreadLoader.updateAllUnreadShortcuts(unreadShortcuts,mContext);
        mContext.sendBroadcast(new Intent(NotificationMonitorListener.NOTIFICATION_CONTROL_ACTION));
    }

    private int getUnreadType(ComponentName componentName){
        int type =  MonsterUnreadLoader.UNREAD_TYPE_NORMAL;
        if(componentName == null)return type;
        if(MonsterUnreadLoader.PHONE_CLASS_NAME.equals(componentName.getClassName()) && MonsterUnreadLoader.PHONE_PACKAGE_NAME.equals(componentName.getPackageName())){
            type = MonsterUnreadLoader.UNREAD_TYPE_PHONE;
        }else if(MonsterUnreadLoader.MMS_CLASS_NAME.equals(componentName.getClassName()) && MonsterUnreadLoader.MMS_PACKAGE_NAME.equals(componentName.getPackageName())){
            type = MonsterUnreadLoader.UNREAD_TYPE_MMS;
        }
        return type;
    }
}
