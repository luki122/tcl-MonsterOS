...
import com.monster.launcher.AutoInstallsLayout.LayoutParserCallback;
import com.monster.launcher.compat.UserHandleCompat;
import com.monster.launcher.compat.UserManagerCompat;
import com.monster.launcher.config.ProviderConfig;
...
public class LauncherProvider extends ContentProvider {
    ...
    static final String TABLE_APP_INFO_CATEGORY = LauncherSettings.AppCategory.TABLE_APP_INFO_CATEGORY;
    static final String TABLE_APP_CATEGORY = LauncherSettings.AppCategory.TABLE_APP_CATEGORY;
    static final String TABLE_FAVORITES_BACKUP = "favorites_backup";
    static final String TABLE_WORKSPACE_SCREENS_BACKUP = "workspace_screens_backup";
    ...
    //add by xiangzx
    public boolean fieldExists(String currentLanguage){
        Cursor fieldCursor = mOpenHelper.getWritableDatabase().rawQuery("select * from sqlite_master where name = ? and sql like ?",
                new String[]{LauncherSettings.AppCategory.TABLE_APP_CATEGORY, "%" + currentLanguage + "%"});
        boolean result = fieldCursor.getCount() > 0 ? true : false;
        fieldCursor.close();
        return result;
    }
    ...
    public void bulkBackup(boolean backup){
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        db.beginTransaction();
        try {
            String fromFavoritesTb;
            String destFavoritesTb;
            String fromScreensTb;
            String destScreensTb;
            if(backup) {
                fromFavoritesTb = TABLE_FAVORITES;
                destFavoritesTb = TABLE_FAVORITES_BACKUP;
                fromScreensTb   = TABLE_WORKSPACE_SCREENS;
                destScreensTb   = TABLE_WORKSPACE_SCREENS_BACKUP;
            }else {
                fromFavoritesTb = TABLE_FAVORITES_BACKUP;
                destFavoritesTb = TABLE_FAVORITES;
                fromScreensTb   = TABLE_WORKSPACE_SCREENS_BACKUP;
                destScreensTb   = TABLE_WORKSPACE_SCREENS;
            }
            db.execSQL("DELETE FROM "+destFavoritesTb);
            db.execSQL("INSERT INTO " +destFavoritesTb+" select * from "+fromFavoritesTb);

            db.execSQL("DELETE FROM "+destScreensTb);
            db.execSQL("INSERT INTO " +destScreensTb+" select * from "+fromScreensTb);
    ...
    synchronized public void loadDefaultFavoritesIfNecessary() {
        ...
        if (sp.getBoolean(EMPTY_DATABASE_CREATED, false)) {
            ...
            insertOriginalAppCategory(mOpenHelper.getWritableDatabase());
            clearFlagEmptyDbCreated();
        }
    }

    public void insertOriginalAppCategory(SQLiteDatabase db){
        final Context context = getContext();
        ArrayList<String> categoryResult = Utilities.readSqlStr(context, LauncherProvider.TABLE_APP_CATEGORY+".txt");
        db.beginTransaction();
        try {
            for(String categoryItem : categoryResult) {
                db.execSQL(categoryItem);
            }
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
        ...
        ArrayList<String> infoResult = Utilities.readSqlStr(context, LauncherProvider.TABLE_APP_INFO_CATEGORY+".txt");
        db.beginTransaction();
        try {
            for(String infoItem : infoResult) {
                db.execSQL(infoItem);
            }
            db.setTransactionSuccessful();
        } finally {
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        public void onCreate(final SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE favorites_backup (" +  //add by xiangzx
                    "_id INTEGER PRIMARY KEY," +
                    "title TEXT," +
                    "intent TEXT," +
                    "container INTEGER," +
            ...
            createAppCategory(db); //add by xiangzx
        ...
        private void addWorkspacesTable(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE " + TABLE_WORKSPACE_SCREENS_BACKUP + " (" +
                    LauncherSettings.WorkspaceScreens._ID + " INTEGER PRIMARY KEY," +
                    LauncherSettings.WorkspaceScreens.SCREEN_RANK + " INTEGER," +
                    LauncherSettings.ChangeLogColumns.MODIFIED + " INTEGER NOT NULL DEFAULT 0" +
                    ");");
        ...
        public void createAppCategory(SQLiteDatabase db){
            db.execSQL("CREATE TABLE " + TABLE_APP_INFO_CATEGORY +"("
                    + "package_name text PRIMARY KEY NOT NULL,"
                    + "app_type text NOT NULL,"
                    + "category_name text  NOT NULL" + ");");

            db.execSQL("CREATE TABLE " + TABLE_APP_CATEGORY +"("
        ...
        public void createEmptyDB(SQLiteDatabase db) {
            ...
            db.execSQL("DROP TABLE IF EXISTS " + TABLE_APP_INFO_CATEGORY);
            db.execSQL("DROP TABLE IF EXISTS " + TABLE_APP_CATEGORY);
            db.execSQL("DROP TABLE IF EXISTS " + TABLE_FAVORITES_BACKUP);
            db.execSQL("DROP TABLE IF EXISTS " + TABLE_WORKSPACE_SCREENS_BACKUP);
            onCreate(db);
        }
