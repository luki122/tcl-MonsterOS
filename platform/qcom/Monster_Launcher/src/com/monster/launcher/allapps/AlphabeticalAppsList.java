...
import com.monster.launcher.model.AppNameComparator;
...
public class AlphabeticalAppsList {
    ...
    public static int SECTION_OFFSET; //add by xiangzx
    ...
    List<AppInfo> mPredictedApps = new ArrayList<>();
    ...
    public AlphabeticalAppsList(Context context) {
        ...
        SECTION_OFFSET = mLauncher.getResources().getDimensionPixelOffset(R.dimen.all_apps_section_top);
    }
    ...
    private void updateAdapterItems() {
        ...
        if (mPredictedAppComponents != null && !mPredictedAppComponents.isEmpty() && !hasFilter()) {
            ...
            if (!mPredictedApps.isEmpty()) {
                //add by xiangzx
                //String predictedTitle = mLauncher.getResources().getString(R.string.allapps_recent_apps);
                String predictedTitle = "☆";
                // Add a section for the predictions
                lastSectionInfo = new SectionInfo();
                lastFastScrollerSectionInfo = new FastScrollSectionInfo(predictedTitle);
                ...
                for (AppInfo info : mPredictedApps) {
                    AdapterItem appItem = AdapterItem.asPredictedApp(position++, lastSectionInfo,
                            predictedTitle, lastSectionInfo.numApps++, info, appIndex++);
        ...
        if (hasFilter()) {
            mLauncher.getAppsView().mAppsRecyclerView.changeIndexLayoutVisibility(false);
            if (hasNoFilteredResults()) {
                mAdapterItems.add(AdapterItem.asEmptySearch(position++));
            } else {
                //mAdapterItems.add(AdapterItem.asDivider(position++));
                //mLauncher.getAppsView().mAppsRecyclerView.changeIndexLayoutVisibility(true);
            }
