...
/**
 * Add class to show predictiveApps by xiangzhenxiong
 */
...
public class PredictiveAppsProvider {
    ...
    public void updateComponentCount(ComponentName component) {
        ...
        /*Begin add by xiangzx to order predictive apps by time desc*/
        long maxCount = 0;
        for (String s : predictiveApps) {
            long count = sharedPreferences.getLong(s, 0);
            if(maxCount < count){
                maxCount = count;
            }
        }
        maxCount += 1;
        sharedPreferences.edit().putLong(key, maxCount).commit();
        /*End add by xiangzx to order predictive apps by time desc*/
