...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent data) {
        ...
        Log.e(TAG, " onReceive install shortcut intent");
        /* modify by xiangzx to avoid appear more than one same app icon */
    ...
    private static PendingInstallShortcutInfo createPendingInfo(Context context, Intent data) {
        ...
        if ( !SmartContainerWrapper.WITH_OUT_VIRTUAL_BOX ) {
            ...
            //try to parse the user from intent
            user = SmartContainerWrapper.getUserHandleCompat(data, user);
            info = new PendingInstallShortcutInfo(data, context, user);
        }
        else{
            info = new PendingInstallShortcutInfo(data, context);
        }
        //Dual app end
...
//        return info.getShortcutInfo();
    ...
    static void flushInstallQueue(Context context) {
        ...
        if (!installQueue.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                //Duall app begin
                if ( !SmartContainerWrapper.WITH_OUT_VIRTUAL_BOX ) {
                    if (pendingInfo.launchIntent.getExtras() != null){
                        Bundle bundle = pendingInfo.launchIntent.getExtras();
                        Log.d(TAG, "   extras:" + pendingInfo.launchIntent.getExtras().toString());
                        for (String key: bundle.keySet())
                        {
                            final Object value = bundle.get(key);
                            Log.d(TAG, "flushInstalQueue:     -->Bundle Content: Key=" + key + ", content=" + value.toString());
                        }
                    }
                }
                //Duall app end
    ...
    private static class PendingInstallShortcutInfo {
        ...
        //Dual app begin
        public PendingInstallShortcutInfo(Intent data, Context context, UserHandleCompat user_in) {
            this.data = data;
            mContext = context;
            activityInfo = null;
            ...
            launchIntent.putExtra(AppInfo.EXTRA_PROFILE, serialNumber);
        }
        //Dual app end
        ...
        public String encodeToString() {
            ...
            try {
                ...
                //Dual app begin
                if ( !SmartContainerWrapper.WITH_OUT_VIRTUAL_BOX ) {
                    json = json.key(USER_HANDLE_KEY).value(UserManagerCompat.getInstance(mContext)
                            .getSerialNumberForUser(user));
                }
                //Dual app end
                if (icon != null) {
        ...
        public ShortcutInfo getShortcutInfo() {
            if (activityInfo != null) {
                ...
                //Dual app begin
                if ( !SmartContainerWrapper.WITH_OUT_VIRTUAL_BOX )
                    return LauncherAppState.getInstance().getModel().infoFromShortcutIntent(mContext, data, user);
                //Dual app end
    ...
    private static PendingInstallShortcutInfo decode(String encoded, Context context) {
        try {
            ...
            if ( !SmartContainerWrapper.WITH_OUT_VIRTUAL_BOX ) {
                UserHandleCompat user = UserManagerCompat.getInstance(context)
                        .getUserForSerialNumber(object.getLong(USER_HANDLE_KEY));
                return new PendingInstallShortcutInfo(data, context, user);
            }
            //Dual app end
    ...
    private static PendingInstallShortcutInfo convertToLauncherActivityIfPossible(
        ...
        if ( !SmartContainerWrapper.WITH_OUT_VIRTUAL_BOX ) {
            try {
                ...
                Method method = c.getMethod("getIdentifier");
                method.setAccessible(true);
                int id = (int)method.invoke(original.user.getUser());
                Class<PackageManager> pc = PackageManager.class;
                method = pc.getMethod("resolveActivityAsUser", Intent.class, int.class, int.class);
                method.setAccessible(true);
                info = (ResolveInfo) method.invoke(pm, original.launchIntent, 0, id);
            } catch ( Exception e) {
                e.printStackTrace();
                info = pm.resolveActivity(original.launchIntent, 0);
            }
        } else{
            info = pm.resolveActivity(original.launchIntent, 0);
        }
