...
public class BaseRecyclerViewFastScrollPopup {
    ...
    public BaseRecyclerViewFastScrollPopup(BaseRecyclerView rv, Resources res) {
        ...
        mTextPaint.setColor(res.getColor(R.color.container_fastscroll_popup_text_color));
    ...
    public Rect updateFastScrollerBounds(BaseRecyclerView rv, int lastTouchY) {
        ...
        if (isVisible() || !mRv.mEnableDrawScrollBar) { //modify by xiangzx
            ...
            int bgPadding = (mBgOriginalSize - mTextBounds.height()) / 2;
            int ratio = 2;
            if(!mRv.mEnableDrawScrollBar){
                ratio = 1;
            }
            int bgHeight = mBgOriginalSize;
            int bgWidth = Math.max(mBgOriginalSize, mTextBounds.width() + ratio * bgPadding);

            mBgBounds.left  = (rv.getWidth() - bgWidth)/2;
            mBgBounds.right = mBgBounds.left + bgWidth;
            mBgBounds.top   =   rv.getHeight()/3;
            mBgBounds.bottom = mBgBounds.top + bgHeight;
