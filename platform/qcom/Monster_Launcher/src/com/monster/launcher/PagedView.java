...
import com.monster.launcher.effect.TransitionEffect;
import com.monster.launcher.util.LauncherEdgeEffect;
 ...
 */
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener ,IChangeLauncherColor {
...
//    private final LauncherEdgeEffect mEdgeGlowRight = new LauncherEdgeEffect();
    ...
    //add by xiangzx
    private static final float OVERSCROLL_DAMP_FACTOR = 0.44f;
    protected int mOverScrollX;
    protected int mUnboundedScrollX;
    private boolean moveByHand = true;
...
//        mEdgeGlowRight.setColor(color);
    ...
    protected void onAttachedToWindow() {
        ...
        if (mPageIndicator == null && mPageIndicatorViewId > -1) {
            ...
            if(grandParent instanceof Folder){
                mPageIndicator.setIsFolderPage(true);
            }
    ...
    protected void onDetachedFromWindow() {
        ...
        //mPageIndicator = null;//lijun detached it to children
    }
    ...
    private void updatePageIndicator() {
        ...
        if (mPageIndicator != null) {
            ...
            if (!isReordering(false)) {
                ...
                onPageIndicatorSwitched(getNextPage());
            }
        }
    }
    ...
    public void scrollTo(int x, int y) {
        mUnboundedScrollX = x; //add by xiangzx
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(DEBUG) Log.d(TAG,"mMinScale = "+mMinScale+"  mUseMinScale = "+mUseMinScale);
    ...
    private float overScrollInfluenceCurve(float f) {
        /*Interpolator pathInterpolatorCompat = PathInterpolatorCompat.create(0.17f, 0.17f, 0.1f, 0.1f);
        return pathInterpolatorCompat.getInterpolation(f);*/
        /*f -= 1.0f;
        return f*f*f + 1;*/
        return f * 0.42f;
    }
    ...
    protected void dampedOverScroll(float amount) {
       ...
       /* if (f < 0) {
        ...
        if(Math.abs(overScrollAmount) > screenSize * OVERSCROLL_DAMP_FACTOR){
            overScrollAmount = overScrollAmount > 0 ? (int)(screenSize * OVERSCROLL_DAMP_FACTOR) : -(int)(screenSize * OVERSCROLL_DAMP_FACTOR);
        }
        if (amount < 0) {
...
//        mEdgeGlowRight.onRelease();
    ...
    protected void onUnhandledTap(MotionEvent ev) {
        if(this instanceof  Workspace && ((Launcher) getContext()).isEditorMode()){
            Workspace workspace = ((Workspace)this);
            Launcher launcher = (Launcher) getContext();
            int clickY = (int) ev.getRawY();
            int top = 0;
            if((workspace.getState() == Workspace.State.OVERVIEW || workspace.getState() == Workspace.State.OVERVIEW_HIDDEN)
                    && launcher.mState == Launcher.State.WORKSPACE){
                top = launcher.getOverviewPanelTop();
                if(clickY>top)return;
            }else if((workspace.getState() == Workspace.State.OVERVIEW || workspace.getState() == Workspace.State.OVERVIEW_HIDDEN)
                    && launcher.mState == Launcher.State.WIDGETS){
                top = launcher.geWidgetsPanelTop();

            }
            if(top > 0 && clickY>top)return;
        }
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
            TimeInterpolator interpolator) {
        whichPage = validateNewPage(whichPage);
//Log.d("--lijun--","---------snapToPage : " + whichPage);
//        android.util.Log.e("---lijun---",android.util.Log.getStackTraceString(new Throwable()));
        ...
        //add by xiangzx to locate app quickly when in current screen
        if(delta == 0){
            immediate = true;
        }
        ...
        if(mUnboundedScrollX < 0 || mUnboundedScrollX > mMaxScrollX){ //add by xiangzx
            moveByHand = false;
            mScroller.setInterpolator(PathInterpolatorCompat.create(0.17f, 0.17f, 0.29f, 1.0f));
            duration = 380;
        }
    ...
    protected void onPageIndicatorSwitched(int activeIndext){
        //do nothing ,just for workspace
    }
    //Modify by xiejun for Transition effect
    ...
    public void onColorChanged(int[] colors){
        ...
        for(int i=0;i<count;i++){
            ...
            if(this instanceof WidgetsContainerPageView)Log.i("cbb","1 child = "+child);
            if(child instanceof IChangeColors.IItemsColorChange){
                if(this instanceof WidgetsContainerPageView)Log.i("cbb","2");
                ((IChangeColors.IItemsColorChange)child).changeItemColors(colors);
            }
        }
        if(mPageIndicator!=null) {
            ArrayList<PageIndicatorMarker> markers = mPageIndicator.mMarkers;
