...
class TransitionStates {
    ...
    final boolean oldStateIsNormalDrag;
    ...
    final boolean stateIsNormalDrag;
    ...
    final boolean workspaceToWorkspaceDrag;
    ...
    public TransitionStates(final Workspace.State fromState, final Workspace.State toState) {
        ...
        oldStateIsNormalDrag = (fromState==Workspace.State.NORMAL_DRAG);
        ...
        stateIsNormalDrag = (toState == Workspace.State.NORMAL_DRAG);
        ...
        workspaceToWorkspaceDrag = (oldStateIsNormal&&stateIsNormalDrag);
...
public class WorkspaceStateTransitionAnimation {
    ...
    @Thunk float mWorspaceNormalDragShrinkFactor;
    ...
    public WorkspaceStateTransitionAnimation(Launcher launcher, Workspace workspace) {
        ...
        mWorspaceNormalDragShrinkFactor = res.getInteger(R.integer.config_workspaceNormalDragShrinkPercentage) / 100f;
    ...
    private void animateWorkspace(final TransitionStates states, int toPage, final boolean animated,
        ...
        final View pageIndicatorCicle = mWorkspace.getPageIndicator();
        final View pageIndicatorCube = mWorkspace.getmPageIndicatorCube();

        // Update the workspace state
        Log.d("Launcher","states.stateIsNormalDrag = "+states.stateIsNormalDrag);
        //lijun modify to hide the background
//        float finalBackgroundAlpha = (states.stateIsSpringLoaded || states.stateIsOverview) ?
//                1.0f : 0.0f;
        float finalBackgroundAlpha = 0.0f;
        //lijun modify for pageindicator show in OverviewHidden
        float finalHotseatAlpha = (states.stateIsNormal || states.stateIsSpringLoaded || states.stateIsNormalDrag)?
                1f : 0f;
        float finalHotseatTranslationY = (states.stateIsNormal || states.stateIsSpringLoaded || states.stateIsNormalDrag)?
                0 : (float) hotseat.getHeight()/2;
        float finalPageIndicatorAlpha = (states.stateIsNormal || states.stateIsSpringLoaded  || states.stateIsOverviewHidden || states.stateIsOverview
                ||states.stateIsFolderImport//liuzuo
        )?
                1f : 0f;
        //lijun modify end
        float finalOverviewPanelAlpha = states.stateIsOverview ? 1f : 0f;
        float finalOverviewPanelTranslationY = states.stateIsOverview ? 0f : (float) overviewPanel.getHeight()/2;
        float finalWorkspaceTranslationY = states.stateIsOverview || states.stateIsOverviewHidden /*|| states.stateIsNormalDrag*/?
                mWorkspace.getOverviewModeTranslationYNew(mWorkspace.getResources()) : 0;

        //lijun add for pageIndicatorCube
        float finalPageIndicatorCubeAlpha = (states.stateIsNormalDrag) ?1f : 0f;
        float finalPageIndicatorCubeScale = (states.stateIsNormalDrag) ?1f : 0.4f;
        float finalPageIndicatorTranslationY = (states.stateIsOverview) ?(finalWorkspaceTranslationY*1.25f) : 0f;
        if(states.oldStateIsNormal && states.stateIsOverview && overviewPanel.getTranslationY()<0.1){
        ...
        if (!states.stateIsNormal) {
            ...
            }else if(states.stateIsNormalDrag){
                mNewScale = mWorspaceNormalDragShrinkFactor;
            }else if(states.stateIsFolderImport){
                mNewScale = mWorspaceNormalDragShrinkFactor;
            }
        }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            float finalAlpha;
            if (states.stateIsNormalHidden) {//lijun remove  || states.stateIsOverviewHidden
                finalAlpha = 0f;
            ...
            Log.i("Launcher","animated = "+animated);
            if (animated) {
        ...
        //M:liuzuo begin
        /*final View folderImportContainer=mLauncher.getFolderImportContainer();
        final View folderImportHint = mLauncher.getFolderImportHint();
        Animator folderImportContainerAlpha = ObjectAnimator.ofFloat(folderImportContainer, "alpha", 0f, 1f);
        folderImportContainerAlpha.setDuration(duration);
        folderImportContainerAlpha.setInterpolator(new AccelerateInterpolator(1.5f));
        Animator folderImportHintAlpha = ObjectAnimator.ofFloat(folderImportHint, "alpha", 0f, 1f);
        folderImportHintAlpha.setDuration(duration);
        folderImportHintAlpha.setInterpolator(new AccelerateInterpolator(1.5f));*/

        //M:liuzuo end
        if (animated) {
            //first deal workspace
            LauncherViewPropertyAnimator scale = new LauncherViewPropertyAnimator(mWorkspace);
            scale.scaleX(mNewScale)
                    .scaleY(mNewScale)
                    .translationY(finalWorkspaceTranslationY)//liuzuo finalhotseatScaleY
                    .setDuration(duration)
                    .setInterpolator(mZoomInInterpolator);
            mStateAnimator.play(scale);
            //second deal hotseat
            float finalhotseatScaleY = states.stateIsNormalDrag?hotseat.getTranslateYByScale(mNewScale):0;
            Log.i("anim","finalhotseatScaleY = "+finalhotseatScaleY);
            /*LauncherViewPropertyAnimator scaleHotseat = new LauncherViewPropertyAnimator(hotseat);
            scaleHotseat.scaleX(mNewScale)
                    .scaleY(mNewScale)
                    .translationY(finalhotseatScaleY*5)
                    .setDuration(duration)
                    .setInterpolator(mZoomInInterpolator);
            mStateAnimator.play(scaleHotseat);*/

            //third deal celllayout background
            ...
            //forth deal page indicator
            Animator pageIndicatorCicleAnima;
            if (pageIndicatorCicle != null) {
                pageIndicatorCicleAnima = new LauncherViewPropertyAnimator(pageIndicatorCicle)
                        .alpha(finalPageIndicatorAlpha).withLayer()
                        .translationY(finalPageIndicatorTranslationY);
                pageIndicatorCicleAnima.addListener(new AlphaUpdateListener(pageIndicatorCicle,
                        accessibilityEnabled));
            } else {
                // create a dummy animation so we don't need to do null checks later
                pageIndicatorCicleAnima = ValueAnimator.ofFloat(0, 0);
            }
            ...
            LauncherViewPropertyAnimator hotseatAlpha = new LauncherViewPropertyAnimator(hotseat)
                    ...
                    .scaleX(mNewScale) //modify by xiangzx
                    .scaleY(mNewScale)
            ...
            LauncherViewPropertyAnimator overviewPanelAlpha =
                    new LauncherViewPropertyAnimator(overviewPanel)
                            .alpha(finalOverviewPanelAlpha)
                            .translationY(finalOverviewPanelTranslationY);
...
//                pageIndicatorCicleAnima.setInterpolator(new DecelerateInterpolator(2));
                hotseatAlpha.setInterpolator(mZoomInInterpolator);
                overviewPanelAlpha.setInterpolator(null);
            } else if (states.overviewToWorkspace) {
//                pageIndicatorCicleAnima.setInterpolator(null);
                hotseatAlpha.setInterpolator(null);
                overviewPanelAlpha.setInterpolator(new DecelerateInterpolator(2));
            }else if(states.stateIsNormalDrag){

            }else if(states.workspaceToFolderImport){
                hotseatAlpha.setInterpolator(null);
                overviewPanelAlpha.setInterpolator(new DecelerateInterpolator(2));
            }
            pageIndicatorCicleAnima.setInterpolator(mZoomInInterpolator);
            overviewPanelAlpha.setDuration(duration);
            pageIndicatorCicleAnima.setDuration(duration);
            if(states.stateIsFolderImport){//liuzuo add
                hotseatAlpha.setDuration(0);
            }else {
                hotseatAlpha.setDuration(duration);
            }
            pageIndicatorCubeAnima.setDuration(duration);
            ...
            mStateAnimator.play(pageIndicatorCicleAnima);
            ...
            mStateAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if(states.oldStateIsNormalDrag && states.stateIsNormal){
                        mWorkspace.getmPageIndicatorManager().reSetPageIndicator();
                    }
                }
            });
            mLauncher.getWidgetsPanel().setVisibility(View.INVISIBLE);
        } else {
            overviewPanel.setAlpha(finalOverviewPanelAlpha);
            AlphaUpdateListener.updateVisibility(overviewPanel, accessibilityEnabled);
            hotseat.setAlpha(finalHotseatAlpha);
            hotseat.setScaleX(mNewScale);
            hotseat.setScaleY(mNewScale);
            AlphaUpdateListener.updateVisibility(hotseat, accessibilityEnabled);
            if (pageIndicatorCicle != null) {
                pageIndicatorCicle.setAlpha(finalPageIndicatorAlpha);
                pageIndicatorCicle.setTranslationY(finalPageIndicatorTranslationY);
                AlphaUpdateListener.updateVisibility(pageIndicatorCicle, accessibilityEnabled);
            }
            if(pageIndicatorCube !=null){
                pageIndicatorCube.setAlpha(finalPageIndicatorCubeAlpha);
                pageIndicatorCube.setScaleX(finalPageIndicatorCubeScale);
                pageIndicatorCube.setScaleY(finalPageIndicatorCubeScale);
                AlphaUpdateListener.updateVisibility(pageIndicatorCube, accessibilityEnabled);
            }
    ...
    //liuzuo
    public float getWorkspaceTranslationY() {
        float workspaceTranslationPaddingY= -mWorkspace.getPaddingTop()*(1-mNewScale);
        float workspaceTranslationY=  -mWorkspace.getHeight()*(1-mNewScale)*0.125f;
        return   workspaceTranslationPaddingY+workspaceTranslationY;
    }
}