...
public class LauncherAppWidgetHostView extends AppWidgetHostView implements DragLayer.TouchCompleteListener {
    ...
    public void updateAppWidget(RemoteViews remoteViews) {
        ...
        try {
            ...
            Log.e("liuzuo55", "updateAppWidget Resources.NotFoundException");
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mContext instanceof Launcher) {
            if (((Launcher) mContext).getImportMode()) {
                return true;
            }
        }
        //M:liuzuo add end
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                mLongPressHelper.cancelLongPress();
                //lijun add begin for widget mode need longclick to drag but don't click
                if (mContext instanceof Launcher) {
                    if (((Launcher) mContext).isEditorMode()) {
//                        if(!mLongPressHelper.hadLongPress()){
//                            ((Launcher) mContext).handleClickOnWidgetHostView(this);
//                        }
                        return true;
                    }
                }
