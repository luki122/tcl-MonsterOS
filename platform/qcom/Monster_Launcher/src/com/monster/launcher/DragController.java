...
public class DragController {
    ...
    //add by xiangzx to pinch
    private double mDistanceStart = -1;
    private double mDistanceEnd;
    private boolean isPinchOverView;
    private boolean isPinchWorkspace;
    ...
    public DragController(Launcher launcher) {
        ...
        mPageIndicatorCubeDragOverVelocity = (int) (50 * density);
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        mLauncher.fullscreenOrNot(true);
        ...
        if(dragInfo instanceof ItemInfo){
            dragView = mDragObject.dragView = ShortcutFactory.getInstance().createDragView((ItemInfo)dragInfo,mLauncher, b, registrationX,
                    registrationY, 0, 0, b.getWidth(), b.getHeight(), initialDragViewScale);
        }else{
            dragView = mDragObject.dragView = new DragView(mLauncher, b, registrationX,
                    registrationY, 0, 0, b.getWidth(), b.getHeight(), initialDragViewScale);
        }
    ...
    private void endDrag() {
        if (mDragging) {
            ...
            mHandler.removeCallbacks(mFullSreenOrNotRunnable);
            mHandler.postDelayed(mFullSreenOrNotRunnable,350);//liuzuo 350 >> 150
        }

        releaseVelocityTracker();
    }

    Runnable mFullSreenOrNotRunnable = new Runnable() {
        @Override
        public void run() {
            mLauncher.fullscreenOrNot(false);
        }
    };
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        /*Begin add by xiangzx to pinch*/
        mDistanceStart = -1;
        int pointerCount = ev.getPointerCount();
        if (ev.getActionMasked() == MotionEvent.ACTION_POINTER_DOWN && pointerCount == 2) {
            if (mLauncher.getWorkspace().mTouchState == mLauncher.getWorkspace().TOUCH_STATE_REST
                    &&mLauncher.getWorkspace().getOpenFolder() == null
                    &&!mDragging
                    &&(mLauncher.getWorkspace().getState() == Workspace.State.NORMAL
                            || mLauncher.getWorkspace().getState() == Workspace.State.OVERVIEW)) {
                float X1_start = ev.getX(0);
                float Y1_start = ev.getY(0);
                float X2_start = ev.getX(1);
                float Y2_start = ev.getY(1);

                if(mLauncher.getDragLayer().mDockDragController.mIsInHotSeat){
                     mLauncher.getDragLayer().mDockDragController.mIsInHotSeat = false;
                }
                float Delta_X_start = Math.abs(X2_start - X1_start);
                float Delta_Y_start = Math.abs(Y2_start - Y1_start);
                mDistanceStart = Math.sqrt(Delta_X_start * Delta_X_start + Delta_Y_start * Delta_Y_start);
                isPinchOverView = false;
                isPinchWorkspace = false;
            }
            return true;
        }
        /*End add by xiangzx to pinch*/
    ...
    public boolean onTouchEvent(MotionEvent ev) {
       /*Begin add by xiangzx to pinch*/
        ...
        int pointerCount = ev.getPointerCount();
        if (pointerCount == 2 && ev.getAction() == MotionEvent.ACTION_MOVE && !mDragging) {
            if(mDistanceStart >= 0 && !isPinchOverView && !isPinchWorkspace) {
                ...
                mDistanceEnd = Math.sqrt(Delta_X_end * Delta_X_end + Delta_Y_end * Delta_Y_end);
                double delta_Dist = mDistanceStart - mDistanceEnd;
                float thresholdIn = mLauncher.getWorkspace().getWidth() * 0.030f;
                float thresholdOut = mLauncher.getWorkspace().getWidth() * 0.015f;
                if (delta_Dist >= thresholdIn && mLauncher.getWorkspace().getState() == Workspace.State.NORMAL) {
                    isPinchOverView = true;
                    mLauncher.showOverviewMode(true);
                    mLauncher.getWorkspace().performHapticFeedback(HapticFeedbackConstants.LONG_PRESS,
                            HapticFeedbackConstants.FLAG_IGNORE_VIEW_SETTING);

                } else if (delta_Dist <= -thresholdOut && mLauncher.getWorkspace().isInOverviewMode()) {
                    isPinchWorkspace = true;
                    mLauncher.showWorkspace(true);
                }
                CellLayout cellLayout = (CellLayout)mLauncher.mWorkspace.getChildAt(mLauncher.mWorkspace.getCurrentPage());
                if(cellLayout != null) {
    ...
    public boolean checkVelocityTrackerToPage(){
        ...
        if (Math.abs(mVelocityTracker.getXVelocity()) < mPageIndicatorCubeDragOverVelocity
                && Math.abs(mVelocityTracker.getYVelocity()) < mPageIndicatorCubeDragOverVelocity){
            return true;
        }
        return false;
    }
