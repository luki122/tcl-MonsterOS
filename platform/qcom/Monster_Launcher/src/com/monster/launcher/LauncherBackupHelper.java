...
import com.monster.launcher.backup.BackupProtos;
import com.monster.launcher.compat.UserHandleCompat;
import com.monster.launcher.compat.UserManagerCompat;
import com.monster.launcher.model.MigrateFromRestoreTask;
...
public class LauncherBackupHelper implements BackupHelper {
    ...
    private static final String[] FAVORITE_PROJECTION = {
        ...
        LauncherSettings.Favorites.MODIFIED,                // 1
        LauncherSettings.Favorites.INTENT,                  // 2
        LauncherSettings.Favorites.APPWIDGET_PROVIDER,      // 3
        LauncherSettings.Favorites.APPWIDGET_ID,            // 4
        LauncherSettings.Favorites.CELLX,                   // 5
        LauncherSettings.Favorites.CELLY,                   // 6
        LauncherSettings.Favorites.CONTAINER,               // 7
        LauncherSettings.Favorites.ICON,                    // 8
        LauncherSettings.Favorites.ICON_PACKAGE,            // 9
        LauncherSettings.Favorites.ICON_RESOURCE,           // 10
        LauncherSettings.Favorites.ICON_TYPE,               // 11
        LauncherSettings.Favorites.ITEM_TYPE,               // 12
        LauncherSettings.Favorites.SCREEN,                  // 13
        LauncherSettings.Favorites.SPANX,                   // 14
        LauncherSettings.Favorites.SPANY,                   // 15
        LauncherSettings.Favorites.TITLE,                   // 16
        LauncherSettings.Favorites.PROFILE_ID,              // 17
        LauncherSettings.Favorites.RANK,                    // 18
    };
    ...
    private static final String[] SCREEN_PROJECTION = {
        LauncherSettings.WorkspaceScreens._ID,              // 0
        LauncherSettings.WorkspaceScreens.MODIFIED,         // 1
        LauncherSettings.WorkspaceScreens.SCREEN_RANK       // 2
    };
    ...
    private final ArrayList<BackupProtos.Key> mKeys;
    ...
    public LauncherBackupHelper(Context context) {
        ...
        mKeys = new ArrayList<BackupProtos.Key>();
    ...
    public void performBackup(ParcelFileDescriptor oldState, BackupDataOutput data,
        ...
        try {
            ...
            for (BackupProtos.Key key : mKeys) {
            ...
            if (!mBackupDataWasUpdated) {
                ...
                mBackupDataWasUpdated = (in.profile == null)
                        || !Arrays.equals(BackupProtos.DeviceProfieData.toByteArray(in.profile),
                            BackupProtos.DeviceProfieData.toByteArray(mDeviceProfileData))
    ...
    private boolean isBackupCompatible(BackupProtos.Journal oldState) {
        BackupProtos.DeviceProfieData currentProfile = mDeviceProfileData;
    ...
    private void backupFavorites(BackupDataOutput data) throws IOException {
        ...
        Cursor cursor = cr.query(LauncherSettings.Favorites.CONTENT_URI, FAVORITE_PROJECTION,
        ...
        try {
            ...
            while(cursor.moveToNext()) {
                ...
                BackupProtos.Key key = getKey(BackupProtos.Key.FAVORITE, id);
                mKeys.add(key);
    ...
    private void backupScreens(BackupDataOutput data) throws IOException {
        // persist things that have changed since the last backup
        ContentResolver cr = mContext.getContentResolver();
        Cursor cursor = cr.query(LauncherSettings.WorkspaceScreens.CONTENT_URI, SCREEN_PROJECTION,
                null, null, null);
        try {
            cursor.moveToPosition(-1);
            ...
            while(cursor.moveToNext()) {
                final long id = cursor.getLong(ID_INDEX);
                final long updateTime = cursor.getLong(ID_MODIFIED);
                BackupProtos.Key key = getKey(BackupProtos.Key.SCREEN, id);
                mKeys.add(key);
                final String backupKey = keyToBackupKey(key);
                if (!mExistingKeys.contains(backupKey) || updateTime >= mLastBackupRestoreTime) {
    ...
    private void backupIcons(BackupDataOutput data) throws IOException {
        ...
        // Don't backup apps in other profiles for now.
        String where = "(" + LauncherSettings.Favorites.ITEM_TYPE + "=" + LauncherSettings.Favorites.ITEM_TYPE_APPLICATION + " OR " +
                LauncherSettings.Favorites.ITEM_TYPE + "=" + LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT + ") AND " +
                getUserSelectionArg();
        Cursor cursor = cr.query(LauncherSettings.Favorites.CONTENT_URI, FAVORITE_PROJECTION,
                where, null, null);
        try {
            ...
            while(cursor.moveToNext()) {
                ...
                try {
                    ...
                    if (cn != null) {
                        key = getKey(BackupProtos.Key.ICON, cn.flattenToShortString());
    ...
    private void backupWidgets(BackupDataOutput data) throws IOException {
        ...
        String where = LauncherSettings.Favorites.ITEM_TYPE + "=" + LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET + " AND "
                + getUserSelectionArg();
        Cursor cursor = cr.query(LauncherSettings.Favorites.CONTENT_URI, FAVORITE_PROJECTION,
                where, null, null);
        try {
            ...
            while(cursor.moveToNext()) {
                ...
                String backupKey = null;
                if (provider != null) {
                    key = getKey(BackupProtos.Key.WIDGET, providerName);
                    backupKey = keyToBackupKey(key);
                } else {
    ...
    private void restoreWidget(BackupProtos.Key key, byte[] buffer, int dataSize) throws IOException {
        ...
        if (DEBUG) Log.d(TAG, "read (" + buffer.length + "): " +
                Base64.encodeToString(buffer, 0, dataSize, Base64.NO_WRAP));
        BackupProtos.Widget widget = unpackProto(new BackupProtos.Widget(), buffer, dataSize);
    ...
    private String keyToBackupKey(BackupProtos.Key key) {
        return Base64.encodeToString(BackupProtos.Key.toByteArray(key), Base64.NO_WRAP);
    }
    ...
    private BackupProtos.Key backupKeyToKey(String backupKey) throws InvalidBackupException {
        try {
            BackupProtos.Key key = BackupProtos.Key.parseFrom(Base64.decode(backupKey, Base64.DEFAULT));
    ...
    private boolean isReplaceableHotseatItem(BackupProtos.Favorite favorite) {
        return favorite.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT
                && favorite.intent != null
                && (favorite.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION
                || favorite.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT);
    }
    ...
    private BackupProtos.Favorite packFavorite(Cursor c) {
        ...
        if (favorite.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET) {
        ...
        } else if (favorite.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT) {
            if (favorite.iconType == LauncherSettings.Favorites.ICON_TYPE_RESOURCE) {
    ...
    private ContentValues unpackFavorite(byte[] buffer, int dataSize)
            throws IOException {
        BackupProtos.Favorite favorite = unpackProto(new BackupProtos.Favorite(), buffer, dataSize);

        // If it is a hotseat item, move it accordingly.
        if (favorite.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
            favorite.screen += mHotseatShift;
        }

        ContentValues values = new ContentValues();
        values.put(LauncherSettings.Favorites._ID, favorite.id);
        ...
        values.put(LauncherSettings.Favorites.SPANX, favorite.spanX);
        values.put(LauncherSettings.Favorites.SPANY, favorite.spanY);
        values.put(LauncherSettings.Favorites.RANK, favorite.rank);

        if (favorite.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT) {
            values.put(LauncherSettings.Favorites.ICON_TYPE, favorite.iconType);
            if (favorite.iconType == LauncherSettings.Favorites.ICON_TYPE_RESOURCE) {
                values.put(LauncherSettings.Favorites.ICON_PACKAGE, favorite.iconPackage);
                values.put(LauncherSettings.Favorites.ICON_RESOURCE, favorite.iconResource);
            }
            values.put(LauncherSettings.Favorites.ICON, favorite.icon);
        }

        if (!TextUtils.isEmpty(favorite.title)) {
            values.put(LauncherSettings.Favorites.TITLE, favorite.title);
        } else {
        ...
        values.put(LauncherSettings.Favorites.ITEM_TYPE, favorite.itemType);
        ...
        // grid migration.
        BackupProtos.DeviceProfieData currentProfile = migrationCompatibleProfileData == null
        ...
        if (favorite.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET) {
            ...
            values.put(LauncherSettings.Favorites.APPWIDGET_ID, favorite.appWidgetId);
            ...
            if (isReplaceableHotseatItem(favorite)
                    && favorite.targetType != BackupProtos.Favorite.TARGET_NONE
    ...
    private ContentValues unpackScreen(byte[] buffer, int dataSize)
        ...
        BackupProtos.Screen screen = unpackProto(new BackupProtos.Screen(), buffer, dataSize);
        ContentValues values = new ContentValues();
        values.put(LauncherSettings.WorkspaceScreens._ID, screen.id);
        values.put(LauncherSettings.WorkspaceScreens.SCREEN_RANK, screen.rank);
        return values;
    }
    ...
    private BackupProtos.Widget packWidget(int dpi, ComponentName provider, UserHandleCompat user) {
    ...
    private void writeRowToBackup(BackupProtos.Key key, MessageNano proto, BackupDataOutput data)
            throws IOException {
    ...
    private String getUserSelectionArg() {
        return LauncherSettings.Favorites.PROFILE_ID + '=' + UserManagerCompat.getInstance(mContext)
