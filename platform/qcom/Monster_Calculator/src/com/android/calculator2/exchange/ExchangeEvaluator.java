...
public class ExchangeEvaluator {
    ...
    private final TCLExchangeActivity mExchangeCalculator;
    private final ExchangeCalculatorResult mResult;
    ...
    ExchangeEvaluator(TCLExchangeActivity calculator,
            ExchangeCalculatorResult resultDisplay) {
        mExchangeCalculator = calculator;
    ...
    private void displayCancelledMessage() {
        new AlertDialog.Builder(mExchangeCalculator)
    ...
    private void displayTimeoutMessage() {
        AlertDialogFragment.showMessageDialog(mExchangeCalculator, mExchangeCalculator.getString(R.string.timeout),
                (mLongTimeout ? null : mExchangeCalculator.getString(R.string.ok_remove_timeout)));
    }
        ...
        protected void onPostExecute(InitialResult result) {
            ...
            if (result.isError()) {
                if (result.errorResourceId == R.string.timeout) {
                    ...
                    mExchangeCalculator.onCancelled();
                } else {
                    mExchangeCalculator.onError(result.errorResourceId);
                }
                return;
            }
            mVal = result.val;
            ...
            mExchangeCalculator.onEvaluate(initPrecOffset, msdIndex, leastDigOffset, truncatedWholePart);
            
            Log.i("zouxu", "onPostExecute evaluatorResultString="+evaluatorResultString);
        }
        @Override
    ...
    static String unflipZeroes(String oldDigs, int oldPrecOffset, String newDigs,
            int newPrecOffset) {
        if(oldDigs == null){
            Log.e("zouxu", "unflipZeroes oldDigs == null");
        } 
        final int oldLen = oldDigs.length();//oldDigs可能为空 重点排查
    ...
    private class AsyncReevaluator extends AsyncTask<Integer, Void, ReevalResult> {
        ...
        protected void onPostExecute(ReevalResult result) {
            if (result == null) {
                ...
                mExchangeCalculator.onError(R.string.error_nan);
            } else {
                if (result.newResultStringOffset < mResultStringOffset) {
                    throw new AssertionError("Unexpected onPostExecute timing");
                }
                
                if(mResultString == null){//zouxu
                    Log.e("zouxu", "onPostExecute mResultString == null");
                } else {
                    Log.e("zouxu", " befor mResultString  ="+mResultString);
                    mResultString = unflipZeroes(mResultString, mResultStringOffset,
                            result.newResultString, result.newResultStringOffset);
                    Log.e("zouxu", "after mResultString  ="+mResultString);

                }
                
                mResultStringOffset = result.newResultStringOffset;
                mExchangeCalculator.onReevaluate();
            }
     ...
     * Will eventually call back mExchangeCalculator to display result or error, or display
    ...
    public void requireResult() {
        if (mResultString == null || mChangedValue) {
            ...
            final int preferredPrecOffset = getPreferredPrec(mResultString, msdIndex,
                    leastDigOffset);
            mExchangeCalculator.onEvaluate(preferredPrecOffset, msdIndex, leastDigOffset,
                    truncatedWholePart);
        }
    }
