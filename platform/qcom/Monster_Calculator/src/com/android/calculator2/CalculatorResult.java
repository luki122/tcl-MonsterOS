...
import com.android.calculator2.utils.TransToChineseUtils;
...
public class CalculatorResult extends AlignedTextView {
    ...
    private TransToChineseCallBack mCallBack;//转换成中文大写的回调 zouxu
    public void setToChineseCallBack(TransToChineseCallBack callback){
        mCallBack = callback;
    }
    ...
    public CalculatorResult(Context context, AttributeSet attrs) {
        ...
        mGestureDetector = new GestureDetector(context,
            new GestureDetector.SimpleOnGestureListener() {
                ...
                @Override
                public boolean onSingleTapUp(MotionEvent e) {//add zouxu
                    if(mCallBack !=null){
                        mCallBack.dismisActionMode();
                    }
...
//    private CalculatorText newResult;
//    
//    public void setNewResult(CalculatorText resut){
//        newResult = resut;
//    }
...
//        final float newCharWidth = Layout.getDesiredWidth("\u2007", paint);
        final float newCharWidth = Layout.getDesiredWidth("1", paint);//zouxu test 20160906 当前版本的字体宽度有问题 导致测量有问题显示不完整　用2来测试
     ...
     */
   public  void displayResult(int initPrec, int msd, int leastDigPos, String truncatedWholePart) {
...
//        if(newResult !=null&&mEvaluator.getCalculatorResult().getText().equals(getContext().getString(R.string.dec_point)) ){
//            newResult.setText("0.");
//        } else if(newResult !=null &&mEvaluator.getCalculatorResult().getText().equals(getContext().getString(R.string.op_sub)) ){
//            newResult.setText(R.string.op_sub);
//        } else {
//            newResult.setText("");
//        }
    ...
    void redisplay() {
        ...
        Log.i("zouxu","redisplay result = "+result);
...
//            setText(formattedResult,BufferType.SPANNABLE); //chg zouxu test
        } else {
            setText(result);
//            setText(result,BufferType.SPANNABLE);//chg zouxu test
        }
        mLastDisplayedOffset = lastDisplayedOffset[0];
        mValid = true;
        
//            newResult.setText(result);
    }
    ...
    public void ToChinese(){
        String chinese_number = TransToChineseUtils.changeDigit(getFullText().toString(), true);
        if(mCallBack !=null){
            mCallBack.callBack(chinese_number);
        }
    }
    
    public boolean isHideToChinese(){

        if(!getContext().getResources().getConfiguration().locale.getCountry().equals("CN")){
            return true;
        }

        String getText  = getFullText();
        if(getText.contains(getContext().getString(R.string.op_sub))||getText.contains("E") ||!isNum(getText)|| getText.length()>9){
            return true;
        }
        return false;
    }
    
    public String getEvaluatorResult(){//add zouxu 20160914
        return mEvaluator.getEvaluatorResultString();
    }

    public void setEvaluatorResult(String str){
        mEvaluator.setEvaluatorResultString(str);
    }


    
    
    public static boolean isNum(String str){//判断是否是数字 因为有可能是显示错误信息
