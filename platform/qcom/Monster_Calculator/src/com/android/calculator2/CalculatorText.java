...
public class CalculatorText extends AlignedTextView implements View.OnLongClickListener {
    ...
    private boolean isPastAble = true;//是否允许粘贴功能

    public void setPastAble(boolean is) {
        isPastAble = is;
    }

    private Calculator.CalculatorState state;

    public void setState(Calculator.CalculatorState mstate){
        state = mstate;
    }
    ...
    private final ActionMode.Callback2 mPasteActionModeCallback = new ActionMode.Callback2() {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            if (!isPastAble || state == Calculator.CalculatorState.ERROR) {
                return false;
            }
    ...
    public void changeTextTo(CharSequence newText) {
        ...
        if (Utils.isNumber(getContext(), newText.toString())) {//判断是否是纯数字　是纯数字就不显示
    ...
    private boolean  isPastAble() {
        ...
        String str = item.coerceToText(getContext()).toString();
        if(TextUtils.isEmpty(str)){
            return false;
        }

        return CalculatorResult.isNum(str);
    }
