...
import mst.provider.Telephony.Mms.Addr;
import mst.provider.Telephony.Mms.Part;
import mst.provider.Telephony.Mms.Rate;
...
import android.provider.TctExtTelephony;
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
    //tangyisen if modify trigger need rm database and reboot phone
    //begin tangyisen need modify snippet when reject = 1 not change
    private static final String SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE =
                        "BEGIN" +
                        "  UPDATE threads SET" +
                        "    date = (strftime('%s','now') * 1000), " +
                        "    snippet = new." + Sms.BODY + ", " +
                        "    time_zone = new."+Sms.TIME_ZONE + ", " +
                        "    snippet_cs = 0" +
                        "  WHERE threads._id = new." + Sms.THREAD_ID + "; " +
    ...
    private static final String SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE_USE_SAME_DATE =
            "BEGIN" +
            "  UPDATE threads SET" +
            "    date = new." + Sms.DATE +", "+
            "    snippet = new." + Sms.BODY + ", " +
  ...
  //end tangyisen
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    //begin tangyisen add for add on words when sms reject from 1 to 0
    private static final String SMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE =
        "  INSERT INTO words " +
        " (_id, index_text, source_id, table_to_use) VALUES" +
        " (new." + Sms._ID + ", new." + Sms.BODY + ", new." + Sms._ID + ", 1);  ";

    private static final String MMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE =
        "  INSERT INTO words " +
        " (_id, index_text, source_id, table_to_use) " +
        " select (2<<32)+_id, text, _id, 2 FROM part where part.mid=new."+ Mms._ID + ";  ";

    private static final String SMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE_CONSTRAINTS =
        "  WHEN new." + Sms.REJECT + "= 0  ";

    private static final String MMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE_CONSTRAINTS =
        "  WHEN new." + Mms.REJECT + "= 0  ";
    /*private static final String SMS_INSERT_WORDS_ON_MMS_REJECT_UPDATE =
        "  INSERT INTO words " +
        " (_id, index_text, source_id, table_to_use) VALUES" +
        " (new." + Sms._ID + ", new." + Sms.BODY + ", new." + Sms._ID + ", 2" +
        "  WHERE new." + Sms.REJECT +" = 0 ; ";*/
    //end tangyisen
...
//Porting TelephonyProvider
    ...
    private static final String PDU_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE_USE_SAME_DATE =
                         "BEGIN" +
                         "  UPDATE threads SET" +
                         "    date = new." + Mms.DATE + " * 1000" + ", "+
                         "    snippet = new." + Mms.SUBJECT + ", " +
                         "    time_zone = new." +Mms.TIME_ZONE + ", " +
                         "    snippet_cs = new." + Mms.SUBJECT_CHARSET +
                         "  WHERE threads._id = new." + Mms.THREAD_ID + "; " +
    ...
    private static final String PDU_SMS_UPDATE_THREAD_READ_ON_DELETE_TRIGGER =
                         "BEGIN " +
                         "  UPDATE threads SET read = " +
                         "    CASE (SELECT COUNT(read) " +
                         "          FROM (select read from pdu where read=0 AND thread_id = OLD.thread_id " +
                         "                union all " +
                         "                select read from sms where read=0 AND thread_id = OLD.thread_id) ) " +
                         "    WHEN 0 THEN 1 " +
                         "    ELSE 0 " +
                         "    END " +
                         "WHERE threads._id = OLD.thread_id; END;";
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        //RcsMmsSmsConstants.init(sCeInstance);
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
        return sCeInstance;
    }
...
//Porting TelephonyProvider
                db.execSQL(
                      "  UPDATE threads SET time_zone = "+
                              " (SELECT timezone FROM"+
                              " (SELECT thread_id,date,timezone FROM sms"+
                              " WHERE thread_id ="+ thread_id+
                              " UNION SELECT thread_id,date * 1000 AS date,timezone FROM pdu)"+
                              " WHERE thread_id ="+ thread_id+
                              " ORDER BY date DESC LIMIT 1)"+
                              " WHERE recipient_ids = " + thread_id);
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        //RcsThreadsProvider.updateThread(db, RcsThreadsProvider.getThreadsIdFromConvId(db, thread_id));
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    }
    ...
    private static void updateThreadDate(SQLiteDatabase db, long thread_id) {
        ...
        try {
            ...
            "        (SELECT date * 1000 AS date, thread_id FROM pdu" +
            "         UNION SELECT date, thread_id FROM sms)" +
            "     WHERE reject = 0 AND thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1)" +
            "  WHERE reject = 0 AND threads._id = " + thread_id + ";");
        } catch (Throwable ex) {
            Log.e(TAG, ex.getMessage(), ex);
        }
    }
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
    public static void updateMultiThreads(SQLiteDatabase db, Set<Long> deletedThreads) {
        Log.d(TAG, "updateMultiThreads start, deletedThreads.length = " + deletedThreads.size());
        ...
        Iterator<Long> it = deletedThreads.iterator();
        while(it.hasNext()){
            long deleteThreadId = it.next(); 
            if (Arrays.binarySearch(threadIds, deleteThreadId) >= 0) {
                updateThread(db, deleteThreadId);
            } else {
                Log.i(TAG, "deletedThreadId =" + deleteThreadId );
            }
        }
       /* for (long deletedThread : deletedThreads) {
            if (Arrays.binarySearch(threadIds, deletedThread) >= 0) {
                ...
                Log.i(TAG, "deletedThread =" + deletedThread );
            }
        }*/
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        //RcsThreadsProvider.updateAllThreads(db, null, null);
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        //RcsMmsSmsConstants.createExtention(sCeInstance, db);
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    }
    ...
    private void populateWordsTable(SQLiteDatabase db) {
        final String TABLE_WORDS = "words";
        {
            ...
            Cursor smsRows = db.query(
                "sms",
                new String[] { Sms._ID, Sms.BODY, Sms.REJECT },
                null, null, null, null, null);
            //end tangyisen
            try {
                if (smsRows != null) {
...

        {
            ...
            /*Cursor mmsRows = db.query(
                    ...
                    null,
                    null);*/
            String rawSql = "select Part._ID, Part.TEXT FROM part,pdu where part.mid=pdu._id AND pdu.reject=0 AND ct = 'text/plain'";
            Cursor mmsRows = db.rawQuery(rawSql, null);
            //end tangyisen
            try {
                if (mmsRows != null) {
    ...
    private void createWordsTables(SQLiteDatabase db) {
        try {
            ...
            /*db.execSQL("CREATE TRIGGER sms_words_update AFTER UPDATE ON sms BEGIN UPDATE words " +
                    ...
                    " END;");*/
            db.execSQL("CREATE TRIGGER sms_words_update AFTER UPDATE ON sms BEGIN UPDATE words " +
                " SET index_text = NEW.body WHERE (NEW.reject=0 AND source_id=NEW._id AND table_to_use=1) ; " +
                " END;");
            //end tangyisen
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + Mms.RECOMMENDED_RETRIEVAL_MODE + " INTEGER DEFAULT 0");

        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + TctExtTelephony.DELIVERY_R +" INTEGER DEFAULT 0");

        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + TIME_ZONE +" TEXT");
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)

        //begin tangyisen
        addColumn_Mms_REJECT(db);
        addColumn_Mms_ImageParam(db);
        //end tangyisen
    ...
    private void createMmsTriggers(SQLiteDatabase db) {
        ...
        //begin tangyisen modify reject not need update,becase not insert,so fine
        ...
        //tangyisen mms_update_thread_reject_on_update modify in code?
        db.execSQL("DROP TRIGGER IF EXISTS mms_update_thread_reject_on_update");
        db.execSQL("CREATE TRIGGER mms_update_thread_reject_on_update AFTER" +
            "  UPDATE OF " + Mms.REJECT +
            "  ON pdu " +
            MMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE_CONSTRAINTS +
            "  BEGIN " +
            MMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE +
            "  END;");
        //end tangyisen
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        if(!Resources.getSystem().getBoolean(com.android.internal.R.bool.feature_mms_threads_use_pdu_or_sms_date_on)) {
            ...
            db.execSQL("CREATE TRIGGER pdu_update_thread_date_subject_on_update AFTER" +
                    ...
                    "  ON " + MmsProvider.TABLE_PDU + " " +
                    PDU_UPDATE_THREAD_CONSTRAINTS +
                    PDU_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE_USE_SAME_DATE);
        }
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
...
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
        ...
        //lichao add begin
        addColumn_Sms_CAR_CODE(db);
        //lichao add end
        //begin tangyisen
        addColumn_Sms_REJECT(db);
        //end tangyisen
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
                   Threads.TIME_ZONE + " TEXT," +
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
                   Threads.ATTACHMENT_INFO + " TEXT," +
                   Threads.NOTIFICATION + " INTEGER DEFAULT 0," +
                   //", sub_id INTEGER DEFAULT -1" + // add by ligy
                   Threads.SUBSCRIPTION_ID + " INTEGER DEFAULT -1" +//add by ligy, lichao modify
                   ");");
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        //tangyisen update sms not change,because can not make reject 0 to 1,just can make 1 to 0
        if(!Resources.getSystem().getBoolean(com.android.internal.R.bool.feature_mms_threads_use_pdu_or_sms_date_on)){
            ...
            db.execSQL("CREATE TRIGGER sms_update_thread_on_insert AFTER INSERT ON sms " +
                    SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE_USE_SAME_DATE);
...
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
        ...
        //begint tangyisen when sms.reject from 1 to 0,need add words
        db.execSQL("CREATE TRIGGER sms_update_thread_reject_on_update AFTER" +
            "  UPDATE OF " + Sms.REJECT +
            "  ON sms " +
            SMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE_CONSTRAINTS +
            "  BEGIN " +
            SMS_INSERT_WORDS_ON_SMS_REJECT_UPDATE +
            "  END;");
        //end tangyisen

        // Updates threads table whenever a message in sms is updated.
        //tangyisen SMS_UPDATE_THREAD_READ_BODY has add reject
...
//                   "END;");
        ...
        db.execSQL("CREATE TRIGGER update_threads_error_on_update_sms " +
            "  AFTER UPDATE OF type ON sms" +
            "  WHEN NEW.reject = 0 AND ((OLD.type != 5 AND NEW.type = 5)" +
            "    OR (OLD.type = 5 AND NEW.type != 5)) " +
            "BEGIN " +
            "  UPDATE threads SET error = " +
            "    CASE" +
...
//[SOLUTION]-Add-BEGIN by TCTNB.(JiangLong Pan), 07/30/2016, SOLUTION-2466195
//Porting TelephonyProvider
        //RcsMmsSmsConstants.onUpgrade(db, oldVersion, currentVersion);
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
...
//Porting TelephonyProvider
        case 64:
            ...
            return;
          //[FEATURE]-Add-End by TCTNB.jian.shi,11/04/2015,Task-857368

        //[BUGFIX]-Add-BEGIN by TSCD.qiang.sun, 10/19/2015,PR-1048659 begin
        case 65:
        ...
        case 67:
            ...
            return;
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
        //lichao add in 2016-10-14 begin
        case 68://oldVersion
    ...
    private void checkAndUpdateSmsTable(SQLiteDatabase db) {
        ...
        //lichao add in 2016-10-24 begin
        try {
            db.query(SmsProvider.TABLE_SMS, new String[] {Sms.CAR_CODE}, null, null, null, null,
                    null);
        } catch (SQLiteException e) {
            Log.e(TAG, "checkAndUpgradeSmsTable: ex. ", e);
            if (e.getMessage().startsWith(NO_SUCH_COLUMN_EXCEPTION_MESSAGE)) {
        ...
        //lichao add in 2016-10-24 end
        //begin tangyisen
        try {
            db.query(SmsProvider.TABLE_SMS, new String[] {Sms.REJECT}, null, null, null, null,
                    null);
        } catch (SQLiteException e) {
            Log.e(TAG, "checkAndUpgradeSmsTable: ex. ", e);
            if (e.getMessage().startsWith(NO_SUCH_COLUMN_EXCEPTION_MESSAGE)) {
...
//Porting TelephonyProvider
    private void upgradeDatabaseToVersion65(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TRIGGER sms_update_thread_read_on_delete "
                 + "AFTER delete ON sms "
                 + PDU_SMS_UPDATE_THREAD_READ_ON_DELETE_TRIGGER);
        db.execSQL("DROP TRIGGER IF EXISTS pdu_update_thread_read_on_delete");
        db.execSQL("CREATE TRIGGER pdu_update_thread_read_on_delete "
                 + "AFTER delete ON pdu "
                 + PDU_SMS_UPDATE_THREAD_READ_ON_DELETE_TRIGGER);
    }
    ...
    private void upgradeDatabaseToVersion66(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + Mms.RECOMMENDED_RETRIEVAL_MODE + " INTEGER DEFAULT 0");
        db.execSQL("ALTER TABLE " + "pdu_temp" + " ADD COLUMN " + Mms.RECOMMENDED_RETRIEVAL_MODE +" INTEGER DEFAULT 0");
    }

    private void upgradeDatabaseToVersion67(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + TctExtTelephony.DELIVERY_R +" INTEGER DEFAULT 0");
		db.execSQL("ALTER TABLE " + "pdu_temp" + " ADD COLUMN " + TctExtTelephony.DELIVERY_R +" INTEGER DEFAULT 0");
    }

    private void upgradeDatabaseToVersion68(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS + " ADD COLUMN " + TIME_ZONE +" TEXT");
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + TIME_ZONE +" TEXT");
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.TIME_ZONE + " TEXT");
        db.execSQL("ALTER TABLE " + "pdu_temp" + " ADD COLUMN " + TIME_ZONE +" TEXT");
    }
//[SOLUTION]-Add-END by TCTNB.(JiangLong Pan)
    ...
    private void addColumn_Sms_CAR_CODE(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS + " ADD COLUMN " + Sms.CAR_CODE +" INTEGER DEFAULT 0");
    }
    //lichao add in 2016-10-14 end
    ...
    private void addColumn_Sms_REJECT(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS + " ADD COLUMN " + Sms.REJECT +" INTEGER DEFAULT 0");
    }

    private void addColumn_Mms_REJECT(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + Mms.REJECT +" INTEGER DEFAULT 0");
    }
    ...
    private void addColumn_Mms_ImageParam(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + Mms.EXTRA +" TEXT");
    }
    //end tangyisen
    ...
    private void upgradeThreadsTableToAutoIncrement(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE threads_temp (" +
                ...
                Threads.ATTACHMENT_INFO + " TEXT," +
                Threads.NOTIFICATION + " INTEGER DEFAULT 0," +
                //", sub_id INTEGER DEFAULT -1" + //add by lgy
                Threads.SUBSCRIPTION_ID + " INTEGER DEFAULT -1" +//add by ligy, lichao modify
                /*
                //lichao: these codes conflic with upgradeDatabaseToVersion69() ?
SQLiteException: duplicate column name: sub_id (code 1): ,
while compiling: ALTER TABLE threads ADD COLUMN sub_id INTEGER DEFAULT -1
at com.android.providers.telephony.MmsSmsDatabaseHelper.upgradeDatabaseToVersion69(MmsSmsDatabaseHelper.java:2136)
                 */
                ");");
