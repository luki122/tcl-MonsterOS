...
import cn.tcl.filemanager.manager.SafeCategoryCountManager;
...
public class SafeCategoryAdapter extends BaseAdapter {
    ...
    public SafeCategoryAdapter(Context context) {
        ...
        mCategoryCountManager = SafeCategoryCountManager.getInstance();
...
//        mGridHolder.mainLayout.setBackground(item.getMainDrawable());
    ...
    private void loadCountText(final TextView textview, final int position) {
        final Handler mmHandler = new Handler() {

            @Override
            public void handleMessage(Message msg) {
                if (String.valueOf(position).equals(textview.getTag())) {
                    textview.setText((String) msg.obj);
                }
            }
        };

        mCategoryCountManager.loadCategoryCountText(position, mContext,
                new SafeCategoryCountManager.CountTextCallback() {

                    @Override
                    public void countTextCallback(String countText) {
                        // if (!TextUtils.isEmpty(countText)) {
                        Message message = mmHandler.obtainMessage(0, 1, 1, countText);
