...
public class VoicemailContentTable implements VoicemailTable.Delegate {
    ...
    public Uri insert(UriData uriData, ContentValues values) {
        ...
        /**M: append data_id & raw_contact_id for calls @{*/
        String strInsNumber = values.getAsString(Calls.NUMBER);
        LogUtils.d(TAG, "[insert] get default insert number:" + strInsNumber);
        // No need to compute and append if number is empty
        if (!TextUtils.isEmpty(strInsNumber)) {
            CallLogProviderEx.getInstance(mContext)
                .appendComputedRawContactForCalls(db, strInsNumber, copiedValues);
        }
        /** @} */

        /** M: ALPS00822600: insert VVM and update VVM Uri in one transaction @{ */
        try {
            db.beginTransaction();
            ...
            if (rowId > 0) {
                ...
                /** M: update dialer search data */
                DialerSearchSupport.getInstance(mContext).handleCallLogsInserted(db, rowId,
                        strInsNumber);
    ...
    public int delete(UriData uriData, String selection, String[] selectionArgs) {
        ...
        if (count > 0) {
            DialerSearchSupport.getInstance(mContext).handleCallLogsDeleted(db);
        }
        return count;
        /** M: [VVM] trigger VVM for dialer search @} */
    }
