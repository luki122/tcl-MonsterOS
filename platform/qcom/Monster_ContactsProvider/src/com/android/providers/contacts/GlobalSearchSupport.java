...
public class GlobalSearchSupport {
    ...
    private static class SearchSuggestion {
        ...
        int mSimIndicator = -1;  // add by MTK, used for sim contacts.
        int mSlotId = -1;        // add by MTK, used for sim contacts.
        int mIsSdnContact = 0;   // add by MTK, used for sim contacts.
    ...
    public Cursor handleSearchSuggestionsQuery(SQLiteDatabase db, Uri uri, String[] projection,
        ...
        LogUtils.d(TAG, "[handleSearchSuggestionsQuery]result cursor count:"
    ...
    public Cursor handleSearchShortcutRefresh(SQLiteDatabase db, String[] projection,
        ...
        LogUtils.d(TAG, "[handleSearchShortcutRefresh]lookupKey:" + lookupKey
                + " ||filter:" + filter);
        long contactId;
        try {
        ...
        LogUtils.d(TAG, "[handleSearchShortcutRefresh]contactId:" + contactId);
        ...
        Cursor c = addSearchSuggestionsBasedOnFilter(cursor,
                db, projection, ContactsColumns.CONCRETE_ID + "=" + contactId, filter, null,
                cancellationSignal);
        LogUtils.d(TAG, "[handleSearchShortcutRefresh]Result cursor count:"
                + (c == null ? "NULL" : c.getCount()));
        c = GlobalSearchSupportEx.processCursor(c,
                projection, lookupKey, SEARCH_SUGGESTIONS_COLUMNS);
        return c;
        /**
         * @}
         */
    ...
    private Cursor addSearchSuggestionsBasedOnFilter(MatrixCursor cursor, SQLiteDatabase db,
        ...
        sb.append(", " + Contacts.INDICATE_PHONE_SIM); /** M: New Feature SimRelated added */
        sb.append(", " + Contacts.IS_SDN_CONTACT); /** M: New Feature SimRelated added */
        sb.append(" FROM ");
        ...
        try {
            while (c.moveToNext()) {
                ...
                /// M: @{
                getCursorField(c, suggestion, ((haveFilter) ? 7 : 6));
                /// M: @}
        ...
        Log.i(TAG, "startTime : " + startTime + " | endTime : " + endTime + " | time : "
                + (endTime - startTime));
        /// @}
        return cursor;
    }
    ...
    private void getCursorField(Cursor c, SearchSuggestion suggestion, int index) {
        suggestion.mSimIndicator = c.getInt(index);
        suggestion.mIsSdnContact = c.getInt(index + 1);
        Context context = mContactsProvider.getContext();
        SubscriptionInfo subInfo = SubscriptionManager
                .from(context).getActiveSubscriptionInfo(suggestion.mSimIndicator);
        if (context != null && suggestion.mSimIndicator > 0) {
