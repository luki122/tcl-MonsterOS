...
import com.android.providers.mst.ContactsContract.Contacts.Photo;
import com.android.providers.mst.ContactsContract.Data;
import com.android.providers.mst.ContactsContract.Directory;
import com.android.providers.mst.ContactsContract.DisplayNameSources;
import com.android.providers.mst.ContactsContract.DisplayPhoto;
import com.android.providers.mst.ContactsContract.FullNameStyle;
import com.android.providers.mst.ContactsContract.Groups;
import com.android.providers.mst.ContactsContract.PhoneticNameStyle;
...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
	...
	/**
	 * M: L-branch Original Code.
	 * static final int DATABASE_VERSION = 910;
	 *
	 * For VoLTE Conference Call Feature, Update:
	 * Android database version 910==> MTK database version 950
	 *
	 * Remove dialer search trigger, update database version 950==> 951
	 *
	 * M branch start with 1011
	 * @{
	 */
	static final int DATABASE_VERSION = 1014;
	/** @} */
	...
	public interface Tables {
		...
		public static final String RAW_CONTACTS_JOIN_SETTINGS_DATA_GROUPS = Tables.RAW_CONTACTS
				...
				+ ContactsProvider2.ACCOUNT_NAME_LOCAL_PHONE + "' OR "
		...
		public static final String MST_DIALER_SEARCH = "mst_dialer_search";
	...
	public interface Joins {
		...
		public static final String GROUP_MEMBER_COUNT =
				" LEFT OUTER JOIN (SELECT "
						...
						+ "FROM view_data "
						+ "WHERE "
						+ " is_privacy=0 AND " // add by liyang 2016-12-6
	...
	public interface Views {
		...
		public static final String MST_DIALER_SEARCH_VIEW = "view_mst_dialer_search";
		/// @}
	}
	...
	private long mMimeTypeIdImsCall;
	/// @}
	...
	private void initializeCache(SQLiteDatabase db) {
		...
		if (ContactsProviderUtils.isImsCallEnabled() && ContactsProviderUtils.isVolteEnabled()) {
			mMimeTypeIdImsCall = lookupMimeTypeId(ImsCall.CONTENT_ITEM_TYPE, db);
		}
		/// @}
	}
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE " + Tables.CONTACTS + " (" +
				...
				"is_privacy INTEGER NOT NULL DEFAULT 0" + // add by liyang 2016-12-6
				");");
		...
		db.execSQL("CREATE TABLE " + Tables.RAW_CONTACTS + " (" +
				...
				"quanpinyin TEXT,"+ //add by liyang
				"jianpinyin TEXT," + //add by liyang
				"is_privacy INTEGER NOT NULL DEFAULT 0," +  //add by liyang 2016-12-6
				 "call_notification_type INTEGER NOT NULL DEFAULT 0"+
				");");
		...
		db.execSQL("CREATE TABLE " + Tables.DATA + " (" +
				...
				", auto_record  INTEGER NOT NULL DEFAULT 0" +    // add by lgy
				", reject  INTEGER NOT NULL DEFAULT 0" +       // add by lgy
				", is_privacy INTEGER NOT NULL DEFAULT 0" +          // add by liyang 2016-12-6
				");");
		...
		db.execSQL("CREATE TABLE " + Tables.CALLS + " (" +
				...
				Calls.CONFERENCE_CALL_ID +  " INTEGER" +             //add by MTK
				",reject" + " INTEGER NOT NULL DEFAULT 0" +  // add by lgy
				",mark" + " TEXT" +
				",black_name" + " TEXT" +
				",user_mark" + " INTEGER NOT NULL DEFAULT 0" +
				",privacy_id INTEGER NOT NULL DEFAULT 0" +
				");");
		...
		/// M: dialer search & conference calls  @{
		DialerSearchSupport.createDialerSearchTable(db);
		CallLogSearchSupport.createConferenceCallsTable(db);
		/// @}
		...
		if (ContactsProviderUtils.isSearchDbSupport()) {        	
			DialerSearchHelperForMst.getInstance().createDialerSearchTable(db);
			DialerSearchHelperForMst.getInstance().createDialerSearchView(db);
		}

		createRejectTables(db);
		createDefaultMarkData(db);
		...
		DialerSearchSupport.dropDialerSearchView(db);
		/*
		 * @}
		 */
	...
	private void createContactsViews(SQLiteDatabase db) {
		...
		String dataColumns =
				Data.IS_PRIMARY + ", "
						...
						+ ", auto_record "    // add by lgy
						+ ", reject "// add by lgy
						+",raw_contacts.quanpinyin"//add by liyang
						+",raw_contacts.jianpinyin"//add by liyang
						+",raw_contacts.phonebook_bucket"; //add by liyang
		;
		...
		String syncColumns =
				RawContactsColumns.CONCRETE_ACCOUNT_ID + ","
						...
						/** M: added by MTK */
						+ RawContactsColumns.CONCRETE_TIMESTAMP + " AS " + RawContacts.TIMESTAMP;
		...
		String contactNameColumns =
				...
				+",name_raw_contact.quanpinyin"//add by liyang
				+",name_raw_contact.jianpinyin"//add by liyang
		       + ",contacts.is_privacy AS is_privacy"  // add by liyang
		       + ", name_raw_contact.call_notification_type AS call_notification_type"; // add by liyang
		...
		String rawContactsSelect = "SELECT "
				...
				+ ", is_privacy" //add by liyang
				+ ", call_notification_type" //add by liyang
				+ " FROM " + Tables.RAW_CONTACTS
				+ " JOIN " + Tables.ACCOUNTS + " ON ("
				+   RawContactsColumns.CONCRETE_ACCOUNT_ID + "=" + AccountsColumns.CONCRETE_ID
				+ ")";
		...
		String rawEntitiesSelect = "SELECT "
				...
				+ ", " + RawContactsColumns.IS_SDN_CONTACT + " AS " + RawContacts.IS_SDN_CONTACT
				/** @} */
				+ " FROM " + Tables.RAW_CONTACTS
				+ " JOIN " + Tables.ACCOUNTS + " ON ("
				+   RawContactsColumns.CONCRETE_ACCOUNT_ID + "=" + AccountsColumns.CONCRETE_ID
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		/**
		 * M: skip version 625
		 * android version 626 ==> MTK version 668
		 * Original code:
        if (oldVersion < 626) {
		...
		/**M: for Android4.3, update database to 770*/
		if (oldVersion < 767) {
			upgradeToVersion767(db);
			upgradeViewsAndTriggers = true;
			oldVersion = 767;
		}
		...
		/**
		 * M: MTK version should not be the same as google version
		 * so we mapping google 803 to MTK 850
		 * if google do a further upgrade, such as version 804, we should:
		 *     1. mapping 850 back to 803
		 *     2. do google upgrade from 803 to 804
		 *     3. mapping new version 804 ==> 851
		 * if MTK upgraded, such as to version 851, we should based on 850, and add a
		 *     new cluster following current one, such as:
		 *         if (oldVersion == 850) {upgradeFrom850To851()})@{
		 */
		if (oldVersion == 803) {
			LogUtils.i(TAG, "[onUpgrade]mapping version 803 to 850");
			oldVersion = 850;
		}
		/** @} */
		...
		/**M: for conference call, update database to 950*/
		if (oldVersion < 950) {
			upgradeToVersion950(db);
			oldVersion = 950;
		}

		/**M: drop dialer search trigger, update database to 951*/
		if (oldVersion < 951) {
			upgradeViewsAndTriggers = true;
			oldVersion = 951;
		}
		...
		if (oldVersion < 1012) {//add by liyang
			upgradeToVersion1012(db);
			upgradeViewsAndTriggers = true;
			oldVersion = 1012;
		}

		if (oldVersion < 1013) {//add by liyang
			upgradeViewsAndTriggers = true;
			oldVersion = 1013;
		}
		
		if (oldVersion < 1014) {//add by liyang 2016-12-6
			upgradeToVersion1014(db);
			upgradeViewsAndTriggers = true;
			oldVersion = 1014;
		}
	...
	private void upgradeDeleteAllSimData(SQLiteDatabase db) {
		if (ContactsProviderUtils.isSearchDbSupport()) {
			db.execSQL(
					"UPDATE calls SET data_id=null, raw_contact_id=null WHERE raw_contact_id IN " +
							"(SELECT _id FROM raw_contacts WHERE " + RawContacts.INDICATE_PHONE_SIM + ">0)"
					);
			db.execSQL(
					"DELETE FROM dialer_search WHERE raw_contact_id IN " +
							"(SELECT _id FROM raw_contacts WHERE " + RawContacts.INDICATE_PHONE_SIM + ">0)"
					);
		}
		db.execSQL("DELETE FROM raw_contacts WHERE " + RawContacts.INDICATE_PHONE_SIM + ">0");
	 ...
	 */
	private void upgradeToVersion767(SQLiteDatabase db) {
	 ...
	 */
	private void upgradeToVersion770(SQLiteDatabase db) {
	...
	private void upgradeToVersion950(SQLiteDatabase db) {
		try {
			db.execSQL("ALTER TABLE calls ADD conference_call_id INTEGER;");

			// New conference calls table.
			CallLogSearchSupport.createConferenceCallsTable(db);
		} catch (SQLiteException e) {
			LogUtils.w(TAG, "upgradeToVersion950 exception:" + e.toString());
		}
	}
	/**
	...
	//add by liyang 2016-11-2
	public void upgradeToVersion1012(SQLiteDatabase db) {
		Log.d(TAG,"upgradeToVersion1012");
		db.execSQL("ALTER TABLE raw_contacts ADD quanpinyin TEXT;");
		db.execSQL("ALTER TABLE raw_contacts ADD jianpinyin TEXT;");
		ArrayList<Token> tokens;
		Token token;
		StringBuilder sb,sb1;
		Cursor cursor=null;
		try{
			cursor=db.query("raw_contacts", new String[]{"_id","display_name"}, null,null,null,null,null);
			Log.d(TAG,"cursor:"+(cursor==null?"null":cursor.getCount()));
			while(cursor.moveToNext()){
				long id=cursor.getLong(0);
				String displayName=cursor.getString(1);
				tokens = HanziToPinyin.getInstance().getTokens(displayName);
				...
				db.execSQL("update raw_contacts set quanpinyin='"+sb.toString()+"',jianpinyin='"+sb1.toString()+"' where _id="+id);
			}
		}catch(Exception e){
			Log.d(TAG,"e:"+e);
		}finally{
	...
	//add by liyang 2016-12-6
	private void upgradeToVersion1014(SQLiteDatabase db) {
		Log.d(TAG,"upgradeToVersion1014");
        try {
            db.execSQL("ALTER TABLE " + Tables.CONTACTS + 
                    " ADD is_privacy INTEGER NOT NULL DEFAULT 0; ");
            db.execSQL("ALTER TABLE " + Tables.RAW_CONTACTS + 
                    " ADD is_privacy INTEGER NOT NULL DEFAULT 0; ");
            db.execSQL("ALTER TABLE " + Tables.RAW_CONTACTS + 
                    " ADD call_notification_type INTEGER NOT NULL DEFAULT 0; ");
            db.execSQL("ALTER TABLE " + Tables.DATA + 
                    " ADD is_privacy INTEGER NOT NULL DEFAULT 0; ");
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD privacy_id INTEGER NOT NULL DEFAULT 0; ");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
    }
	...
	/**
	 * M: get Setting Account Name.
	 * @param accountWithDataSet account data
	 * @return account name
	 */
	...
	public void createSimSettingInTransaction(AccountWithDataSet accountWithDataSet) {
		...
		if (accountWithDataSet == null || accountType == null) {
			return;
		}

		if (accountType.equals(ACCOUNT_TYPE_SIM) || accountType.equals(ACCOUNT_TYPE_UIM) ||
				accountType.equals(ACCOUNT_TYPE_USIM) || accountType.equals(ACCOUNT_TYPE_CSIM)) {
			String accountName = getSettingAccountNameOrNull(accountWithDataSet);
			if (accountName != null) {
				return;
			}
			final SQLiteStatement insert = getWritableDatabase().compileStatement(
			...
			try {
				...
				DatabaseUtils.bindObjectToProgram(insert, 2, accountWithDataSet.getAccountType());
				DatabaseUtils.bindObjectToProgram(insert, 3, accountWithDataSet.getDataSet());
				DatabaseUtils.bindObjectToProgram(insert, 4, "1");
				DatabaseUtils.bindObjectToProgram(insert, 5, "1");
	...
	public void updateRawContactDisplayName(SQLiteDatabase db, long rawContactId) {
		Log.d(TAG,"updateRawContactDisplayName,rawContactId:"+rawContactId);
		...
		if (bestDisplayNameSource == DisplayNameSources.STRUCTURED_NAME
			...
			Log.d(TAG,"displayNamePrimary0:"+displayNamePrimary);
			...
			/** M: dialer search @{ */
			if (ContactsProviderUtils.isSearchDbSupport()) {
				DialerSearchSupport.getInstance(mContext).handleContactsUpdated(db, rawContactId, -1,
						displayNamePrimary, displayNameAlternative, StructuredName.CONTENT_ITEM_TYPE);
			}
			/** @} */
		} else {
			displayNamePrimary = displayNameAlternative = bestDisplayName;
			Log.d(TAG,"displayNamePrimary1:"+displayNamePrimary);
		...
		if (bestPhoneticName != null) {
			if (displayNamePrimary == null) {
				...
				Log.d(TAG,"displayNamePrimary2:"+displayNamePrimary);
			}
		...
		if (ContactsProviderUtils.isSearchDbSupport()) {
			...
			//			DialerSearchHelperForMst.getInstance().updateOrInsertNameDialerSearch(db,
			//					rawContactId, displayNamePrimary);			
			updateContactsPinyin(db,rawContactId,displayNamePrimary);
		}
		
//		//add by liyang for calllog name
//		CallLogProviderEx.getInstance(mContext).mstUpdateCallsName(db,
//				rawContactId, displayNamePrimary);
	}

	public void updateContactsPinyin(SQLiteDatabase db,long rawContactId,String displayNameString){
		//add by liyang
		Log.d(TAG,"updateContactsPinyin,rawContactId:"+rawContactId+" name:"+displayNameString);
		Token token;
		ArrayList<Token> tokens = HanziToPinyin.getInstance().getTokens(displayNameString);
		StringBuilder sb=new StringBuilder();
		...
		ContentValues values=new ContentValues();
		values.put("quanpinyin",sb.toString());
		values.put("jianpinyin",sb1.toString());
		db.update("raw_contacts", values,"_id="+rawContactId, null);
	}
	...
	private void createContactsTriggersEx(SQLiteDatabase db) {
		createContactsTriggers(db);
		if (ContactsProviderUtils.isSearchDbSupport()) {
			DialerSearchSupport.createContactsTriggersForDialerSearch(db);
		}
	}
	/// @}
	...
	private void appendPresenceAndStautsTable(StringBuilder sb) {
		...
		sb.append(" LEFT OUTER JOIN " + Tables.AGGREGATED_PRESENCE +
				" ON (" + RawContacts.CONTACT_ID + " = "
				+ AggregatedPresenceColumns.CONCRETE_CONTACT_ID + ")");
	}
	/// @}

	private static final class BlackColumns implements BaseColumns { 		
	...
	private static final class MarkColumns implements BaseColumns {
		public static final String LABLE = "lable";
	}

	private void createRejectTables(SQLiteDatabase db) {
		db.execSQL("create table " + Tables.BLACK + "("
				+ BlackColumns._ID + " INTEGER PRIMARY KEY autoincrement,"
				+ BlackColumns.ISBLACK + " INTEGER,"
				+ BlackColumns.LABLE + " TEXT,"
				+ "user_mark" + " INTEGER NOT NULL DEFAULT 0,"
				+ BlackColumns.BLACK_NAME + " TEXT,"
				+ BlackColumns.NUMBER + " TEXT,"
				+ BlackColumns.REJECT + " INTEGER)" + ";");

		db.execSQL("create table " + Tables.MARK + "("
				+ MarkColumns._ID
				+ " INTEGER PRIMARY KEY autoincrement,"
				+ MarkColumns.LABLE + " TEXT,"
				+ "number TEXT)" + ";");
	}

	private void createDefaultMarkData(SQLiteDatabase db) {
		String[] marks = new String[]{null, null, null, null, null};
		marks[0] = mContext.getString(R.string.default_mark_1);
		marks[1] = mContext.getString(R.string.default_mark_2);
		marks[2] = mContext.getString(R.string.default_mark_3);
		marks[3] = mContext.getString(R.string.default_mark_4);
		marks[4] = mContext.getString(R.string.default_mark_5);

		for (int i = 0; i < marks.length; i++) {
			ContentValues values = new ContentValues();
			values.put("lable", marks[i]);
			db.insert(Tables.MARK, null, values);            
		}
	}
