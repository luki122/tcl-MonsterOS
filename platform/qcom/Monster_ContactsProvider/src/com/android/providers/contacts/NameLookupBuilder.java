...
public abstract class NameLookupBuilder {
	...
	private static final String TAG = "NameLookupBuilder";
...
//    	Log.d(TAG,"appendToSearchIndex,name:"+name+" fullNameStyle:"+fullNameStyle);
        int tokenCount = mSplitter.tokenize(mNames, name);
        if (tokenCount == 0) {
            return;
        }
...
//    	Log.d(TAG,"appendNameLookupForLocaleBasedName,fullName:"+fullName+" fullNameStyle:"+fullNameStyle);
...
//    	Log.d(TAG,"appendNameShorthandLookup,name:"+name+" fullNameStyle:"+fullNameStyle);
        Iterator<String> it =
                ContactLocaleUtils.getInstance().getNameLookupKeys(name, fullNameStyle);
        if (it != null) {
            while (it.hasNext()) {
            	String nextString=it.next();
//            	Log.d(TAG,"nextString:"+nextString);
                builder.appendName(nextString);
            }
        }
    }
}
