...
public class TmsServiceManager{
	...
	public void getUsefulNumbers(SQLiteDatabase db){
		if(db != null) {
	         final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mContext);
	            ...
	            if(loadedUsefulNumber <= 0){
	                ArrayList<UsefulNumberResult> list=TmsServiceManager.getInstance(mContext).getUsefulNumber("mstQueryAll");
	                Log.d(TAG,"list:"+list);
	                if(list!=null&&list.size()>0){
	                    int i=-10000;
	                    for(UsefulNumberResult item:list){
	                        DialerSearchHelperForMst.getInstance(mContext).insertUserNumber(db, i, item.getName(), item.getNumber());
//	                        DialerSearchHelperForMst.getInstance(mContext).updateOrInsertNameDialerSearch(db,
//	                                i--, item.getName());   
	                    }
	                    Log.i(TAG, "getUsefulNumbers2");
	                    prefs.edit().putInt("usefulnumber", 1).commit();
	                    
	                    DialerSearchHelperForMst.getInstance(mContext).test(db);
