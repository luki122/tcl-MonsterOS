...
import com.android.providers.mst.CallLog;
import com.android.providers.mst.CallLog.Calls;
import com.android.providers.mst.CallLog.ConferenceCalls;
...
public class CallLogProviderEx {
	...
	static {
		...
		//add by lgy
		sCallsJoinDataViewProjectionMap.put("reject", Tables.CALLS + ".reject as reject");
	...
	public SQLiteQueryBuilder queryCallLog(Uri uri, String[] projection,
		...
		case CALLS_SEARCH_FILTER: {
			...
			/// like "*" "#", etc. @{
			String number = query;
			if (!PhoneNumberUtils.isUriNumber(query)) {
		...
		case CALL_DETAILS:{
			Log.d(TAG,"CALL_DETAILS");
			qb.setTables("calls left outer join view_contacts on calls.raw_contact_id=view_contacts.name_raw_contact_id");
			qb.setProjectionMap(CallLogProvider.sCallsProjectionMapForCallDetail);
			String query = uri.getPathSegments().get(2);
			String number=null;
			if (!PhoneNumberUtils.isUriNumber(query)) {
				number = PhoneNumberUtils.stripSeparators(query);
			}
			qb.appendWhere("calls.number='"+number+"'");
		}
...
//	public int mstUpdateCallsName(SQLiteDatabase db,
//			long rawContactId, String name) {
//
//		SQLiteStatement updateCallsWithNewCallable = db.compileStatement(
//				" UPDATE " + Tables.CALLS
//				+ " SET name=?"
//				+ " WHERE raw_contact_id="+rawContactId
//				);
//
//		updateCallsWithNewCallable.bindString(1, name);
//		int count = updateCallsWithNewCallable.executeUpdateDelete();
//		updateCallsWithNewCallable.close();
//		Log.d(TAG, "mstUpdateCallsName:" + updateCallsWithNewCallable+" name="+name);
//		return count;
//	}
