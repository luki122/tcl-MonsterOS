 ...
 * register all alarm instances with the state manager if you want them to be
 * activated. If a major time change has occurred (ie. TIMEZONE_CHANGE,
 * TIMESET_CHANGE), then you must also re-register instances to fix their
 * states.
 * <p>
 * Please see {@link #registerInstance) for special transitions when major time
 * changes occur.
 * <p>
 * Following states:
 * <p>
 * SILENT_STATE: This state is used when the alarm is activated, but doesn't
 * need to display anything. It is in charge of changing the alarm instance
 * state to a LOW_NOTIFICATION_STATE.
 * <p>
 * LOW_NOTIFICATION_STATE: This state is used to notify the user that the alarm
 * will go off {@link AlarmInstance#LOW_NOTIFICATION_HOUR_OFFSET}. This state
 * handles the state changes to HIGH_NOTIFICATION_STATE, HIDE_NOTIFICATION_STATE
 * and DISMISS_STATE.
 * <p>
 * HIDE_NOTIFICATION_STATE: This is a transient state of the
 * LOW_NOTIFICATION_STATE, where the user wants to hide the notification. This
 * will sit and wait until the HIGH_PRIORITY_NOTIFICATION should go off.
 * <p>
 * HIGH_NOTIFICATION_STATE: This state behaves like the LOW_NOTIFICATION_STATE,
 * but doesn't allow the user to hide it. This state is in charge of triggering
 * a FIRED_STATE or DISMISS_STATE.
 * <p>
 * SNOOZED_STATE: The SNOOZED_STATE behaves like a HIGH_NOTIFICATION_STATE, but
 * with a different message. It also increments the alarm time in the instance
 * to reflect the new snooze time.
 * <p>
 * FIRED_STATE: The FIRED_STATE is used when the alarm is firing. It will start
 * the AlarmService, and wait until the user interacts with the alarm via
 * SNOOZED_STATE or DISMISS_STATE change. If the user doesn't then it might be
 * change to MISSED_STATE if auto-silenced was enabled.
 * <p>
 * MISSED_STATE: The MISSED_STATE is used when the alarm already fired, but the
 * user could not interact with it. At this point the alarm instance is dead and
 * we check the parent alarm to see if we need to disable or schedule a new
 * alarm_instance. There is also a notification shown to the user that he/she
 * missed the alarm and that stays for
 * {@link AlarmInstance#MISSED_TIME_TO_LIVE_HOUR_OFFSET} or until the user
 * acknownledges it.
 * <p>
 * DISMISS_STATE: This is really a transient state that will properly delete the
 * alarm instance. Use this state, whenever you want to get rid of the alarm
 * instance. This state will also check the alarm parent to see if it should
 * disable or schedule a new alarm instance.
 */
public final class AlarmStateManager extends BroadcastReceiver {
    // These defaults must match the values in res/xml/settings.xml
    private static final String DEFAULT_SNOOZE_MINUTES = "5";
    ...
    // Intent category tag used when schedule state change intents in alarm
    // manager.
...
//    public static final String TCT_ACTION_POWER_OFF_ALARM_ALERT = "android.intent.action.POWER_OFF_ALARM_ALERT";
//    public static final String EXTRA_KEY_CONFIRM = "android.intent.extra.KEY_CONFIRM";
//    private static final String FIRST_ALARM_FLAG = "first_alarm";
//    private static Context mContext;
//    private static AlarmInstance mInstance;
//    private static final int POWER_OFF_ALARM = 1000;
//    // zouxu   [APP][Alarm]Enable Power off alarm  end
     ...
     *
     * @param context application context
     * @return an alarm instance that will fire earliest relative to current
     * time.
     */
    ...
    /**
     * Used in L and later devices where "next alarm" is stored in the Alarm
     * Manager.
     */
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    private static void updateNextAlarmInAlarmManager(Context context, AlarmInstance nextAlarm) {
        // Sets a surrogate alarm with alarm manager that provides the
        // AlarmClockInfo for the
        // alarm that is going to fire next. The operation is constructed such
        // that it is ignored
        ...
        if (nextAlarm != null) {
            ...
            // Create an intent that can be used to show or edit details of the
            // next alarm.
            PendingIntent viewIntent = PendingIntent.getActivity(context, nextAlarm.hashCode(),
                    AlarmNotifications.createViewAlarmIntent(context, nextAlarm), PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    /**
     * This will set the alarm instance to the SILENT_STATE and update the
     * application notifications and schedule any state changes that need to
     * occur in the future.
     *
     * @param context  application context
     ...
     * the application notifications and schedule any state changes that need to
     * occur in the future.
     *
     * @param context  application context
     * @param instance to set state to
    ...
    /**
     * This will set the alarm instance to the HIDE_NOTIFICATION_STATE and
     * update the application notifications and schedule any state changes that
    ...
    /**
     * This will set the alarm instance to the HIGH_NOTIFICATION_STATE and
     * update the application notifications and schedule any state changes that
     * need to occur in the future.
     *
     * @param context  application context
    ...
    /**
     * This will set the alarm instance to the FIRED_STATE and update the
     * application notifications and schedule any state changes that need to
     * occur in the future.
     *
     * @param context  application context
    ...
    public static void setFiredState(Context context, AlarmInstance instance) {
        ...
        if (instance.mAlarmId != null) {
            // if the time changed *backward* and pushed an instance from missed
            // back to fired,
        ...
        // Instance not valid anymore, so find next alarm that will fire and
        // notify system
        updateNextAlarm(context);
    }

    /**
     * This will set the alarm instance to the SNOOZE_STATE and update the
     * application notifications and schedule any state changes that need to
     * occur in the future.
     *
     * @param context  application context
    ...
    public static void setSnoozeState(final Context context, AlarmInstance instance, boolean showToast) {
        ...
        if(instance ==null){
            Log.e("zouxu","setSnoozeState instance ==null");
            return ;
        }
        ...
        boolean is_remind_later = SharePreferencesUtils.istRemindLaterALarm(context, instance.mAlarmId);
        if (is_remind_later) {
            int sleep_count = SharePreferencesUtils.getSleepCount(context, instance.mAlarmId);
            sleep_count++;
            SharePreferencesUtils.setSleepCount(context, instance.mAlarmId, sleep_count);

        }

        // Calculate the new snooze alarm time
        String snoozeMinutesStr = PreferenceManager.getDefaultSharedPreferences(context).getString(
                SettingsActivity.KEY_ALARM_SNOOZE, DEFAULT_SNOOZE_MINUTES);
            ...
            final Runnable myRunnable = new Runnable() {
                ...
                public void run() {
                    String displayTime = String.format(
                            context.getResources().getQuantityText(R.plurals.alarm_alert_snooze_set, snoozeMinutes)
        ...
        // Instance time changed, so find next alarm that will fire and notify
        // system
        updateNextAlarm(context);
...
//            try {
//                Intent powerOnAlert = new Intent(TCT_ACTION_POWER_OFF_ALARM_ALERT);
//                powerOnAlert.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                context.startActivity(powerOnAlert);
...
//        // zouxu   [APP][Alarm]Enable Power off alarm  end
    ...
    public static int getSnoozedMinutes(Context context) {
        final String snoozeMinutesStr = PreferenceManager.getDefaultSharedPreferences(context).getString(
                SettingsActivity.KEY_ALARM_SNOOZE, DEFAULT_SNOOZE_MINUTES);
        return Integer.parseInt(snoozeMinutesStr);
    }

    /**
     * This will set the alarm instance to the MISSED_STATE and update the
     * application notifications and schedule any state changes that need to
     * occur in the future.
     *
     * @param context  application context
    ...
    public static void setMissedState(Context context, AlarmInstance instance) {
        ...
        // Instance is not valid anymore, so find next alarm that will fire and
        // notify system
        updateNextAlarm(context);
    }

    /**
     * This will set the alarm instance to the PREDISMISSED_STATE and schedule
     * an instance state change to DISMISSED_STATE at the regularly scheduled
     * firing time.
     *
     * @param context
     * @param instance
    ...
    public static void setPreDismissState(Context context, AlarmInstance instance) {
        ...
        SharePreferencesUtils.setSkipThisALarm(context, alarm.id, true);// 跳过这个闹钟
    ...
    public static void setDismissState(Context context, AlarmInstance instance) {
        ...
        if(instance == null){//add zouxu 20161102
            return;
        }

        LogUtils.v("Setting dismissed state to instance " + instance.mId);

        SharePreferencesUtils.setSleepCount(context, instance.mAlarmId, 0);// 关闭闹钟的时候
        // 睡眠次数清零
...
//        // zouxu   [APP][Alarm]Enable Power off alarm  end
    ...
    /**
     * This will not change the state of instance, but remove it's notifications
     * and alarm timers.
     *
     * @param context  application context
    ...
    /**
     * This registers the AlarmInstance to the state manager. This will look at
     * the instance and choose the most appropriate state to put it in. This is
     * primarily used by new alarms, but it can also be called when the system
     * time changes.
     * <p>
     * Most state changes are handled by the states themselves, but during major
     * time changes we have to correct the alarm instance state. This means we
     * have to handle special cases as describe below:
     * <p>
     * <ul>
     * <li>Make sure all dismissed alarms are never re-activated</li>
     * <li>Make sure pre-dismissed alarms stay predismissed</li>
     * <li>Make sure firing alarms stayed fired unless they should be
     * auto-silenced</li>
     * <li>Missed instance that have parents should be re-enabled if we went
     * back in time</li>
     * <li>If alarm was SNOOZED, then show the notification but don't update
     * time</li>
     * <li>If low priority notification was hidden, then make sure it stays
     * hidden</li>
     * </ul>
     * <p>
     * If none of these special case are found, then we just check the time and
     * see what is the proper state for the instance.
     *
     * @param context  application context
    ...
    public static void registerInstance(Context context, AlarmInstance instance, boolean updateNextAlarm) {
        ...
        if (alarm != null) {
            SharePreferencesUtils.setSkipThisALarm(context, alarm.id, false);
        }
        ...
        if (instance.mAlarmState == AlarmInstance.DISMISSED_STATE) {
            ...
            if (currentTime.before(alarmTime)) {
                ...
                // use our normal notifications. This is not ideal, but very
                // rare use-case.
        ...
        if (currentTime.after(missedTTL)) {
            ...
            // There is a chance that the TIME_SET occurred right when the alarm
            // should go off, so
            // we need to add a check to see if we should fire the alarm instead
            // of marking it
            // missed.
            ...
            // We only want to display snooze notification and not update the
            // time,
    ...
    /**
     * This will delete and unregister all instances associated with alarmId,
     * without affect the alarm itself. This should be used whenever modifying
    ...
    public static void deleteAllInstances(Context context, long alarmId) {
        ...
        for (AlarmInstance instance : instances) {
            unregisterInstance(context, instance);
            boolean deleteInstance = AlarmInstance.deleteInstance(context.getContentResolver(), instance.mId);
            if (!deleteInstance) {
                Log.e("zouxu", "deleteAllInstances fail!");
            }
        }
        updateNextAlarm(context);
    }
    ...
    public static void fixAlarmInstances(Context context) {
        // Register all instances after major time changes or when phone
        // restarts
        final ContentResolver contentResolver = context.getContentResolver();
        final Calendar currentTime = getCurrentTime();
        for (AlarmInstance instance : AlarmInstance.getInstances(contentResolver, null)) {
            ...
            if (currentTime.before(priorAlarmTime) || currentTime.after(missedTTLTime)) {
                ...
                // The time change is so dramatic the AlarmInstance doesn't make
                // any sense;
...
//            // should add more judgement,not only just minute.
//            int minute = mCalendar.get(Calendar.MINUTE);
//            int hour = mCalendar.get(Calendar.HOUR);
//            int day = mCalendar.get(Calendar.DAY_OF_MONTH);
//            if (minute == instance.mMinute && hour == instance.mHour && day == instance.mDay && Utils.isAlarmBoot()) {
//                mContext = context;
//                mInstance = instance;
//                mBootAlarmHandler.removeMessages(POWER_OFF_ALARM);
//                mBootAlarmHandler.sendEmptyMessage(POWER_OFF_ALARM);
//            }
//            // zouxu [APP][Alarm]Enable Power off alarm  end
...
//    // PR832683 by xing.zhao   [APP][Alarm]Enable Power off alarm  begin
//    final static Handler mBootAlarmHandler = new Handler() {
//        public void handleMessage(Message msg) {
//            switch (msg.what) {
//                case POWER_OFF_ALARM:
//                    AlarmStateManager.setRtcPowerUp(mContext, true);
//                    Log.e("AlarmClock", "POWER_OFF_ALARM and setFiredState"); // PR1021460 by xing.zhao for add log
//                    setFiredState(mContext, mInstance);
//                    break;
//                default:
//                    return;
//            }
//        }
//    };
//    // PR832683 by xing.zhao   [APP][Alarm]Enable Power off alarm  end
    ...
    /**
     * Creates an intent that can be used to set an AlarmManager alarm to set
     * the next alarm indicators.
     */
    ...
    /**
     * Abstract away how the current time is computed. If no implementation of
     * this interface is given the default is to return
     * {@link Calendar#getInstance()}. Otherwise, the factory instance is
     * consulted for the current time.
     */
    ...
    /**
     * Abstracts away how state changes are scheduled. The
     * {@link AlarmManagerStateChangeScheduler} implementation schedules
     * callbacks within the system AlarmManager. Alternate implementations, such
     * as test case mocks can subvert this behavior.
     */
    ...
    private static class AlarmManagerStateChangeScheduler implements StateChangeScheduler {
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time, AlarmInstance instance, int newState) {
            ...
            // Treat alarm state change as high priority, use foreground
            // broadcasts
            ...
            Intent stateChangePowerOffIntent = createStateChangeIntent(context, ALARM_MANAGER_TAG, instance,
                    AlarmInstance.POWER_OFF_ALARM_STATE);
            PendingIntent pendingPowerOffIntent = PendingIntent.getBroadcast(context, instance.hashCode(),
                    stateChangePowerOffIntent, PendingIntent.FLAG_ONE_SHOT);
        ...
        public void cancelScheduledInstanceStateChange(Context context, AlarmInstance instance) {
            ...
            // Create a PendingIntent that will match any one set for this
            // instance
            PendingIntent pendingIntent = PendingIntent.getBroadcast(context, instance.hashCode(),
                    createStateChangeIntent(context, ALARM_MANAGER_TAG, instance, null), PendingIntent.FLAG_NO_CREATE);

            PendingIntent pendingPowerOffIntent = PendingIntent.getBroadcast(context, instance.hashCode(),
                    createStateChangeIntent(context, ALARM_MANAGER_TAG, instance, null), PendingIntent.FLAG_ONE_SHOT);
