...
public final class AlarmNotifications {
    ...
    public static void showLowPriorityNotification(Context context, AlarmInstance instance) {
        ...
        Log.i("zouxu","showLowPriorityNotification!!");
        if(true){//关闭低权限的通知（默认距离２小时的时候会提醒）
            return;
        }
    ...
    public static void showHighPriorityNotification(Context context, AlarmInstance instance) {
        ...
        if(hour_now>11 || hour_now<5){//只有在早上５点到１１点才提醒
            return;
        }
        
        NotificationManagerCompat nm = NotificationManagerCompat.from(context);
        
        Log.i("zouxu","showHighPriorityNotification!!");
...
//                instance.hashCode(), dismissIntent, PendingIntent.FLAG_UPDATE_CURRENT), true);
//        notification.setPriority(NotificationCompat.PRIORITY_MAX); //可以弹出到界面上来 类似闹铃
    ...
    public static void showAlarmNotification(Context context, AlarmInstance instance) {
        ...
        if(is_remind_later && sleep_count<2){// Setup Snooze Action  有稍后提醒的功能　并且睡眠次数<2
