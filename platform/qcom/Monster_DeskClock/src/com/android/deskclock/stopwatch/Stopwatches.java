...
/**
 * Stopwatch utility class providing access to stopwatch resources and data
 * formatting strings of stopwatch data.
 */
public class Stopwatches {
    ...
    /***
     * Format the string of the time running on the stopwatch up to hundred of a
     * second accuracy
     * 
     * @param context
     *            context with resources
     * @param time
     *            - in hundreds of a second since the stopwatch started
     * @param lap
     *            lap number
     * @return formatted text
     */
    public static String getTimeText(Context context, long time, int lap) {
        ...
        return String.format(formats[formatIndex], hours, minutes, seconds, hundreds, decimalSeparator, lap);
    }

    /***
     * Sets the string of the time running on the stopwatch up to hundred of a
     * second accuracy
     * 
     * @param time
     *            - in hundreds of a second since the stopwatch started
    ...
    public static String formatTimeText(long time, final String format) {
        ...
        final String TWO_DIGITS = "%02d";
        String mMinutes = String.format(TWO_DIGITS, minutes);// 分钟一直保留两位小数
        String mSeconds = String.format(TWO_DIGITS, seconds);
        String mHundredths = String.format(TWO_DIGITS, hundreds);

        String mHours;
        if (hours > 0) {
            mHours = String.format(TWO_DIGITS, hours);
        } else {
            mHours = null;
        }
        if (mHours == null) {
        ...
        // DecimalFormatSymbols.getInstance().getDecimalSeparator();//去掉原生的
        // return String.format(format, hours, minutes, seconds, hundreds,
        // decimalSeparator);
    }
}
