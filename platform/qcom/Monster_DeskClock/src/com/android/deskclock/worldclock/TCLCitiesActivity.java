...
//        }.executeOnExecutor(Executors.newCachedThreadPool());//zouxu 20161102
        ...
        if(intent!=null && !trans_pinying){
            select_city_id = intent.getStringExtra("select_city_id");
            if(select_city_id!=null){
                for(int i=0;i<dataList.size();i++){
                    if(select_city_id.equals(dataList.get(i).mCityId)){
                        if(i>0){
                            my_listview.setSelection(i-1);
                        } else {
                            my_listview.setSelection(i);
                        }
                        break;
                    }
                }
...
//        dataList = Utils.loadCitiesListFromXml(this);
    ...
    private void initView() {
        ...
        index_bar.setBalloonFocusColor(getColor(R.color.red_float_bt));
...
//        my_listview.setSelection(listindex);
    ...
    public void saveSelect() {
        HashMap<String, CityObj> m_select = new HashMap<String, CityObj>();

        for (int i = 0; i < dataList.size(); i++) {
            CityObj city = dataList.get(i);
            if (city.isChoose) {
                m_select.put(city.mCityId, city);
            }

        }
        Cities.saveCitiesToSharedPrefs(PreferenceManager.getDefaultSharedPreferences(TCLCitiesActivity.this),
                m_select);
        Intent i = new Intent(Cities.WORLDCLOCK_UPDATE_INTENT);
        sendBroadcast(i);
        setResult(RESULT_OK, i);
