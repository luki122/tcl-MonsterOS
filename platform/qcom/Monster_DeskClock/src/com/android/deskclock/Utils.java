...
import com.android.deskclock.worldclock.HanziToPinyin;
...
public class Utils {
    ...
    /**
     * The background colors of the app - it changes throughout out the day to
     * mimic the sky.
     */
    private static final int[] BACKGROUND_SPECTRUM = {0xFF212121 /* 12 AM */, 0xFF20222A /*
                                                                                           * 1
                                                                                           * AM
                                                                                           */, 0xFF202233 /*
                                                                                                           * 2
                                                                                                           * AM
                                                                                                           */,
            0xFF1F2242 /* 3 AM */, 0xFF1E224F /* 4 AM */, 0xFF1D225C /* 5 AM */, 0xFF1B236B /*
                                                                                             * 6
                                                                                             * AM
                                                                                             */, 0xFF1A237E /*
                                                                                                             * 7
                                                                                                             * AM
                                                                                                             */,
            0xFF1D2783 /* 8 AM */, 0xFF232E8B /* 9 AM */, 0xFF283593 /* 10 AM */, 0xFF2C3998 /*
                                                                                              * 11
                                                                                              * AM
                                                                                              */, 0xFF303F9F /*
                                                                                                              * 12
                                                                                                              * PM
                                                                                                              */,
            0xFF2C3998 /* 1 PM */, 0xFF283593 /* 2 PM */, 0xFF232E8B /* 3 PM */, 0xFF1D2783 /*
                                                                                             * 4
                                                                                             * PM
                                                                                             */, 0xFF1A237E /*
                                                                                                             * 5
                                                                                                             * PM
                                                                                                             */,
            0xFF1B236B /* 6 PM */, 0xFF1D225C /* 7 PM */, 0xFF1E224F /* 8 PM */, 0xFF1F2242 /*
                                                                                             * 9
                                                                                             * PM
                                                                                             */, 0xFF202233 /*
                                                                                                             * 10
    ...
    /**
     * @return {@code true} if the device is
     * {@link Build.VERSION_CODES#JELLY_BEAN_MR2} or later
     */
    ...
    /**
     * @return {@code true} if the device is
     * {@link Build.VERSION_CODES#LOLLIPOP} or later
     */
    ...
    /**
     * @return {@code true} if the device is
     * {@link Build.VERSION_CODES#LOLLIPOP_MR1} or later
     */
    ...
    /**
     * @return {@code true} if the device is {@link Build.VERSION_CODES#M} or
     * later
     */
    ...
    public static void prepareHelpMenuItem(Context context, MenuItem helpMenuItem) {
        ...
        if (TextUtils.isEmpty(helpUrlString)) {
            // The help url string is empty or null, so set the help menu item
            // to be invisible.
            helpMenuItem.setVisible(false);
            return;
        }
        // The help url string exists, so first add in some extra query
        // parameters. 87
        ...
        // Set the intent to the help menu item, show the help menu item in the
        // overflow
    ...
    /**
     * Adds two query parameters into the Uri, namely the language code and the
     * version code of the application's package as gotten via the context.
     *
    ...
    private static Uri uriWithAddedParameters(Context context, Uri baseUri) {
        ...
        if (sCachedVersionCode == null) {
            // There is no cached version code, so try to get it from the
            // package manager.
            try {
                ...
                // Cannot find the package name, so don't add in the version
                // parameter
    ...
    /**
     * Calculate the amount by which the radius of a CircleTimerView should be
     * offset by the any of the extra painted objects.
     */
    ...
    /**
     * Uses {@link Utils#calculateRadiusOffset(float, float, float)} after
     * fetching the values from the resources just as
     * {@link CircleTimerView#init(android.content.Context)} does.
     */
    ...
    /**
     * Clears the persistent data of stopwatch (start time, state, laps,
     * etc...).
     */
    ...
    /**
     * Runnable for use with screensaver and dream, to move the clock every
     * minute. registerViews() must be called prior to posting.
     */
        ...
        public void run() {
            ...
            if (xrange == 0 && yrange == 0) {
                ...
                delay = delay + (MOVE_DELAY - adjust) // minute aligned
                        - (SLIDE ? 0 : FADE_TIME) // start moving before the
                // fade
                ;
            }
    ...
    /**
     * Setup to find out when the quarter-hour changes (e.g. Kathmandu is
     * GMT+5:45)
     **/
    ...
    static long getAlarmOnQuarterHour(Calendar calendar, long now) {
        ...
        if (0 >= delta || delta > 901000) {
            // Something went wrong in the calculation, schedule something that
            // is
            // about 15 minutes. Next time , it will align with the 15 minutes
            // border.
    ...
    // Setup a thread that starts at midnight plus one second. The extra second
    // is added to ensure
    ...
    // Setup a thread that starts at the quarter-hour plus one second. The extra
    // second is added to
    ...
    /**
     * For screensavers to set whether the digital or analog clock should be
     * displayed. Returns the view to be displayed.
     */
    ...
    public static String getNextAlarm(Context context) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            timeString = Settings.System.getString(context.getContentResolver(), Settings.System.NEXT_ALARM_FORMATTED);
        } else {
            final AlarmManager.AlarmClockInfo info = ((AlarmManager) context.getSystemService(Context.ALARM_SERVICE))
                    .getNextAlarmClock();
            if (info != null) {
    ...
    /**
     * Clock views can call this to refresh their alarm to the next upcoming
     * value.
     **/
    public static void refreshAlarm(Context context, View clock) {
        ...
        if (!TextUtils.isEmpty(nextAlarm) && nextAlarmView != null) {
            ...
            nextAlarmView.setContentDescription(context.getResources().getString(R.string.next_alarm_description,
                    nextAlarm));
            nextAlarmView.setVisibility(View.VISIBLE);
            Log.i("zouxu", "refreshAlarm nextAlarm = " + nextAlarm);
        } else {
            nextAlarmView.setVisibility(View.GONE);
            if (nextAlarm == null) {
                Log.i("zouxu", "refreshAlarm nextAlarm = null ");
            }
        }
    }
    ...
    public static void updateDate(String dateFormat, String dateFormatForAccessibility, View clock) {
        ...
        if (dateDisplay != null) {
            ...
            dateDisplay.setText(isJBMR2OrLater() ? new SimpleDateFormat(DateFormat
                    .getBestDateTimePattern(l, dateFormat), l).format(now) : SimpleDateFormat.getDateInstance().format(
                    now));
            dateDisplay.setVisibility(View.VISIBLE);
            dateDisplay.setContentDescription(isJBMR2OrLater() ? new SimpleDateFormat(DateFormat
                    .getBestDateTimePattern(l, dateFormatForAccessibility), l).format(now) : SimpleDateFormat
                    .getDateInstance(java.text.DateFormat.FULL).format(now));
        }
    }
    ...
    public static CharSequence get12ModeFormat(Context context, int amPmFontSize) {
        ...
        String pattern = context.getString(R.string.time_format_12_mode_tcl);//chg zouxu 20160919
...
//        sp.setSpan(new StyleSpan(Typeface.NORMAL), amPmPos, amPmPos + 1, Spannable.SPAN_POINT_MARK);//delete zouxu 20160921
        sp.setSpan(new AbsoluteSizeSpan(amPmFontSize), amPmPos, amPmPos + 1, Spannable.SPAN_POINT_MARK);
        sp.setSpan(new TypefaceSpan("monster-normal"), amPmPos, amPmPos + 1, Spannable.SPAN_POINT_MARK);
        return sp;
    }

    public static CharSequence get24ModeFormat() {
        return isJBMR2OrLater() ? DateFormat.getBestDateTimePattern(Locale.getDefault(), "Hm") : (new SimpleDateFormat(
                "k:mm", Locale.getDefault())).toLocalizedPattern();
    }
    ...
    public static CityObj[] loadCitiesFromXml(Context c) {
        ...
        for (int i = 0; i < cities.length; i++) {
            // Default to using the first character of the city name as the
            // index unless one is
            // specified. The indicator for a specified index is the addition of
            // character(s)
            ...
            if (separatorIndex == 0) {
                // Default to using second character (the first character after
                // the = separator)
                // as the index.
     ...
     *
     * @param useShortForm Whether to return a short form of the header that rounds to
     *                     the nearest hour and excludes the "GMT" prefix
     */
     ...
     * @param defValue the default color value to use if the attribute cannot be
     *                 resolved
     ...
     * @return Single-char version of day name, e.g.: 'S', 'M', 'T', 'W', 'T',
     * 'F', 'S'
     */
    ...
    /**
     * @param firstDay is the result from getZeroIndexedFirstDayOfWeek
     * @return Long-version of day name, e.g.: 'Sunday', 'Monday', 'Tuesday',
     * etc
     */
    ...
    // Return the first day of the week value corresponding to
    // Calendar.<WEEKDAY> value, which is
    // 1-indexed starting with Sunday.
    public static int getFirstDayOfWeek(Context context) {
        // return
        // Integer.parseInt(PreferenceManager.getDefaultSharedPreferences(context).getString(
        // SettingsActivity.KEY_WEEK_START,
        // String.valueOf(DEFAULT_WEEK_START)));
        return 2;// 修改默认为２即第一天是星期一 ０:星期六 １:星期天
    }

    // Return the first day of the week value corresponding to a week with
    // Sunday at 0 index.
    ...
    public static CityObj getCityByTZId(Context c, String id) {
        Resources r = c.getResources();
        // Read strings array of name,timezone, id
        // make sure the list are the same length
        String[] cityNames = r.getStringArray(R.array.cities_names);
        String[] timezones = r.getStringArray(R.array.cities_tz);
        String[] ids = r.getStringArray(R.array.cities_id);
        String[] tz_id = r.getStringArray(R.array.timezone_values);
        int minLength = cityNames.length;
        if (cityNames.length != timezones.length || ids.length != cityNames.length) {
            minLength = Math.min(cityNames.length, Math.min(timezones.length, ids.length));
            LogUtils.e("City lists sizes are not the same, truncating");
        ...
        for (int i = 0; i < minLength; i++) {
            ...
            if (i < tz_id.length && id.equals(tz_id[i])) {
                CityObj cities = new CityObj(cityName, timezones[i], ids[i], index);
                return cities;
            }
        }
        return null;
    ...
    public static ArrayList<CityObj> loadCitiesListFromXml(Context c, boolean is_trans_pinyin) {

        HashMap<String, CityObj> mUserSelectedCities = Cities.readCitiesFromSharedPrefs(PreferenceManager
                .getDefaultSharedPreferences(c));
        catalog_hashmap.clear();
        Resources r = c.getResources();
        // Read strings array of name,timezone, id
        // make sure the list are the same length
        String[] cityNames = r.getStringArray(R.array.cities_names);
...
//        HanziToPinyin engine = HanziToPinyin.getInstance();
        ...
        for (int i = 0; i < minLength; i++) {
            ...
            CityObj mcity = new CityObj(cityName, timezones[i], ids[i], index);

            if (is_trans_pinyin) {

                mcity.pinyin = HanziToPinyin.getPingYin(cityName);
                for (int j = 0; j < cityName.length(); j++) {
                    String fistLetter = HanziToPinyin.getPingYin(cityName.substring(j, j + 1));
                    if (!TextUtils.isEmpty(fistLetter)) {
                        fistLetter = fistLetter.substring(0, 1);
                    } else {
                        fistLetter = cityName.substring(0, 1);
                    }

                    mcity.firstLetter.add(fistLetter);
                }
            }

            if (mUserSelectedCities.containsKey(mcity.mCityId)) {
                mcity.isChoose = true;
            } else {
                mcity.isChoose = false;
            }

            cities.add(mcity);
            String letter = mcity.mCityIndex.substring(0, 1);
            catalog_hashmap.put(letter, letter);

        }
        Collections.sort(cities, new EmployeePinyinComparator());

        return cities;
    }

    public static HashMap<String, String> catalog_hashmap = new HashMap<String, String>();

    public static HashMap<String, String> getSortLetterMap() {
        return catalog_hashmap;
    }

    public static boolean isTime24(Context mContext) {
    ...
    public static void setStatusBarColor(MstActivity activity, int color_id){
        Window window = activity.getWindow();
        window.setStatusBarColor(activity.getResources().getColor(color_id));
        activity.getToolbar().setBackgroundColor(activity.getResources().getColor(color_id));
    }

//    // zouxu   [APP][Alarm]Enable Power off alarm  begin
//    public static boolean isAlarmBoot() {
//        String bootReason = SystemProperties.get("sys.boot.alarm", "0");
//        Log.e("AlarmClock", "AlarmActivity isAlarmBoot = " + bootReason);
