...
public final class Alarm implements Parcelable, ClockContract.AlarmsColumns {
    ...
    public boolean isWorkDayAlarm = false;//是否是工作日（智能跳过节假日）的闹钟
    public boolean isNoWorkDayAlarm = false;//是否是节假日（智能跳过工作日）的闹钟
    public boolean is_remaind_later = true;//是否稍后提醒
    ...
    public Alarm(int hour, int minutes) {
        ...
        this.isWorkDayAlarm = false;
        this.isNoWorkDayAlarm = false;
        this.is_remaind_later = true;
    }
    ...
    Alarm(Parcel p) {
        ...
        isWorkDayAlarm = p.readInt() == 1;
        is_remaind_later = p.readInt() == 1;
        isNoWorkDayAlarm = p.readInt() == 1;
    }
    ...
    public void writeToParcel(Parcel p, int flags) {
        ...
        p.writeInt(isWorkDayAlarm ? 1 : 0);
        p.writeInt(is_remaind_later ? 1 : 0);
        p.writeInt(isNoWorkDayAlarm ? 1 : 0);
    }
    ...
    public Calendar getNextAlarmTime(Calendar currentTime,Context context) {
        ...
        boolean is_weekday_alarm= SharePreferencesUtils.isWorkDayALarm(context, id);//只有工作日响
        if(is_weekday_alarm){
            while(!SharePreferencesUtils.shouldAlarm(context,nextInstanceTime)){
                nextInstanceTime.add(Calendar.DAY_OF_YEAR, 1);
            }
            return nextInstanceTime;
        }
        
        boolean is_no_weekday_alarm= SharePreferencesUtils.isNoWorkDayALarm(context, id);//只有节假日响
        if(is_no_weekday_alarm){
            while(SharePreferencesUtils.shouldAlarm(context,nextInstanceTime)){
                nextInstanceTime.add(Calendar.DAY_OF_YEAR, 1);
            }
            return nextInstanceTime;
