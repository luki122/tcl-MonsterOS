...
public class NumberPickerView extends View{
    ...
    private static final int DEFAULT_TEXT_SIZE_HINT_SP = 13;
    ...
    private static final int DEFAULT_DIVIDER_COLOR = 0Xff4cb6ac;
    ...
    private static final int DEFAULT_INTERVAL_REVISE_DURATION = 500;//300 zouxu
    ...
    private int mTextColorMin = DEFAULT_TEXT_COLOR_NORMAL;//add zouxu 显示５个数据的时候　最小的字体颜色
    private int mTextColorHint = DEFAULT_TEXT_COLOR_SELECTED;
    private int mTextSizeNormal = 0;
    private int mTextSizeMin= 0;//add zouxu 显示５个数据的时候　最小的字体大小
    ...
    private float mTextSizeMinCenterYOffset = 0f;
    ...
    private void initAttr(Context context, AttributeSet attrs){
        ...
        for (int i = 0; i < n; i++) {
            ...
            }else if(attr == R.styleable.NumberPickerView_npv_TextColorMin){
                mTextSizeMin = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
            }else if(attr == R.styleable.NumberPickerView_npv_TextColorMin){
                mTextColorMin = a.getColor(attr, DEFAULT_TEXT_COLOR_NORMAL);
            }
        }
        a.recycle();
        
        mTextSizeMin =  dp2px(context, 20);
        mTextColorMin = context.getColor(R.color.picker_min_clolor);
    }
    ...
    boolean is_text_thin = true;
    
    public void setTextThin(boolean is){
        is_text_thin = is;
        invalidate();
    }
    ...
    private void updateFontAttr(){
        ...
        mPaintText.setTextSize(mTextSizeMin);//add zouxu
        mTextSizeMinCenterYOffset = getTextCenterYOffset(mPaintText.getFontMetrics());//add zouxu
    }
    ...
    public void setTouchAble(boolean is){
        isTouchAble = is;
    }
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch(event.getAction()){
            case MotionEvent.ACTION_DOWN:
                ...
                onScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
                //getParent().requestDisallowInterceptTouchEvent(true); //允许父view拦截不然viewpager在这个控件上面左右滑不了
                break;
    ...
    int []heightList = new int[6];
    int []itemheightList = new int[7];
    ...
    private void drawContent(Canvas canvas){
        ...
        if(heightList[1]==0){
            heightList[0] = dp2px(mContext, 0);
            heightList[1] = dp2px(mContext, 32);
            heightList[2] = dp2px(mContext, 72);
            heightList[3] = dp2px(mContext, 128);
            heightList[4] = dp2px(mContext, 168);
            heightList[5] = dp2px(mContext, 200);
            itemheightList[0] = dp2px(mContext, 32);
            itemheightList[1] = dp2px(mContext, 32);
            itemheightList[2] = dp2px(mContext, 40);
            itemheightList[3] = dp2px(mContext, 56);
            itemheightList[4] = dp2px(mContext, 40);
            itemheightList[5] = dp2px(mContext, 32);
            itemheightList[6] = dp2px(mContext, 32);
        }
        
        if(mShowCount == 5){//add zouxu 对显示５个数据特殊处理
            for(int i = 0; i < mShowCount + 1; i++){
//                float y = mCurrDrawFirstItemY + mItemHeight * i;
                float y = mCurrDrawFirstItemY + heightList [i];
                index = getIndexByRawIndex(mCurrDrawFirstItemIndex + i, getOneRecycleSize(), mWrapSelectorWheel && mWrapSelectorWheelCheck);
                //fraction = (float)(itemheightList[i] + mCurrDrawFirstItemY) / itemheightList[i];
                if(i == mShowCount / 2){//this will be picked 3
//                    fraction = (float)(mItemHeight + mCurrDrawFirstItemY) / mItemHeight;
                    ...
                    textSizeCenterYOffset = getEvaluateSize(1 - fraction, mTextSizeNormalCenterYOffset,
                            mTextSizeSelectedCenterYOffset);
                }else if(i == mShowCount / 2 -1){ // 2
//                    fraction = (float)(mItemHeight + mCurrDrawFirstItemY) / mItemHeight;
                    fraction = (float)(itemheightList[i] + mCurrDrawFirstItemY) / itemheightList[i];
                    textColor = getEvaluateColor(fraction, mTextColorMin, mTextColorNormal);
                    textSize = getEvaluateSize(fraction, mTextSizeMin, mTextSizeNormal);
                    textSizeCenterYOffset = getEvaluateSize(fraction, mTextSizeMinCenterYOffset,
                            mTextSizeNormalCenterYOffset);
//                    Log.i("zouxu", "fraction ="+fraction+",textSize="+textSize);

                } else if(i == mShowCount / 2 +2){ // 5
                    
                    textColor = getEvaluateColor(1-fraction, mTextColorMin, mTextColorNormal);
                    textSize = getEvaluateSize(1-fraction, mTextSizeMin, mTextSizeNormal);
                    textSizeCenterYOffset = getEvaluateSize(1-fraction, mTextSizeMinCenterYOffset,
                            mTextSizeNormalCenterYOffset);

                } else { // 1
                    textColor = mTextColorMin;
                    textSize = mTextSizeMin;
                    textSizeCenterYOffset = mTextSizeMinCenterYOffset;
                }
                Log.i("zouxu","mCurrDrawFirstItemY="+mCurrDrawFirstItemY);
                
                mPaintText.setColor(textColor);
                mPaintText.setTextSize(textSize);
                
                if(is_text_thin){
                    Typeface tf = Typeface.create("monster-thin", -1);
...
//                            y + mItemHeight / 2 + textSizeCenterYOffset, mPaintText);
                    float y_offset = y + itemheightList[i+1] / 2 + textSizeCenterYOffset;
                    
                    if(i == mShowCount / 2){
                        Log.i("zouxu", "y_offset = "+y_offset);
                    }
...
//                (dividerY0 + dividerY1) / 2 + mTextSizeHintCenterYOffset, mPaintHint);
    ...
    private float getHintY(){
//        Log.i("zouxu","maigin_bt="+maigin_bt+",select_text_height="+select_text_height+",mTextSizeSelectedCenterYOffset="+mTextSizeSelectedCenterYOffset);
//        Log.i("zouxu", "mItemHeight = "+mItemHeight+",mTextSizeSelected="+mTextSizeSelected);
        return mItemHeight*mShowCount/2+mTextSizeSelectedCenterYOffset;
    }
