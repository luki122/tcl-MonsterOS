...
public class TimerObj implements Parcelable {
    ...
    public static final long MAX_TIMER_LENGTH = (99 * 3600 + 99 * 60 + 99) * 1000;
    public static final long MINUTE_IN_MILLIS = 60 * 1000;

    public int mTimerId; // Unique id
    public long mStartTime; // With mTimeLeft , used to calculate the correct
                            // time
    public long mTimeLeft; // in the timer.
    public long mOriginalLength; // length set at start of timer and by +1 min
                                 // after times up
    ...
    public void writeToSharedPref(SharedPreferences prefs) {
        ...
        final SharedPreferences.Editor editor = prefs.edit().putInt(PREF_TIMER_ID + id, mTimerId)
                .putLong(PREF_START_TIME + id, mStartTime).putLong(PREF_TIME_LEFT + id, mTimeLeft)
                .putLong(PREF_ORIGINAL_TIME + id, mOriginalLength).putLong(PREF_SETUP_TIME + id, mSetupLength)
                .putInt(PREF_STATE + id, mState).putInt(PREF_PRIOR_STATE + id, mPriorState)
                .putStringSet(PREF_TIMERS_LIST, timerIds).putString(PREF_LABEL + id, mLabel)
                .putBoolean(PREF_DELETE_AFTER_USE + id, mDeleteAfterUse);

        if(lastShowTime !=mTimeLeft && lastShowTime>0){//add zouxu 20160829
            editor.putLong(PREF_TIME_LEFT + id, lastShowTime);
            lastShowTime = mTimeLeft;
        }
        
        Log.i("timer", "writeToSharedPref  mTimeLeft="+mTimeLeft);
        editor.apply();
    }
    
    private long lastShowTime;
    
    public void seLastShowTime(long time){
        lastShowTime = time;
    }
    ...
    public void readFromSharedPref(SharedPreferences prefs) {
        ...
        Log.i("timer", "readFromSharedPref  mTimeLeft="+mTimeLeft);
    ...
    public boolean deleteFromSharedPref(SharedPreferences prefs) {
        ...
        final SharedPreferences.Editor editor = prefs.edit().remove(PREF_TIMER_ID + id).remove(PREF_START_TIME + id)
                .remove(PREF_TIME_LEFT + id).remove(PREF_ORIGINAL_TIME + id).remove(PREF_SETUP_TIME + id)
                .remove(PREF_STATE + id).remove(PREF_PRIOR_STATE + id).putStringSet(PREF_TIMERS_LIST, timerIds)
                .remove(PREF_LABEL + id).remove(PREF_DELETE_AFTER_USE + id);
    ...
    public boolean deleteAllFromSharedPref(SharedPreferences prefs) {
        final String id = Integer.toString(mTimerId);
    ...
    public void init(long length, int timerId) {
        /*
         * TODO: mTimerId must avoid StopwatchService.NOTIFICATION_ID,
         * TimerReceiver.IN_USE_NOTIFICATION_ID, and alarm ID's (which seem to
         * be 1, 2, ..)
         */
        mTimerId = timerId;
    ...
    public String getLabelOrDefault(Context context) {
        return (mLabel == null || mLabel.length() == 0) ? context.getString(R.string.timer_notification_label) : mLabel;
    }
    ...
    public static TimerObj getTimerFromSharedPrefs(SharedPreferences prefs) {
        for (String timerIdString : getTimerIds(prefs)) {
            final int timerId = Integer.parseInt(timerIdString);
            TimerObj obj= getTimerFromSharedPrefs(prefs, timerId);
            if(obj != null){
                return obj;
            }
        }
    ...
    public static void getTimersFromSharedPrefs(SharedPreferences prefs, List<TimerObj> timers, int state) {
        for (String timerIdString : getTimerIds(prefs)) {
            final int timerId = Integer.parseInt(timerIdString);
    ...
    public static void putTimersInSharedPrefs(SharedPreferences prefs, TimerObj timer) {
        timer.writeToSharedPref(prefs);
    }
    ...
    private static Set<String> getTimerIds(SharedPreferences prefs) {
        // return a defensive copy that is safe to mutate; see doc for
        // getStringSet() for details
