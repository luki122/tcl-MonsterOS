...
public class AlarmUtils {
    ...
    public static String  getAlarmWhenToRing(Context context, long alarmTime) {//仿照上面的方法计算还有多久响铃
        final long alarmTimeDelta = alarmTime - System.currentTimeMillis();
         return  formatWhenToAlarm(context, alarmTimeDelta);
    }

    static String formatWhenToAlarm(Context context, long delta) {
        // If the alarm will ring within 60 seconds, just report "less than a minute."
        final String[] formats = context.getResources().getStringArray(R.array.when_to_alarm);
        if (delta < DateUtils.MINUTE_IN_MILLIS) {
            return formats[0];
        }
